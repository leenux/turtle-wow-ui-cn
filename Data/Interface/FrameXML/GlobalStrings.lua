-- AUTOMATICALLY GENERATED -- DO NOT EDIT!

ABANDON_PET = "你是否决定永远地遗弃你的宠物？"; -- Confirmation dialog
ABANDON_QUEST = "放弃任务"; -- Quit the currently selected quest
ABANDON_QUEST_CONFIRM = "放弃\"%s\"？";
ABANDON_QUEST_CONFIRM_WITH_ITEMS = "确定要放弃\"%s\"并摧毁%s吗？";
ABILITIES = "技能"; -- The player's abilities ''
ABILITYBOOK_BUTTON = "技能";
ABSORB = "吸收";
ABSORB_CAPS = "吸收"; -- Absorb an attack
ACCEPT = "接受"; -- Generic "accept"
ACTIONBAR_LABEL = "动作条";
ADDITIONAL_COMMENTS = "附加评论：";
ADDMEMBER = "添加成员";
ADDON_ACTION_FORBIDDEN = "%s已被禁用，因为该功能只对暴雪的UI开放。\n你可以禁用这个插件并重新装载UI。";
ADDON_BANNED = "非法";
ADDON_CORRUPT = "损坏";
ADDON_DEP_BANNED = "依赖功能非法";
ADDON_DEP_CORRUPT = "依赖功能损坏";
ADDON_DEP_DISABLED = "依赖功能被禁用";
ADDON_DEP_INSECURE = "依赖功能不安全";
ADDON_DEP_INTERFACE_VERSION = "依赖功能过期";
ADDON_DEP_MISSING = "依赖功能缺失";
ADDON_DEP_NOT_DEMAND_LOADED = "依赖功能无法装载";
ADDON_DISABLED = "禁用";
ADDON_INSECURE = "不安全";
ADDON_INTERFACE_VERSION = "过期";
ADDON_LOAD_FAILED = "无法读取%s：%s";
ADDON_MISSING = "缺失";
ADDON_NOT_DEMAND_LOADED = "无法装载";
ADDON_UNKNOWN_ERROR = "未知读取问题";
ADD_CHANNEL = "输入频道名称";
ADD_CHAT_CHANNEL = "增加一个频道";
ADD_FRIEND = "添加好友"; -- Add a friend to your friends list
ADD_FRIEND_LABEL = "输入想要添加的好友名字："; -- Editbox label for adding a friend
ADD_GUILDMEMBER_LABEL = "添加公会成员：";
ADD_GUILDRANK_LABEL = "添加公会会阶：";
ADD_IGNORE_LABEL = "输入想要屏蔽的玩家名字\n或者在聊天窗口中\n按住Shift点击该玩家的名字："; -- Editbox label for ignoring a player
ADD_RAIDMEMBER_LABEL = "增加团队成员：";
ADD_RAID_MEMBER = "添加成员";
ADVANCED_OPTIONS = "高级选项";
ADVANCED_OPTIONS_TOOLTIP = "配置高级界面选项";
AGILITY_COLON = "敏捷："; -- Character's agility, with colon
AGILITY_TOOLTIP = "提高所有人物的致命一击几率。对盗贼\n的提升效果更高。敏捷还能提高躲避\n几率。盗贼从每一点敏捷获得的躲避加成比其它\n职业更多。敏捷直接提高护甲值。"; -- Tooltip description for "Agility"
ALL = "全部"; -- Generic All tag
ALLIED = "盟友"; -- Label for whether you're allied with a faction or not
ALL_INVENTORY_SLOTS = "所有空格"; -- Text for inventory slot filter in the tradeskills frame
ALL_SUBCLASSES = "所有分支"; -- Text for subclass filter in the tradeskills frame
ALREADY_LEARNED = "已经学会"; -- This service has already been learned
ALWAYS = "总是";
ALWAYS_SHOW_MULTIBARS_TEXT = "总是显示动作条";
AMBIENCE_VOLUME = "环境音量"; -- Sound options tag
AMMOSLOT = "弹药";
AMMO_DAMAGE_TEMPLATE = "每秒伤害提高%g";
AMMO_SCHOOL_DAMAGE_TEMPLATE = "每秒增加%g点%s系伤害";
AMOUNT_PAID_COLON = "付费金额：";
AMOUNT_RECEIVED_COLON = "收款金额：";
AMOUNT_TO_SEND = "寄送金额：";
ANIMATION = "动画";
ANISOTROPIC = "各向异性过滤";
AREA_SPIRIT_HEAL = "%d%s后复活";
ARMOR = "护甲"; -- Generic "armor"
ARMOR_COLON = "护甲："; -- Character's armor modifier, with colon
ARMOR_TEMPLATE = "%d点护甲"; -- Item's armor display template
ARMOR_TOOLTIP = "降低你在受到物理攻击时所承受的伤害，其效果与攻击者的等级有关。\n受到等级%d的敌人攻击时减免伤害：%.1f%%"; -- Tooltip description for "Armor"
ASSIGNED_COLON = "当前指派："; -- Header for currently assigned quests, with colon
ASSIST_ATTACK = "协助攻击"; -- Option to automatically attack after an "assist" command
ATTACHMENT_TEXT = "将物品放在这里随邮件发送";
ATTACK = "攻击"; -- Attack a target
ATTACK_COLON = "攻击：";
ATTACK_POWER_COLON = "强度：";
ATTACK_POWER_TOOLTIP = "攻击强度";
ATTACK_SPEED_COLON = "攻击速度（秒）："; -- Character's attack speed modifier, with colon
ATTACK_SPEED_TOOLTIP1 = "每次攻击间隔时间"; -- Tooltip for "Attack Speed"
ATTACK_TOOLTIP = "攻击命中率"; -- Tooltip description for "Attack"
ATTACK_TOOLTIP_SUBTEXT = "攻击命中率影响到你击中目标的几率，其数值由你对目前所装备的武器的使用熟练度决定。";
AT_WAR = "交战状态"; -- At war tag for faction that the player is at war us
AUCTIONS = "拍卖";
AUCTION_BUYOUT_ERROR = "（必须高于或等于起始价格）";
AUCTION_CREATOR = "出售者";
AUCTION_DURATION = "拍卖时限";
AUCTION_DURATION_ERROR = "（拍卖时间将被设置为5分钟）";
AUCTION_EXPIRED_MAIL_SUBJECT = "拍卖已到期：%s"; -- %s is the item name
AUCTION_HOUSE_CUT_COLON = "拍卖费：";
AUCTION_ITEM = "拍卖物品";
AUCTION_ITEM_TEXT = "将物品放在这里以供拍卖";
AUCTION_OUTBID_MAIL_SUBJECT = "竞标%s失败"; -- %s is the item name
AUCTION_REMOVED_MAIL_SUBJECT = "拍卖取消：%s";
AUCTION_SOLD_MAIL_SUBJECT = "拍卖成功：%s"; -- %s is the item name
AUCTION_TIME_LEFT1 = "短";
AUCTION_TIME_LEFT1_DETAIL = "少于30分钟";
AUCTION_TIME_LEFT2 = "中";
AUCTION_TIME_LEFT2_DETAIL = "30分钟到2小时";
AUCTION_TIME_LEFT3 = "长";
AUCTION_TIME_LEFT3_DETAIL = "2小时到8小时";
AUCTION_TIME_LEFT4 = "非常长";
AUCTION_TIME_LEFT4_DETAIL = "大于8小时";
AUCTION_TITLE = "%s的拍卖";
AUCTION_WON_MAIL_SUBJECT = "竞拍获胜：%s"; -- %s is the item name
AURAADDEDOTHERHARMFUL = "%s受到了%s效果的影响。"; -- Combat log text for aura events
AURAADDEDOTHERHELPFUL = "%s获得了%s的效果。"; -- Combat log text for aura events
AURAADDEDSELFHARMFUL = "你受到了%s效果的影响。"; -- Combat log text for aura events
AURAADDEDSELFHELPFUL = "你获得了%s的效果。"; -- Combat log text for aura events
AURAAPPLICATIONADDEDOTHERHARMFUL = "%s受到了%s效果的影响（%d）。";
AURAAPPLICATIONADDEDOTHERHELPFUL = "%s获得了%s的效果（%d）。";
AURAAPPLICATIONADDEDSELFHARMFUL = "你受到了%s效果的影响（%d）。";
AURAAPPLICATIONADDEDSELFHELPFUL = "你获得了%s的效果（%d）。";
AURACHANGEDOTHER = "%s用%s替换了%s。"; -- Combat log text for aura events
AURACHANGEDSELF = "你将%s替换成%s。"; -- Combat log text for aura events
AURADISPELOTHER = "%s的%s被移除了。";
AURADISPELSELF = "你的%s被移除了。";
AURAREMOVEDOTHER = "%s效果从%s身上消失。"; -- Combat log text for aura events
AURAREMOVEDSELF = "%s效果从你身上消失了。"; -- Combat log text for aura events
AURASTOLENOTHEROTHER = "%s偷取了%s的%s。";
AURASTOLENOTHERSELF = "%s偷取了你的%s。";
AURASTOLENSELFOTHER = "你偷取了%s的%s。";
AURASTOLENSELFSELF = "你偷取了你的%s。";
AURA_END = "<%s>效果消失";
AUTOFOLLOWSTART = "正在跟随%s";
AUTOFOLLOWSTOP = "已停止跟随%s。";
AUTOFOLLOWSTOPCOMBAT = "你停止移动，开始攻击%s";
AUTO_FOLLOW_SPEED = "自动跟随速度";
AUTO_JOIN_GUILD_CHANNEL = "自动加入公会招募频道";
AUTO_QUEST_WATCH_TEXT = "自动任务追踪";
AUTO_SELF_CAST_TEXT = "自动自我施法";
AVAILABLE = "可用"; -- Generic available tag
AVAILABLE_QUESTS = "可以取得的任务"; -- Header for currently available quests
AVAILABLE_SERVICES = "可以使用的服务";
AVERAGE_WAIT_TIME = "平均等待时间：";
BACK = "后退";
BACKGROUND = "底色"; -- Title in the chat preferences menu
BACKPACK_TOOLTIP = "行囊"; -- Backpack tooltip
BACKSLOT = "背部"; -- Back slot
BAGSLOT = "背包"; -- Bag slot
BAGSLOTTEXT = "背包栏位"; -- label for bags
BAGS_ONLY = "这里只能放置背包！"; -- The player can only put bags in this slot
BAG_CANT_STACK = "此物品无法堆积存放"; -- Can't have a stack of these
BAG_CANT_SWAP = "这些物品无法交换"; -- An error of sorts?
BAG_DROP_TOO_FAR_AWAY = "指定距离太远，无法丢弃物品"; -- You clicked too far away.
BAG_ERROR = "由于未知的原因无法移动物品。"; -- The client doesn't understand the specific result code, or the server had some sort of bug outside the bounds of things the player should know about.
BAG_FULL = "这个背包已经装满了！"; -- The player is trying to put an item into a full bag
BAG_ITEM_ALREADY_BOUND = "此物品已被绑定"; -- The item is bound to its current owner and cannot be dropped or traded.
BAG_ITEM_CLASS_MISMATCH = "不能将该类型物品放入此背包中。"; -- E.G. Only arrows go in quivers
BAG_ITEM_MAX_COUNT_EXCEEDED = "已经携带过多物品"; -- Can only carry a limited number of the item total
BAG_ITEM_NOT_FOUND = "未找到指定物品"; -- A request for an item that doesn't exist (could be caused by lag)
BAG_ITEM_SPLIT_FAILED = "无法分开这些物品"; -- Some sort of internal error?
BAG_ITEM_SUBTYPE_MISMATCH = "不能将该物品放入此背包中。"; -- E.G. Only CERTAIN arrows?!
BAG_ITEM_TOO_FEW_TO_SPLIT = "你试图分开的数量大于物品原有的数量。"; -- Tried to split out too many of the item
BAG_NOT_A_CONTAINER = "不是一个可以放置东西的器具"; -- Something that should have been a container, wasn't
BAG_NOT_EMPTY = "这个背包不是空的。"; -- Can't trade bags with stuff in 'em
BAG_NOT_ENOUGH_GOLD = "金钱不足"; -- Transaction related
BAG_NOT_EQUIPPABLE = "此物品无法装备"; -- Item is not equippable, by anyone, ever
BAG_NOT_OWNER = "你并未拥有这件物品。"; -- Not owner of something specified
BAG_SLOT_EMPTY = "这个栏位是空的";
BAG_SLOT_NOT_EMPTY = "这个栏位不是空的";
BANKSLOTPURCHASE = "购买"; -- Purchase button label
BANKSLOTPURCHASE_LABEL = "你想要再购买一个用来放置背包的栏位吗？";
BANK_BAG = "背包栏位"; -- Bag slot
BANK_BAG_PURCHASE = "可以购买的背包栏位"; -- Bank slot that is not yet opened
BASIC_LABEL = "基本";
BASIC_OPTIONS_TOOLTIP = "配置基本界面选项";
BATTLEFIELDMINIMAP_OPACITY_LABEL = "改变不透明度";
BATTLEFIELDMINIMAP_OPTIONS_LABEL = "作战地图设置";
BATTLEFIELDS = "战场";
BATTLEFIELD_CONFIRM_STATUS = "（准备进入）";
BATTLEFIELD_FULL = "%s（已满）";
BATTLEFIELD_GROUP_JOIN = "小队加入";
BATTLEFIELD_IN_BATTLEFIELD = "你正在%s中\n|cffffffff<左键点击这里显示得分>\n<右键点击这里显示战场地图>|r";
BATTLEFIELD_IN_QUEUE = "你正在等待%s的队列中\n平均等待时间：%s（前10个玩家）\n在队列中的时间：%s";
BATTLEFIELD_JOIN = "加入战斗";
BATTLEFIELD_LEVEL = "等级范围：";
BATTLEFIELD_MINIMAP = "作战地图";
BATTLEFIELD_NAME = "战场名称：";
BATTLEFIELD_QUEUE_CONFIRM = "你现在可以进入%s了\n你将在%s内离开队列";
BATTLEFIELD_QUEUE_STATUS = "（在队列中）";
BATTLEGROUND_INSTANCE_TOOLTIP = "当战场中有可用的空位时再加入战场。这样会比选择加入首先可用的战场的等待时间稍长。";
BID = "竞标";
BIDS = "竞标";
BID_STATUS = "竞标状态";
BILLING_NAG_DIALOG = "您的帐户中还有%d%s的剩余游戏时间";
BILLING_NAG_WARNING = "您的帐户中还有%d分钟的剩余游戏时间";
BILLING_NAG_WARNING_P1 = "您的帐户中还有%d分钟的剩余游戏时间";
BINDING_HEADER_ACTIONBAR = "动作条按键";
BINDING_HEADER_BLANK = "";
BINDING_HEADER_CAMERA = "视角按键";
BINDING_HEADER_CHAT = "聊天按键";
BINDING_HEADER_INTERFACE = "界面面板按键";
BINDING_HEADER_ITUNES_REMOTE = "iTunes遥控";
BINDING_HEADER_MISC = "其他按键";
BINDING_HEADER_MOVEMENT = "移动按键";
BINDING_HEADER_MULTIACTIONBAR = "多动作条绑定";
BINDING_HEADER_RAID_TARGET = "团队目标";
BINDING_HEADER_TARGETING = "选择目标按键";
BINDING_NAME_ACTIONBUTTON1 = "快捷键1"; -- Keybinding name for ACTIONBUTTON1
BINDING_NAME_ACTIONBUTTON10 = "快捷键10";
BINDING_NAME_ACTIONBUTTON11 = "快捷键11";
BINDING_NAME_ACTIONBUTTON12 = "快捷键12";
BINDING_NAME_ACTIONBUTTON2 = "快捷键2"; -- Keybinding name for ACTIONBUTTON2
BINDING_NAME_ACTIONBUTTON3 = "快捷键3";
BINDING_NAME_ACTIONBUTTON4 = "快捷键4";
BINDING_NAME_ACTIONBUTTON5 = "快捷键5";
BINDING_NAME_ACTIONBUTTON6 = "快捷键6";
BINDING_NAME_ACTIONBUTTON7 = "快捷键7";
BINDING_NAME_ACTIONBUTTON8 = "快捷键8";
BINDING_NAME_ACTIONBUTTON9 = "快捷键9";
BINDING_NAME_ACTIONPAGE1 = "动作条1";
BINDING_NAME_ACTIONPAGE2 = "动作条2";
BINDING_NAME_ACTIONPAGE3 = "动作条3";
BINDING_NAME_ACTIONPAGE4 = "动作条4";
BINDING_NAME_ACTIONPAGE5 = "动作条5";
BINDING_NAME_ACTIONPAGE6 = "动作条6";
BINDING_NAME_ALLNAMEPLATES = "显示所有姓名板";
BINDING_NAME_ASSISTTARGET = "协助目标";
BINDING_NAME_ATTACKTARGET = "攻击目标";
BINDING_NAME_BONUSACTIONBUTTON1 = "副快捷按钮1";
BINDING_NAME_BONUSACTIONBUTTON10 = "副快捷按钮10";
BINDING_NAME_BONUSACTIONBUTTON2 = "副快捷按钮2";
BINDING_NAME_BONUSACTIONBUTTON3 = "副快捷按钮3";
BINDING_NAME_BONUSACTIONBUTTON4 = "副快捷按钮4";
BINDING_NAME_BONUSACTIONBUTTON5 = "副快捷按钮5";
BINDING_NAME_BONUSACTIONBUTTON6 = "副快捷按钮6";
BINDING_NAME_BONUSACTIONBUTTON7 = "副快捷按钮7";
BINDING_NAME_BONUSACTIONBUTTON8 = "副快捷按钮8";
BINDING_NAME_BONUSACTIONBUTTON9 = "副快捷按钮9";
BINDING_NAME_CAMERAZOOMIN = "放大";
BINDING_NAME_CAMERAZOOMOUT = "缩小";
BINDING_NAME_CHATBOTTOM = "翻至对话最下端";
BINDING_NAME_CHATPAGEDOWN = "对话向下翻页";
BINDING_NAME_CHATPAGEUP = "对话向上翻页";
BINDING_NAME_COMBATLOGBOTTOM = "战斗日志翻至最下端";
BINDING_NAME_COMBATLOGPAGEDOWN = "战斗日志向下翻页";
BINDING_NAME_COMBATLOGPAGEUP = "战斗日志向上翻页";
BINDING_NAME_FLIPCAMERAYAW = "水平视角";
BINDING_NAME_FOLLOWTARGET = "跟随目标";
BINDING_NAME_FRIENDNAMEPLATES = "显示友方姓名板";
BINDING_NAME_ITUNES_BACKTRACK = "iTunes上一首";
BINDING_NAME_ITUNES_NEXTTRACK = "iTunes下一首";
BINDING_NAME_ITUNES_PLAYPAUSE = "iTunes播放/暂停";
BINDING_NAME_ITUNES_VOLUMEDOWN = "iTunes音量降低";
BINDING_NAME_ITUNES_VOLUMEUP = "iTunes音量提高";
BINDING_NAME_JUMP = "跳跃";
BINDING_NAME_MASTERVOLUMEDOWN = "主音量缩小";
BINDING_NAME_MASTERVOLUMEUP = "主音量放大";
BINDING_NAME_MINIMAPZOOMIN = "放大地图";
BINDING_NAME_MINIMAPZOOMOUT = "缩小地图";
BINDING_NAME_MOVEANDSTEER = "移动控制";
BINDING_NAME_MOVEBACKWARD = "后退";
BINDING_NAME_MOVEFORWARD = "前进";
BINDING_NAME_MULTIACTIONBAR1BUTTON1 = "左下方动作按钮1";
BINDING_NAME_MULTIACTIONBAR1BUTTON10 = "左下方动作按钮10";
BINDING_NAME_MULTIACTIONBAR1BUTTON11 = "左下方动作按钮11";
BINDING_NAME_MULTIACTIONBAR1BUTTON12 = "左下方动作按钮12";
BINDING_NAME_MULTIACTIONBAR1BUTTON2 = "左下方动作按钮2";
BINDING_NAME_MULTIACTIONBAR1BUTTON3 = "左下方动作按钮3";
BINDING_NAME_MULTIACTIONBAR1BUTTON4 = "左下方动作按钮4";
BINDING_NAME_MULTIACTIONBAR1BUTTON5 = "左下方动作按钮5";
BINDING_NAME_MULTIACTIONBAR1BUTTON6 = "左下方动作按钮6";
BINDING_NAME_MULTIACTIONBAR1BUTTON7 = "左下方动作按钮7";
BINDING_NAME_MULTIACTIONBAR1BUTTON8 = "左下方动作按钮8";
BINDING_NAME_MULTIACTIONBAR1BUTTON9 = "左下方动作按钮9";
BINDING_NAME_MULTIACTIONBAR2BUTTON1 = "右下方动作按钮1";
BINDING_NAME_MULTIACTIONBAR2BUTTON10 = "右下方动作按钮10";
BINDING_NAME_MULTIACTIONBAR2BUTTON11 = "右下方动作按钮11";
BINDING_NAME_MULTIACTIONBAR2BUTTON12 = "右下方动作按钮12";
BINDING_NAME_MULTIACTIONBAR2BUTTON2 = "右下方动作按钮2";
BINDING_NAME_MULTIACTIONBAR2BUTTON3 = "右下方动作按钮3";
BINDING_NAME_MULTIACTIONBAR2BUTTON4 = "右下方动作按钮4";
BINDING_NAME_MULTIACTIONBAR2BUTTON5 = "右下方动作按钮5";
BINDING_NAME_MULTIACTIONBAR2BUTTON6 = "右下方动作按钮6";
BINDING_NAME_MULTIACTIONBAR2BUTTON7 = "右下方动作按钮7";
BINDING_NAME_MULTIACTIONBAR2BUTTON8 = "右下方动作按钮8";
BINDING_NAME_MULTIACTIONBAR2BUTTON9 = "右下方动作按钮9";
BINDING_NAME_MULTIACTIONBAR3BUTTON1 = "右边动作按钮1";
BINDING_NAME_MULTIACTIONBAR3BUTTON10 = "右边动作按钮10";
BINDING_NAME_MULTIACTIONBAR3BUTTON11 = "右边动作按钮11";
BINDING_NAME_MULTIACTIONBAR3BUTTON12 = "右边动作按钮12";
BINDING_NAME_MULTIACTIONBAR3BUTTON2 = "右边动作按钮2";
BINDING_NAME_MULTIACTIONBAR3BUTTON3 = "右边动作按钮3";
BINDING_NAME_MULTIACTIONBAR3BUTTON4 = "右边动作按钮4";
BINDING_NAME_MULTIACTIONBAR3BUTTON5 = "右边动作按钮5";
BINDING_NAME_MULTIACTIONBAR3BUTTON6 = "右边动作按钮6";
BINDING_NAME_MULTIACTIONBAR3BUTTON7 = "右边动作按钮7";
BINDING_NAME_MULTIACTIONBAR3BUTTON8 = "右边动作按钮8";
BINDING_NAME_MULTIACTIONBAR3BUTTON9 = "右边动作按钮9";
BINDING_NAME_MULTIACTIONBAR4BUTTON1 = "右边动作按钮2 - 1";
BINDING_NAME_MULTIACTIONBAR4BUTTON10 = "右边动作按钮2 - 10";
BINDING_NAME_MULTIACTIONBAR4BUTTON11 = "右边动作按钮2 - 11";
BINDING_NAME_MULTIACTIONBAR4BUTTON12 = "右边动作按钮2 - 12";
BINDING_NAME_MULTIACTIONBAR4BUTTON2 = "右边动作按钮2 - 2";
BINDING_NAME_MULTIACTIONBAR4BUTTON3 = "右边动作按钮2 - 3";
BINDING_NAME_MULTIACTIONBAR4BUTTON4 = "右边动作按钮2 - 4";
BINDING_NAME_MULTIACTIONBAR4BUTTON5 = "右边动作按钮2 - 5";
BINDING_NAME_MULTIACTIONBAR4BUTTON6 = "右边动作按钮2 - 6";
BINDING_NAME_MULTIACTIONBAR4BUTTON7 = "右边动作按钮2 - 7";
BINDING_NAME_MULTIACTIONBAR4BUTTON8 = "右边动作按钮2 - 8";
BINDING_NAME_MULTIACTIONBAR4BUTTON9 = "右边动作按钮2 - 9";
BINDING_NAME_NAMEPLATES = "显示姓名板";
BINDING_NAME_NEXTACTIONPAGE = "下一动作条";
BINDING_NAME_NEXTVIEW = "下一个视角";
BINDING_NAME_OPENALLBAGS = "打开/关闭所有的背包";
BINDING_NAME_OPENCHAT = "打开对话框";
BINDING_NAME_OPENCHATSLASH = "打开目标对话框";
BINDING_NAME_PETATTACK = "宠物攻击";
BINDING_NAME_PITCHDOWN = "向下走路";
BINDING_NAME_PITCHUP = "向上攀爬";
BINDING_NAME_PREVIOUSACTIONPAGE = "前一动作条";
BINDING_NAME_PREVVIEW = "前一个视角";
BINDING_NAME_RAIDTARGET1 = "为目标指定星形";
BINDING_NAME_RAIDTARGET2 = "为目标指定圆形";
BINDING_NAME_RAIDTARGET3 = "为目标指定菱形";
BINDING_NAME_RAIDTARGET4 = "为目标指定三角";
BINDING_NAME_RAIDTARGET5 = "为目标指定月亮";
BINDING_NAME_RAIDTARGET6 = "为目标指定方块";
BINDING_NAME_RAIDTARGET7 = "为目标指定十字";
BINDING_NAME_RAIDTARGET8 = "为目标指定骷髅";
BINDING_NAME_RAIDTARGETNONE = "清除团队目标图标";
BINDING_NAME_REPLY = "回复对话";
BINDING_NAME_REPLY2 = "再次密语";
BINDING_NAME_RESETVIEW1 = "重置视角1";
BINDING_NAME_RESETVIEW2 = "重置视角2";
BINDING_NAME_RESETVIEW3 = "重置视角3";
BINDING_NAME_RESETVIEW4 = "重置视角4";
BINDING_NAME_RESETVIEW5 = "重置视角5";
BINDING_NAME_SAVEVIEW1 = "保存视角1";
BINDING_NAME_SAVEVIEW2 = "保存视角2";
BINDING_NAME_SAVEVIEW3 = "保存视角3";
BINDING_NAME_SAVEVIEW4 = "保存视角4";
BINDING_NAME_SAVEVIEW5 = "保存视角5";
BINDING_NAME_SCREENSHOT = "截图";
BINDING_NAME_SELFACTIONBUTTON1 = "快捷按钮1";
BINDING_NAME_SELFACTIONBUTTON10 = "快捷按钮10";
BINDING_NAME_SELFACTIONBUTTON11 = "快捷按钮11";
BINDING_NAME_SELFACTIONBUTTON12 = "快捷按钮12";
BINDING_NAME_SELFACTIONBUTTON2 = "快捷按钮2";
BINDING_NAME_SELFACTIONBUTTON3 = "快捷按钮3";
BINDING_NAME_SELFACTIONBUTTON4 = "快捷按钮4";
BINDING_NAME_SELFACTIONBUTTON5 = "快捷按钮5";
BINDING_NAME_SELFACTIONBUTTON6 = "快捷按钮6";
BINDING_NAME_SELFACTIONBUTTON7 = "快捷按钮7";
BINDING_NAME_SELFACTIONBUTTON8 = "快捷按钮8";
BINDING_NAME_SELFACTIONBUTTON9 = "快捷按钮9";
BINDING_NAME_SETVIEW1 = "设置1号视角";
BINDING_NAME_SETVIEW2 = "设置2号视角";
BINDING_NAME_SETVIEW3 = "设置3号视角";
BINDING_NAME_SETVIEW4 = "设置4号视角";
BINDING_NAME_SETVIEW5 = "设置5号视角";
BINDING_NAME_SHAPESHIFTBUTTON1 = "特殊快捷按钮1";
BINDING_NAME_SHAPESHIFTBUTTON10 = "特殊快捷按钮10";
BINDING_NAME_SHAPESHIFTBUTTON2 = "特殊快捷按钮2";
BINDING_NAME_SHAPESHIFTBUTTON3 = "特殊快捷按钮3";
BINDING_NAME_SHAPESHIFTBUTTON4 = "特殊快捷按钮4";
BINDING_NAME_SHAPESHIFTBUTTON5 = "特殊快捷按钮5";
BINDING_NAME_SHAPESHIFTBUTTON6 = "特殊快捷按钮6";
BINDING_NAME_SHAPESHIFTBUTTON7 = "特殊快捷按钮7";
BINDING_NAME_SHAPESHIFTBUTTON8 = "特殊快捷按钮8";
BINDING_NAME_SHAPESHIFTBUTTON9 = "特殊快捷按钮9";
BINDING_NAME_SITORSTAND = "坐下/起立";
BINDING_NAME_STRAFELEFT = "向左平移";
BINDING_NAME_STRAFERIGHT = "向右平移";
BINDING_NAME_TARGETLASTHOSTILE = "选择最后一个有敌意的目标";
BINDING_NAME_TARGETNEARESTENEMY = "选择最近的敌人目标";
BINDING_NAME_TARGETNEARESTFRIEND = "选择最近的盟友";
BINDING_NAME_TARGETPARTYMEMBER1 = "选择队友1";
BINDING_NAME_TARGETPARTYMEMBER2 = "选择队友2";
BINDING_NAME_TARGETPARTYMEMBER3 = "选择队友3";
BINDING_NAME_TARGETPARTYMEMBER4 = "选择队友4";
BINDING_NAME_TARGETPARTYPET1 = "选择队友宠物1";
BINDING_NAME_TARGETPARTYPET2 = "选择队友宠物2";
BINDING_NAME_TARGETPARTYPET3 = "选择队友宠物3";
BINDING_NAME_TARGETPARTYPET4 = "选择队友宠物4";
BINDING_NAME_TARGETPET = "选择宠物";
BINDING_NAME_TARGETPREVIOUSENEMY = "选择前一个敌人目标";
BINDING_NAME_TARGETPREVIOUSFRIEND = "选择前一个盟友";
BINDING_NAME_TARGETSELF = "选择自己";
BINDING_NAME_TOGGLEABILITYBOOK = "打开/关闭能力界面";
BINDING_NAME_TOGGLEACTIONBARLOCK = "打开/关闭动作条锁定";
BINDING_NAME_TOGGLEAUTORUN = "自动奔跑";
BINDING_NAME_TOGGLEAUTOSELFCAST = "自动自我施法";
BINDING_NAME_TOGGLEBACKPACK = "打开/关闭行囊";
BINDING_NAME_TOGGLEBAG1 = "打开/关闭1号背包";
BINDING_NAME_TOGGLEBAG2 = "打开/关闭2号背包";
BINDING_NAME_TOGGLEBAG3 = "打开/关闭3号背包";
BINDING_NAME_TOGGLEBAG4 = "打开/关闭4号背包";
BINDING_NAME_TOGGLEBAG5 = "打开/关闭5号背包";
BINDING_NAME_TOGGLEBATTLEFIELDMINIMAP = "战场地图开关";
BINDING_NAME_TOGGLECHARACTER0 = "打开/关闭角色界面";
BINDING_NAME_TOGGLECHARACTER1 = "打开/关闭技能界面";
BINDING_NAME_TOGGLECHARACTER2 = "打开/关闭声望界面";
BINDING_NAME_TOGGLECHARACTER3 = "切换宠物面板";
BINDING_NAME_TOGGLECHARACTER4 = "打开荣誉面板";
BINDING_NAME_TOGGLECOMBATLOG = "打开/关闭战斗日志";
BINDING_NAME_TOGGLEFPS = "开启/关闭祯数显示";
BINDING_NAME_TOGGLEFRIENDSTAB = "切换好友面板";
BINDING_NAME_TOGGLEGAMEMENU = "打开/关闭游戏菜单";
BINDING_NAME_TOGGLEGUILDTAB = "开启/关闭公会面板";
BINDING_NAME_TOGGLEIGNORETAB = "切换忽略面板";
BINDING_NAME_TOGGLEKEYRING = "钥匙链开关";
BINDING_NAME_TOGGLEMINIMAP = "打开/关闭地图";
BINDING_NAME_TOGGLEMUSIC = "开启音乐";
BINDING_NAME_TOGGLEPETBOOK = "打开/关闭宠物书";
BINDING_NAME_TOGGLEQUESTLOG = "打开/关闭任务日志";
BINDING_NAME_TOGGLERAIDTAB = "开启/关闭团队面板";
BINDING_NAME_TOGGLERUN = "跑/走";
BINDING_NAME_TOGGLESHEATH = "取出/收起武器";
BINDING_NAME_TOGGLESOCIAL = "打开/关闭社交界面";
BINDING_NAME_TOGGLESOUND = "开启声效"; -- Key binding tag
BINDING_NAME_TOGGLESPELLBOOK = "打开/关闭法术书";
BINDING_NAME_TOGGLETALENTS = "打开/关闭天赋面板";
BINDING_NAME_TOGGLEUI = "打开／关闭用户界面"; -- Keybinding name for TOGGLEUI
BINDING_NAME_TOGGLEWHOTAB = "切换查询面板";
BINDING_NAME_TOGGLEWORLDMAP = "打开/关闭世界地图";
BINDING_NAME_TOGGLEWORLDSTATESCORES = "积分窗口";
BINDING_NAME_TURNLEFT = "左转";
BINDING_NAME_TURNRIGHT = "右转";
BIND_ENCHANT = "对这件物品进行附魔将使其与你绑定。";
BIND_KEY_TO_COMMAND = "点击按键以绑定命令 -> %s";
BIND_ZONE_DISPLAY = "你现在绑定在%s。"; -- The zone name where your character is bound
BLOCK = "格挡"; -- Block an attack
BLOCK_TRADES = "阻止交易";
BLOCK_TRAILER = "（%d点被格挡）"; -- Appended to attack strings in the combat log when damage is blocked. (NOTE SPACE AT BEGINNING)
BOSS = "首领";
BREATH_LABEL = "呼吸"; -- Used as the label for the breath status bar
BROWSE = "浏览";
BROWSE_AUCTIONS = "浏览拍卖";
BROWSE_NO_RESULTS = "未发现物品";
BROWSE_SEARCH_TEXT = "选择搜索条件，然后按下“搜索”";
BUFFERING = "增益法术";
BUFFER_DOUBLE = "双倍";
BUG_BUTTON = "Bug及建议";
BUG_CATEGORY15 = "语言翻译";
BUG_CATEGORY13 = "其它";
BUG_CATEGORY6 = "怪物——平衡/技能";
BUG_CATEGORY7 = "怪物——位置";
BUG_CATEGORY1 = "角色职业";
BUG_CATEGORY10 = "声效";
BUG_CATEGORY12 = "专业技能";
BUG_CATEGORY11 = "物品";
BUG_CATEGORY9 = "美工";
BUG_CATEGORY8 = "任务和故事";
BUG_CATEGORY4 = "城市";
BUG_CATEGORY2 = "户外区域";
BUG_CATEGORY5 = "用户界面";
BUG_CATEGORY14 = "玩家对玩家";
BUG_CATEGORY3 = "地下城";
BUG_CATEGORY_CHOOSE = "——〉请选择一个类别";
BUG_CATEGORY_ERROR = "你必须选择一个类别来提交Bug或建议。";
BUG_SUBMITTED = "Bug已提交";
BUG_SUBMIT_FAILED = "提交Bug失败";
BUYBACK = "购回";
BUYBACK_THIS_ITEM = "买回这件物品";
BUYOUT = "一口价";
BUYOUT_AUCTION_CONFIRMATION = "以一口价购买：";
BUYOUT_COST = "一口价";
BUYOUT_PRICE = "一口价";
CALIBRATION_TEXT = "你应该可以区分以下的21个灰格。否则的话请调整反差系数，直到你能够区分这些灰格。如果单单调整反差系数仍然效果不佳的话，请调节您的显示器亮度和对比度来达到最满意的效果。";
CAMERA_ALWAYS = "总是";
CAMERA_FOLLOWING_STYLE = "镜头跟随模式";
CAMERA_LABEL = "镜头";
CAMERA_LOCKED = "已锁";
CAMERA_MODE = "分离镜头";
CAMERA_NEVER = "从不";
CAMERA_SMART = "智能";
CAMP_NOW = "立刻返回角色选择画面"; -- Gives the time in minutes/seconds until you logout
CAMP_TIMER = "%d%s后返回角色选择画面"; -- Gives the time in minutes/seconds until you logout
CANCEL = "取消"; -- Generic "cancel"
CANCEL_AUCTION = "取消拍卖";
CANCEL_AUCTION_CONFIRMATION = "取消拍卖将使你失去保证金。";
CANCEL_AUCTION_CONFIRMATION_MONEY = "取消拍卖会没收你所有的委托费用和：";
CANT_AFFORD_ITEM = "你买不起这个物品。"; -- The player tried to buy an item he couldn't afford
CANT_USE_ITEM = "你不能使用这件物品。"; -- The player tried to use an item he doesn't have requisites for
CASH_ON_DELIVERY = "付款取信";
CAST_WHILE_MOVING = "%s失败：不能在移动中施放。"; -- The player moved while casting a spell
CHANCE_TO_BLOCK = "%.2f%%格挡几率";
CHANCE_TO_CRIT = "%.2f%%致命一击率";
CHANCE_TO_DODGE = "%.2f%%闪躲几率";
CHANCE_TO_PARRY = "%.2f%%招架几率";
CHANGE_INSTANCE = "变更副本";
CHANGE_MACRO_NAME_ICON = "改变名字/图标";
CHANNELING = "正在引导"; -- Channeling tag for the channeling bar
CHANNELS = "频道";
CHARACTER = "角色"; -- Generic "character", as is the player's character
CHARACTER_BUTTON = "角色信息";
CHARACTER_INFO = "角色信息"; -- Screen that displays all of a character's information
CHARACTER_KEY_BINDINGS = "%s的按键设置"; -- Title text for the keybinding ui interface.  %s is the player's name
CHARACTER_POINTS1_COLON = "天赋点数："; -- Character's character points
CHARACTER_POINTS2_COLON = "技能点数："; -- Character's skill points
CHARACTER_POINTS_CHANGED = "角色点数已改变";
CHARACTER_SPECIFIC_KEYBINDINGS = "角色专用按键设置";
CHARACTER_SPECIFIC_KEYBINDING_TOOLTIP = "点击这里在通用按键设置和本角色的专用按键设置之间切换。";
CHARACTER_SPECIFIC_MACROS = "%s专用宏"; -- Title for character specific macros %s is the player's name
CHATLOGDISABLED = "聊天记录已被禁止。";
CHATLOGENABLED = "聊天记录保存在Logs\\WoWChatLog.txt中";
CHAT_AFK_GET = "%s目前不在屏幕前：\32";
CHAT_ANNOUNCEMENTS_OFF_NOTICE = "[%s] 频道公告已被%s禁止。";
CHAT_ANNOUNCEMENTS_ON_NOTICE = "[%s] 频道公告已被%s打开。";
CHAT_BATTLEGROUND_GET = "[Battleground] %s:\32";
CHAT_BATTLEGROUND_LEADER_GET = "[Battleground Leader] %s:\32";
CHAT_BATTLEGROUND_SEND = "Battleground:\32";
CHAT_BUBBLES_TEXT = "显示聊天泡泡";
CHAT_CHANNEL_GET = "%s：\32";
CHAT_CHANNEL_JOIN_GET = "%s进入频道。"; -- another player joined
CHAT_CHANNEL_LEAVE_GET = "%s离开频道。"; -- another player left
CHAT_CHANNEL_LIST_GET = "[%s]\32"; -- Channel user list, user list is the "message", this is the channel name.
CHAT_CHANNEL_OWNER_NOTICE = "[%s] 频道所有者被设定为%s。";
CHAT_CHANNEL_SEND = "[%d. %s]：\32";
CHAT_COMBAT_CREATURE_VS_CREATURE_HITS_GET = "";
CHAT_COMBAT_CREATURE_VS_CREATURE_MISSES_GET = "";
CHAT_COMBAT_CREATURE_VS_PARTY_HITS_GET = "";
CHAT_COMBAT_CREATURE_VS_PARTY_MISSES_GET = "";
CHAT_COMBAT_CREATURE_VS_SELF_HITS_GET = "";
CHAT_COMBAT_CREATURE_VS_SELF_MISSES_GET = "";
CHAT_COMBAT_ERROR_GET = "";
CHAT_COMBAT_FRIENDLYPLAYER_HITS_GET = "";
CHAT_COMBAT_FRIENDLYPLAYER_MISSES_GET = "";
CHAT_COMBAT_FRIENDLY_DEATH_GET = "";
CHAT_COMBAT_HOSTILEPLAYER_HITS_GET = "";
CHAT_COMBAT_HOSTILEPLAYER_MISSES_GET = "";
CHAT_COMBAT_HOSTILE_DEATH_GET = "";
CHAT_COMBAT_LOG_ERROR_GET = "";
CHAT_COMBAT_LOG_MISC_INFO_GET = "";
CHAT_COMBAT_MISC_INFO_GET = "";
CHAT_COMBAT_PARTY_HITS_GET = "";
CHAT_COMBAT_PARTY_MISSES_GET = "";
CHAT_COMBAT_PET_HITS_GET = "";
CHAT_COMBAT_PET_MISSES_GET = "";
CHAT_COMBAT_SELF_HITS_GET = "";
CHAT_COMBAT_SELF_MISSES_GET = "";
CHAT_DND_GET = "%s不愿被打扰：\32";
CHAT_EMOTE_GET = "%s\32"; -- Just the player's name for an emote, as in "Pimlet jumps up and down"
CHAT_EMOTE_SEND = "%s\32"; -- Just the player's name for an emote, as in "Pimlet jumps up and down"
CHAT_EMOTE_UNKNOWN = "做了一些奇怪的手势。";
CHAT_FILTERED = "无法向%s发送信息，因为你的话语中带有屏蔽词。";
CHAT_FLAG_AFK = "<暂离>";
CHAT_FLAG_DND = "<勿扰>";
CHAT_FLAG_GM = "<GM>";
CHAT_GUILD_DEMOTE_SEND = "降职"; -- Demote player within your guild
CHAT_GUILD_GET = "[公会] %s:\32"; -- Guild message from player %s
CHAT_HARDCORE_GET = "[硬核] %s:\32"; -- Hardcore chat message from player %s
CHAT_GUILD_INVITE_SEND = "邀请加入公会："; -- Invite a player to your guild
CHAT_GUILD_LEADER_SEND = "设定公会领袖："; -- Set player as the new guild leader
CHAT_GUILD_MOTD_SEND = "公会纲领："; -- Set the guild MOTD
CHAT_GUILD_PROMOTE_SEND = "提升"; -- Promote player within your guild
CHAT_GUILD_SEND = "公会：\32"; -- Chat message to your guild
CHAT_HARDCORE_SEND = "硬核:\32"; -- Chat message to hardcore homies
CHAT_GUILD_UNINVITE_SEND = "从公会中开除："; -- Remove player from your guild
CHAT_HELP_TEXT_LINE5 = "/leave, /chatleave, /chatexit [channel] - Leave a channel (or all channels)";
CHAT_HELP_TEXT_LINE10 = "/mute, /squelch, /unvoice, /unmute, /unsquelch, /voice <频道> <玩家> - 改变一个玩家的发言权限";
CHAT_HELP_TEXT_LINE2 = "/#, /c, /csay - 向#频道发送信息 (例如：/1 你好！)";
CHAT_HELP_TEXT_LINE9 = "/mod, /moderator, /unmod, /unmoderator <频道> <玩家> - 改变一个玩家的频道管理权限";
CHAT_HELP_TEXT_LINE8 = "/own <频道> [玩家] - 显示或改变频道管理员";
CHAT_HELP_TEXT_LINE4 = "/join, /channel, /chan - 加入一个频道";
CHAT_HELP_TEXT_LINE15 = "/moderate <频道> - 打开/关闭频道设置";
CHAT_HELP_TEXT_LINE12 = "/ckick <频道> <玩家> - 将一个玩家踢出频道";
CHAT_HELP_TEXT_LINE11 = "/cinvite, /chatinvite <频道> <玩家> - 邀请玩家进入频道";
CHAT_HELP_TEXT_LINE7 = "/password, /pass <频道> <密码> - 改变密码";
CHAT_HELP_TEXT_LINE6 = "/chatlist, /chatwho, /chatinfo [频道] - 列出频道或频道中的用户";
CHAT_HELP_TEXT_LINE3 = "/chat, /chathelp - This help";
CHAT_HELP_TEXT_LINE16 = "/afk, /dnd - 设置为离开或者请勿打扰状态";
CHAT_HELP_TEXT_LINE1 = "聊天命令：";
CHAT_HELP_TEXT_LINE14 = "/announce, /ann <频道名> - 开启或者关闭加入/离开频道时的提示";
CHAT_HELP_TEXT_LINE13 = "/ban, /unban <channel> <player> - ban/unban a player from a channel";
CHAT_IGNORED = "%s已将你屏蔽。";
CHAT_INVALID_NAME_NOTICE = "无效的频道名";
CHAT_INVITE_NOTICE = "%s邀请你加入'%s'频道。";
CHAT_INVITE_SEND = "邀请"; -- Invite another player to a group
CHAT_INVITE_WRONG_FACTION_NOTICE = "%s的目标所属阵营错误。";
CHAT_JOIN_HELP = "输入 /join <频道名称> [密码] 以创建新的频道或加入已有频道。";
CHAT_LABEL = "聊天";
CHAT_LOCKED_TEXT = "锁定聊天设置";
CHAT_MODERATION_OFF_NOTICE = "[%s] 频道修改已被%s禁止。";
CHAT_MODERATION_ON_NOTICE = "[%s] 频道修改已被%s禁止。";
CHAT_MONSTER_EMOTE_GET = ""; -- The emote text has %s embedded in it, so no formatting is necessary here.
CHAT_MONSTER_SAY_GET = "%s说：\32"; -- Monster chat form monster named %s
CHAT_MONSTER_WHISPER_GET = "%s悄悄地说：\32"; -- Whisper from NPC\Monster %s
CHAT_MONSTER_YELL_GET = "%s喊道：\32"; -- Monster yell form monster named %s
CHAT_MSG_AFK = "AFK";
CHAT_MSG_BATTLEGROUND = "战场";
CHAT_MSG_BATTLEGROUND_LEADER = "战场领袖";
CHAT_MSG_BG_SYSTEM_ALLIANCE = "联盟区域信息";
CHAT_MSG_BG_SYSTEM_HORDE = "部落区域信息";
CHAT_MSG_BG_SYSTEM_NEUTRAL = "中立区域信息";
CHAT_MSG_CHANNEL_LIST = "频道列表";
CHAT_MSG_COMBAT_CREATURE_VS_CREATURE_HITS = "怪物击中怪物";
CHAT_MSG_COMBAT_CREATURE_VS_CREATURE_MISSES = "怪物未击中怪物";
CHAT_MSG_COMBAT_CREATURE_VS_PARTY_HITS = "怪物击中队友";
CHAT_MSG_COMBAT_CREATURE_VS_PARTY_MISSES = "怪物未击中队友";
CHAT_MSG_COMBAT_CREATURE_VS_SELF_HITS = "怪物击中你";
CHAT_MSG_COMBAT_CREATURE_VS_SELF_MISSES = "怪物未击中你";
CHAT_MSG_COMBAT_FACTION_CHANGE = "阵营";
CHAT_MSG_COMBAT_FRIENDLYPLAYER_HITS = "友方玩家命中";
CHAT_MSG_COMBAT_FRIENDLYPLAYER_MISSES = "友方玩家未击中";
CHAT_MSG_COMBAT_FRIENDLY_DEATH = "友方死亡";
CHAT_MSG_COMBAT_HONOR_GAIN = "荣誉点数";
CHAT_MSG_COMBAT_HOSTILEPLAYER_HITS = "敌方玩家命中";
CHAT_MSG_COMBAT_HOSTILEPLAYER_MISSES = "敌方玩家未击中";
CHAT_MSG_COMBAT_HOSTILE_DEATH = "敌方死亡";
CHAT_MSG_COMBAT_LOG_ERROR = "错误";
CHAT_MSG_COMBAT_LOG_MISC_INFO = "其它";
CHAT_MSG_COMBAT_MISC_INFO = "其它信息";
CHAT_MSG_COMBAT_PARTY_HITS = "队友命中";
CHAT_MSG_COMBAT_PARTY_MISSES = "队友未击中";
CHAT_MSG_COMBAT_PET_HITS = "宠物命中";
CHAT_MSG_COMBAT_PET_MISSES = "宠物未击中";
CHAT_MSG_COMBAT_SELF_HITS = "你攻击命中";
CHAT_MSG_COMBAT_SELF_MISSES = "你未击中";
CHAT_MSG_COMBAT_XP_GAIN = "经验值获得";
CHAT_MSG_DND = "DND";
CHAT_MSG_EMOTE = "表情";
CHAT_MSG_FILTERED = "被过滤聊天信息";
CHAT_MSG_GUILD = "公会";
CHAT_MSG_HARDCORE = "硬核";
CHAT_MSG_IGNORED = "已屏蔽";
CHAT_MSG_LOOT = "战利品";
CHAT_MSG_MONEY = "钱";
CHAT_MSG_MONSTER_EMOTE = "怪物表情";
CHAT_MSG_MONSTER_SAY = "怪物说";
CHAT_MSG_MONSTER_WHISPER = "怪物悄悄话";
CHAT_MSG_MONSTER_YELL = "怪物大喊";
CHAT_MSG_OFFICER = "公会官员";
CHAT_MSG_PARTY = "小队";
CHAT_MSG_RAID = "团队";
CHAT_MSG_RAID_BOSS_EMOTE = "首领怪物表情";
CHAT_MSG_RAID_LEADER = "团队领袖";
CHAT_MSG_RAID_WARNING = "团队通知";
CHAT_MSG_SAY = "说";
CHAT_MSG_SKILL = "技能";
CHAT_MSG_SPELL_AURA_GONE_OTHER = "光环从其他人身上消失";
CHAT_MSG_SPELL_AURA_GONE_PARTY = "光环从你队友身上消失";
CHAT_MSG_SPELL_AURA_GONE_SELF = "光环从你身上消失";
CHAT_MSG_SPELL_BREAK_AURA = "打断光环";
CHAT_MSG_SPELL_CREATURE_VS_CREATURE_BUFF = "怪物对其他人的增益法术";
CHAT_MSG_SPELL_CREATURE_VS_CREATURE_DAMAGE = "怪物对其他人的伤害性法术";
CHAT_MSG_SPELL_CREATURE_VS_PARTY_BUFF = "怪物对队友的增益法术";
CHAT_MSG_SPELL_CREATURE_VS_PARTY_DAMAGE = "怪物对队友的伤害性法术";
CHAT_MSG_SPELL_CREATURE_VS_SELF_BUFF = "怪物对你的增益法术";
CHAT_MSG_SPELL_CREATURE_VS_SELF_DAMAGE = "怪物对你的伤害性法术";
CHAT_MSG_SPELL_DAMAGESHIELDS_ON_OTHERS = "魔法护盾对其他人的伤害";
CHAT_MSG_SPELL_DAMAGESHIELDS_ON_SELF = "魔法护盾对你的伤害";
CHAT_MSG_SPELL_FAILED_LOCALPLAYER = "没有足够的法术资源错误";
CHAT_MSG_SPELL_FRIENDLYPLAYER_BUFF = "友方玩家的增益法术";
CHAT_MSG_SPELL_FRIENDLYPLAYER_DAMAGE = "友方玩家的伤害法术";
CHAT_MSG_SPELL_HOSTILEPLAYER_BUFF = "敌方玩家的增益法术";
CHAT_MSG_SPELL_HOSTILEPLAYER_DAMAGE = "敌方玩家的伤害法术";
CHAT_MSG_SPELL_ITEM_ENCHANTMENTS = "物品附魔";
CHAT_MSG_SPELL_PARTY_BUFF = "队友的增益法术";
CHAT_MSG_SPELL_PARTY_DAMAGE = "队友的伤害法术";
CHAT_MSG_SPELL_PERIODIC_CREATURE_BUFFS = "怪物的周期性增益法术";
CHAT_MSG_SPELL_PERIODIC_CREATURE_DAMAGE = "怪物的周期性伤害";
CHAT_MSG_SPELL_PERIODIC_FRIENDLYPLAYER_BUFFS = "友方玩家的周期性增益法术";
CHAT_MSG_SPELL_PERIODIC_FRIENDLYPLAYER_DAMAGE = "友方玩家的周期性伤害";
CHAT_MSG_SPELL_PERIODIC_HOSTILEPLAYER_BUFFS = "敌方玩家的周期性增益法术";
CHAT_MSG_SPELL_PERIODIC_HOSTILEPLAYER_DAMAGE = "敌方玩家的周期性伤害";
CHAT_MSG_SPELL_PERIODIC_PARTY_BUFFS = "队友的周期性增益法术";
CHAT_MSG_SPELL_PERIODIC_PARTY_DAMAGE = "队友的周期性伤害";
CHAT_MSG_SPELL_PERIODIC_SELF_BUFFS = "你的周期性增益法术";
CHAT_MSG_SPELL_PERIODIC_SELF_DAMAGE = "你的周期性伤害";
CHAT_MSG_SPELL_PET_BUFF = "宠物的增益法术";
CHAT_MSG_SPELL_PET_DAMAGE = "宠物的伤害法术";
CHAT_MSG_SPELL_SELF_BUFF = "你的增益法术";
CHAT_MSG_SPELL_SELF_DAMAGE = "你的伤害法术";
CHAT_MSG_SPELL_TRADESKILLS = "专业";
CHAT_MSG_SYSTEM = "系统";
CHAT_MSG_TEXT_EMOTE = "文字表情";
CHAT_MSG_WHISPER = "收到悄悄话";
CHAT_MSG_WHISPER_INFORM = "悄悄话";
CHAT_MSG_YELL = "大喊";
CHAT_MUTED_NOTICE = "[%s] 你没有发言权限。";
CHAT_NAME_TEMPLATE = "交谈 %d"; -- Default names for chat windows
CHAT_NOT_MEMBER_NOTICE = "不在%s频道里。";
CHAT_NOT_MODERATED_NOTICE = "%s未修改";
CHAT_NOT_MODERATOR_NOTICE = "不是%s的管理员。";
CHAT_NOT_OWNER_NOTICE = "[%s] 你不是频道的所有者";
CHAT_OFFICER_GET = "[官员] %s：\32"; -- Officer chat from player %s
CHAT_OFFICER_SEND = "官员：\32"; -- Chat message to officers in your guild
CHAT_OPTIONS_LABEL = "聊天选项";
CHAT_OWNER_CHANGED_NOTICE = "[%s] 频道所有者被设定为%s。";
CHAT_PARTY_GET = "[小队] %s：\32"; -- Party message from player %s
CHAT_PARTY_SEND = "小队：\32"; -- Chat message to your party
CHAT_PASSWORD_CHANGED_NOTICE = "[%s] 密码已被%s修改。";
CHAT_PLAYER_ALREADY_MEMBER_NOTICE = "[%s] %s已在此频道中。";
CHAT_PLAYER_BANNED_NOTICE = "[%s] %s已被%s禁止进入频道。";
CHAT_PLAYER_INVITED_NOTICE = "[%s] 你邀请%s加入频道";
CHAT_PLAYER_INVITE_BANNED_NOTICE = "[%s] %s已经被踢出频道。";
CHAT_PLAYER_KICKED_NOTICE = "[%s] %s被%s逐出频道。";
CHAT_PLAYER_NOT_BANNED_NOTICE = "[%s] 玩家%s未被开除。";
CHAT_PLAYER_NOT_FOUND_NOTICE = "[%s] %s不在此频道中。";
CHAT_PLAYER_UNBANNED_NOTICE = "[%s] %s被%s允许重新进入频道。";
CHAT_PROMOTE_SEND = "提升"; -- Promote another player to leader of the party
CHAT_RAID_BOSS_EMOTE_GET = "";
CHAT_RAID_GET = "[团队] %s:\32"; -- Raid message from player %s
CHAT_RAID_LEADER_GET = "[团队领袖] %s：\32";
CHAT_RAID_SEND = "团队：\32"; -- Chat message to your raid
CHAT_RAID_WARNING_GET = "[团队通知] %s：\32";
CHAT_RAID_WARNING_SEND = "团队通知：\32";
CHAT_SAY_GET = "%s说：\32"; -- Normal message from player %s
CHAT_SAY_SEND = "说：\32"; -- Normal chat message
CHAT_SET_MODERATOR_NOTICE = "[%s] %s获得修改权限。";
CHAT_SET_VOICE_NOTICE = "[%s] %s获得发言权限。";
CHAT_SPELL_AURA_GONE_OTHER_GET = "";
CHAT_SPELL_AURA_GONE_PARTY_GET = "";
CHAT_SPELL_AURA_GONE_SELF_GET = "";
CHAT_SPELL_BREAK_AURA_GET = "";
CHAT_SPELL_CREATURE_VS_CREATURE_BUFF_GET = "";
CHAT_SPELL_CREATURE_VS_CREATURE_DAMAGE_GET = "";
CHAT_SPELL_CREATURE_VS_PARTY_BUFF_GET = "";
CHAT_SPELL_CREATURE_VS_PARTY_DAMAGE_GET = "";
CHAT_SPELL_CREATURE_VS_SELF_BUFF_GET = "";
CHAT_SPELL_CREATURE_VS_SELF_DAMAGE_GET = "";
CHAT_SPELL_DAMAGESHIELDS_ON_OTHERS_GET = "";
CHAT_SPELL_DAMAGESHIELDS_ON_SELF_GET = "";
CHAT_SPELL_FRIENDLYPLAYER_BUFF_GET = "";
CHAT_SPELL_FRIENDLYPLAYER_DAMAGE_GET = "";
CHAT_SPELL_HOSTILEPLAYER_BUFF_GET = "";
CHAT_SPELL_HOSTILEPLAYER_DAMAGE_GET = "";
CHAT_SPELL_ITEM_ENCHANTMENTS_GET = "";
CHAT_SPELL_PARTY_BUFF_GET = "";
CHAT_SPELL_PARTY_DAMAGE_GET = "";
CHAT_SPELL_PERIODIC_CREATURE_BUFFS_GET = "";
CHAT_SPELL_PERIODIC_CREATURE_DAMAGE_GET = "";
CHAT_SPELL_PERIODIC_FRIENDLYPLAYER_BUFFS_GET = "";
CHAT_SPELL_PERIODIC_FRIENDLYPLAYER_DAMAGE_GET = "";
CHAT_SPELL_PERIODIC_HOSTILEPLAYER_BUFFS_GET = "";
CHAT_SPELL_PERIODIC_HOSTILEPLAYER_DAMAGE_GET = "";
CHAT_SPELL_PERIODIC_PARTY_BUFFS_GET = "";
CHAT_SPELL_PERIODIC_PARTY_DAMAGE_GET = "";
CHAT_SPELL_PERIODIC_SELF_BUFFS_GET = "";
CHAT_SPELL_PERIODIC_SELF_DAMAGE_GET = "";
CHAT_SPELL_PET_BUFF_GET = "";
CHAT_SPELL_PET_DAMAGE_GET = "";
CHAT_SPELL_SELF_BUFF_GET = "";
CHAT_SPELL_SELF_DAMAGE_GET = "";
CHAT_SPELL_TRADESKILLS_GET = "";
CHAT_SUSPENDED_NOTICE = "离开频道：[%s]";
CHAT_THROTTLED_NOTICE = "[%s] 短时间内发送到此频道的信息条数是受限制的。请稍候再发送新的消息。";
CHAT_UNINVITE_SEND = "开除"; -- Kick a player out of your party
CHAT_UNSET_MODERATOR_NOTICE = "[%s] %s失去修改权限。";
CHAT_UNSET_VOICE_NOTICE = "[%s] %s已被禁言。";
CHAT_WHISPER_GET = "%s悄悄地说：\32"; -- Whisper from player %s
CHAT_WHISPER_INFORM_GET = "发送给%s：\32"; -- A whisper already sent to player %s
CHAT_WHISPER_SEND = "告诉%s：\32"; -- Whisper to a particular player
CHAT_WRONG_FACTION_NOTICE = "与%s错误地结盟。";
CHAT_WRONG_PASSWORD_NOTICE = "%s的密码错误。";
CHAT_YELL_GET = "%s喊道：\32"; -- Yelled message from player %s
CHAT_YELL_SEND = "大喊：\32"; -- Yell a message
CHAT_YOU_CHANGED_NOTICE = "改变频道：[%s]";
CHAT_YOU_JOINED_NOTICE = "加入频道：[%s]"; -- You joined
CHAT_YOU_LEFT_NOTICE = "离开频道：[%s]"; -- You left
CHESTSLOT = "胸部"; -- Chest slot
CHOOSE_BOX = "选择一个盒子：";
CHOOSE_STATIONERY = "选择信纸";
CHOSEN_FOR_GMSURVEY = "你被选择填写一份GM调查问卷。";
CINEMATIC_SUBTITLES = "动画字幕";
CLASS = "职业";
CLASS_SKILLS = "%s技能："; -- Skills specific to the characters class
CLEARED_AFK = "你现在不再处于离开状态";
CLEARED_DND = "你现在不再处于“请勿打扰”状态";
CLEAR_AFK = "自动清除暂离标签";
CLICK_CAMERA_STYLE = "点击移动视角模式";
CLICK_TO_LEARN = "点击学习技能"; -- Tooltip in the skill frame for learn skill buttons
CLICK_TO_MOVE = "点击移动";
CLIENT_RESTART_ALERT = "你的有些设置需要你重新启动游戏才能够生效。";
CLOSE = "关闭";
CLOSES_IN = "剩余时间";
CLOSE_CHAT_WINDOW = "移除窗口";
CLOSE_LOG = "关闭日志"; -- Close the quest log
COD = "付款取信"; -- Cash on delivery 
COD_AMOUNT = "付款取信邮件的金额";
COD_CONFIRMATION = "收下这件物品将花费：";
COD_INSUFFICIENT_MONEY = "你没有足够的钱来支付付款取信邮件。";
COD_PAYMENT = "付款取信邮件的费用：%s"; -- %s is the subject of the original mail
COLOR = "颜色";
COLOR_PICKER = "颜色选择器";
COMBATHITCRITOTHEROTHER = "%s对%s造成%d的致命一击伤害。";
COMBATHITCRITOTHERSELF = "%s的致命一击对你造成%d点伤害。";
COMBATHITCRITSCHOOLOTHEROTHER = "%s的致命一击对%s造成%d点%s伤害。";
COMBATHITCRITSCHOOLOTHERSELF = "%s的致命一击对你造成%d点%s伤害。";
COMBATHITCRITSCHOOLSELFOTHER = "你的致命一击对%s造成%d点%s伤害。";
COMBATHITCRITSELFOTHER = "你对%s造成%d的致命一击伤害。";
COMBATHITOTHEROTHER = "%s击中%s造成%d点伤害。";
COMBATHITOTHERSELF = "%s击中你造成%d点伤害。";
COMBATHITSCHOOLOTHEROTHER = "%s击中%s造成%d点%s伤害。";
COMBATHITSCHOOLOTHERSELF = "%s击中你造成%d点%s伤害。";
COMBATHITSCHOOLSELFOTHER = "你击中%s造成%d点%s伤害。";
COMBATHITSELFOTHER = "你击中%s造成%d点伤害。";
COMBATLOGDISABLED = "战斗记录已被禁止。";
COMBATLOGENABLED = "战斗记录保存在Logs\\WoWCombatLog.txt中";
COMBATLOG_DISHONORGAIN = "%s死亡，非荣誉击杀。";
COMBATLOG_HONORAWARD = "你得到了%d点荣誉的奖励。";
COMBATLOG_HONORGAIN = "%s死亡，荣誉击杀军衔：%s（贡献点数预估：%d）";
COMBATLOG_XPGAIN = "%s获得%d点经验值";
COMBATLOG_XPGAIN_EXHAUSTION1 = "%s死亡，你获得了%d点经验值。（%s点经验值的%s奖励）";
COMBATLOG_XPGAIN_EXHAUSTION1_GROUP = "%s死亡，你获得了%d点经验值。（%s点%s奖励，+%d组队奖励）";
COMBATLOG_XPGAIN_EXHAUSTION1_RAID = "%s死亡，你获得了%d点经验值。（%s点%s奖励，-%d点团队惩罚）";
COMBATLOG_XPGAIN_EXHAUSTION2 = "%s死亡，你获得了%d点经验值。（%s点经验值的%s奖励）";
COMBATLOG_XPGAIN_EXHAUSTION2_GROUP = "%s死亡，你获得了%d点经验值。（%s点%s奖励，+%d组队奖励）";
COMBATLOG_XPGAIN_EXHAUSTION2_RAID = "%s死亡，你获得了%d点经验值。（%s点%s奖励，-%d点团队惩罚）";
COMBATLOG_XPGAIN_EXHAUSTION4 = "%s死亡，你获得了%d点经验值。（%s点经验值的%s惩罚）";
COMBATLOG_XPGAIN_EXHAUSTION4_GROUP = "%s死亡，你获得了%d点经验值。（%s点%s惩罚，+%d组队奖励）";
COMBATLOG_XPGAIN_EXHAUSTION4_RAID = "%s死亡，你获得了%d点经验值。（%s点%s惩罚，-%d点团队惩罚）";
COMBATLOG_XPGAIN_EXHAUSTION5 = "%s死亡，你获得了%d点经验值。（%s点经验值的%s惩罚）";
COMBATLOG_XPGAIN_EXHAUSTION5_GROUP = "%s死亡，你获得了%d点经验值。（%s点%s惩罚，+%d组队奖励）";
COMBATLOG_XPGAIN_EXHAUSTION5_RAID = "%s死亡，你获得了%d点经验值。（%s点%s惩罚，-%d点团队惩罚）";
COMBATLOG_XPGAIN_FIRSTPERSON = "%s死亡，你获得%d点经验值。";
COMBATLOG_XPGAIN_FIRSTPERSON_GROUP = "%s死亡，你获得了%d点经验值。（+%d点组队奖励）";
COMBATLOG_XPGAIN_FIRSTPERSON_RAID = "%s死亡，你获得了%d点经验值。（-%d点团队惩罚）";
COMBATLOG_XPGAIN_FIRSTPERSON_UNNAMED = "你获得了%d点经验值。";
COMBATLOG_XPGAIN_FIRSTPERSON_UNNAMED_GROUP = "你获得了%d点经验值。（+%d点组队奖励）";
COMBATLOG_XPGAIN_FIRSTPERSON_UNNAMED_RAID = "你获得%d点经验值。（-%d点团队惩罚）";
COMBATLOG_XPLOSS_FIRSTPERSON_UNNAMED = "你损失了%d点经验值。";
COMBAT_ENEMY = "作战敌人";
COMBAT_ERROR = "作战错误";
COMBAT_LOG = "战斗记录";
COMBAT_MESSAGES = "作战信息";
COMBAT_MISC = "作战其它";
COMBAT_PARTY = "作战小队";
COMBAT_SELF = "作战自身";
COMBAT_TEXT_COMBO_POINTS = "<%d连击>";
COMBAT_TEXT_COMBO_POINTS_P1 = "<%d连击>";
COMBAT_TEXT_FLOAT_MODE_LABEL = "战斗信息飘浮模式";
COMBAT_TEXT_HONOR_GAINED = "荣誉 %s";
COMBAT_TEXT_LABEL = "浮动战斗信息";
COMBAT_TEXT_SCROLL_ARC = "弧形";
COMBAT_TEXT_SCROLL_DOWN = "向下滚动";
COMBAT_TEXT_SCROLL_DOWN_TEXT = "记录向下滚动";
COMBAT_TEXT_SCROLL_UP = "向上滚动";
COMBAT_TEXT_SHOW_AURAS_TEXT = "效果显示";
COMBAT_TEXT_SHOW_AURA_FADE_TEXT = "效果消失显示";
COMBAT_TEXT_SHOW_COMBAT_STATE_TEXT = "战斗状态显示";
COMBAT_TEXT_SHOW_COMBO_POINTS_TEXT = "连击点显示";
COMBAT_TEXT_SHOW_DODGE_PARRY_MISS_TEXT = "躲闪/招架/未击中显示";
COMBAT_TEXT_SHOW_FRIENDLY_NAMES_TEXT = "友方治疗者姓名显示";
COMBAT_TEXT_SHOW_HONOR_GAINED_TEXT = "荣誉显示";
COMBAT_TEXT_SHOW_LOW_HEALTH_MANA_TEXT = "生命法力过低显示";
COMBAT_TEXT_SHOW_MANA_TEXT = "能量获取显示";
COMBAT_TEXT_SHOW_REACTIVES_TEXT = "反击法术和技能显示";
COMBAT_TEXT_SHOW_REPUTATION_TEXT = "声望显示";
COMBAT_TEXT_SHOW_RESISTANCES_TEXT = "伤害减免显示";
COMMAND = "命令"; -- Command label for keybinding UI
COMMENTS_COLON = "评论：";
COMPLETE = "完成"; -- Generic "complete"
COMPLETE_QUEST = "完成任务"; -- Give items to a quest giver to complete a quest
CONFIRM_BATTLEFIELD_ENTRY = "你现在可以进入%s了，请选择：";
CONFIRM_BINDER = "你想要将%s设为你的新家吗？";
CONFIRM_BUY_BANK_SLOT = "你愿意付钱购买银行空位吗？";
CONFIRM_DELETING_CHARACTER_SPECIFIC_BINDINGS = "确定要切换到通用键位设定吗？所有本角色专用的键位设定都将被永久删除。";
CONFIRM_GUILD_DISBAND = "你真的要解散公会吗？";
CONFIRM_GUILD_LEAVE = "确定要退出%s？";
CONFIRM_GUILD_PROMOTE = "确定要将%s提升为会长？";
CONFIRM_LEAVE_QUEUE = "你确定要离开这块集合石的队列吗？";
CONFIRM_LOOT_DISTRIBUTION = "你想要将%s分配给%s，确定吗？";
CONFIRM_LOSE_BINDING_CHANGES = "如果你在通用键位设定和角色专用键位设定之间切换，所有未保存的更改都将丢失。";
CONFIRM_PET_UNLEARN = "你确定要让宠物遗忘所有技能吗？每次进行这种操作的费用都会递增。";
CONFIRM_RESET_INSTANCES = "你确定想要重置你的所有副本吗？";
CONFIRM_SUMMON = "%s想将你召唤到%s去。这个法术将在%d%s后取消。";
CONFIRM_TALENT_WIPE = "你确定要遗忘所有的天赋吗？每次遗忘天赋的花费都会比上一次更高。";
CONFIRM_XP_LOSS = "如果你找到你的尸体，那么你可以在没有任何惩罚的情况下复活。现在由我来复活你，那么你的所有物品（包括已装备的和物品栏中的）将损失25%%的耐久度，你也要承受%s的复活虚弱时间。";
CONFIRM_XP_LOSS_AGAIN = "记住，如果你找到自己的尸体的话，将不会有任何惩罚。你决定现在以承受%s的复活虚弱时间并损失所有装备25%%耐久度的代价来复活吗？";
CONFIRM_XP_LOSS_AGAIN_NO_SICKNESS = "记住，如果你找到自己的尸体的话，将不会有任何惩罚。你决定现在以损失所有装备25%耐久度的代价来复活吗？";
CONFIRM_XP_LOSS_NO_SICKNESS = "如果你找到你的尸体，你可以复活而且不会受到任何惩罚。如果让我复活你，那么你所有的物品会损失25%%的耐久度（包括装备着的和包中的物品）。";
CONTAINER_SLOTS = "%d格%s"; -- # of slots and bag type
CONTESTED_TERRITORY = "争夺中的领土";
CONTINENT = "大陆"; -- Label for "Continent" field in the world map
CONTINUE = "继续";
CONTROLS_LABEL = "控制";
CONVERT_TO_RAID = "转化为团队";
COOLDOWN_REMAINING = "剩余冷却时间：";
COPPER = "铜币"; -- Lowest value coin denomination
COPY_NAME = "复制名字";
CORPSE = "尸体";
CORPSE_RED = "尸体"; -- Generic "corpse" in red
CORPSE_TOOLTIP = "%s的尸体"; -- Tooltip for corpses
COSTS_LABEL = "花费："; -- Character point and money costs of a skill or bank slot
CREATE = "制造"; -- Create a trade skill item
CREATED_ITEM = "%s制造了：%s。"; -- The first %s is the player name, the other %s is the item name.
CREATED_ITEM_MULTIPLE = "%s制造了：%sx%d。"; -- The first %s is the player name, the other %s is the item name, and the %d is the quantity of items created.
CREATE_ALL = "全部制造";
CREATE_AUCTION = "开始拍卖";
CREATE_MACROS = "创建宏"; -- Title for the MacroFrame
CREATURE = "怪物";
CRUSHING_TRAILER = "(碾压)"; -- Appended to crushing blows in the combat log. (NOTE SPACE AT BEGINNING)
CURRENTLY_EQUIPPED = "当前装备"; -- Comparison tooltip text
CURRENT_BID = "当前价格";
CURRENT_PET = "当前宠物：";
CURRENT_QUESTS = "现有任务"; -- Header for currently assigned quests
DAMAGE = "伤害"; -- Generic "damage"
DAMAGESHIELDOTHEROTHER = "%s将%d点%s伤害反射给%s。";
DAMAGESHIELDOTHERSELF = "%s将%d点%s伤害反弹给你。";
DAMAGESHIELDSELFOTHER = "你将%d点%s伤害反弹给%s。";
DAMAGE_BONUS_TOOLTIP = "增加武器伤害"; -- Tooltop for damage increasing stat
DAMAGE_COLON = "伤害："; -- Character's damage modifier, with colon
DAMAGE_PER_SECOND = "每秒伤害：";
DAMAGE_TEMPLATE = "%d - %d伤害"; -- Weapon damage template
DAMAGE_TEMPLATE_WITH_SCHOOL = "%d - %d %s伤害"; -- Weapon damage template, for weapons doing school damage
DAMAGE_TOOLTIP = "武器伤害"; -- Tooltip description for "Damage"
DAYS_P1 = "天";
DAYS = "天";
DAYS_ABBR = "天";
DAYS_ABBR_P1 = "天";
DAY_ONELETTER_ABBR = "%d d";
DEAD = "死亡"; -- Generic "dead"
DEATHBINDALREADYBOUND = "你已经绑定在这里了！";
DEATHBIND_SUCCESSFUL = "你的灵魂已经绑定在这里了。"; -- Clicked on a bindstone
DEATHS = "死亡"; -- Generic Deaths label
DEATHS_TOOLTIP = "你被杀死的次数。";
DEATH_CORPSE_SKINNED = "徽记被取走 - 你只能在墓地复活";
DEATH_EFFECT = "死亡效果";
DEATH_RELEASE = "释放灵魂"; -- "Release" from your body after death before the timer expires
DEATH_RELEASE_NOTIMER = "你死亡了。要释放灵魂到最近的墓地吗？";
DEATH_RELEASE_TIMER = "%d%s后释放灵魂"; -- Gives the time in minutes/seconds until you automaticaly release
DECLINE = "拒绝"; -- Generic "decline"
DEFAULTS = "默认设置";
DEFAULT_AFK_MESSAGE = "暂离";
DEFAULT_AGILITY_TOOLTIP = "提高你的远程武器攻击强度。\n提高用所有武器造成致命一击的几率。\n提高你的护甲值和躲避攻击的几率。";
DEFAULT_DND_MESSAGE = "请勿打扰";
DEFAULT_INTELLECT_TOOLTIP = "提高你的武器技能熟练度提升速度。";
DEFAULT_SPIRIT_TOOLTIP = "提高你的生命值和法力值回复速度。";
DEFAULT_STAMINA_TOOLTIP = "提高你的生命值上限。";
DEFAULT_STRENGTH_TOOLTIP = "提高你的近战武器攻击强度。";
DEFENSE_COLON = "防御："; -- Character's defense modifier, with colon
DEFENSE_TOOLTIP = "防御等级"; -- Tooltip description for "Defense"
DEFLECT = "偏斜";
DELETE = "删除";
DELETE_GOOD_ITEM = "你真的要摧毁%s吗？\n\n请在输入框中输入\"DELETE\"以确认。";
DELETE_ITEM = "你确定要摧毁%s？"; -- %s is the item name
DELETE_ITEM_CONFIRM_STRING = "DELETE"; -- String to type in to confirm deletion of a blue item or better
DELETE_MAIL_CONFIRMATION = "删除这封邮件会摧毁%s";
DELETE_MONEY_CONFIRMATION = "删除这封邮件会摧毁：";
DEMOTE = "降职";
DEPOSIT_COLON = "保管费：";
DESKTOP_GAMMA = "使用桌面Gamma";
DISABLE = "禁用";
DISABLE_SPAM_FILTER = "禁用垃圾信息过滤";
DISGUISE = "伪装";
DISHONORABLE_KILLS = "非荣誉击杀";
DISPELFAILEDOTHEROTHER = "%s未能驱散%s的%s。";
DISPELFAILEDOTHERSELF = "%s未能驱散你的%s。";
DISPELFAILEDSELFOTHER = "你未能驱散%s的%s。";
DISPELFAILEDSELFSELF = "你试图驱散你的%s，但是失败了。";
DISPLAY = "显示";
DISPLAY_ON_CHARACTER = "试穿效果";
DISPLAY_ON_CHAR_TOOLTIP = "钩选此框可以在你的人物身上显示任何已选中的拍卖物品的装备效果。\n\n按住CTRL并左键点击游戏中的任何可装备的物品均可让你的人物查看试穿效果。";
DK = "非荣誉击杀"; -- Abbreviation for "dishonorable kill"
DODGE = "躲闪"; -- Dodge an attack
DPS_TEMPLATE = "（每秒伤害%.1f）";
DRESSUP_FRAME = "试衣间";
DRESSUP_FRAME_INSTRUCTIONS = "按住CTRL并左键点击额外的物品可以预览该物品的装备效果";
DRUID_INTELLECT_TOOLTIP = "提高你的法力值上限和使用法术造成致命一击的几率。\n使你更快地提升武器技能熟练度。";
DRUNK_MESSAGE_OTHER3 = "%s看上去喝醉了。"; -- %s is the name of the player.
DRUNK_MESSAGE_OTHER2 = "%s看上去有些喝醉了。"; -- %s is the name of the player.
DRUNK_MESSAGE_OTHER1 = "%s开始变得冷静下来了。"; -- %s is the name of the player.
DRUNK_MESSAGE_OTHER4 = "%s看上去完全喝醉了。"; -- %s is the name of the player.
DRUNK_MESSAGE_SELF4 = "你感觉完全喝醉了。";
DRUNK_MESSAGE_SELF2 = "你感到喝醉了。喔哦！";
DRUNK_MESSAGE_SELF3 = "你感觉喝醉了。喔哦！";
DRUNK_MESSAGE_SELF1 = "你再次感觉清醒。";
DUEL = "决斗";
DUEL_COUNTDOWN = "决斗开始：%d"; -- %d is the number of seconds until the beginning of the duel.
DUEL_OUTOFBOUNDS_TIMER = "正在离开决斗区域,你将在%d%s内失败。";
DUEL_REQUESTED = "%s向你发出决斗要求。";
DUEL_WINNER_KNOCKOUT = "%1$s在决斗中战胜了%2$s"; -- %1$s is the winner, %2$s is the loser
DUEL_WINNER_RETREAT = "%2$s在决斗中输给了%1$s"; -- %1$s is the winner, %2$s is the loser
DUNGEON_DIFFICULTY1 = "普通";
DUNGEON_DIFFICULTY3 = "精英";
DUNGEON_DIFFICULTY2 = "困难";
DURABILITYDAMAGE_DEATH = "你所装备的物品遭受了10%%的耐久度损失。";
DURABILITY_TEMPLATE = "耐久度 %d / %d"; -- Item's durability display template
EDIT_TICKET = "改动存盘";
EFFECTS_LABEL = "效果：";
ELITE = "精英";
EMBLEM_BACKGROUND = "底色";
EMBLEM_BORDER = "镶边";
EMBLEM_BORDER_COLOR = "镶边颜色";
EMBLEM_SYMBOL = "图标";
EMBLEM_SYMBOL_COLOR = "图标颜色";
EMOTE100_CMD2 = "/疲倦";
EMOTE100_CMD1 = "/tired";
EMOTE101_CMD2 = "/victory";
EMOTE101_CMD = "/victory";
EMOTE101_CMD3 = "/victory";
EMOTE101_CMD1 = "/胜利";
EMOTE102_CMD2 = "/wave";
EMOTE102_CMD1 = "/招手";
EMOTE103_CMD2 = "/welcome";
EMOTE103_CMD1 = "/欢迎";
EMOTE104_CMD2 = "/whine";
EMOTE104_CMD1 = "/哀诉";
EMOTE105_CMD1 = "/口哨";
EMOTE105_CMD2 = "/whistle";
EMOTE106_CMD2 = "/work";
EMOTE106_CMD1 = "/工作";
EMOTE107_CMD2 = "/哈欠";
EMOTE107_CMD3 = "/yawn";
EMOTE107_CMD1 = "/yawn";
EMOTE108_CMD2 = "/犹豫";
EMOTE108_CMD1 = "/boggle";
EMOTE109_CMD2 = "/calm";
EMOTE109_CMD1 = "/冷静";
EMOTE109_CMD3 = "/冷静";
EMOTE10_CMD2 = "/流血";
EMOTE10_CMD4 = "/blood";
EMOTE10_CMD3 = "/bleed";
EMOTE10_CMD1 = "/流血";
EMOTE110_CMD2 = "/cold";
EMOTE110_CMD1 = "/寒冷";
EMOTE111_CMD2 = "/comfort";
EMOTE111_CMD1 = "/安慰";
EMOTE112_CMD2 = "/spoon";
EMOTE112_CMD4 = "/被拥抱";
EMOTE112_CMD3 = "/cuddle";
EMOTE112_CMD1 = "/拥抱";
EMOTE113_CMD2 = "/duck";
EMOTE113_CMD1 = "/闪避";
EMOTE114_CMD3 = "/凌辱";
EMOTE114_CMD1 = "/insult";
EMOTE114_CMD2 = "/insult";
EMOTE115_CMD2 = "/introduce";
EMOTE115_CMD1 = "/介绍";
EMOTE116_CMD2 = "/jk";
EMOTE116_CMD1 = "/玩笑";
EMOTE117_CMD2 = "/lick";
EMOTE117_CMD1 = "/舔";
EMOTE118_CMD2 = "/listen";
EMOTE118_CMD1 = "/聆听";
EMOTE119_CMD2 = "/lost";
EMOTE119_CMD1 = "/失落";
EMOTE11_CMD2 = "/blink";
EMOTE11_CMD1 = "/眨眼";
EMOTE120_CMD2 = "/mock";
EMOTE120_CMD1 = "/嘲笑";
EMOTE121_CMD2 = "/ponder";
EMOTE121_CMD1 = "/考虑";
EMOTE122_CMD2 = "/pounce";
EMOTE122_CMD1 = "/冲";
EMOTE123_CMD1 = "/表扬";
EMOTE123_CMD3 = "/praise";
EMOTE123_CMD4 = "/lavish";
EMOTE123_CMD2 = "/慷慨";
EMOTE124_CMD2 = "/purr";
EMOTE124_CMD1 = "/咕噜";
EMOTE125_CMD2 = "/puzzled";
EMOTE125_CMD1 = "/迷惑";
EMOTE126_CMD4 = "/volunteer";
EMOTE126_CMD1 = "/伸手";
EMOTE126_CMD3 = "/raise";
EMOTE126_CMD2 = "/志愿";
EMOTE127_CMD3 = "/ready";
EMOTE127_CMD2 = "/rdy";
EMOTE127_CMD4 = "/rdy";
EMOTE127_CMD1 = "/就绪";
EMOTE128_CMD2 = "/shimmy";
EMOTE128_CMD1 = "/摇晃";
EMOTE129_CMD2 = "/shiver";
EMOTE129_CMD1 = "/颤抖";
EMOTE12_CMD1 = "/羞愧";
EMOTE12_CMD2 = "/blush";
EMOTE130_CMD3 = "/shoo";
EMOTE130_CMD2 = "/pest";
EMOTE130_CMD4 = "/pest";
EMOTE130_CMD1 = "/嘘声";
EMOTE131_CMD2 = "/slap";
EMOTE131_CMD1 = "/耳光";
EMOTE132_CMD1 = "/傻笑";
EMOTE132_CMD2 = "/smirk";
EMOTE133_CMD2 = "/sniff";
EMOTE133_CMD1 = "/吸气";
EMOTE134_CMD2 = "/snub";
EMOTE134_CMD1 = "/斥责";
EMOTE135_CMD2 = "/soothe";
EMOTE135_CMD1 = "/安抚";
EMOTE136_CMD3 = "/stink";
EMOTE136_CMD2 = "/嗅";
EMOTE136_CMD4 = "/smell";
EMOTE136_CMD1 = "/臭味";
EMOTE137_CMD1 = "/嘲讽";
EMOTE137_CMD2 = "/taunt";
EMOTE138_CMD2 = "/tease";
EMOTE138_CMD1 = "/戏弄";
EMOTE139_CMD2 = "/thirsty";
EMOTE139_CMD1 = "/干渴";
EMOTE13_CMD2 = "/咚";
EMOTE13_CMD1 = "/敲击";
EMOTE13_CMD3 = "/bonk";
EMOTE13_CMD4 = "/doh";
EMOTE140_CMD2 = "/veto";
EMOTE140_CMD1 = "/否决";
EMOTE141_CMD2 = "/snicker";
EMOTE141_CMD1 = "/窃笑";
EMOTE142_CMD1 = "/胳肢";
EMOTE142_CMD2 = "/tickle";
EMOTE143_CMD2 = "/stand";
EMOTE143_CMD1 = "/站立";
EMOTE144_CMD2 = "/violin";
EMOTE144_CMD1 = "/低泣";
EMOTE145_CMD2 = "/smile";
EMOTE145_CMD1 = "/微笑";
EMOTE146_CMD1 = "/粗鲁";
EMOTE146_CMD2 = "/rasp";
EMOTE147_CMD1 = "/怒号";
EMOTE147_CMD2 = "/growl";
EMOTE148_CMD1 = "/狂哮";
EMOTE148_CMD2 = "/bark";
EMOTE149_CMD1 = "/怜悯";
EMOTE149_CMD2 = "/pity";
EMOTE14_CMD2 = "/bored";
EMOTE14_CMD1 = "/无聊";
EMOTE150_CMD2 = "/scared";
EMOTE150_CMD1 = "/恐慌";
EMOTE151_CMD2 = "/flop";
EMOTE151_CMD1 = "/摔落";
EMOTE152_CMD2 = "/love";
EMOTE152_CMD1 = "/爱";
EMOTE153_CMD2 = "/moo";
EMOTE153_CMD1 = "/嗼";
EMOTE154_CMD1 = "/赞扬";
EMOTE154_CMD2 = "/commend";
EMOTE155_CMD2 = "/train";
EMOTE155_CMD1 = "/火车";
EMOTE156_CMD1 = "/呼救";
EMOTE156_CMD2 = "/helpme";
EMOTE157_CMD1 = "/敌人";
EMOTE157_CMD2 = "/incoming";
EMOTE158_CMD1 = "/开火";
EMOTE158_CMD2 = "/openfire";
EMOTE159_CMD1 = "/冲锋";
EMOTE159_CMD2 = "/charge";
EMOTE15_CMD1 = "/弹跳";
EMOTE15_CMD2 = "/bounce";
EMOTE160_CMD1 = "/逃跑";
EMOTE160_CMD2 = "/flee";
EMOTE161_CMD1 = "/攻击目标";
EMOTE161_CMD2 = "/attacktarget";
EMOTE162_CMD1 = "/魔法耗尽";
EMOTE162_CMD2 = "/oom";
EMOTE163_CMD1 = "/跟着我";
EMOTE163_CMD2 = "/followme";
EMOTE164_CMD1 = "/等等";
EMOTE164_CMD2 = "/wait";
EMOTE165_CMD1 = "/调情";
EMOTE165_CMD2 = "/flirt";
EMOTE166_CMD1 = "/治疗我";
EMOTE166_CMD2 = "/healme";
EMOTE167_CMD1 = "/笑话";
EMOTE167_CMD2 = "/silly";
EMOTE168_CMD1 = "/wink";
EMOTE168_CMD2 = "/wink";
EMOTE169_CMD1 = "/pat";
EMOTE169_CMD2 = "/pat";
EMOTE16_CMD2 = "/brb";
EMOTE16_CMD1 = "/马上回来";
EMOTE170_CMD1 = "/golfclap";
EMOTE170_CMD2 = "/golfclap";
EMOTE17_CMD2 = "/bow";
EMOTE17_CMD1 = "/鞠躬";
EMOTE18_CMD1 = "/打嗝";
EMOTE18_CMD3 = "/burp";
EMOTE18_CMD4 = "/belch";
EMOTE18_CMD2 = "/belch";
EMOTE19_CMD1 = "/再见";
EMOTE19_CMD2 = "/goodbye";
EMOTE19_CMD4 = "/bye";
EMOTE19_CMD6 = "/farewell";
EMOTE19_CMD5 = "/goodbye";
EMOTE19_CMD3 = "/再会";
EMOTE1_CMD2 = "/agree";
EMOTE1_CMD1 = "/赞同";
EMOTE20_CMD2 = "/cackle";
EMOTE20_CMD1 = "/咯咯笑";
EMOTE21_CMD2 = "/cheer";
EMOTE21_CMD1 = "/欢呼";
EMOTE22_CMD4 = "/chicken";
EMOTE22_CMD1 = "/小鸡";
EMOTE22_CMD6 = "/strut";
EMOTE22_CMD5 = "/flap";
EMOTE22_CMD2 = "/flap";
EMOTE22_CMD3 = "/strut";
EMOTE23_CMD1 = "/chuckle";
EMOTE23_CMD2 = "/chuckle";
EMOTE24_CMD1 = "/拍手";
EMOTE24_CMD2 = "/clap";
EMOTE25_CMD1 = "/糊涂";
EMOTE25_CMD2 = "/confused";
EMOTE26_CMD2 = "/祝贺";
EMOTE26_CMD3 = "/cong";
EMOTE26_CMD1 = "/祝贺";
EMOTE26_CMD5 = "/congrats";
EMOTE26_CMD4 = "/congratulate";
EMOTE26_CMD6 = "/cong";
EMOTE27_CMD2 = "/unused";
EMOTE27_CMD1 = "/unused";
EMOTE28_CMD1 = "/cough";
EMOTE28_CMD2 = "/咳嗽";
EMOTE29_CMD1 = "/退缩";
EMOTE29_CMD2 = "/恐惧";
EMOTE29_CMD3 = "/cower";
EMOTE29_CMD4 = "/fear";
EMOTE2_CMD2 = "/amaze";
EMOTE2_CMD1 = "/惊奇";
EMOTE30_CMD3 = "/crack";
EMOTE30_CMD1 = "/关节";
EMOTE30_CMD4 = "/knuckles";
EMOTE30_CMD2 = "/knuckles";
EMOTE31_CMD1 = "/畏缩";
EMOTE31_CMD2 = "/cringe";
EMOTE32_CMD4 = "/cry";
EMOTE32_CMD5 = "/sob";
EMOTE32_CMD2 = "/抽泣";
EMOTE32_CMD3 = "/哭泣";
EMOTE32_CMD6 = "/weep";
EMOTE32_CMD1 = "/哭泣";
EMOTE33_CMD1 = "/好奇";
EMOTE33_CMD2 = "/curious";
EMOTE34_CMD1 = "/屈膝";
EMOTE34_CMD2 = "/curtsey";
EMOTE35_CMD2 = "/dance";
EMOTE35_CMD1 = "/跳舞";
EMOTE36_CMD1 = "/drink";
EMOTE36_CMD4 = "/shindig";
EMOTE36_CMD2 = "/干杯";
EMOTE36_CMD3 = "/喝酒";
EMOTE37_CMD1 = "/流口水";
EMOTE37_CMD2 = "/drool";
EMOTE38_CMD4 = "/eat";
EMOTE38_CMD3 = "/feast";
EMOTE38_CMD2 = "/吃饭";
EMOTE38_CMD6 = "/feast";
EMOTE38_CMD5 = "/chew";
EMOTE38_CMD1 = "/吃饭";
EMOTE39_CMD1 = "/眯眼";
EMOTE39_CMD2 = "/eye";
EMOTE3_CMD3 = "/angry";
EMOTE3_CMD4 = "/mad";
EMOTE3_CMD2 = "/疯狂";
EMOTE3_CMD1 = "/愤怒";
EMOTE40_CMD1 = "/放屁";
EMOTE40_CMD2 = "/fart";
EMOTE41_CMD1 = "/坐立不安";
EMOTE41_CMD2 = "/不耐烦";
EMOTE41_CMD3 = "/fidget";
EMOTE41_CMD4 = "/impatient";
EMOTE42_CMD3 = "/flex";
EMOTE42_CMD1 = "/强壮";
EMOTE42_CMD2 = "/strong";
EMOTE42_CMD4 = "/strong";
EMOTE43_CMD3 = "/失望";
EMOTE43_CMD1 = "/皱眉";
EMOTE43_CMD2 = "/失望";
EMOTE43_CMD4 = "/frown";
EMOTE43_CMD5 = "/disappointed";
EMOTE43_CMD6 = "/disappointment";
EMOTE44_CMD3 = "/喘气";
EMOTE44_CMD1 = "/gasp";
EMOTE44_CMD2 = "/gasp";
EMOTE45_CMD2 = "/凝视";
EMOTE45_CMD1 = "/gaze";
EMOTE46_CMD2 = "/giggle";
EMOTE46_CMD1 = "/giggle";
EMOTE47_CMD1 = "/瞪眼";
EMOTE47_CMD2 = "/glare";
EMOTE48_CMD1 = "/幸灾乐祸";
EMOTE48_CMD2 = "/gloat";
EMOTE49_CMD1 = "/greet";
EMOTE49_CMD4 = "/greetings";
EMOTE49_CMD3 = "/greet";
EMOTE49_CMD2 = "/greetings";
EMOTE4_CMD3 = "/apologize";
EMOTE4_CMD1 = "/抱歉";
EMOTE4_CMD4 = "/抱歉";
EMOTE4_CMD2 = "/sorry";
EMOTE50_CMD3 = "/顽皮";
EMOTE50_CMD2 = "/顽皮";
EMOTE50_CMD4 = "/grin";
EMOTE50_CMD6 = "/wickedly";
EMOTE50_CMD5 = "/wicked";
EMOTE50_CMD1 = "/咧嘴笑";
EMOTE51_CMD1 = "/呻吟";
EMOTE51_CMD2 = "/groan";
EMOTE52_CMD3 = "/卑微";
EMOTE52_CMD1 = "/grovel";
EMOTE52_CMD2 = "/苦工";
EMOTE52_CMD4 = "/peon";
EMOTE53_CMD2 = "/guffaw";
EMOTE53_CMD1 = "/大笑";
EMOTE54_CMD2 = "/hail";
EMOTE54_CMD1 = "/hail";
EMOTE55_CMD3 = "/高兴";
EMOTE55_CMD4 = "/happy";
EMOTE55_CMD6 = "/yay";
EMOTE55_CMD2 = "/高兴";
EMOTE55_CMD1 = "/高兴";
EMOTE55_CMD5 = "/glad";
EMOTE56_CMD1 = "/你好";
EMOTE56_CMD2 = "/hi";
EMOTE56_CMD4 = "/你好";
EMOTE56_CMD3 = "/hello";
EMOTE57_CMD2 = "/hug";
EMOTE57_CMD1 = "/拥抱";
EMOTE58_CMD5 = "/pizza";
EMOTE58_CMD6 = "/pizza";
EMOTE58_CMD1 = "/饥饿";
EMOTE58_CMD4 = "/food";
EMOTE58_CMD3 = "/hungry";
EMOTE58_CMD2 = "/食物";
EMOTE59_CMD3 = "/kiss";
EMOTE59_CMD1 = "/亲吻";
EMOTE59_CMD2 = "/飞吻";
EMOTE59_CMD4 = "/blow";
EMOTE5_CMD4 = "/applaud";
EMOTE5_CMD3 = "/applause";
EMOTE5_CMD6 = "/欢呼";
EMOTE5_CMD1 = "/鼓掌";
EMOTE5_CMD5 = "/喝彩";
EMOTE5_CMD2 = "/bravo";
EMOTE60_CMD2 = "/kneel";
EMOTE60_CMD3 = "/kneel";
EMOTE60_CMD1 = "/下跪";
EMOTE61_CMD3 = "/laugh";
EMOTE61_CMD4 = "/lol";
EMOTE61_CMD1 = "/大笑";
EMOTE61_CMD2 = "/lol";
EMOTE62_CMD7 = "/lay";
EMOTE62_CMD4 = "/lie";
EMOTE62_CMD5 = "/laydown";
EMOTE62_CMD1 = "/躺下";
EMOTE62_CMD2 = "/liedown";
EMOTE62_CMD3 = "/lay";
EMOTE62_CMD6 = "/liedown";
EMOTE62_CMD8 = "/lie";
EMOTE63_CMD2 = "/massage";
EMOTE63_CMD1 = "/按摩";
EMOTE64_CMD1 = "/哀号";
EMOTE64_CMD2 = "/moan";
EMOTE65_CMD2 = "/moon";
EMOTE65_CMD1 = "/屁股";
EMOTE66_CMD2 = "/mourn";
EMOTE66_CMD1 = "/哀悼";
EMOTE67_CMD2 = "/no";
EMOTE67_CMD1 = "/不";
EMOTE68_CMD3 = "/nod";
EMOTE68_CMD1 = "/点头";
EMOTE68_CMD2 = "/yes";
EMOTE68_CMD4 = "/yes";
EMOTE69_CMD1 = "/抬鼻";
EMOTE69_CMD2 = "/pick";
EMOTE69_CMD4 = "/挖鼻孔";
EMOTE69_CMD3 = "/nosepick";
EMOTE6_CMD1 = "/害臊";
EMOTE6_CMD2 = "/bashful";
EMOTE70_CMD2 = "/惊恐";
EMOTE70_CMD1 = "/panic";
EMOTE71_CMD2 = "/peer";
EMOTE71_CMD1 = "/观察";
EMOTE72_CMD2 = "/恳求";
EMOTE72_CMD1 = "/plead";
EMOTE73_CMD2 = "/point";
EMOTE73_CMD1 = "/指点";
EMOTE74_CMD1 = "/戳";
EMOTE74_CMD2 = "/poke";
EMOTE75_CMD2 = "/pray";
EMOTE75_CMD1 = "/祈祷";
EMOTE76_CMD2 = "/roar";
EMOTE76_CMD1 = "/咆哮";
EMOTE77_CMD2 = "/rofl";
EMOTE77_CMD1 = "/狂笑";
EMOTE78_CMD2 = "/rude";
EMOTE78_CMD1 = "/侮辱";
EMOTE79_CMD2 = "/salute";
EMOTE79_CMD1 = "/敬礼";
EMOTE7_CMD2 = "/招手";
EMOTE7_CMD1 = "/beckon";
EMOTE80_CMD4 = "/scratch";
EMOTE80_CMD5 = "/cat";
EMOTE80_CMD1 = "/瘙痒";
EMOTE80_CMD3 = "/catty";
EMOTE80_CMD2 = "/猫咪";
EMOTE80_CMD6 = "/catty";
EMOTE81_CMD2 = "/性感";
EMOTE81_CMD1 = "/sexy";
EMOTE82_CMD3 = "/shake";
EMOTE82_CMD1 = "/摇摆";
EMOTE82_CMD2 = "/rear";
EMOTE82_CMD4 = "/扭屁股";
EMOTE83_CMD1 = "/shout";
EMOTE84_CMD2 = "/shrug";
EMOTE84_CMD1 = "/耸肩";
EMOTE85_CMD2 = "/shy";
EMOTE85_CMD1 = "/害羞";
EMOTE86_CMD2 = "/sigh";
EMOTE86_CMD1 = "/叹气";
EMOTE87_CMD2 = "/sit";
EMOTE87_CMD1 = "/坐下";
EMOTE88_CMD2 = "/sleep";
EMOTE88_CMD1 = "/睡觉";
EMOTE89_CMD2 = "/snarl";
EMOTE89_CMD1 = "/怒骂";
EMOTE8_CMD2 = "/beg";
EMOTE8_CMD1 = "/乞求";
EMOTE90_CMD2 = "/吐口水";
EMOTE90_CMD1 = "/spit";
EMOTE91_CMD2 = "/stare";
EMOTE91_CMD1 = "/凝视";
EMOTE92_CMD2 = "/surprised";
EMOTE92_CMD1 = "/惊奇";
EMOTE93_CMD2 = "/surrender";
EMOTE93_CMD1 = "/头像";
EMOTE94_CMD2 = "/talk";
EMOTE94_CMD1 = "/谈话";
EMOTE95_CMD3 = "/talkex";
EMOTE95_CMD1 = "/交谈";
EMOTE95_CMD2 = "/激动";
EMOTE95_CMD4 = "/excited";
EMOTE96_CMD3 = "/领悟";
EMOTE96_CMD4 = "/question";
EMOTE96_CMD2 = "/疑问";
EMOTE96_CMD1 = "/talkq";
EMOTE97_CMD2 = "/tap";
EMOTE97_CMD1 = "/掂脚";
EMOTE98_CMD4 = "/thank";
EMOTE98_CMD2 = "/感谢";
EMOTE98_CMD1 = "/感谢";
EMOTE98_CMD6 = "/ty";
EMOTE98_CMD5 = "/thanks";
EMOTE98_CMD3 = "/ty";
EMOTE99_CMD1 = "/威胁";
EMOTE99_CMD8 = "/wrath";
EMOTE99_CMD6 = "/doom";
EMOTE99_CMD4 = "/愤怒";
EMOTE99_CMD3 = "/威胁";
EMOTE99_CMD2 = "/末日";
EMOTE99_CMD5 = "/threaten";
EMOTE99_CMD7 = "/threat";
EMOTE9_CMD1 = "/bite";
EMOTE9_CMD2 = "/撕咬";
EMOTE_MESSAGE = "表情"; -- Text emote
EMOTE_STATE_KNEEL = "/kneel";
EMPTY = "空"; -- Generic "Empty" label
EMPTY_STABLE_SLOT = "空的兽栏位置";
ENABLE_ALL_SHADERS = "开启所有遮罩效果";
ENABLE_ALL_SOUND = "开启所有音效";
ENABLE_AMBIENCE = "开启环境音效";
ENABLE_EMOTE_SOUNDS = "开启表情音效";
ENABLE_ERROR_SPEECH = "开启错误提示"; -- Sound option tag
ENABLE_GROUP_SPEECH = "开启队伍聊天"; -- Sound option tag
ENABLE_MUSIC = "开启音乐"; -- Sound options tag
ENABLE_MUSIC_LOOPING = "音乐循环";
ENABLE_SOUND = "开启声效"; -- Sound options tag
ENABLE_SOUND_AT_CHARACTER = "开启人物声音";
ENABLE_TUTORIAL_TEXT = "继续教程";
ENCHANT_SLOT = "附魔/开锁栏";
ENCLOSED_MONEY = "已附加的金额";
ENERGY = "能量"; -- Generic "energy"
ENERGY_COST = "%d能量";
ENERGY_COST_PER_TIME = "%d能量，外加每秒%d";
ENERGY_POINTS = "能量";
ENSCRIBE = "附魔";
ENTER_MACRO_LABEL = "输入宏命令";
ENTER_BATTLE = "Enter Battle";
ENTER_CODE = "Please enter code:";
ENTER_MACRO_LABEL = "Enter Macro Commands:";
ENVIRONMENT_DETAIL = "环境细节";
EQUIP_CONTAINER = "装备容器"; -- Equip a container
EQUIP_NO_DROP = "装备之后，该物品将与你绑定。"; -- Tells player he is about to equip a no drop item
ERROR_CAPS = "错误"; -- Generic "error", in all capitals
ERR_2HANDED_EQUIPPED = "不能在使用双手武器的同时装备这件物品。";
ERR_2HSKILLNOTFOUND = "你不能装备双武器";
ERR_ABILITY_COOLDOWN = "技能还没有准备好。";
ERR_ALREADY_INVITED_TO_GUILD_S = "%s已经加入了另一个公会。";
ERR_ALREADY_IN_GROUP_S = "%s已经加入了别的队伍";
ERR_ALREADY_IN_GUILD = "你已经加入了一个公会";
ERR_ALREADY_IN_GUILD_S = "%s已经加入了一个公会";
ERR_ALREADY_TRADING = "你已经在交易了";
ERR_AMMO_ONLY = "这里只能放置弹药";
ERR_APPROACHING_NO_PLAY_TIME = "%s后进入疲劳游戏时间，届时您将无法获得任何经验值和战利品，必须下线累计5小时才能恢复正常状态。"; -- Sent every half hour in tired time
ERR_APPROACHING_PARTIAL_PLAY_TIME = "您的健康游戏时间还剩余%s。您的游戏获益将被减半。"; -- Sent every hour in normal time, plus once more at 15 minutes remaining
ERR_ATTACK_CHARMED = "当被魅惑的时候无法攻击。";
ERR_ATTACK_CONFUSED = "无法在混乱状态下攻击。";
ERR_ATTACK_DEAD = "你不能在死亡状态的时候攻击";
ERR_ATTACK_FLEEING = "无法在逃跑状态下攻击。";
ERR_ATTACK_MOUNTED = "无法在骑乘时攻击。";
ERR_ATTACK_PACIFIED = "无法在平静状态下攻击。";
ERR_ATTACK_PREVENTED_BY_MECHANIC_S = "在%s状态下无法攻击。"; -- %s is the mechanic state that prevented the action (e.g. disoriented, frozen, etc.)
ERR_ATTACK_STUNNED = "无法在昏迷状态下攻击。";
ERR_AUCTION_BAG = "你不能出售非空的背包";
ERR_AUCTION_BID_INCREMENT = "你的提价幅度太小了。";
ERR_AUCTION_BID_OWN = "你不能对自己拍卖的物品竞标";
ERR_AUCTION_BID_PLACED = "竞标已被接受。";
ERR_AUCTION_BOUND_ITEM = "你不能出售灵魂绑定的物品";
ERR_AUCTION_CONJURED_ITEM = "你不能拍卖魔法制造的物品";
ERR_AUCTION_DATABASE_ERROR = "内部拍卖错误";
ERR_AUCTION_EXPIRED_S = "你拍卖的%s已经过期。"; -- %s is the item name
ERR_AUCTION_HIGHER_BID = "已经有更高的出价了。";
ERR_AUCTION_LOOT_ITEM = "你不能拍卖一件可拾取的物品";
ERR_AUCTION_MIN_BID = "你的出价必须不低于最低竞标价格。";
ERR_AUCTION_OUTBID_S = "你对%s的出价被人超过了。"; -- %s is the item name
ERR_AUCTION_QUEST_ITEM = "你不能出售任务物品";
ERR_AUCTION_REMOVED = "拍卖已取消。";
ERR_AUCTION_REMOVED_S = "你竞标的%s被卖家取消。";
ERR_AUCTION_REPAIR_ITEM = "你必须先修理该物品才能将其拍卖。";
ERR_AUCTION_SOLD_S = "你拍卖的%s已经售出。"; -- %s is the item name
ERR_AUCTION_STARTED = "已开始拍卖。";
ERR_AUCTION_WON_S = "你赢得了对%s的竞标"; -- %s is the item name
ERR_AUCTION_WRAPPED_ITEM = "你不能拍卖一件打包的物品";
ERR_AUTOFOLLOW_TOO_FAR = "目标距离太远";
ERR_AUTOLOOT_MONEY_S = "你捡起了%s";
ERR_BADATTACKFACING = "你面朝错误的方向！"; -- Melee combat error
ERR_BADATTACKPOS = "你距离太远！"; -- Melee combat error
ERR_BAD_PLAYER_NAME_S = "无法找到'%s'"; -- %s is the target's name
ERR_BAG_FULL = "这个背包已经满了";
ERR_BAG_IN_BAG = "不能将非空的背包放在别的背包中";
ERR_BANKSLOT_FAILED_TOO_MANY = "你的背包栏位已经全部占满了！";
ERR_BANKSLOT_INSUFFICIENT_FUNDS = "你买不起这个物品。";
ERR_BANKSLOT_NOTBANKER = "目标不是银行职员！";
ERR_BANK_FULL = "你的银行已满";
ERR_BG_PLAYER_JOINED_SS = "|Hplayer:%s|h[%s]|h加入了战斗";
ERR_BG_PLAYER_LEFT_S = "%s离开了战斗";
ERR_BUTTON_LOCKED = "这已经被使用了";
ERR_CANNOTCREATEDIRECTORY = "不能创建目录%s";
ERR_CANNOTCREATEFILE = "不能创建文件%s";
ERR_CANTATTACK_NOTSTANDING = "你必须处于站立状态下才能进行攻击！";
ERR_CANT_EQUIP_EVER = "你永远不能使用这个物品";
ERR_CANT_EQUIP_LEVEL_I = "你必须达到等级%d才能使用这件物品。"; -- %d is the required level
ERR_CANT_EQUIP_RANK = "你还未达到军阶要求";
ERR_CANT_EQUIP_REPUTATION = "你的声望还没有达到可以使用该物品的条件";
ERR_CANT_EQUIP_SKILL = "你现在还没有足够的技能使用这个物品";
ERR_CANT_INTERACT_SHAPESHIFTED = "不能够在变形状态对话";
ERR_CANT_STACK = "这件物品无法堆积";
ERR_CANT_SWAP = "这些物品无法交换";
ERR_CANT_USE_DISARMED = "你不能使用一件已被缴械的物品。"; -- This error message happens when you attempt to use an item that a monster has disarmed from you.
ERR_CANT_USE_ITEM = "你不能使用这件物品。";
ERR_CANT_WRAP_BAGS = "背包无法打包。";
ERR_CANT_WRAP_BOUND = "绑定的物品无法打包";
ERR_CANT_WRAP_EQUIPPED = "已经装备的物品不能被打包";
ERR_CANT_WRAP_STACKABLE = "可以堆积的物品不能被打包";
ERR_CANT_WRAP_UNIQUE = "唯一物品无法打包。";
ERR_CANT_WRAP_WRAPPED = "已经打包的物品不能再被打包";
ERR_CHAT_PLAYER_AMBIGUOUS_S = "%s：找到多个匹配玩家，请输入他们的服务器名以进一步查找";
ERR_CHAT_PLAYER_NOT_FOUND_S = "未找到名为“%s”的在线玩家。";
ERR_CHAT_RESTRICTED = "硬核玩家不能执行该动作。";
ERR_CHAT_WHILE_DEAD = "你无法在死亡状态下交谈！";
ERR_CHAT_WRONG_FACTION = "你只能够和你的盟友一方角色进行密语";
ERR_CHEST_IN_USE = "该物品正被使用";
ERR_CLICK_ON_ITEM_TO_FEED = "点击一个物品来喂养你的宠物";
ERR_CLIENT_LOCKED_OUT = "你还不能那样做。";
ERR_COMBAT_DAMAGE_SSI = "%s击中%s造成%d点伤害！"; -- Attacker, target, and damage
ERR_COMMAND_NEEDS_TARGET = "你必须指定一个目标：/<命令><目标名称>";
ERR_DEATHBINDALREADYBOUND = "你已经绑定在这里了！";
ERR_DEATHBIND_SUCCESS_S = "%s现在是你的家。";
ERR_DECLINE_GROUP_S = "%s拒绝了你邀请其加入队伍的请求。";
ERR_DESTROY_NONEMPTY_BAG = "只能使用空的背包";
ERR_DISMOUNT_NOPET = "内部错误，你没有可以解散的宠物";
ERR_DISMOUNT_NOTMOUNTED = "你现在不处于骑乘状态！";
ERR_DISMOUNT_NOTYOURPET = "内部错误，从一只非宠物身上下来";
ERR_DOOR_LOCKED = "门被锁住了。";
ERR_DROP_BOUND_ITEM = "你不能丢弃一件灵魂绑定的物品";
ERR_DUEL_CANCELLED = "决斗已取消。";
ERR_DUEL_REQUESTED = "你已经发出了决斗要求";
ERR_DUNGEON_DIFFICULTY_CHANGED_S = "地下城难度已设置为%s";
ERR_DUNGEON_DIFFICULTY_FAILED = "无法改变地下城难度";
ERR_EAT_WHILE_MOVNG = "你不能在移动中吃东西";
ERR_EMBLEMERROR_NOTABARDGEOSET = "先要恢复到正常状态！"; -- Displayed when attempting to create a guild emblem while in shapeshift form
ERR_EQUIP_TRADE_ITEM = "该物品正在被交易";
ERR_EXHAUSTION_EXHAUSTED = "你现在处于精疲力竭状态。";
ERR_EXHAUSTION_NORMAL = "你进入了正常休息状态。";
ERR_EXHAUSTION_RESTED = "你现在处于精神良好状态。";
ERR_EXHAUSTION_TIRED = "你现在处于疲倦乏力状态。";
ERR_EXHAUSTION_WELLRESTED = "你进入了精力充沛状态。";
ERR_FEIGN_DEATH_RESISTED = "抵抗";
ERR_FILTERING_YOU_S = "无法向%s发送信息，因为你的话语中带有屏蔽词。";
ERR_FISH_ESCAPED = "你的鱼逃走了！";
ERR_FISH_NOT_HOOKED = "没有鱼上钩";
ERR_FOOD_COOLDOWN = "你已经吃得太饱了。";
ERR_FRIEND_ADDED_S = "%s已被加入好友名单";
ERR_FRIEND_ALREADY_S = "%s已经在你的好友名单中了";
ERR_FRIEND_DB_ERROR = "好友搜寻系统数据库错误";
ERR_FRIEND_ERROR = "来自于服务器中未知好友的回音";
ERR_FRIEND_LIST_FULL = "你的好友列表已满。";
ERR_FRIEND_NOT_FOUND = "没有找到玩家。";
ERR_FRIEND_OFFLINE_S = "%s下线了。";
ERR_FRIEND_ONLINE_SS = "|Hplayer:%s|h[%s]|h上线了。"; -- The name is passed in twice to accomodate player name links - "|Hplayer:%s|h[%s]|h" should not be localized.
ERR_FRIEND_REMOVED_S = "%s已被从好友名单中删除";
ERR_FRIEND_SELF = "你不能把自己加入好友名单中";
ERR_FRIEND_WRONG_FACTION = "好友必须与你属于同一阵营。"; -- Player tried to set an enemy player as a friend
ERR_GENERIC_NO_TARGET = "你没有目标";
ERR_GENERIC_STUNNED = "你被击昏了"; -- Generic failure due to being stunned (not spell/loot)
ERR_GROUP_DISBANDED = "你的队伍已经解散。";
ERR_GROUP_FULL = "你的队伍已经满了";
ERR_GROUP_JOIN_BATTLEGROUND_DESERTERS = "你无法加入战场，因为你或你的某个队友被标记为逃亡者。";
ERR_GROUP_JOIN_BATTLEGROUND_FAIL = "你所在的小队加入了一个战场序列，但你不符合条件";
ERR_GROUP_JOIN_BATTLEGROUND_S = "你所在的小队加入了%s的等待队列";
ERR_GROUP_JOIN_BATTLEGROUND_TOO_MANY = "你的小队人数太多，无法加入该战场";
ERR_GUILDEMBLEM_COLORSPRESENT = "你的公会已经有了徽章！";
ERR_GUILDEMBLEM_INVALIDVENDOR = "这不是一个徽章出售者！";
ERR_GUILDEMBLEM_INVALID_TABARD_COLORS = "无效的公会徽章颜色";
ERR_GUILDEMBLEM_NOGUILD = "你现在不属于任何一个公会！";
ERR_GUILDEMBLEM_NOTENOUGHMONEY = "你没有足够的钱。";
ERR_GUILDEMBLEM_NOTGUILDMASTER = "只有公会首领可以创建徽章";
ERR_GUILDEMBLEM_SAME = "未存储，你的公会徽章没有变更。";
ERR_GUILDEMBLEM_SUCCESS = "公会徽章已经保存";
ERR_GUILD_ACCEPT = "你已经加入公会";
ERR_GUILD_CREATE_S = "%s已经创建";
ERR_GUILD_DECLINE_S = "%s拒绝了你邀请其加入公会的请求。";
ERR_GUILD_DEMOTE_SSS = "%s将%s降职为%s。"; -- demoter's name, player's name, new rank
ERR_GUILD_DISBANDED = "公会已经被解散";
ERR_GUILD_DISBAND_S = "%s解散了公会。"; -- %s is the guild leader
ERR_GUILD_DISBAND_SELF = "你解散了公会";
ERR_GUILD_ENTER_NAME = "请为你的公会输入一个名字。";
ERR_GUILD_FOUNDER_S = "恭喜你，你现在是%s的一员了！";
ERR_GUILD_INTERNAL = "内部公会错误";
ERR_GUILD_INVITE_S = "你邀请%s加入你的公会";
ERR_GUILD_JOIN_S = "%s加入了公会。"; -- %s is the player's name
ERR_GUILD_LEADER_CHANGED_SS = "%s让%s成为新的公会首领";
ERR_GUILD_LEADER_IS_S = "%s现在是你们公会的首领。";
ERR_GUILD_LEADER_LEAVE = "你必须在离开公会之前使用/gpromote命令指定一个新的公会首领。";
ERR_GUILD_LEADER_S = "%s成为公会首领。";
ERR_GUILD_LEADER_SELF = "你现在是公会首领";
ERR_GUILD_LEAVE_RESULT = "你离开了公会";
ERR_GUILD_LEAVE_S = "%s离开了公会。"; -- %s is the player's name
ERR_GUILD_NAME_EXISTS_S = "已经存在名为“\%s\”的公会。";
ERR_GUILD_NAME_INVALID = "名字包括非法字符，请输入一个新的名字。";
ERR_GUILD_NAME_INVALID_SPACE = "公会名称无法以空格开始或结束。请输入一个新名称。";
ERR_GUILD_NAME_MIXED_LANGUAGES = "名字中只能够包含一种语言。请输入一个新的名字。";
ERR_GUILD_NAME_NAME_CONSECUTIVE_SPACES = "无法使用连续的空格。请输入一个新名称。";
ERR_GUILD_NAME_PROFANE = "这个名字含有亵渎性内容。请输入一个新的名字。";
ERR_GUILD_NAME_RESERVED = "这个名字已经被保留。请输入一个新的名字。";
ERR_GUILD_NAME_TOO_SHORT = "这个名字太短了，请输入一个新的名字。";
ERR_GUILD_NOT_ALLIED = "你不能邀请敌对势力的玩家加入你的公会";
ERR_GUILD_PERMISSIONS = "你没有权利这样做";
ERR_GUILD_PLAYER_NOT_FOUND_S = "\"%s\"没有找到";
ERR_GUILD_PLAYER_NOT_IN_GUILD = "你现在没有加入任何一个公会";
ERR_GUILD_PLAYER_NOT_IN_GUILD_S = "%s不在你的公会中";
ERR_GUILD_PROMOTE_SSS = "%s将%s晋升为%s。"; -- promoter's name, player's name, new rank
ERR_GUILD_QUIT_S = "现在你不再是%s的一员";
ERR_GUILD_RANKS_LOCKED = "临时性公会错误。请重试！";
ERR_GUILD_RANK_IN_USE = "这个公会等级目前正在使用中";
ERR_GUILD_RANK_TOO_HIGH_S = "%s的级别太高了";
ERR_GUILD_RANK_TOO_LOW_S = "%s的会阶已经是最低了。";
ERR_GUILD_REMOVE_SELF = "你被开除出公会";
ERR_GUILD_REMOVE_SS = "%s被%s开除出公会。"; -- %s is the player's name, then remover's name
ERR_IGNORE_ADDED_S = "%s现在被加入屏蔽名单了";
ERR_IGNORE_ALREADY_S = "%s已经被你加入屏蔽名单了";
ERR_IGNORE_AMBIGUOUS = "该名字不够明确，请输入该玩家的服务器名。";
ERR_IGNORE_FULL = "你的屏蔽列表已满";
ERR_IGNORE_NOT_FOUND = "没有找到玩家。";
ERR_IGNORE_REMOVED_S = "%s已被取消屏蔽。";
ERR_IGNORE_SELF = "你不能屏蔽自己";
ERR_IGNORING_YOU_S = "%s已将你屏蔽。";
ERR_INITIATE_TRADE_S = "你对%s发出交易请求"; -- %s is your trade target
ERR_INSPECT_S = "%s正在观察你。";
ERR_INTERNAL_BAG_ERROR = "内部包裹错误";
ERR_INVALID_ATTACK_TARGET = "你不能攻击那个目标";
ERR_INVALID_FOLLOW_TARGET = "你不能跟随这个目标";
ERR_INVALID_INSPECT_TARGET = "你无法观察那个单位。";
ERR_INVALID_ITEM_TARGET = "这个物品是个无效的目标";
ERR_INVALID_PETNAME = "错误：无效的宠物名称。";
ERR_INVALID_PROMOTION_CODE = "无法验证号码，请重试。";
ERR_INVALID_RAID_TARGET = "目标无效，为敌方玩家";
ERR_INVITED_TO_GROUP_S = "%s邀请你加入队伍。"; -- %s is the inviter's name
ERR_INVITED_TO_GUILD = "你已经被邀请加入了一个公会";
ERR_INVITED_TO_GUILD_SS = "%s邀请你加入%s。"; -- %s's are the inviter's name and the guild name respectively
ERR_INVITE_PLAYER_S = "你邀请%s加入你的队伍"; -- %s is the target player's name
ERR_INVITE_SELF = "你不能邀请自己加入一个队伍";
ERR_INV_FULL = "物品栏已满。";
ERR_ITEM_CANT_BE_DESTROYED = "这件物品无法被摧毁。";
ERR_ITEM_COOLDOWN = "物品还没有准备好。";
ERR_ITEM_LOCKED = "物品被锁住了。";
ERR_ITEM_MAX_COUNT = "你不能携带更多此类物品。";
ERR_ITEM_NOT_FOUND = "未找到指定物品";
ERR_JOINED_GROUP_S = "%s加入了队伍。"; -- %s is the player's name
ERR_KILLED_BY_S = "你被%s杀死了。";
ERR_LEARN_ABILITY_S = "你学会新的技能：%s"; -- %s is the ability name
ERR_LEARN_RECIPE_S = "你已经学会了制作新物品：%s"; -- %s is the recipe name
ERR_LEARN_SPELL_S = "你学会新的法术：%s"; -- %s is the spell name
ERR_LEFT_GROUP_S = "%s离开了队伍。";
ERR_LEFT_GROUP_YOU = "你离开了队伍。";
ERR_LOGGING_OUT = "你正在登出";
ERR_LOGOUT_FAILED = "你不能在无法坐下的情况下登出";
ERR_LOOT_BAD_FACING = "你必须面向尸体才能进行搜索。";
ERR_LOOT_CANT_LOOT_THAT_NOW = "你现在不能拾取那件物品。";
ERR_LOOT_DIDNT_KILL = "你没有被允许搜索这个尸体";
ERR_LOOT_GONE = "物品已经被拾取";
ERR_LOOT_LOCKED = "有人已经正在搜索尸体了";
ERR_LOOT_MASTER_INV_FULL = "该玩家的物品栏已满"; -- Master looter error
ERR_LOOT_MASTER_OTHER = "无法将物品分配给该玩家"; -- Master looter error, "unknown" reason
ERR_LOOT_MASTER_UNIQUE_ITEM = "该玩家已经有太多同类物品了"; -- Master looter error for unique items
ERR_LOOT_NOTSTANDING = "你必须处于站立状态下才能进行搜索！";
ERR_LOOT_NO_UI = "现在不能搜索尸体";
ERR_LOOT_PLAYER_NOT_FOUND = "没有找到玩家";
ERR_LOOT_ROLL_PENDING = "这个物品还在等待掷骰子决定归属";
ERR_LOOT_STUNNED = "你不能在晕眩状态进行搜索！";
ERR_LOOT_TOO_FAR = "你离尸体太远，无法进行搜索";
ERR_LOOT_WHILE_INVULNERABLE = "不能在无敌状态下搜索尸体";
ERR_MAIL_BAG = "你不能邮寄非空的背包";
ERR_MAIL_BOUND_ITEM = "你不能邮寄灵魂绑定的物品。";
ERR_MAIL_CONJURED_ITEM = "你不能邮寄魔法制造的物品";
ERR_MAIL_DATABASE_ERROR = "内部邮件数据库错误";
ERR_MAIL_QUEST_ITEM = "你不能邮寄任务物品";
ERR_MAIL_REACHED_CAP = "你已经达到了收取邮件的数量上限"; -- Stopping mail bots
ERR_MAIL_SENT = "邮件已发出。";
ERR_MAIL_TARGET_NOT_FOUND = "找不到收件人。";
ERR_MAIL_TO_SELF = "你不能给自己发送邮件。";
ERR_MEETING_STONE_GROUP_FULL = "你所在的小队已满";
ERR_MEETING_STONE_INVALID_LEVEL = "你没有达到使用这块集合石所需的等级。";
ERR_MEETING_STONE_IN_PROGRESS = "你仍在使用集合石寻找更多玩家加入队伍。";
ERR_MEETING_STONE_IN_QUEUE_S = "你正在一个等待进入%s的队列中。";
ERR_MEETING_STONE_LEFT_QUEUE_S = "你离开了等待前往%s的队列。";
ERR_MEETING_STONE_MEMBER_ADDED_S = "%s被集合石加入了小队。";
ERR_MEETING_STONE_MEMBER_STILL_IN_QUEUE = "正在集合石队列中寻找新的队友。";
ERR_MEETING_STONE_MUST_BE_LEADER = "只有小队的队长才能与集合石互动";
ERR_MEETING_STONE_NOT_LEADER = "只有队长才能离开集合石的队列";
ERR_MEETING_STONE_NO_RAID_GROUP = "你不能在团队中使用集合石";
ERR_MEETING_STONE_OTHER_MEMBER_LEFT = "队友已离开。正在集合石队列中寻找新的队友。";
ERR_MEETING_STONE_PARTY_KICKED_FROM_QUEUE = "一个队友已经被移除，全队已从集合石队列中移除。";
ERR_MEETING_STONE_SUCCESS = "你的小队已满，你离开了集合石队列。";
ERR_MOUNT_ALREADYMOUNTED = "你已经处于骑乘状态！";
ERR_MOUNT_FORCEDDISMOUNT = "在继续之前先要从坐骑上下来。";
ERR_MOUNT_INVALIDMOUNTEE = "你不能骑上它！";
ERR_MOUNT_LOOTING = "你不能在搜索尸体的时候骑乘";
ERR_MOUNT_NOTMOUNTABLE = "它不能被当作坐骑！";
ERR_MOUNT_NOTYOURPET = "这个坐骑不是你的宠物！";
ERR_MOUNT_OTHER = "未知的骑乘错误";
ERR_MOUNT_RACECANTMOUNT = "由于你的种族限制而无法骑乘";
ERR_MOUNT_SHAPESHIFTED = "在变形状态下无法骑乘！";
ERR_MOUNT_TOOFARAWAY = "坐骑距离太远！";
ERR_MUST_EQUIP_ITEM = "你必须先装备这件物品才能使用它。";
ERR_MUST_REPAIR_DURABILITY = "你必须先修理该物品才能使用它。"; -- This is the error message displayed when a player tries to use an item that has run out of durability.
ERR_NEWTAXIPATH = "发现新的飞行路线！";
ERR_NEW_LEADER_S = "%s现在是队长"; -- Player %s is the new group leader
ERR_NEW_LEADER_YOU = "你现在是队长";
ERR_NEW_LOOT_MASTER_S = "%s现在负责拾取并分配所有战利品";
ERR_NOAMMO_S = "%s";
ERR_NOEMOTEWHILERUNNING = "你不能在移动中实施这个动作！";
ERR_NOTYOURPET = "这不是你的宠物！";
ERR_NOT_A_BAG = "不是背包。";
ERR_NOT_ENOUGH_GOLD = "金钱不足";
ERR_NOT_ENOUGH_MONEY = "你的钱不够。";
ERR_NOT_EQUIPPABLE = "这件物品无法装备";
ERR_NOT_IN_BATTLEGROUND = "你不在战场中";
ERR_NOT_IN_COMBAT = "你无法在战斗中实施那个动作";
ERR_NOT_IN_GROUP = "你现在没有在一个队伍中";
ERR_NOT_IN_RAID = "你不在一个团队中";
ERR_NOT_LEADER = "你现在不是队长";
ERR_NOT_OWNER = "你并未拥有这件物品。";
ERR_NOT_WHILE_DISARMED = "你无法在被缴械时进行该动作";
ERR_NOT_WHILE_SHAPESHIFTED = "你不能在变形的时候实施这个动作。";
ERR_NO_ATTACK_TARGET = "现在没有可以攻击的目标";
ERR_NO_BANK_HERE = "你离银行太远";
ERR_NO_BANK_SLOT = "你没有足够的背包栏位";
ERR_NO_GUILD_CHARTER = "你现在没有公会登记表";
ERR_NO_ITEMS_WHILE_SHAPESHIFTED = "不能在变形状态下使用物品";
ERR_NO_PET = "你现在还没有宠物！";
ERR_NO_REPLY_TARGET = "还没有人回复你";
ERR_NO_SLOT_AVAILABLE = "你没有可以装备这件物品的栏位。";
ERR_NULL_PETNAME = "错误：宠物名称不能为空。";
ERR_OBJECT_IS_BUSY = "目标正在忙碌";
ERR_ONLY_ONE_AMMO = "你只能装备一个弹药袋"; -- Only one ammo pouch at a time
ERR_ONLY_ONE_BOLT = "你只能装备一个箭袋"; -- Only one quiver at a time (crossbows)
ERR_ONLY_ONE_QUIVER = "你只能装备一个箭袋"; -- Only one quiver at a time
ERR_OUT_OF_ENERGY = "能量值不足";
ERR_OUT_OF_FOCUS = "集中值不足";
ERR_OUT_OF_HEALTH = "生命值不足";
ERR_OUT_OF_MANA = "法力值不足";
ERR_OUT_OF_RAGE = "怒气值不足";
ERR_OUT_OF_RANGE = "距离太远。";
ERR_PASSIVE_ABILITY = "你不能在动作条中放置一个被动技能";
ERR_PETITION_ALREADY_SIGNED = "你已经在那张公会登记表上签名了";
ERR_PETITION_CREATOR = "你不能够在自己的公会登记表上签名";
ERR_PETITION_DECLINED_S = "%s拒绝加入你的公会。";
ERR_PETITION_IN_GUILD = "你已经加入了一个公会";
ERR_PETITION_NOT_ENOUGH_SIGNATURES = "你需要更多的签名。";
ERR_PETITION_NOT_SAME_SERVER = "该玩家与你不在同一个服务器";
ERR_PETITION_OFFERED_S = "你要求%s的签名";
ERR_PETITION_SIGNED = "公会登记表";
ERR_PETITION_SIGNED_S = "%s已经在你的公会登记表上签名。";
ERR_PET_BROKEN = "你的宠物逃跑了";
ERR_PET_NOT_RENAMEABLE = "你的宠物无法被重新命名。";
ERR_PET_SPELL_AFFECTING_COMBAT = "你的宠物正在战斗。";
ERR_PET_SPELL_ALREADY_KNOWN_S = "你的宠物已经学会%s了。";
ERR_PET_SPELL_DEAD = "你的宠物已经死亡。";
ERR_PET_SPELL_NOT_BEHIND = "你的宠物必须在目标的背后。";
ERR_PET_SPELL_OUT_OF_RANGE = "宠物距离目标太远。";
ERR_PET_SPELL_ROOTED = "你的宠物无法移动。";
ERR_PET_SPELL_TARGETS_DEAD = "宠物的目标已经死亡。";
ERR_PLAYER_BUSY_S = "%s现在很忙";
ERR_PLAYER_DEAD = "你已经死亡，无法实施该动作";
ERR_PLAYER_DIED_S = "%s已经死亡。"; -- %s is the player's name
ERR_PLAYER_WRONG_FACTION = "目标不是你的盟友";
ERR_PLAY_TIME_EXCEEDED = "已超过最大游戏时间"; -- Sent when trying to loot anything after play time has been exceeded
ERR_POTION_COOLDOWN = "你已经喝得太饱了。";
ERR_PROFICIENCY_GAINED_S = "你掌握了%s的使用方法。";
ERR_PROFICIENCY_NEEDED = "你不会使用这种物品。";
ERR_PVP_TOGGLE_OFF = "关闭PvP战斗";
ERR_PVP_TOGGLE_ON = "开启PvP战斗";
ERR_QUEST_ACCEPTED_S = "接受任务：%s"; -- %s is the quest name
ERR_QUEST_ADD_FOUND_SII = "%s：%d/%d"; -- %s is the object name
ERR_QUEST_ADD_ITEM_SII = "%s：%d/%d"; -- %s is the item name
ERR_QUEST_ADD_KILL_SII = "已杀死%s：%d/%d"; -- %s is the monster name
ERR_QUEST_ALREADY_ON = "你已经接到了该任务";
ERR_QUEST_COMPLETE_S = "%s完成。"; -- %s is the quest name
ERR_QUEST_FAILED_BAG_FULL_S = "%s失败：背包已满。"; -- %s is the quest name
ERR_QUEST_FAILED_LOW_LEVEL = "你现在的等级没有达到任务的等级要求";
ERR_QUEST_FAILED_MAX_COUNT_S = "%s失败：发现重复的物品。"; -- %s is the quest name
ERR_QUEST_FAILED_MISSING_ITEMS = "你没有所需要的物品。请查看自己的物品栏。";
ERR_QUEST_FAILED_NOT_ENOUGH_MONEY = "你没有足够的钱去完成这个任务";
ERR_QUEST_FAILED_S = "%s失败。"; -- %s is the quest name
ERR_QUEST_FAILED_WRONG_RACE = "你的种族不能接受这个任务。";
ERR_QUEST_LOG_FULL = "你的任务日志已经满了。";
ERR_QUEST_MUST_CHOOSE = "你必须选择一件奖励品。";
ERR_QUEST_NEED_PREREQS = "你还未达到接受该任务的前提条件";
ERR_QUEST_OBJECTIVE_COMPLETE_S = "%s（完成）"; -- %s is the objective text
ERR_QUEST_ONLY_ONE_TIMED = "你不能同时做多个限时任务";
ERR_QUEST_PUSH_ACCEPTED_S = "%s接受了你的任务。";
ERR_QUEST_PUSH_ALREADY_DONE_S = "%s已经完成这个任务了";
ERR_QUEST_PUSH_BUSY_S = "%s很忙。";
ERR_QUEST_PUSH_DECLINED_S = "%s拒绝了你的任务。";
ERR_QUEST_PUSH_INVALID_S = "无法与%s共享任务";
ERR_QUEST_PUSH_LOG_FULL_S = "%s的任务记录已满";
ERR_QUEST_PUSH_ONQUEST_S = "%s已经有这个任务了";
ERR_QUEST_PUSH_SUCCESS_S = "已与%s共享任务";
ERR_QUEST_PUSH_TOO_FAR_S = "%s距离太远，无法接受你的任务。";
ERR_QUEST_REWARD_EXP_I = "获得经验值：%d"; -- %d is amount of xp gain
ERR_QUEST_REWARD_ITEM_S = "获得物品：%s。"; -- %s is the item name
ERR_QUEST_REWARD_MONEY_S = "获得%s。"; -- %s is string containing money amounts
ERR_QUEST_UNKNOWN_COMPLETE = "目标达成";
ERR_RAID_GROUP_FULL = "此副本已满";
ERR_RAID_GROUP_ONLY = "你必须在一个团队中才能进入这个副本";
ERR_RAID_LEADER_READY_CHECK_START_S = "%s开始进行就位确认";
ERR_RAID_MEMBER_ADDED_S = "%s加入了团队";
ERR_RAID_MEMBER_REMOVED_S = "%s离开了团队";
ERR_RAID_YOU_JOINED = "你加入了一个团队。";
ERR_RAID_YOU_LEFT = "你已经离开了这个团队";
ERR_READY_CHECK_IN_PROGRESS = "你已经在进行就位确认了";
ERR_RECEIVE_ITEM_S = "获得%s。"; -- player was given an item called %s
ERR_RESTRICTED_ACCOUNT = "硬核帐号无法进行该动作";
ERR_SET_LOOT_FREEFORALL = "分配方式设定为自由拾取。";
ERR_SET_LOOT_GROUP = "分配方式设定为队伍分配。";
ERR_SET_LOOT_MASTER = "分配方式设定为队长分配。";
ERR_SET_LOOT_NBG = "分配方式设定为需求优先。";
ERR_SET_LOOT_ROUNDROBIN = "分配方式设定为轮流拾取。";
ERR_SET_LOOT_THRESHOLD_S = "分配物品界限设定为%s";
ERR_SKILL_GAINED_S = "你学会了%s技能。"; -- %s is the skill name
ERR_SKILL_UP_SI = "你的%s技能提高到了%d。"; -- %s is the skill name, %d is the new rank
ERR_SLOT_EMPTY = "这个栏位是空的";
ERR_SPECIFY_MASTER_LOOTER = "你必须指定一个物品分配人员。";
ERR_SPELL_ALREADY_KNOWN_S = "你已经学会%s了。"; -- %s is the spell name
ERR_SPELL_COOLDOWN = "法术还没有准备好。";
ERR_SPELL_FAILED_ALREADY_AT_FULL_HEALTH = "你的生命值已满。";
ERR_SPELL_FAILED_ALREADY_AT_FULL_POWER_S = "你的%s已满。";
ERR_SPELL_FAILED_EQUIPPED_ITEM = "必须装备正确的物品";
ERR_SPELL_FAILED_EQUIPPED_ITEM_CLASS_S = "%s";
ERR_SPELL_FAILED_NOTUNSHEATHED = "你必须拔出武器来实施这个动作！";
ERR_SPELL_FAILED_REAGENTS = "%s";
ERR_SPELL_FAILED_REAGENTS_GENERIC = "缺少材料。";
ERR_SPELL_FAILED_S = "%s";
ERR_SPELL_FAILED_SHAPESHIFT_FORM_S = "%s";
ERR_SPELL_FAILED_SS = "%s失败：%s。";
ERR_SPELL_FAILED_TITLE = "%s失败：%s。";
ERR_SPELL_FAILED_TOTEMS = "%s";
ERR_SPELL_OUT_OF_RANGE = "距离太远。";
ERR_SPELL_UNLEARNED_S = "你忘却了%s";
ERR_SPLIT_FAILED = "无法分开这些物品";
ERR_TALENT_WIPE_ERROR = "你还没有分配任何天赋点数";
ERR_TAME_FAILED = "%s。";
ERR_TARGET_LOGGING_OUT = "该玩家已经登出。";
ERR_TARGET_NOT_IN_GROUP_S = "%s不在你的队伍中";
ERR_TARGET_STUNNED = "目标被晕眩"; -- Trade failure
ERR_TAXINOPATH = "你选择的路径不存在！";
ERR_TAXINOPATHS = "你还未发现任何连接到这里的飞行点。";
ERR_TAXINOSUCHPATH = "没有到该地点的直接路径！";
ERR_TAXINOTENOUGHMONEY = "你没有足够的钱！";
ERR_TAXINOTSTANDING = "你必须先站起来。";
ERR_TAXINOTVISITED = "你还没有步行到达过该空运站！";
ERR_TAXINOVENDORNEARBY = "附近没有空运站！";
ERR_TAXIPLAYERALREADYMOUNTED = "你已经上了坐骑！请先下来。";
ERR_TAXIPLAYERBUSY = "你太忙了，无法使用空中运输服务。";
ERR_TAXIPLAYERMOVING = "你正在移动。";
ERR_TAXIPLAYERSHAPESHIFTED = "你不能在变形状态下使用空中运输服务！";
ERR_TAXISAMENODE = "你已经在那里了！";
ERR_TAXITOOFARAWAY = "你离空运站太远了。";
ERR_TAXIUNSPECIFIEDSERVERERROR = "未知的服务器错误";
ERR_TICKET_ALREADY_EXISTS = "你已经有一张GM请求了。";
ERR_TICKET_CREATE_ERROR = "创建GM请求错误";
ERR_TICKET_DB_ERROR = "获取GM请求错误";
ERR_TICKET_NO_TEXT = "你必须输入正文的内容。";
ERR_TICKET_UPDATE_ERROR = "更新GM请求错误";
ERR_TOOBUSYTOFOLLOW = "你现在很忙，无法跟随！";
ERR_TOO_FAR_TO_ATTACK = "你离你的攻击目标太远了！"; -- Player is too far from the target
ERR_TOO_FEW_TO_SPLIT = "你试图分开的数量大于物品原有的数量。";
ERR_TOO_MANY_CHAT_CHANNELS = "你最多同时进入10个频道";
ERR_TOO_MANY_SPECIAL_BAGS = "你只能装备一个该类型的背包";
ERR_TRADE_BAG = "你不能交易装有物品的包";
ERR_TRADE_BAG_FULL = "交易失败，你没有足够的物品栏空间。";
ERR_TRADE_BLOCKED_S = "%s要求进行交易，你拒绝了。";
ERR_TRADE_BOUND_ITEM = "你不能交易一件灵魂绑定物品";
ERR_TRADE_CANCELLED = "交易取消";
ERR_TRADE_COMPLETE = "交易完成";
ERR_TRADE_EQUIPPED_BAG = "你无法交易已经装备的包裹。";
ERR_TRADE_GROUND_ITEM = "你不能交易一件还放在地上的物品。";
ERR_TRADE_MAX_COUNT_EXCEEDED = "你拥有超过一件";
ERR_TRADE_QUEST_ITEM = "你不能交易一件任务物品";
ERR_TRADE_REQUEST_S = "%s想要和你进行交易。"; -- %s is the trade initiator
ERR_TRADE_TARGET_BAG_FULL = "交易失败，交易目标没有足够的物品栏空间。";
ERR_TRADE_TARGET_DEAD = "你不能和已死亡的玩家交易";
ERR_TRADE_TARGET_MAX_COUNT_EXCEEDED = "交易对象已经拥有该类唯一物品。";
ERR_TRADE_TOO_FAR = "交易目标太远";
ERR_TRADE_WRONG_REALM = "你只能与来自其它服务器的玩家交易魔法制造的物品";
ERR_UNHEALTHY_TIME = "您正处于疲劳游戏时间，应该尽快下线休息。"; -- Sent every 15 minutes in unhealthy time.
ERR_UNINVITE_YOU = "你已经被移出队伍";
ERR_UNIT_NOT_FOUND = "未知的单位";
ERR_USE_BAD_ANGLE = "你面朝的角度不对！";
ERR_USE_CANT_IMMUNE = "无法在免疫状态下那样做。"; -- This is the error message when a player is under any kind of immunity effect and they are not allowed to do something because of it
ERR_USE_CANT_OPEN = "你不能打开这个";
ERR_USE_DESTROYED = "已经被摧毁。";
ERR_USE_LOCKED = "物品被锁住了。";
ERR_USE_LOCKED_WITH_ITEM_S = "需要%s"; -- %s is the item required
ERR_USE_LOCKED_WITH_SPELL_KNOWN_SI = "需要%s %d"; -- %s is the open lock skill required, %d is the required rank
ERR_USE_LOCKED_WITH_SPELL_S = "需要%s"; -- %s is the open lock skill required
ERR_USE_OBJECT_MOVING = "目标在移动中";
ERR_USE_SPELL_FOCUS = "目标是个法术焦点。";
ERR_USE_TOO_FAR = "你距离太远了。";
ERR_VENDOR_HATES_YOU = "商人不喜欢你。";
ERR_VENDOR_NOT_INTERESTED = "商人不要这件物品";
ERR_VENDOR_SOLD_OUT = "这件物品已经售完";
ERR_VENDOR_TOO_FAR = "你距离太远了。";
ERR_WRONG_BAG_TYPE = "该物品无法装入这个包中。";
ERR_WRONG_BAG_TYPE_SUBCLASS = "只有%s能放在那个位置。";
ERR_WRONG_DIRECTION_FOR_ATTACK = "你必须面对正确的方向进行攻击！"; -- Player is not facing the target
ERR_WRONG_SLOT = "物品无法放入该空位。";
ERR_ZONE_EXPLORED = "发现：%s";
ERR_ZONE_EXPLORED_XP = "发现%s：获得%d点经验值";
EVADE = "闪避"; -- Evade, first letter capitalized
EXHAUSTION_LABEL = "疲倦"; -- Used as the label for the exhaustion status bar
EXHAUST_TOOLTIP1 = "|cffffd200%s|r\n|cffffffff从怪物身上获得正常经验值的%d%%。|r";
EXHAUST_TOOLTIP2 = "\n|cffff0000你应该找一家旅店休息。|r";
EXHAUST_TOOLTIP3 = "\n\n在当前的状态下，你可以得到\n比基础经验值多出%d的经验值\n，直到进入下一个休息状态阶段。";
EXHAUST_TOOLTIP4 = "\n\n|cffffd200正在休息|r\n|cffffffff你必须再休息%d\n分钟才能恢复到|r |cffffd200精力充沛状态|r|cffffffff。|r";
EXIT = "退出"; -- Generic "exit"
EXIT_GAME = "退出游戏";
EXOTICS = "异种武器"; -- Exotic Weapons
EXPERIENCE_COLON = "经验值："; -- Character's experience, with colon
FACING_WRONG_DIRECTION = "你没有面向正确的方向！"; -- The player is facing the wrong direction
FACTION = "阵营"; -- Generic faction label
FACTION_ALLIANCE = "联盟";
FACTION_CONTROLLED_TERRITORY = "%s领地"; -- Faction name
FACTION_HORDE = "部落";
FACTION_INACTIVE = "隐藏";
FACTION_OTHER = "其它";
FACTION_STANDING_CHANGED = "你在%2$s中的声望达到了%1$s。";
FACTION_STANDING_DECREASED = "你在%s中的声望值降低了%d点。";
FACTION_STANDING_INCREASED = "你在%s中的声望值提高了%d点。";
FACTION_STANDING_LABEL1 = "仇恨"; -- "Hated" faction standing
FACTION_STANDING_LABEL7 = "崇敬"; -- "Revered" faction standing
FACTION_STANDING_LABEL8_FEMALE = "崇拜"; -- "Exalted" faction standing
FACTION_STANDING_LABEL7_FEMALE = "崇敬"; -- "Revered" faction standing
FACTION_STANDING_LABEL6_FEMALE = "尊敬"; -- "Honored" faction standing
FACTION_STANDING_LABEL5_FEMALE = "友善"; -- "Friendly" faction standing
FACTION_STANDING_LABEL4_FEMALE = "中立"; -- "Neutral" faction standing
FACTION_STANDING_LABEL3_FEMALE = "冷淡"; -- "Unfriendly" faction standing
FACTION_STANDING_LABEL2_FEMALE = "敌对"; -- "Hostile" faction standing
FACTION_STANDING_LABEL1_FEMALE = "仇恨"; -- "Hated" faction standing
FACTION_STANDING_LABEL8 = "崇拜"; -- "Exalted" faction standing
FACTION_STANDING_LABEL6 = "尊敬"; -- "Honored" faction standing
FACTION_STANDING_LABEL2 = "敌对"; -- "Hostile" faction standing
FACTION_STANDING_LABEL3 = "冷淡"; -- "Unfriendly" faction standing
FACTION_STANDING_LABEL4 = "中立"; -- "Neutral" faction standing
FACTION_STANDING_LABEL5 = "友善"; -- "Friendly" faction standing
FAILED = "失败"; -- Something failed
FARCLIP = "远景显示";
FEEDPET_LOG_FIRSTPERSON = "你的宠物开始食用%s。";
FEEDPET_LOG_THIRDPERSON = "%s的宠物开始食用%s。";
FEETSLOT = "脚"; -- Feet slot
FEMALE = "女性"; -- Generic "Female" gender tag
FILTER = "过滤器";
FILTERS = "过滤器";
FINGER0SLOT = "手指"; -- Finger slot
FINGER1SLOT = "手指"; -- Finger slot
FIRST_AVAILABLE = "首先可用";
FIRST_AVAILABLE_TOOLTIP = "加入首先可用的战场等待序列。如果你选择了“首先可用”战场，而另一个队友已经进入了“首先可用”战场，那么你的默认战场副本也会被更改为该战场。";
FIX_LAG = "修正延迟";
FLAG_COUNT_TEMPLATE = "x %d";
FOCUS = "集中"; -- Generic "focus"
FOCUS_COST = "%d集中";
FOCUS_COST_PER_TIME = "%d集中，外加每秒%d";
FOCUS_POINTS = "集中";
FOLLOW = "跟随";
FOLLOW_TERRAIN = "跟随地形";
FONT_SIZE = "字体大小";
FONT_SIZE_TEMPLATE = "%d pt";
FOOD_TIMER = "你已经吃得太饱了。"; -- The player is trying to eat food before the cooldown timer expires
FOREIGN_SERVER_LABEL = " (*)"; -- Indicates a player that is from another server
FRAMERATE_LABEL = "每秒帧数:";
FREE_FOR_ALL_TERRITORY = "PvP区域";
FRIENDS = "好友";
FRIENDS_LEVEL_TEMPLATE = "等级%d %s"; -- For "name location" in friends list
FRIENDS_LIST = "好友名单";
FRIENDS_LIST_OFFLINE_TEMPLATE = "|cff999999%s - 离线|r"; -- For "name location" in friends list
FRIENDS_LIST_TEMPLATE = "%s |cffffffff- %s|r %s"; -- For "name location <AFK>" in friends list
FROM = "来自：";
FULL_SCREEN_GLOW = "全屏幕泛光效果";
GAINING_LOYALTY = "忠诚度提升";
GAMEFIELD_DESELECT_TEXT = "目标锁定";
GAME_VERSION_LABEL = "版本"; -- generic version
GAMMA = "Gamma";
GENERAL = "综合"; -- Name for the general chat tab
GENERAL_MACROS = "通用宏"; -- Header for general account macros
GENERIC_MIN = "%d分钟";
GENERIC_MIN_P1 = "%d分钟";
GENERIC_SEC = "%d秒";
GENERIC_SEC_P1 = "%d秒";
GIVE_LOOT = "将尸体上的物品给予：";
GLANCING_TRAILER = "(偏斜)"; -- Appended to glancing blows in the combat log. (NOTE SPACE AT BEGINNING)
GMSURVEYRATING1 = "极差";
GMSURVEYRATING2 = "较差";
GMSURVEYRATING3 = "普通";
GMSURVEYRATING5 = "出色";
GMSURVEYRATING4 = "优秀";
GMSURVEY_EXCELLENT = "5 (卓越)";
GMSURVEY_POOR = "1 (极差)";
GMSURVEY_REQUEST_TEXT = "请回答下面的问题：";
GMSURVEY_SUBMITTED = "你的调查已经提交";
GMSURVEY_TITLE = "GM调查问卷"; -- Header for the gm survey frame
GM_EMAIL_NAME = "客户支持";
GM_SURVEY_NOT_APPLICABLE = "N/A";
GM_TICKET_ESCALATED = "你的请求升级了。";
GM_TICKET_HIGH_VOLUME = "我们目前正在处理大量请求。";
GM_TICKET_SERVICE_SOON = "你的请求将很快得到答复。";
GM_TICKET_UNAVAILABLE = "等待时间不可预估。";
GM_TICKET_WAIT_TIME = "剩余时间：\n%s";
GOLD = "金币"; -- Highest value coin denomination
GOODBYE = "再见"; -- Say "Goodbye" to an NPC
GOSSIP_OPTIONS = "闲谈选项";
GREED = "贪婪";
GROUP = "小队";
GROUPS = "组队";
GROUP_INVITE = "组队邀请"; -- Invite a friend into your group
GUILD = "公会";
HARDCORE = "硬核";
GUILDADDRANK_BUTTON_TOOLTIP = "点击这里增加新会阶";
GUILDCONTROL = "公会控制";
GUILDCONTROL_ALLOWRANK = "此会阶的权限：";
GUILDCONTROL_OPTION1 = "听取公会聊天";
GUILDCONTROL_OPTION10 = "编辑公共信息";
GUILDCONTROL_OPTION11 = "查看官员信息";
GUILDCONTROL_OPTION12 = "编辑官员信息";
GUILDCONTROL_OPTION13 = "修改公会信息";
GUILDCONTROL_OPTION2 = "公会聊天讲话";
GUILDCONTROL_OPTION3 = "听取官员聊天";
GUILDCONTROL_OPTION4 = "官员聊天讲话";
GUILDCONTROL_OPTION5 = "提升";
GUILDCONTROL_OPTION6 = "降职";
GUILDCONTROL_OPTION7 = "邀请成员";
GUILDCONTROL_OPTION8 = "移除成员";
GUILDCONTROL_OPTION9 = "设置今日信息";
GUILDCONTROL_RANKLABEL = "会阶名称：";
GUILDCONTROL_SELECTRANK = "选择要修改的会阶：";
GUILDMEMBER_ALERT = "公会成员提示";
GUILDMOTD_BUTTON_TOOLTIP = "点击这里查看/编辑本日公会信息。";
GUILDNOTE_BUTTON_TOOLTIP = "点击这里查看/编辑已选中玩家的公共信息。";
GUILDOFFICERNOTE_BUTTON_TOOLTIP = "点击这里查看/编辑已选中玩家的公会官员信息。";
GUILDREMOVERANK_BUTTON_TOOLTIP = "点击这里移除这个会阶";
GUILD_CHARTER = "公会登记表";
GUILD_CHARTER_CREATOR = "公会首领：%s";
GUILD_CHARTER_PURCHASE = "购买公会申请表";
GUILD_CHARTER_REGISTER = "注册公会登记表";
GUILD_CHARTER_TEMPLATE = "%s公会登记表";
GUILD_CHARTER_TITLE = "公会登记表：%s";
GUILD_CREST_DESIGN = "设计公会徽章";
GUILD_FRAME_TITLE = "公会名册";
GUILD_HELP_TEXT_LINE10 = "'/gquit' removes you from your guild";
GUILD_HELP_TEXT_LINE3 = "'/g <信息>' 向公会内的所有成员发送一条信息";
GUILD_HELP_TEXT_LINE9 = "'/gmotd <信息>' 设置公会通知";
GUILD_HELP_TEXT_LINE5 = "'/ginvite <玩家>' 邀请玩家加入你的公会";
GUILD_HELP_TEXT_LINE4 = "'/o <信息>' 向公会中所有的管理人员发送一条信息";
GUILD_HELP_TEXT_LINE1 = "要创建一个公会，请在控制台中输入'guildcreate <公会名称>'";
GUILD_HELP_TEXT_LINE13 = "'/gdisband' 解散公会";
GUILD_HELP_TEXT_LINE12 = "'/gleader <玩家>' 指定另一个玩家担任公会首领";
GUILD_HELP_TEXT_LINE7 = "'/gpromote <玩家>' 提升一个玩家在公会内的会阶";
GUILD_HELP_TEXT_LINE6 = "'/gremove <玩家>' 从公会中开除玩家";
GUILD_HELP_TEXT_LINE2 = "'/ginfo' 显示公会的基本信息";
GUILD_HELP_TEXT_LINE8 = "'/gdemote <玩家>' 降低一个玩家在公会中的会阶";
GUILD_HELP_TEXT_LINE11 = "'/groster' 列出公会成员名单";
GUILD_INFORMATION = "公会信息";
GUILD_INFO_EDITLABEL = "点击这里设置消息";
GUILD_INFO_TEMPLATE = "公会创立于%d-%d-%d，%d名成员，%d个账号"; -- Guild creation date (m-d-y), # players and accounts
GUILD_INVITATION = "%s邀请你加入%s"; -- Tells the player he has been invited to a guild
GUILD_LEAVE = "退出公会";
GUILD_MEMBER_OPTIONS = "会员设置";
GUILD_MEMBER_TEMPLATE = "%s，%s"; -- Member name, rank
GUILD_MESSAGE = "公会聊天"; -- Message to your guild
HARDCORE_MESSAGE = "硬核"; -- Message to your Hardcore homies
GUILD_MOTD = "公会今日信息";
GUILD_MOTD_EDITLABEL = "点击这里设置本日公会信息。";
GUILD_MOTD_LABEL = "今日公会信息：";
GUILD_MOTD_LABEL2 = "公会今日信息";
GUILD_MOTD_TEMPLATE = "今日公会信息：%s"; -- %s is the guild MOTD
GUILD_NAME = "公会名称";
GUILD_NAME_TEMPLATE = "公会：%s"; -- Name of guild
GUILD_NOTES_LABEL = "玩家记录：";
GUILD_NOTE_EDITLABEL = "点击这里设置公共信息。";
GUILD_NOT_ALLIED_S = "%s不是你的盟友";
GUILD_OFFICERNOTES_LABEL = "公会官员信息";
GUILD_OFFICERNOTE_EDITLABEL = "点击这里设置公会官员信息。";
GUILD_OFFICER_NOTE = "公会官员注释";
GUILD_ONLINE_LABEL = "在线";
GUILD_PETITION_LEADER_INSTRUCTIONS = "选择一个你想要邀请的玩家然后点击<要求签名>。当你完成了所有签名之后将登记表交给公会注册者。";
GUILD_PETITION_MEMBER_INSTRUCTIONS = "点击<签署登记表>注册成为此公会的一员。";
GUILD_PROMOTE = "提升为会长";
GUILD_RANK0_DESC = "公会首领";
GUILD_RANK1_DESC = "公会官员";
GUILD_RANK2_DESC = "资深成员";
GUILD_RANK3_DESC = "普通成员";
GUILD_RANK4_DESC = "见习成员";
GUILD_REGISTRAR_PURCHASE_TEXT = "要创建一个公会，你必须购买一张公会登记表，在获得9个不同玩家的签名之后将其交还给我。请输入你想要成立的公会的名称。";
GUILD_ROSTER_TEMPLATE = "%d玩家，%d账号"; -- # players and accounts in the guild
GUILD_STATUS = "公会状态";
GUILD_TEMPLATE = "%2$s - %1$s"; -- Character's rank and guild, as in "Guildmaster of Doomhammer's Slammers"
GUILD_TITLE_TEMPLATE = "%2$s - %1$s"; -- Guild rank, guild name
GUILD_TOTAL_P1 = "|cffffffff%d|r位公会成员";
GUILD_TOTAL = "|cffffffff%d|r位公会成员";
GUILD_TOTALONLINE = "(|cffffffff%d|r |cff00ff00在线|r)";
HANDSSLOT = "手"; -- Hands slot
HAPPINESS = "快乐"; -- Generic "happiness"
HAPPINESS_POINTS = "快乐"; -- For hunter pets
HARASSMENT = "骚扰";
HARASSMENT_POLICY_TEXT = "想要了解更多关于骚扰的条例，请访问：\nwww.wowchina.com/policy/harassment.shtml";
HARASSMENT_TEXT = "请选择以下的选项：";
HARDWARE_CURSOR = "硬件指针";
HAVE_MAIL = "你有未阅读的邮件";
HEADSLOT = "头部"; -- Head slot
HEAD_BOB = "摆头";
HEALEDCRITOTHEROTHER = "%s的%s发挥极效，为%s恢复了%d点生命值。";
HEALEDCRITOTHERSELF = "%s的%s发挥极效，为你恢复了%d点生命值。";
HEALEDCRITSELFOTHER = "你的%s对%s产生极效治疗效果，恢复了%d点生命值。";
HEALEDCRITSELFSELF = "你的%s对你造成极效治疗，恢复了%d点生命值。";
HEALEDOTHEROTHER = "%s的%s为%s恢复了%d点生命值。";
HEALEDOTHERSELF = "%s的%s为你恢复了%d点生命值。";
HEALEDSELFOTHER = "你的%s治疗了%s%d点生命值。";
HEALEDSELFSELF = "你的%s治疗了你%d点生命值。";
HEALTH = "生命值"; -- Generic "Health" string
HEALTH_COLON = "生命值："; -- Character's health, with colon
HEALTH_COST = "%d生命值";
HEALTH_COST_PER_TIME = "%d生命值，每秒增加%d";
HEALTH_LOW = "生命值过低";
HELPFRAME_ACCOUNT_BULLET1 = "创建账号出现问题";
HELPFRAME_ACCOUNT_BULLET2 = "关于支付方式的问题";
HELPFRAME_ACCOUNT_BULLET3 = "账号信息修改的问题";
HELPFRAME_ACCOUNT_BULLET4 = "付费选择";
HELPFRAME_ACCOUNT_BULLET_TITLE1 = "支付和账号服务和以下事务相关：";
HELPFRAME_ACCOUNT_BUTTON_TEXT = "报告账号/付费问题";
HELPFRAME_ACCOUNT_ENDTEXT = "想要联系任何此类问题，请联系支付和帐户服务：\n\n通过网站 - www.wowchina.com/support\n电子邮件 - billing@wowchina.com";
HELPFRAME_ACCOUNT_TEXT = "创建账号的时候遇到问题或者支付问题";
HELPFRAME_ACCOUNT_TITLE = "支付和账号服务";
HELPFRAME_BUG_BUTTON_DESCRIPTION = "提交游戏中的一个错误或者Bug";
HELPFRAME_BUG_BUTTON_TEXT = "提交一个Bug：";
HELPFRAME_CHARACTER_BULLET1 = "技能/属性缺失或者无法正常发挥效力";
HELPFRAME_CHARACTER_BULLET2 = "无法登录魔兽世界";
HELPFRAME_CHARACTER_BULLET3 = "技能等级显示为负";
HELPFRAME_CHARACTER_BULLET4 = "专业技能无法列出";
HELPFRAME_CHARACTER_BULLET5 = "天赋不正常工作或者出现缺失";
HELPFRAME_CHARACTER_BULLET_TITLE1 = "以下是角色相关问题的例子：";
HELPFRAME_CHARACTER_BUTTON_TEXT = "报告角色问题";
HELPFRAME_CHARACTER_TEXT = "关于技能，职业，声望和天赋的问题";
HELPFRAME_CHARACTER_TITLE = "角色";
HELPFRAME_ENVIRONMENTAL_BULLET1 = "无法使用一个熔炉";
HELPFRAME_ENVIRONMENTAL_BULLET2 = "能够走过/跳过一堵墙";
HELPFRAME_ENVIRONMENTAL_BULLET3 = "被传送到错误的墓地";
HELPFRAME_ENVIRONMENTAL_BULLET4 = "掉落穿越世界";
HELPFRAME_ENVIRONMENTAL_BULLET5 = "玩家死亡后希望被传送到原来的位置";
HELPFRAME_ENVIRONMENTAL_BULLET6 = "询问一个很难找到的地方";
HELPFRAME_ENVIRONMENTAL_BULLET_TITLE1 = "以下是环境相关问题的例子：";
HELPFRAME_ENVIRONMENTAL_BULLET_TITLE2 = "以下不被认为是环境问题：";
HELPFRAME_ENVIRONMENTAL_BUTTON_TEXT = "报告环境问题";
HELPFRAME_ENVIRONMENTAL_TEXT = "关于一个角色和游戏环境进行交互或者无法在游戏环境中实施动作的问题";
HELPFRAME_ENVIRONMENTAL_TITLE = "环境";
HELPFRAME_GENERAL_BUTTON_DESCRIPTION = "基本游戏性问题/信息";
HELPFRAME_GENERAL_BUTTON_TEXT = "综合性游戏问题：";
HELPFRAME_GM_BUTTON_DESCRIPTION = "联系一个GM来得到帮助";
HELPFRAME_GM_BUTTON_TEXT = "呼叫GM：";
HELPFRAME_GUILD_BULLET1 = "无法增加/移除公会会员";
HELPFRAME_GUILD_BULLET2 = "无法创建一个公会";
HELPFRAME_GUILD_BULLET3 = "无法重新设置和命名你在公会的等级";
HELPFRAME_GUILD_BULLET_TITLE1 = "以下是公会相关问题的例子：";
HELPFRAME_GUILD_BUTTON_TEXT = "报告公会问题";
HELPFRAME_GUILD_TEXT = "任何公会创建或者公会相关功能的问题";
HELPFRAME_GUILD_TITLE = "公会";
HELPFRAME_HARASSMENT_BUTTON_DESCRIPTION = "行为或者语言行为如果冒犯了其他玩家可以被归类在这个选项";
HELPFRAME_HARASSMENT_BUTTON_TEXT = "骚扰：";
HELPFRAME_HOME_BUTTON_TEXT = "GM可以协助解决的事宜";
HELPFRAME_HOME_ISSUE1 = "所有任务、非玩家人物（NPC）、物品信息、地点信息或者任何有关游戏中的世界的信息，玩家都可以通过与世界或其他用户互动来获得。";
HELPFRAME_HOME_ISSUE1_HEADER = "游戏提示";
HELPFRAME_HOME_ISSUE2 = "包括下列信息：下一次补丁的内容、发布时间和发布方式，即将增加的内容，游戏性变动和未来的规则改动。";
HELPFRAME_HOME_ISSUE2_HEADER = "游戏内提示";
HELPFRAME_HOME_ISSUE3 = "大多数PvP问题都可以通过游戏内的机制解决。GM在大多数情况下不会介入此类问题，除非涉及到《魔兽世界》骚扰条款中列举的情况。有关PvP的特殊规则，请访问：";
HELPFRAME_HOME_ISSUE3_HEADER = "PvP";
HELPFRAME_HOME_TEXT = "请选择能够最好地描述你的帮助请求的选项："; -- Greeting text on the home tab of the HelpFrame
HELPFRAME_HOME_TEXT1 = "GM一天24小时应该都能提供帮助。无论你现在使用的是什么角色，GM都可以帮助你。GM|cffffd200不会|r帮助你做的事情包括但|cffffd200不限于|r以下这些：";
HELPFRAME_HOME_TEXT2 = "另外，我们鼓励所有的玩家首先选择论坛和官方网站来寻找相关问题的解决信息，网址是|cffffd200www.wowchina.com|r。";
HELPFRAME_ITEM_BULLET1 = "崩溃之后物品损失";
HELPFRAME_ITEM_BULLET2 = "武器效果没有激活";
HELPFRAME_ITEM_BULLET3 = "附魔效果无法正常生效";
HELPFRAME_ITEM_BULLET4 = "其它附加效果无法正常生效";
HELPFRAME_ITEM_BULLET5 = "更多细节请访问官方网站";
HELPFRAME_ITEM_BULLET6 = "关于如何获得一件物品的询问";
HELPFRAME_ITEM_BULLET7 = "需要物品的请求";
HELPFRAME_ITEM_BULLET_TITLE1 = "以下是物品相关问题的例子：";
HELPFRAME_ITEM_BULLET_TITLE2 = "不包括以下类型的问题：";
HELPFRAME_ITEM_BUTTON_TEXT = "报告物品问题";
HELPFRAME_ITEM_TEXT = "关于任何物品功能性或者装备的问题。";
HELPFRAME_ITEM_TITLE = "物品";
HELPFRAME_NONQUEST_BULLET1 = "野生生物一直保持躲避状态";
HELPFRAME_NONQUEST_BULLET2 = "NPC行走路径不正确";
HELPFRAME_NONQUEST_BULLET3 = "野生生物重生速度过快或者过慢";
HELPFRAME_NONQUEST_BULLET4 = "NPC商人没有物品出售或者没有反映";
HELPFRAME_NONQUEST_BULLET5 = "需要关于非任务NPC/野生怪物重生或者移除问题信息的请求";
HELPFRAME_NONQUEST_BULLET6 = "需要关于非任务NPC/野生怪物信息的请求";
HELPFRAME_NONQUEST_BULLET7 = "和任务相关的NPC或者怪物的问题";
HELPFRAME_NONQUEST_BULLET_TITLE1 = "以下是非任务NPC/怪物的例子：";
HELPFRAME_NONQUEST_BULLET_TITLE2 = "以下不被认为是非任务NPC/怪物的问题：";
HELPFRAME_NONQUEST_BUTTON_TEXT = "报告非任务NPC/野生怪物问题";
HELPFRAME_NONQUEST_TEXT = "与NPC和野生怪物交流的问题或者别的交流问题";
HELPFRAME_NONQUEST_TITLE = "非任务NPC/怪物";
HELPFRAME_OPENTICKET_EDITTEXT = "你现在的问题：";
HELPFRAME_OPENTICKET_TEXT = "描述你的问题：";
HELPFRAME_OTHER_BUTTON_DESCRIPTION = "任何需要GM协助的事件";
HELPFRAME_OTHER_BUTTON_TEXT = "其他事件：";
HELPFRAME_QUEST_BULLET1 = "任务相关NPC或者物体无法正常工作";
HELPFRAME_QUEST_BULLET2 = "无法获取任务相关物品";
HELPFRAME_QUEST_BULLET3 = "任务要求所杀的怪物不掉落任务所需要的物品";
HELPFRAME_QUEST_BULLET4 = "需要任务NPC/野生怪物信息的请求";
HELPFRAME_QUEST_BULLET5 = "需要关于简化一个任务的信息请求";
HELPFRAME_QUEST_BULLET_TITLE1 = "以下是任务/任务NPC的例子：";
HELPFRAME_QUEST_BULLET_TITLE2 = "以下不被认为是任务/任务NPC问题：";
HELPFRAME_QUEST_BUTTON_TEXT = "报告任务/NPC问题";
HELPFRAME_QUEST_TEXT = "关于无法实施或者开始或者完成任务的问题";
HELPFRAME_QUEST_TITLE = "任务/任务NPC";
HELPFRAME_STUCK_BUTTON_DESCRIPTION = "在地图的某处卡住";
HELPFRAME_STUCK_BUTTON_TEXT = "卡死：";
HELPFRAME_STUCK_TEXT1 = "如果你被卡住而无法移动，请首先尝试自动脱离卡死功能，然后再联络GM寻求帮助。此功能在大部份的情况下都能帮助你解决困难。自动脱离卡死功能首先会尝试使用你的炉石将你送回绑定的旅店。若无法使用炉石，则它会尝试将你的角色从卡住的地点移开。\n\n请注意，这个功能每5分钟只能使用一次。使用自动脱离卡死功能将记录你的角色所在的位置，以便我们今后修正这个位置。";
HELPFRAME_SUGGESTION_BUTTON_DESCRIPTION = "所有关于这个游戏的综合性建议和反馈";
HELPFRAME_SUGGESTION_BUTTON_TEXT = "发送一个建议：";
HELPFRAME_TECHNICAL_BULLET1 = "游戏运行表现降低（游戏速度变慢或者出现停顿）";
HELPFRAME_TECHNICAL_BULLET2 = "显示错误（黑色方形物体，物体显示闪烁，显示扭曲等等）";
HELPFRAME_TECHNICAL_BULLET3 = "过场动画或者别的游戏动画无法正常播放";
HELPFRAME_TECHNICAL_BULLET4 = "声音问题（没有声音或者音乐，杂音，循环音效等等）";
HELPFRAME_TECHNICAL_BULLET5 = "连接/断线问题";
HELPFRAME_TECHNICAL_BULLET6 = "游戏中出现的崩溃或者错误信息";
HELPFRAME_TECHNICAL_BULLET7 = "任何其它的技术问题";
HELPFRAME_TECHNICAL_BULLET_TITLE1 = "以下是技术问题的例子：";
HELPFRAME_TECHNICAL_BULLET_TITLE2 = "您可以在技术支持论坛中找到各种常见问题的解决方法，请访问魔兽世界官方网站：\n\nwww.wowchina.com\n\n如果您的问题无法通过该方法找到解决方案的话，请和我们的技术支持部门联系：\n\n网页：www.wowchina.com/support\n电子邮件：tech@wowchina.com";
HELPFRAME_TECHNICAL_BUTTON_TEXT = "报告技术问题";
HELPFRAME_TECHNICAL_TEXT = "我们的技术团队会解决您在魔兽世界游戏中遇到的任何技术问题。";
HELPFRAME_TECHNICAL_TITLE = "技术支持";
HELP_BUTTON = "帮助请求";
HELP_FRAME_TITLE = "魔兽世界帮助";
HELP_LABEL = "帮助";
HELP_TEXT_LINE10 = "- '1' 或 't' 攻击当前目标（如无当前目标则攻击最近目标）";
HELP_TEXT_LINE8 = "- 'v' 查看附近目标，也可用于选择目标";
HELP_TEXT_LINE3 = "- 组队/观察/交易：点击目标玩家，然后右键点击其头像";
HELP_TEXT_LINE2 = "- 'x' 坐下/起立";
HELP_TEXT_LINE1 = "魔兽世界帮助：";
HELP_TEXT_LINE13 = "- 输入/who 获取玩家列表";
HELP_TEXT_LINE7 = "- 'z' 拿出/收起武器";
HELP_TEXT_LINE11 = "- 'PageUp/PageDown' 翻动聊天记录";
HELP_TEXT_LINE9 = "- Tab 切换最近敌人为目标";
HELP_TEXT_LINE5 = "- 'Shift-1 到 Shift-6' 切换动作条。Shift 上/下 和 Shift-鼠标滚轮也有同样功能。";
HELP_TEXT_LINE12 = "- 'r' 或 '/r' 回复最后一个悄悄话";
HELP_TEXT_LINE15 = "- 输入/ghelp 获取公会命令列表";
HELP_TEXT_LINE14 = "- 输入/chat可以显示聊天命令列表";
HELP_TEXT_LINE4 = "- 'F1-F5' 选定自己/队友为目标";
HELP_TEXT_LINE6 = "- 按下'Numlock'自动奔跑";
HELP_TEXT_SIMPLE = "输入 '/help' 以获得命令列表";
HELP_TICKET_ABANDON = "取消请求";
HELP_TICKET_ABANDON_CONFIRM = "确定要取消发送给GM的记录内容?";
HELP_TICKET_EDIT = "编辑请求";
HELP_TICKET_EDIT_ABANDON = "你现在选择设置";
HELP_TICKET_QUEUE_DISABLED = "GM帮助请求暂时不可用。";
HERTZ = "Hz";
HIDE = "隐藏";
HIDE_OUTDOOR_WORLD_STATE_TEXT = "隐藏区域目标追踪";
HIDE_PARTY_INTERFACE_TEXT = "隐藏小队界面";
HIDE_PULLOUT_BG = "隐藏背景";
HIGH = "高";
HIGH_BIDDER = "最高出价";
HK = "荣誉击杀"; -- Abbreviation for "honorable kill"
HOME = "Home";
HOME_INN = "你所处的旅店";
HONOR = "荣誉";
HONORABLE_KILLS = "荣誉击杀";
HONORABLE_KILLS_TOOLTIP = "在死亡之前被你（或你所在的队伍）击伤的敌方玩家";
HONOR_CONTRIBUTION_POINTS = "荣誉";
HONOR_GAINED = "获得荣誉";
HONOR_GAINED_TOOLTIP = "通过达成战场任务目标和赢得战场胜利获得的荣誉点数。";
HONOR_HIGHEST_RANK = "最高级别";
HONOR_LASTWEEK = "上周";
HONOR_LIFETIME = "总计";
HONOR_STANDING = "排名";
HONOR_THISWEEK = "本周";
HONOR_THIS_SESSION = "今日";
HONOR_YESTERDAY = "昨天";
HOURS_P1 = "小时";
HOURS = "小时";
HOURS_ABBR = "小时";
HOURS_ABBR_P1 = "小时";
HOUR_ONELETTER_ABBR = "%d h";
HP = "生命值"; -- Abbreviation for "health"
HP_TEMPLATE = "%d生命值";
HUNTER_AGILITY_TOOLTIP = "提高你的近战武器和远程武器的攻击强度，并提高用所有武器造成致命一击的几率。\n提高你的护甲值和躲避攻击的几率。";
HUNTER_INTELLECT_TOOLTIP = "提高你的法力值上限和使用法术造成致命一击的几率。\n使你更快地提升武器技能熟练度。";
ID = "ID";
IDLE_MESSAGE = "因长时间没有动作，将自动退出游戏。如果你希望继续游戏，请点击取消键。";
IGNORE = "屏蔽";
IGNORE_LIST = "屏蔽列表";
IGNORE_PLAYER = "屏蔽玩家"; -- Add a player to your ignore list
IGR_BILLING_NAG_DIALOG = "你的IGR游戏时间即将用尽，你很快会被断开连接。";
IMMUNE = "免疫"; -- Immune to a spell
IMMUNEDAMAGECLASSOTHEROTHER = "%s对%s的%s伤害免疫。";
IMMUNEDAMAGECLASSOTHERSELF = "你对%s的%s伤害免疫。";
IMMUNEDAMAGECLASSSELFOTHER = "%s对你的%s伤害免疫。";
IMMUNEOTHEROTHER = "%s击中了%s，但是目标对攻击免疫。";
IMMUNEOTHERSELF = "%s击中了你，但是你对攻击免疫。";
IMMUNESELFOTHER = "你击中了%s，但是目标对攻击免疫。";
IMMUNESELFSELF = "你击中了自己，但是你对攻击免疫。";
IMMUNESPELLOTHEROTHER = "%s对%s的%s免疫。";
IMMUNESPELLOTHERSELF = "你对%s的%s免疫。";
IMMUNESPELLSELFOTHER = "%s对你的%s免疫。";
IMMUNESPELLSELFSELF = "你对你的%s免疫。";
INBOX = "收件箱";
INCREASE_POTENTIAL = "将|cffffffff%s|r的上限提升|cffffffff%d|r点";
INPUT_CHINESE = "中";
INPUT_JAPANESE = "日";
INPUT_KOREAN = "韩";
INPUT_ROMAN = "英";
INSPECT = "观察"; -- Player inspects another player
INSPECT_NOTIFY = "%s正在观察你。"; -- Tells the player that another player is inspecting him
INSTAKILLOTHER = "%s被%s杀死了。";
INSTAKILLSELF = "你被%s杀死了。";
INSTANCE = "副本";
INSTANCE_BOOT_TIMER = "你现在不在这个副本的队伍中。你将在%2$d%3$s内被传送到%1$s。";
INSTANCE_COMPLETE_MESSAGE = "战斗已结束。战场将在%s内关闭";
INSTANCE_RESET_FAILED = "无法重置%s，该副本中仍有玩家。";
INSTANCE_RESET_FAILED_OFFLINE = "无法重置%s，你的队伍中有人离线。";
INSTANCE_RESET_FAILED_ZONING = "无法重置%s，你的队伍中有人正在试图进入某个副本。";
INSTANCE_RESET_SUCCESS = "%s 已被重置。";
INSTANCE_SAVED = "你在此副本中的进度已保存";
INSTANCE_SHUTDOWN_MESSAGE = "没有足够的玩家。服务器将在%s内关闭"; -- Displayed when a battleground instance does not have the minimum number of players required
INTELLECT_COLON = "智力："; -- Character's intellect, with colon
INTELLECT_TOOLTIP = "提高法力值上限，并且提高法术的致命\n一击几率。智力还能让你更快地提升\n武器技能。（但不会影响专业技能的\n提升速度。）"; -- Tooltip description for intellect
INTERRUPT = "中断"; -- Interrupt an attack
INTERRUPTED = "被打断"; -- Interrupted an attack
INT_SPELL_DURATION_DAYS = "%d天";
INT_SPELL_DURATION_HOURS_P1 = "%d小时";
INT_SPELL_DURATION_HOURS = "%d小时";
INT_SPELL_DURATION_MIN = "%d分钟";
INT_SPELL_DURATION_SEC = "%d秒";
INT_SPELL_POINTS_SPREAD_TEMPLATE = "%d到%d";
INVENTORY_FULL = "物品栏已满。"; -- The player's inventory is full
INVENTORY_TOOLTIP = "物品栏"; -- Inventory tooltip
INVERT_MOUSE = "反转鼠标";
INVITATION = "%s邀请你加入队伍"; -- Tells the player he has been invited to a group
INVTYPE_2HWEAPON = "双手"; -- Two-hand type
INVTYPE_BAG = "背包"; -- Bag type
INVTYPE_BODY = "衬衣"; -- Body type
INVTYPE_CHEST = "胸部"; -- Chest type
INVTYPE_CLOAK = "背部"; -- Cloak type
INVTYPE_FEET = "脚"; -- Feet type
INVTYPE_FINGER = "手指"; -- Finger type
INVTYPE_HAND = "手"; -- Hands type
INVTYPE_HEAD = "头部"; -- Head type
INVTYPE_HOLDABLE = "副手物品"; -- Holdable type
INVTYPE_LEGS = "腿部"; -- Legs type
INVTYPE_NECK = "颈部"; -- Neck type
INVTYPE_RANGED = "远程"; -- Ranged type
INVTYPE_RELIC = "圣物";
INVTYPE_ROBE = "胸部"; -- Robe type
INVTYPE_SHIELD = "副手"; -- Shield type
INVTYPE_SHOULDER = "肩部"; -- Shoulder type
INVTYPE_TABARD = "公会徽章"; -- Tabard type
INVTYPE_TRINKET = "饰品"; -- Trinket type
INVTYPE_WAIST = "腰部"; -- Waist type
INVTYPE_WEAPON = "单手"; -- One-hand type, either hand
INVTYPE_WEAPONMAINHAND = "主手"; -- One-hand type, main hand
INVTYPE_WEAPONOFFHAND = "副手"; -- One-hand type, off hand
INVTYPE_WRIST = "手腕"; -- Wrist type
ITEMENCHANTMENTADDOTHEROTHER = "%1$s在%3$s的%4$s上施放了%2$s。";
ITEMENCHANTMENTADDOTHERSELF = "%1$s在你的%3$s上施放了%2$s。";
ITEMENCHANTMENTADDSELFOTHER = "你在%2$s的%3$s上施放了%1$s。";
ITEMENCHANTMENTADDSELFSELF = "你在你的%2$s上施放了%1$s。";
ITEMENCHANTMENTREMOVEOTHER = "%s效果从%s的%s上消失。";
ITEMENCHANTMENTREMOVESELF = "%s效果从你的%s上消失。";
ITEMPRESENTINOFFHAND = "不能装备，因为你的副手已持有物品！";
ITEMS = "物品"; -- Generic "items"
ITEMSLOTTEXT = "物品栏位"; -- label for items
ITEM_BIND_ON_EQUIP = "装备后绑定"; -- Item will be bound when equipped
ITEM_BIND_ON_PICKUP = "拾取后绑定"; -- Item wil be bound when picked up
ITEM_BIND_ON_USE = "使用后绑定"; -- Item will be bound when used
ITEM_BIND_QUEST = "任务物品"; -- Item is a quest item (same logic as ON_PICKUP)
ITEM_CANT_BE_DESTROYED = "这件物品无法被摧毁。"; -- Attempted to destroy a NO_DESTROY item
ITEM_CLASSES_ALLOWED = "职业：%s"; -- Lists the classes allowed to use this item
ITEM_CONJURED = "魔法制造的物品"; -- Item expires
ITEM_COOLDOWN_TIME_DAYS_P1 = "剩余冷却时间：%d天";
ITEM_COOLDOWN_TIME_DAYS = "剩余冷却时间：%d天";
ITEM_COOLDOWN_TIME_HOURS = "剩余冷却时间：%d小时";
ITEM_COOLDOWN_TIME_HOURS_P1 = "剩余冷却时间：%d小时";
ITEM_COOLDOWN_TIME_MIN = "冷却时间剩余：%d 分钟";
ITEM_COOLDOWN_TIME_SEC = "冷却时间剩余：%d秒";
ITEM_CREATED_BY = "|cff00ff00<由%s制造>|r"; -- %s is the creator of the item
ITEM_DURATION_DAYS = "持续时间：%d天";
ITEM_DURATION_HOURS = "持续时间：%d小时";
ITEM_DURATION_MIN = "持续时间：%d分钟";
ITEM_DURATION_SEC = "持续时间：%d秒";
ITEM_ENCHANT_DISCLAIMER = "物品将不会被交易！"; -- Temporary string
ITEM_ENCHANT_TIME_LEFT_DAYS = "%s（%d天）";
ITEM_ENCHANT_TIME_LEFT_DAYS_P1 = "%s（%d天）";
ITEM_ENCHANT_TIME_LEFT_HOURS = "%s（%d小时）";
ITEM_ENCHANT_TIME_LEFT_HOURS_P1 = "%s（%d小时）";
ITEM_ENCHANT_TIME_LEFT_MIN = "%s（%d分钟）"; -- Enchantment name, followed by the time left in minutes
ITEM_ENCHANT_TIME_LEFT_SEC = "%s（%d秒）"; -- Enchantment name, followed by the time left in seconds
ITEM_LEVEL = "等级 %d"; -- The actual level of the item
ITEM_LEVEL_AND_MIN = "等级 %d （最小 %d）"; -- The level and minimum required level for an item
ITEM_MIN_LEVEL = "需要等级 %d"; -- Required level to use the item
ITEM_MIN_SKILL = "需要%s（%d）"; -- Required skill rank to use the item
ITEM_MOD_AGILITY = "%c%d 敏捷"; -- Item adds %d to agility, %c is +/-
ITEM_MOD_HEALTH = "%c%d 生命值"; -- Item adds %d to health, %c is +/-
ITEM_MOD_INTELLECT = "%c%d 智力"; -- Item adds %d to intellect, %c is +/-
ITEM_MOD_MANA = "%c%d 法力值"; -- Item adds %d to mana, %c is +/-
ITEM_MOD_SPIRIT = "%c%d 精神"; -- Item adds %d to spirit, %c is +/-
ITEM_MOD_STAMINA = "%c%d 耐力"; -- Item adds %d to stamina, %c is +/-
ITEM_MOD_STRENGTH = "%c%d 力量"; -- Item adds %d to strength, %c is +/-
ITEM_MOUSE_OVER = "将鼠标移动到图标上可以获得更多的信息";
ITEM_NO_DROP = "无法丢弃"; -- This is a No Drop item
ITEM_OPENABLE = "<右键点击打开>";
ITEM_PROPOSED_ENCHANT = "将获得%s的效果。"; -- Enchantment proposed in trade
ITEM_PURCHASED_COLON = "物品购入：";
ITEM_QUALITY0_DESC = "粗糙";
ITEM_QUALITY1_DESC = "普通";
ITEM_QUALITY2_DESC = "优秀";
ITEM_QUALITY3_DESC = "精良";
ITEM_QUALITY4_DESC = "史诗";
ITEM_QUALITY5_DESC = "传说";
ITEM_QUALITY6_DESC = "神器";
ITEM_RACES_ALLOWED = "种族：%s"; -- Lists the races allowed to use this item
ITEM_RANDOM_ENCHANT = "<随机附魔>";
ITEM_READABLE = "<右键点击阅读>"; -- Tooltip text for readable items
ITEM_REQ_REPUTATION = "需要 %s - %s"; -- Required faction reputation to use the item
ITEM_REQ_SKILL = "需要%s"; -- Required skill to use the item
ITEM_RESIST_ALL = "%c%d 所有抗性"; -- Item adds %d to all resistances, %c is +/-
ITEM_RESIST_SINGLE = "%c%d %s抗性"; -- Item adds %d to %s school resistance, first %c is +/-
ITEM_SET_BONUS = "套装：%s";
ITEM_SET_BONUS_GRAY = "(%d) 套装：%s";
ITEM_SET_NAME = "%s（%d/%d）"; -- Set name (2/5)
ITEM_SIGNABLE = "<右键点击以了解详情>";
ITEM_SOLD_COLON = "物品售出：";
ITEM_SOULBOUND = "已绑定"; -- Item is Soulbound
ITEM_SPELL_CHARGES_P1 = "%d次"; -- Spell charges on an item
ITEM_SPELL_CHARGES = "%d次"; -- Spell charges on an item
ITEM_SPELL_EFFECT = "效果：%s"; -- Item can cast spell %s
ITEM_SPELL_KNOWN = "已经学会";
ITEM_SPELL_TRIGGER_ONEQUIP = "装备：";
ITEM_SPELL_TRIGGER_ONPROC = "击中时可能：";
ITEM_SPELL_TRIGGER_ONUSE = "使用：";
ITEM_STARTS_QUEST = "该物品将触发一个任务"; -- Item is a quest giver
ITEM_SUFFIX_TEMPLATE = "%s %s"; -- Name, suffix
ITEM_TEXT_FROM = "发信人，";
ITEM_UNIQUE = "唯一"; -- Item is unique
ITEM_UNIQUE_MULTIPLE = "唯一（%d）"; -- Item is unique
ITEM_UNSELLABLE = "无法出售";
ITEM_WRAPPED_BY = "|cff00ff00<%s的礼物>|r"; -- %s is the person who wrapped the gift
ITEM_WRITTEN_BY = "由%s撰写";
ITEM_WRONG_CLASS = "你的职业无法使用这件物品！";
ITEM_WRONG_RACE = "你的种族无法使用这件物品！";
JOINED_PARTY = "%s加入了队伍。"; -- Player %s has joined the party
JOIN_AS_GROUP = "小队加入";
JOIN_AS_GROUP_TOOLTIP = "小队的队长可以选择将自己的整个小队加入战场等待序列。你的小队成员将同时进入战场。";
JOIN_NEW_CHANNEL = "加入新频道";
KEY1 = "按键设置1"; -- Label for the first key in the keybinding UI
KEY2 = "按键设置2"; -- Label for the second key in the keybinding UI
KEYBINDINGFRAME_MOUSEWHEEL_ERROR = "无法将鼠标滚轮的上下滚动状态绑定在动作条上";
KEYRING = "钥匙链"; -- Keyring bag name
KEY_APOSTROPHE = "'";
KEY_BACKSLASH = "\\";
KEY_BACKSPACE = "退格";
KEY_BACKSPACE_MAC = "删除";
KEY_BINDINGS = "按键设置"; -- Title for the key bindings frame
KEY_BOUND = "按键设置成功";
KEY_BUTTON3 = "鼠标中键";
KEY_BUTTON4 = "鼠标按键4";
KEY_BUTTON5 = "鼠标按键5";
KEY_COMMA = ",";
KEY_DELETE = "Delete";
KEY_DELETE_MAC = "Del";
KEY_DOWN = "方向键下";
KEY_END = "End";
KEY_ENTER = "回车";
KEY_ENTER_MAC = "返回";
KEY_ESCAPE = "Escape";
KEY_HOME = "Home";
KEY_INSERT = "Insert";
KEY_INSERT_MAC = "帮助";
KEY_LEFT = "方向键左";
KEY_LEFTBRACKET = "[";
KEY_MINUS = "-";
KEY_MOUSEWHEELDOWN = "鼠标滚轮向下滚动";
KEY_MOUSEWHEELUP = "鼠标滚轮向上滚动";
KEY_NUMLOCK = "Num Lock";
KEY_NUMLOCK_MAC = "清除";
KEY_NUMPAD0 = "数字键盘0";
KEY_NUMPAD1 = "数字键盘1";
KEY_NUMPAD2 = "数字键盘2";
KEY_NUMPAD3 = "数字键盘3";
KEY_NUMPAD4 = "数字键盘4";
KEY_NUMPAD5 = "数字键盘5";
KEY_NUMPAD6 = "数字键盘6";
KEY_NUMPAD7 = "数字键盘7";
KEY_NUMPAD8 = "数字键盘8";
KEY_NUMPAD9 = "数字键盘9";
KEY_NUMPADDECIMAL = "数字键盘.";
KEY_NUMPADDIVIDE = "数字键盘/";
KEY_NUMPADMINUS = "数字键盘-";
KEY_NUMPADMULTIPLY = "数字键盘*";
KEY_NUMPADPLUS = "数字键盘+";
KEY_PAGEDOWN = "Page Down";
KEY_PAGEUP = "Page Up";
KEY_PAUSE = "Pause";
KEY_PAUSE_MAC = "F15";
KEY_PERIOD = ".";
KEY_PLUS = "+";
KEY_PRINTSCREEN = "Print Screen";
KEY_PRINTSCREEN_MAC = "F13";
KEY_RIGHT = "方向键右";
KEY_RIGHTBRACKET = "]";
KEY_SCROLLLOCK = "Scroll Lock";
KEY_SCROLLLOCK_MAC = "F14";
KEY_SEMICOLON = ";";
KEY_SLASH = "/";
KEY_SPACE = "空格键"; -- Spacebar
KEY_TAB = "Tab";
KEY_TILDE = "~";
KEY_UNBOUND_ERROR = "|cffff0000%s功能未设置！|r";
KEY_UP = "方向键上";
KILLING_BLOWS = "杀死敌人";
KILLING_BLOW_TOOLTIP = "你亲手击杀的敌人";
KILLS = "杀敌"; -- Generic kills label
KNOWLEDGE_BASE = "知识库";
KNOWN_TALENTS_HEADER = "我的天赋"; -- Header for talents already known by the player
LABEL_NOTE = "注释";
LANGUAGE = "语言";
LASTONLINE = "最后上线";
LASTONLINE_DAYS_P1 = "%d天";
LASTONLINE_DAYS = "%d天";
LASTONLINE_HOURS = "%d小时";
LASTONLINE_HOURS_P1 = "%d小时";
LASTONLINE_MINS = "<1个小时";
LASTONLINE_MONTHS_P1 = "%d月";
LASTONLINE_MONTHS = "%d月之前";
LASTONLINE_YEARS_P1 = "%d年";
LASTONLINE_YEARS = "%d年";
LAST_ONLINE_COLON = "最后上线：";
LAUGH_WORD9 = "rofl";
LAUGH_WORD2 = "rofl";
LAUGH_WORD5 = "哈哈"; -- index 5
LAUGH_WORD6 = "哈哈";
LAUGH_WORD8 = "哈哈";
LAUGH_WORD7 = "哈哈";
LAUGH_WORD4 = "哈哈";
LAUGH_WORD3 = "hehe";
LAUGH_WORD1 = "lol";
LEARN_SKILL_TEMPLATE = "学习%s"; -- Used in the SkillFrame to display skills that are learnable
LEAVE_QUEUE = "离开队列";
LEAVING_COMBAT = "脱离战斗";
LEFT_PARTY = "%s离开了队伍。"; -- Player %s has left the party
LEGSSLOT = "腿部"; -- Legs slot
LESS_THAN_ONE_MINUTE = "< 1分钟";
LEVEL = "等级"; -- Level of a monster, character or item
LEVEL_ABBR = "Lvl"; -- Level abbreviation
LEVEL_GAINED = "等级 %d"; -- Indicates the required level to gain a skill line
LEVEL_RANGE = "等级范围";
LEVEL_REQUIRED = "需要等级%d"; -- Level required to use an item
LEVEL_TOO_LOW = "你需要达到等级%d才能装备这件物品。"; -- The player is not high enough level to equip this item
LEVEL_UP = "祝贺你，你升到了%d级！"; -- Tell player when he levels up
LEVEL_UP_CHAR_POINTS = "你获得了%d个天赋点数。";
LEVEL_UP_CHAR_POINTS_P1 = "你获得了%d个天赋点数。";
LEVEL_UP_HEALTH = "你获得%d的生命值。";
LEVEL_UP_HEALTH_MANA = "你获得%d的生命值和%d的法力值。";
LEVEL_UP_SKILL_POINTS = "你还可以再学习%d个专业。";
LEVEL_UP_SKILL_POINTS_P1 = "你还可以再学习%d个专业。";
LEVEL_UP_STAT = "你的%s提高了%d点。"; -- %s is the stat name, %d is the increase amount
LFG_LABEL = "我希望组队";
LOCATION_COLON = "所在地区：";
LOCK = "锁定";
LOCKED = "已锁"; -- Used in tooltips for locked items
LOCKED_WITH_ITEM = "需要%s"; -- A game object is locked with an item
LOCKED_WITH_SPELL = "需要%s"; -- A game object is locked with a spell
LOCKED_WITH_SPELL_KNOWN = "需要%s"; -- A game object is locked with a spell the player knows
LOCK_ACTIONBAR_TEXT = "锁定动作条";
LOCK_BATTLEFIELDMINIMAP = "锁定作战地图";
LOCK_WINDOW = "锁定窗口";
LOGOUT = "返回角色选择";
LOG_PERIODIC_EFFECTS = "周期性伤害";
LOOKING = "正在寻找";
LOOT = "战利品";
LOOT_FREE_FOR_ALL = "分配方式：自由拾取";
LOOT_GONE = "物品已经被搜刮";
LOOT_GROUP_LOOT = "分配方式：队伍分配";
LOOT_ITEM = "%s获得了物品：%s。"; -- The first %s is the player name, the other %s is the item name.
LOOT_ITEM_CREATED_SELF = "你制造了：%s。";
LOOT_ITEM_CREATED_SELF_MULTIPLE = "你制造了：%sx%d。";
LOOT_ITEM_MULTIPLE = "%s获得了物品：%sx%d。"; -- The first %s is the player name, the other %s is the item name, and the %d is the number of items they received.
LOOT_ITEM_PUSHED_SELF = "你获得了物品：%s。";
LOOT_ITEM_PUSHED_SELF_MULTIPLE = "你获得了：%sx%d。";
LOOT_ITEM_SELF = "你获得了物品：%s。";
LOOT_ITEM_SELF_MULTIPLE = "你得到了物品：%sx%d。";
LOOT_MASTER_LOOTER = "分配方式：队长分配";
LOOT_METHOD = "物品分配方式";
LOOT_MONEY = "%s拾取了%s。"; -- Tells player how much money a party member looted
LOOT_MONEY_SPLIT = "你分到%s。"; -- Tells player how much money the party looted
LOOT_NEED_BEFORE_GREED = "分配方式：需求优先";
LOOT_NEXT_PAGE = "切换页面"; -- Tells player how much money the party looted
LOOT_NO_DROP = "拾取该物品后会将它和你绑定"; -- Tells player he is about to pick up a no drop item
LOOT_PROMOTE = "提升为物品分配者";
LOOT_ROLL_ALL_PASSED = "所有人都放弃了：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_GREED = "%s选择了贪婪取向：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_GREED_SELF = "你选择了贪婪取向：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_NEED = "%s选择了需求取向：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_NEED_SELF = "你选择了需求取向：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_PASSED = "%s放弃了：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_PASSED_SELF = "你放弃了：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_ROLLED = "%s掷出%d，对象物品：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_ROLLED_GREED = "（贪婪）%d点：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s（%s）";
LOOT_ROLL_ROLLED_GREED_SELF = "你掷出了%d点（贪婪）：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_ROLLED_NEED = "（需求）%d点：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s（%s）";
LOOT_ROLL_ROLLED_NEED_SELF = "你掷出了%d点（需求）：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_ROLLED_SELF = "你掷出%d点：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_START = "开始掷骰子决定物品归属：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_WON = "%s赢得了：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_WON_NO_SPAM_GREED = "%1$s赢得了：%3$s|Hitem:%4$d:%5$d:%6$d:%7$d|h[%8$s]|h%9$s |cff818181（贪婪 - %2$d）|r";
LOOT_ROLL_WON_NO_SPAM_NEED = "%1$s赢得了：%3$s|Hitem:%4$d:%5$d:%6$d:%7$d|h[%8$s]|h%9$s |cff818181（需求 - %2$d）|r";
LOOT_ROLL_YOU_WON = "你赢得了：%s|Hitem:%d:%d:%d:%d|h[%s]|h%s";
LOOT_ROLL_YOU_WON_NO_SPAM_GREED = "你赢得了：%2$s|Hitem:%3$d:%4$d:%5$d:%6$d|h[%7$s]|h%8$s |cff818181（贪婪 - %1$d）|r";
LOOT_ROLL_YOU_WON_NO_SPAM_NEED = "你赢得了：%2$s|Hitem:%3$d:%4$d:%5$d:%6$d|h[%7$s]|h%8$s |cff818181（需求 - %1$d）|r";
LOOT_ROUND_ROBIN = "分配方式：轮流拾取";
LOOT_THRESHOLD = "物品分配界限";
LOSING_LOYALTY = "忠诚度下降";
LOW = "低";
MACRO = "宏";
MACROFRAME_CHAR_LIMIT = "已使用%d个字符,最多256个";
MACROS = "宏命令设置";
MACRO_HELP_TEXT_LINE2 = "- 输入 /macro 可打开宏UI，或从聊天命令菜单中选择";
MACRO_HELP_TEXT_LINE4 = "- 在宏中施放法术： /cast <法术名称> (<台词>)";
MACRO_HELP_TEXT_LINE3 = "- 使用宏UI可以编制聊天文字，表情和动作命令";
MACRO_HELP_TEXT_LINE1 = "宏帮助：";
MACRO_HELP_TEXT_LINE5 = "- 在已打开宏UI的情况下按住Shift点击法术书中的法术可将此法术添加到你的宏里";
MACRO_POPUP_CHOOSE_ICON = "选择一个图标：";
MACRO_POPUP_TEXT = "输入宏的名字（最多16个字符）"; -- Text for the macro popup
MAGE_INTELLECT_TOOLTIP = "提高你的法力值上限和使用法术造成致命一击的几率。\n使你更快地提升武器技能熟练度。";
MAGIC_RESISTANCES_COLON = "魔法抗性："; -- Character's magical resistances, with colon
MAIL_COD_ERROR = "付费取信邮件的金额不得超过%d";
MAIL_LABEL = "信件";
MAIL_LETTER_TOOLTIP = "点击这里来获得一份这封信\n永久性的副本。";
MAIL_REPLY_PREFIX = "回复：";
MAIL_RETURN = "退信";
MAIL_SUBJECT_LABEL = "主题：";
MAIL_TO_LABEL = "收件人：";
MAINHANDSLOT = "主手"; -- Main hand slot
MAINMENUBAR_LATENCY_LABEL = "延迟："; -- Latency label for the performance bar tooltip
MAINMENU_BUTTON = "主菜单";
MAIN_MENU = "主菜单"; -- The main menu
MALE = "男性"; -- Generic "Male" gender tag
MANA = "法力值"; -- Generic "mana"
MANAGE_ACCOUNT = "帐户管理";
MANAGE_ACCOUNT_URL = "http://signup.wowchina.com";
MANA_COLON = "法力值："; -- Character's mana, with colon
MANA_COST = "%d法力值";
MANA_COST_PER_TIME = "%d点法力值，外加每秒%d点";
MANA_LOW = "法力值过低";
MANA_POINTS = "法力值"; -- Generic "Mana"
MARKED_AFK = "你现在处于离开状态";
MARKED_AFK_MESSAGE = "你现在处于离开状态：%s";
MARKED_DND = "你现在处于请勿打扰状态：%s。";
MASTERY_POINTS_SPENT = "在%s天赋中花费的点数：";
MASTER_VOLUME = "主音量"; -- Sound options tag
MAX_FOLLOW_DIST = "最大镜头距离";
MAX_HP_TEMPLATE = "%d 最大生命值";
MEETINGSTONE_LEVEL = "等级 %d-%d";
MEETINGSTONE_TOOLTIP = "寻找更多队友去%s"; -- Displayed on the meeting stone minimap button letting you know which queue you are in.  suchas: Looking for more for Wailing Caverns
MELEE_ATTACK = "近战攻击";
MELEE_ATTACK_POWER = "近战攻击强度";
MELEE_ATTACK_POWER_TOOLTIP = "使近战武器的每秒伤害提高%.1f。";
MEMBERS = "成员";
MEMORY_EXHAUSTED = "Interface使用了多于%dMB的内存，请禁止一些附加工具并重新启动游戏。";
MERCHANT = "商人";
MERCHANT_BUYBACK = "从商人处购回";
MERCHANT_STOCK = "（%d）"; -- Label for the number of items in the stock of a merchant
MESSAGE_TYPES = "信息类型";
MILLISECONDS_ABBR = "毫秒"; -- Milliseconds abbreviation
MINIMAP_LABEL = "微缩地图";
MINUTES = "分钟"; -- Minutes of time
MINUTES_P1 = "分钟";
MINUTES_ABBR = "分钟";
MINUTES_ABBR_P1 = "分钟";
MINUTE_ONELETTER_ABBR = "%d m";
MISCELLANEOUS = "其它";
MISS = "未击中"; -- Miss on an attack
MISSEDOTHEROTHER = "%s没有击中%s。";
MISSEDOTHERSELF = "%s没有击中你。";
MISSEDSELFOTHER = "你没有击中%s。";
MODIFIERS_COLON = "管理员："; -- Character's statistical modifiers, with colon
MONEY = "钱";
MORE_REAGENTS = "更多材料";
MOUSE_LOOK_SPEED = "鼠标观察速度";
MOUSE_SENSITIVITY = "鼠标灵敏度";
MOVE_TO_INACTIVE = "隐藏";
MP = "法力值"; -- Abbreviation for "mana"
MULTISAMPLE = "多重采样";
MULTISAMPLING_FORMAT_STRING = "%d位色 %d位色深 %dx采样";
MUSIC_VOLUME = "音乐音量"; -- Sound options tag
NAME = "名字";
NAME_CHAT_WINDOW = "输入对话窗口名称";
NECKSLOT = "颈部"; -- Neck slot
NEED = "需求";
NEW = "新建";
NEWBIE_TOOLTIP_ABANDONQUEST = "放弃选定的任务，将其从任务日志中移除。任何被放弃的任务都可以重新获得。相关的任务物品将会留在你的物品栏中，放弃一个任务不会给你带来任何惩罚。";
NEWBIE_TOOLTIP_ADDFRIEND = "将一个玩家添加到你的好友名单中。当该玩家登录或者退出游戏的时候，你会收到相应的提示。其他玩家不会知道他们是否在你的好友名单之上。";
NEWBIE_TOOLTIP_ADDMEMBER = "添加新的公会成员。";
NEWBIE_TOOLTIP_ALLIANCE = "一名联盟的成员，和部落的成员相敌对（兽人，巨魔，牛头人，亡灵）。";
NEWBIE_TOOLTIP_BATTLEFIELDMINIMAP_OPTIONS = "鼠标右键点击以打开此窗口的自定义设置列表。可以用鼠标左键点击并拖拽此窗口。";
NEWBIE_TOOLTIP_BATTLEFIELD_GROUP_JOIN = "如果你是一个小队的队长，这个按钮可以将你的整个小队一起加入战场等待序列，并保证你的小队同时进入同一个战场副本。但是，如果你在此之后添加新的队员，或者某个队员正在其它战场的等待队列中，那么他们也许无法和你进入同一个战场。";
NEWBIE_TOOLTIP_CHARACTER = "关于你的人物的各种信息，包括装备、属性、技能和声望。";
NEWBIE_TOOLTIP_CHATMENU = "与别人联系的命令。使用聊天菜单，你可以和周围的玩家聊天，可以发送悄悄话，或者和队伍成员聊天，和一个朋友挥手致意或者创建一个宏。";
NEWBIE_TOOLTIP_CHATOPTIONS = "右键点击获得这个窗口的自定义选项列表。左键点击拖拉来移动这个窗口。";
NEWBIE_TOOLTIP_DEMOTE = "降低所选择玩家在公会中的等级。";
NEWBIE_TOOLTIP_DISHONORABLE_KILLS = "每当你在PvP中协助或主动杀死了一个等级过低（对你来说是灰色）的平民，则你获得一个非荣誉击杀。每次非荣誉击杀都会立刻稍稍降低你的总体排名，而非荣誉击杀的次数越多，每次遭受的惩罚也越严重。非荣誉击杀直接影响你的排名。";
NEWBIE_TOOLTIP_ENCHANTSLOT = "将物品放入这个空格中可以让其他玩家进行开锁、附魔或淬毒。放在这里的物品不会被交易，在交易结束之后，它会回到原拥有者的物品栏中。";
NEWBIE_TOOLTIP_ENTER_BATTLEGROUND = "当战场中有可用的空位时再加入战场。这样会比选择加入首先可用的战场的等待时间稍长。";
NEWBIE_TOOLTIP_FIRST_AVAILABLE = "加入首先可用的战场等待序列。如果你选择了“首先可用”，而另一个队友已经进入了“首先可用”的战场，那么你的首选战场将被改为这个战场。";
NEWBIE_TOOLTIP_FRIENDSTAB = "编辑你比较乐意一起游戏的玩家的名单。你可以收到这些玩家上线和下线的提示。";
NEWBIE_TOOLTIP_GROUPINVITE = "邀请选择的玩家加入队伍。";
NEWBIE_TOOLTIP_GUILDCONTROL = "自定义公会等级的名称和它们的权限。";
NEWBIE_TOOLTIP_GUILDGROUPINVITE = "邀请选择的公会成员加入队伍。";
NEWBIE_TOOLTIP_GUILDPUBLICNOTE = "点击察看你的公共信息。这个信息是别人通过公会用户界面可以查看到的。如果你点击这条信息，那你就可以对其进行编辑。";
NEWBIE_TOOLTIP_GUILDREMOVE = "将选定的玩家移除公会。";
NEWBIE_TOOLTIP_GUILDTAB = "查看关于你所在的公会及其会员的信息。如果你是公会的管理人员，还可以在这个窗口中进行公会管理工作。";
NEWBIE_TOOLTIP_GUILD_MEMBER_OPTIONS = "右键点击一名会员可以看到更多设置选项。";
NEWBIE_TOOLTIP_HEALTHBAR = "你现在所拥有的生命值。如果你的生命值为0的话，那你就会死亡。当你脱离战斗的时候，生命值会自动恢复。";
NEWBIE_TOOLTIP_HELP = "你可以通过这个菜单给我们发送建议和报告错误。如果你有什么问题或者需要帮助的话，你还可以使用帮助请求菜单来与我们的GM在游戏中对话。";
NEWBIE_TOOLTIP_HONORABLE_KILLS = "每当你或你所在的队伍在PvP中对一个敌方玩家造成伤害，并且该玩家随后被杀死，则你可以获得一次荣誉击杀。只有在目标等级对你来说是绿色或更高（即杀死与该玩家等级相当的怪物可以获得经验值）的情况下才能获得荣誉击杀。只有敌方玩家会提供荣誉击杀。";
NEWBIE_TOOLTIP_HONOR_CONTRIBUTION_POINTS = "荣誉点数是通过在PvP战斗中进行荣誉击杀获得的。杀死敌人可以获得多少点数取决于你（或你所在的队伍）对其造成了多少伤害，并由你所在的小队或团队均分。如果你在短时间内反复杀死同一个敌方玩家，则每次杀死他所获得的荣誉点数也会越来越少，因此，在数次杀死同一个敌方玩家之后，你将获得极少的荣誉点数，甚至不再获得任何荣誉点数。";
NEWBIE_TOOLTIP_HONOR_STANDING = "排名是根据你与其他同阵营玩家（联盟或部落）的对比得出的。如果你的排名是150，那就是说有149名玩家在上一阶段中获得的荣誉点数比你多。你每周必须获得至少25个荣誉击杀点数才能参与军阶评定。";
NEWBIE_TOOLTIP_HORDE = "一名部落的成员，和联盟的成员相敌对（暗夜精灵，矮人，人类，侏儒）。";
NEWBIE_TOOLTIP_IGNOREPLAYER = "将一个玩家添加到你的屏蔽名单中。你将不会收到屏蔽名单上玩家的信息或者别的文字，被屏蔽的玩家也无法使用任何方式与你联络。";
NEWBIE_TOOLTIP_IGNORETAB = "编辑你想要屏蔽的玩家的名单。";
NEWBIE_TOOLTIP_LATENCY = "和游戏服务器通讯的平均时间。较低的通讯时间将被显示为绿色，较高的通讯时间将被显示为黄色甚至是红色。如果延迟一直很高的话，那么你的互联网连接可能有问题。";
NEWBIE_TOOLTIP_MAINMENU = "你可以在这里修改你的视频、音频和界面设置，或者自定义快捷键。你还可以在这里选择返回人物选择画面或者退出游戏。";
NEWBIE_TOOLTIP_MANABAR0 = "你现在所拥有的法力值。玩家需要法力值来施放法术。如果你不进行施法动作的时间超过5秒，法力值就会开始自动回复。";
NEWBIE_TOOLTIP_MANABAR1 = "战士使用技能需要怒气。怒气可以通过制造伤害或者承受伤害获得。";
NEWBIE_TOOLTIP_MANABAR2 = "已废弃";
NEWBIE_TOOLTIP_MANABAR3 = "盗贼需要能量来使用他们的技能。能量会不断地自动恢复。";
NEWBIE_TOOLTIP_MANABAR4 = "你现在宠物的喜悦程度。让你的宠物感到满足会让它对敌人造成普通伤害，而高兴状态下的宠物会制造更多的伤害，而不高兴状态下的宠物制造的伤害会减少。当你的宠物死亡或者饥饿状态的时候会降低喜悦程度，而喂食你的宠物能增加它的喜悦程度。";
NEWBIE_TOOLTIP_MINIMAPTOGGLE = "开启/关闭小地图显示";
NEWBIE_TOOLTIP_PARTYOPTIONS = "如果你在一个小队中，你可以右键点击你的头像来打开小队选项菜单。这个菜单可以让你退出队伍或者查看现在使用的物品捡拾方式。小队的领袖可以增加或者移除队员，并且还可以改变物品捡拾方式。";
NEWBIE_TOOLTIP_PLAYEROPTIONS = "右键点击可以实施和别的玩家交互的特殊命令。你可以查看他们的装备，发出组队邀请，开启交易窗口或者和他进行对决。一个队伍的领导可以提升队伍中队员的作用或者将其移除队伍。";
NEWBIE_TOOLTIP_PROMOTE = "提升所选择玩家在公会中的等级。";
NEWBIE_TOOLTIP_PVPFFA = "你可以在游戏中攻击和被任何玩家攻击。";
NEWBIE_TOOLTIP_QUESTLOG = "你现在所拥有的任务。你最多可以同时拥有20条任务记录。";
NEWBIE_TOOLTIP_RAIDTAB = "查看或者编辑你的大型团队。团队是一支由超过5个玩家组成的队伍，可以用来击败高等级的非常强大的敌人。";
NEWBIE_TOOLTIP_RANK = "荣誉系统的计算结果将以军阶的形式表现。根据你的本周排名和当前军阶级别的对比，你的军阶级别可能会提高、降低或保持不变。高级的玩家提升军阶的速度更快（下降也更慢）。另外，低级军阶提升的速度比高级军阶提升的速度更快。";
NEWBIE_TOOLTIP_RANK_POSITION = "你在排名体系中的位置。它会告诉你距离晋级或降级还有多远，但是只在每周更新排名时才会随之更新。";
NEWBIE_TOOLTIP_REMOVEFRIEND = "将选定的玩家从你的好友名单之中移除。";
NEWBIE_TOOLTIP_SENDMESSAGE = "给选定的玩家发送信息。";
NEWBIE_TOOLTIP_SHAREQUEST = "和附近符合条件的队伍队员共享任务。有些任务，比如需要使用任务物品的任务无法共享。";
NEWBIE_TOOLTIP_SOCIAL = "关于游戏中的其他玩家人物的信息。你可以使用社交窗口来设置你的好友名单和屏蔽名单，以及查看有哪些玩家在线。";
NEWBIE_TOOLTIP_SPELLBOOK = "你的所有法术和技能。如果你想要将法术或技能图标拖到动作条上，就打开法术和技能窗口，左键点击你想要拖拉的法术或者技能，然后将其拖到动作条上。";
NEWBIE_TOOLTIP_STOPIGNORE = "将选定的玩家从你的屏蔽名单之中移除。";
NEWBIE_TOOLTIP_TALENTS = "天赋的各种组合选择能够强化你的角色，并使你的角色与众不同。";
NEWBIE_TOOLTIP_UNIT_DUEL = "向选定的玩家发出挑战，进行对决。生命值首先降到零的玩家将被判负。";
NEWBIE_TOOLTIP_UNIT_FOLLOW = "让你开始跟随选定的玩家。你将自动跟随这个玩家，直到自动跟随模式被取消。";
NEWBIE_TOOLTIP_UNIT_FREE_FOR_ALL = "在自由拾取规则下，所有的队伍队员都可以捡拾任何杀死怪物掉下的物品。金钱自动平分。";
NEWBIE_TOOLTIP_UNIT_GROUP_LOOT = "在队伍分配规则下，玩家通常使用轮流拾取规则来获得战利品，但是品质高于预设级别的物品会经过掷骰子决定归属。";
NEWBIE_TOOLTIP_UNIT_INSPECT = "观察选定的玩家所装备的护甲和武器。";
NEWBIE_TOOLTIP_UNIT_INVITE = "邀请选择的玩家加入队伍。";
NEWBIE_TOOLTIP_UNIT_LEAVE_PARTY = "从玩家目前所在的小队中开除该玩家。";
NEWBIE_TOOLTIP_UNIT_LOOT_THRESHOLD = "从怪物身上拾取战利品时，品质不低于此级别的物品将经过掷骰子决定归属。";
NEWBIE_TOOLTIP_UNIT_MASTER_LOOTER = "在队长分配规则下，只有队伍领袖才能够捡拾品质等于或高于预设级别的物品。在该级别之下的物品和金钱采用轮流拾取方式分配。";
NEWBIE_TOOLTIP_UNIT_NEED_BEFORE_GREED = "在需求优先规则下，玩家通常使用轮流拾取规则来获得战利品，但是品质高于预设级别的物品会由能使用该物品的队友掷骰子决定归属。";
NEWBIE_TOOLTIP_UNIT_PET_ABANDON = "放弃你的宠物，让它回归自然。一旦你放弃了宠物，它将永远不再回来。";
NEWBIE_TOOLTIP_UNIT_PET_DISMISS = "解散由你控制的宠物。";
NEWBIE_TOOLTIP_UNIT_PET_PAPERDOLL = "关于你宠物的信息，包括它的属性、法术抗性和食谱。";
NEWBIE_TOOLTIP_UNIT_PET_RENAME = "给你的宠物设定一个新名字。一个宠物只能被改名一次。";
NEWBIE_TOOLTIP_UNIT_PROMOTE = "交出队伍的领导权，选定的玩家将继续行使这一职责。";
NEWBIE_TOOLTIP_UNIT_ROUND_ROBIN = "在轮流拾取规则下，队伍队员按照顺序捡拾怪物掉落物品。金钱自动平分。";
NEWBIE_TOOLTIP_UNIT_TRADE = "与选择的玩家进行交易。";
NEWBIE_TOOLTIP_UNIT_UNINVITE = "将选定的玩家移出队伍。";
NEWBIE_TOOLTIP_WHOTAB = "找到其他玩家在这个世界中的位置。";
NEWBIE_TOOLTIP_WORLDMAP = "在你探索一个地区时，它附近的地区会显示在世界地图上。你可以使用左键放大地图，右键缩小地图。";
NEWBIE_TOOLTIP_XPBAR = "你所得到的经验值（XP）。你的经验槽的颜色代表了你的休息状态，蓝色代表精力充沛，紫色代表正常。精力充沛的玩家通过杀死怪物所得的经验值是正常状态下的两倍，你可以在城市或者旅馆中逗留或者下线以获得更多的休息良好状态时间。";
NEW_CHAT_WINDOW = "创建新窗口";
NEW_LEADER = "新的领袖";
NEXT = "下一页"; -- Generic "next"
NEXT_ABILITY = "下一个技能"; -- The next ability in a particular skill line
NO = "否"; -- Generic "no"
NONE = "无";
NONEQUIPSLOT = "无法装备"; -- Items that cannot be equipped
NONE_CAPS = "无"; -- "None", in all capitals
NOTE = "公共信息";
NOTE_COLON = "备注：";
NOTE_SUBMITTED = "信件已经提交";
NOTE_SUBMIT_FAILED = "信件提交失败";
NOT_APPLICABLE = "无";
NOT_BOUND = "未设置";
NOT_ENOUGH_MANA = "你没有足够的法力值来施放%s。"; -- Tell player he doesn't have enough mana to cast a spell
NOT_IN_GROUP = "你没有加入任何队伍"; -- You have left the party
NOT_TAMEABLE = "不可驯服";
NOT_YET_SIGNED = "<未被签名>";
NO_ATTACHMENTS = "无附件";
NO_BIDS = "无竞价";
NO_EMPTY_KEYRING_SLOTS_ERROR = "你的钥匙链已经满了。";
NO_EQUIPMENT_SLOTS_AVAILABLE = "你没有可以装备这件物品的栏位。"; -- An item dropped on the player's picture rather than a particular slot can't be equipped
NO_RAID_INSTANCES_SAVED = "你在任何副本中的进度都未保存";
NO_RESPONSE = "无回复";
NUMBER_OF_RESULTS_TEMPLATE = "物品%d - %d（总数%d）";
NUM_RAID_MEMBERS = "%d团队成员";
OBJECT_ALPHA = "物体半透明效果";
OFFICER_NOTE_COLON = "官员备注";
OKAY = "确定"; -- Generic "okay"
ONLY_EMPTY_BAGS = "只能捡起空的背包！"; -- The player can only pick up empty bags
OPACITY = "不透明";
OPENMAIL = "打开邮件";
OPEN_LOCK_OTHER = "%1$s对%3$s使用%2$s。";
OPEN_LOCK_SELF = "你对%2$s使用%1$s。";
OPTIONAL = "可选";
OPTIONS_BRIGHTNESS = "亮度";
OPTIONS_MENU = "选项菜单";
OPTION_LOGOUT_REQUIREMENT = "需要重登录才能生效";
OPTION_RESTART_REQUIREMENT = "需要重新启动游戏才能生效";
OPTION_TOOLTIP_ALWAYS_SHOW_MULTIBARS = "勾选此选项后，额外的动作条将总是被显示出来。";
OPTION_TOOLTIP_AMBIENCE_VOLUME = "调整环境音效的音量。";
OPTION_TOOLTIP_ANIMATION = "PLACE_HOLDER";
OPTION_TOOLTIP_ANISOTROPIC = "提高材质锐度，特别是远处的物体材质。降低这个数值可以提高整体运行速度。";
OPTION_TOOLTIP_ASSIST_ATTACK = "自动攻击使用\"/assist\"指令选定的目标。";
OPTION_TOOLTIP_AUTO_FOLLOW_SPEED = "调节在总是跟随和智能跟随模式下的镜头移动速度。";
OPTION_TOOLTIP_AUTO_JOIN_GUILD_CHANNEL = "钩选这个选项可以让你自动加入公会招募频道，无论你是否已经加入了某个公会。";
OPTION_TOOLTIP_AUTO_QUEST_WATCH = "当你达到一项任务目标时，该任务会被自动追踪5分钟。";
OPTION_TOOLTIP_AUTO_SELF_CAST = "开启此选项后，如果你周围没有友方目标或没有目标没，则可以对友方目标施放的法术会自动对你本人施放。这个功能可以指定快捷键。";
OPTION_TOOLTIP_BLOCK_TRADES = "阻止所有的交易要求。";
OPTION_TOOLTIP_BUFFERING = "当垂直同步开启的时候启用三倍桢数缓冲。选择三倍缓冲可以提高你的桢数。";
OPTION_TOOLTIP_CAMERA1 = "设定视角，使其固定在设置的地方，除非你的角色移动了。";
OPTION_TOOLTIP_CAMERA2 = "设定视角，使视角总是处于你的角色后方。";
OPTION_TOOLTIP_CAMERA3 = "设定视角，使其固定在一点，永远不自动调节。";
OPTION_TOOLTIP_CAMERA_ALWAYS = "设定视角，使视角总是处于你的角色后方。";
OPTION_TOOLTIP_CAMERA_NEVER = "设定视角，使其固定在一点，永远不自动调节。";
OPTION_TOOLTIP_CAMERA_SMART = "将视角固定在你所设置的角度，但你的人物移动时则恢复到跟踪视角。（推荐模式）";
OPTION_TOOLTIP_CHAT_BUBBLES = "在人物头顶上显示“/说”指令和“/喊”指令的文字泡泡。";
OPTION_TOOLTIP_CHAT_LOCKED = "锁定所有聊天窗口，这样它们就不会被修改了。";
OPTION_TOOLTIP_CINEMATIC_SUBTITLES = "开启开场动画字幕。";
OPTION_TOOLTIP_CLEAR_AFK = "移动或者说话时\n自动解除暂离状态。";
OPTION_TOOLTIP_CLICKCAMERA_LOCKED = "设置视角，使其在你使用鼠标点击移动的同时，视角保持与你行进方向的一致。";
OPTION_TOOLTIP_CLICKCAMERA_NEVER = "设置视角，使其在使用鼠标点击移动的时候不改变视角。";
OPTION_TOOLTIP_CLICKCAMERA_SMART = "设置视角，使其在你使用点击鼠标移动改变方向的时候稍作延时，然后跟随你的人物角色。（推荐模式）";
OPTION_TOOLTIP_CLICK_CAMERA2 = "设置视角，使其在你使用鼠标点击移动的同时，视角保持与你行进方向的一致。";
OPTION_TOOLTIP_CLICK_CAMERA3 = "设置视角，使其在使用鼠标点击移动的时候不改变视角。";
OPTION_TOOLTIP_CLICK_CAMERA1 = "设置视角，使其在你使用点击鼠标移动改变方向的时候稍作延时，然后跟随你的人物角色。（推荐模式）";
OPTION_TOOLTIP_CLICK_CAMERA_STYLE = "决定点击移动方式的时候视角跟随方式。";
OPTION_TOOLTIP_CLICK_TO_MOVE = "使用鼠标点击来把你的角色移动到指定的地点。";
OPTION_TOOLTIP_COMBAT_TEXT_MODE = "设置战斗信息滚动的方向。";
OPTION_TOOLTIP_COMBAT_TEXT_SCROLL_DOWN = "战斗记录向下滚动，而非向上滚动。";
OPTION_TOOLTIP_COMBAT_TEXT_SHOW_AURAS = "当玩家获得魔法效果时显示信息。";
OPTION_TOOLTIP_COMBAT_TEXT_SHOW_AURA_FADE = "当魔法效果从玩家身上消失时显示信息。";
OPTION_TOOLTIP_COMBAT_TEXT_SHOW_COMBAT_STATE = "当你进入或脱离战斗时显示信息。";
OPTION_TOOLTIP_COMBAT_TEXT_SHOW_COMBO_POINTS = "当你获得新的连击点时，显示连击点的数量。";
OPTION_TOOLTIP_COMBAT_TEXT_SHOW_DODGE_PARRY_MISS = "当敌人的攻击被你躲闪、招架或未击中你时显示信息。";
OPTION_TOOLTIP_COMBAT_TEXT_SHOW_FRIENDLY_NAMES = "当一个友方施法者对你施放治疗法术时，显示他/她的名字。";
OPTION_TOOLTIP_COMBAT_TEXT_SHOW_HONOR_GAINED = "显示你通过杀死其他玩家所得到的荣誉值。";
OPTION_TOOLTIP_COMBAT_TEXT_SHOW_LOW_HEALTH_MANA = "当你的生命值或法力值低于20%的时候显示信息。";
OPTION_TOOLTIP_COMBAT_TEXT_SHOW_MANA = "显示所有立即获取的法力值、怒气值和能量值。";
OPTION_TOOLTIP_COMBAT_TEXT_SHOW_REACTIVES = "当特定的反击型职业技能和法术（如战士的压制）可以使用时显示信息。";
OPTION_TOOLTIP_COMBAT_TEXT_SHOW_REPUTATION = "当玩家在某个阵营中的声望提高或降低时显示信息。";
OPTION_TOOLTIP_COMBAT_TEXT_SHOW_RESISTANCES = "当你抵抗攻击或法术时显示信息。";
OPTION_TOOLTIP_DEATH_EFFECT = "关闭此选项可以禁止鬼魂状态下的泛光效果，可略微提高运行速度。";
OPTION_TOOLTIP_DISABLE_SPAM_FILTER = "禁用聊天文字中的垃圾信息过滤功能。";
OPTION_TOOLTIP_ENABLE_ALL_SHADERS = "开启所有像素遮罩。";
OPTION_TOOLTIP_ENABLE_ALL_SOUND = "开启或禁止所有游戏声音。";
OPTION_TOOLTIP_ENABLE_AMBIENCE = "开启环境音效。";
OPTION_TOOLTIP_ENABLE_EMOTE_SOUNDS = "开启/关闭表情指令的音效。";
OPTION_TOOLTIP_ENABLE_ERROR_SPEECH = "开启错误提示语音（比如“目标太远”）。";
OPTION_TOOLTIP_ENABLE_GROUP_SPEECH = "开启队友语音。";
OPTION_TOOLTIP_ENABLE_MUSIC = "开启背景音乐。";
OPTION_TOOLTIP_ENABLE_MUSIC_LOOPING = "循环播放背景音乐。";
OPTION_TOOLTIP_ENABLE_SOUND_AT_CHARACTER = "将声源接收位置设定为来自玩家角色所在位置，而非镜头位置。";
OPTION_TOOLTIP_ENVIRONMENT_DETAIL = "控制你所能看见物体的远近。降低这个数值可以提高运行速度。";
OPTION_TOOLTIP_FARCLIP = "改变可视地形的范围。减少这个数值可以提高运行速度。";
OPTION_TOOLTIP_FIX_LAG = "开启此选项可以减少用户界面延迟，但也会降低画面祯数。";
OPTION_TOOLTIP_FOLLOW_TERRAIN = "根据地形自动调节视角。如果你的角色在爬坡，视角会向上旋转；如果你的角色在往下走，那视角会向下旋转。";
OPTION_TOOLTIP_FULL_SCREEN_GLOW = "开启全屏幕柔化和光影效果。关闭这个选项有时可以提高运行速度。";
OPTION_TOOLTIP_GAMEFIELD_DESELECT = "钩选此框可以让你避免因为点击画面上的空白区域而取消对当前目标的选择。你只能通过按下ESC键或点击另一个目标来切换你所选择的目标。";
OPTION_TOOLTIP_GAMMA = "控制游戏的亮度。增加亮度直到你清晰的辨别右侧的21级灰度条。";
OPTION_TOOLTIP_GUILDMEMBER_ALERT = "当你的公会会员登录或者退出游戏时显示提示信息。";
OPTION_TOOLTIP_HARDWARE_CURSOR = "开启这个选项来获得最佳运行表现，除非你遇到了图标问题。";
OPTION_TOOLTIP_HEAD_BOB = "在第一人称视角时候，模拟头部晃动。";
OPTION_TOOLTIP_HIDE_OUTDOOR_WORLD_STATE = "钩选此框将隐藏界面上的所有本区域范围内的目标指示。";
OPTION_TOOLTIP_HIDE_PARTY_INTERFACE = "点击这里隐藏你的队员的头像和生命槽。";
OPTION_TOOLTIP_INVERT_MOUSE = "鼠标移动方向与改变视角方向相反。";
OPTION_TOOLTIP_LOCK_ACTIONBAR = "使玩家无法拖动动作条上的快捷技能图标。这项功能可以通过按键设置界面设置相应的快捷键。";
OPTION_TOOLTIP_LOG_PERIODIC_EFFECTS = "显示周期性伤害效果数值，比如撕裂和暗言术：痛。";
OPTION_TOOLTIP_MASTER_VOLUME = "调整主音量。";
OPTION_TOOLTIP_MAX_FOLLOW_DIST = "调节最大跟随距离。";
OPTION_TOOLTIP_MOUSE_LOOK_SPEED = "调节使用鼠标改变视角时的移动速度。";
OPTION_TOOLTIP_MOUSE_SENSITIVITY = "调节鼠标指针移动速度。";
OPTION_TOOLTIP_MULTISAMPLING = "提高多重采样率可使模型边缘更加平滑。";
OPTION_TOOLTIP_MUSIC_VOLUME = "调整背景音乐的音量。";
OPTION_TOOLTIP_OBJECT_ALPHA = "PLACE_HOLDER";
OPTION_TOOLTIP_PARTY_CHAT_BUBBLES = "在队友头顶上显示组队聊天的文字泡泡。";
OPTION_TOOLTIP_PET_NAMEPLATES = "开启/关闭你的宠物、图腾和守护者的名字显示。";
OPTION_TOOLTIP_PET_SPELL_DAMAGE = "显示你的宠物造成的法术伤害。";
OPTION_TOOLTIP_PHONG_SHADING = "开启这个选项可以平滑角色光线表现。";
OPTION_TOOLTIP_PROFANITY_FILTER = "开启不良语句过滤。";
OPTION_TOOLTIP_REMOVE_CHAT_DELAY = "选择后当鼠标移动到聊天窗口之后窗口立即显示。";
OPTION_TOOLTIP_SCROLL_ARC = "战斗信息文字以弧线路径远离玩家。";
OPTION_TOOLTIP_SCROLL_DOWN = "战斗信息文字向屏幕下方滚动。";
OPTION_TOOLTIP_SCROLL_UP = "战斗信息文字向屏幕上方滚动。";
OPTION_TOOLTIP_SHOW_BUFF_DURATION = "显示/隐藏增益效果的持续时间。";
OPTION_TOOLTIP_SHOW_CASTABLE_BUFFS = "只显示玩家可施放的增益法术。";
OPTION_TOOLTIP_SHOW_CLOAK = "取消这个选项来隐藏你的角色的披风。";
OPTION_TOOLTIP_SHOW_COMBAT_TEXT = "钩选此项将在屏幕上直观显示额外的战斗信息。";
OPTION_TOOLTIP_SHOW_DAMAGE = "显示攻击敌对生物所造成的伤害。";
OPTION_TOOLTIP_SHOW_DISPELLABLE_DEBUFFS = "只显示玩家可驱散的负面效果。";
OPTION_TOOLTIP_SHOW_GUILD_NAMES = "总是显示游戏世界中的玩家公会名字。";
OPTION_TOOLTIP_SHOW_HELM = "取消这个选项来隐藏你的角色的头盔。";
OPTION_TOOLTIP_SHOW_LOOT_SPAM = "不选中此项可以隐藏各人掷骰子的信息，只显示获胜者的掷骰子信息。";
OPTION_TOOLTIP_SHOW_MULTIBAR1 = "打开/关闭屏幕左下角的可选动作条。";
OPTION_TOOLTIP_SHOW_MULTIBAR2 = "打开/关闭屏幕右下角的可选动作条。";
OPTION_TOOLTIP_SHOW_MULTIBAR3 = "打开/关闭屏幕右边的可选动作条。";
OPTION_TOOLTIP_SHOW_MULTIBAR4 = "打开/关闭屏幕右边的第二个可选动作条。";
OPTION_TOOLTIP_SHOW_NEWBIE_TIPS = "显示不同玩家界面元素的详细信息。高级玩家可以将这个选项关闭。";
OPTION_TOOLTIP_SHOW_NPC_NAMES = "总是显示游戏世界中的NPC名字。";
OPTION_TOOLTIP_SHOW_OWN_NAME = "显示你的角色的名字。";
OPTION_TOOLTIP_SHOW_PARTY_BACKGROUND = "在小队成员后面显示背景。";
OPTION_TOOLTIP_SHOW_PARTY_PETS = "显示队友宠物的头像。";
OPTION_TOOLTIP_SHOW_PET_MELEE_DAMAGE = "显示宠物造成的伤害。";
OPTION_TOOLTIP_SHOW_PLAYER_NAMES = "总是显示游戏世界中的玩家姓名。";
OPTION_TOOLTIP_SHOW_PLAYER_TITLES = "总是显示游戏世界中的玩家头衔。";
OPTION_TOOLTIP_SHOW_QUEST_FADING = "钩选此框以立刻显示所有任务文本。";
OPTION_TOOLTIP_SHOW_TARGET_OF_TARGET = "显示或隐藏你的目标所选定的目标。";
OPTION_TOOLTIP_SHOW_TIPOFTHEDAY = "在游戏读取界面上显示或隐藏每日小窍门。";
OPTION_TOOLTIP_SHOW_TUTORIALS = "显示介绍你如何进行魔兽世界游戏的教程。";
OPTION_TOOLTIP_SHOW_UNIT_NAMES = "在游戏中显示单位名称。";
OPTION_TOOLTIP_SIMPLE_CHAT = "自动设置无法修改设定的主对话窗口和战斗日志窗口。";
OPTION_TOOLTIP_SMART_PIVOT = "当镜头固定在地面上时，使你可以自由观察。";
OPTION_TOOLTIP_SOUND_VOLUME = "调整音效的音量。";
OPTION_TOOLTIP_SPELL_DETAIL = "控制法术效果的细节级别。调低此选项可以提高运行速度。";
OPTION_TOOLTIP_STATUS_BAR = "用数字表示经验值、生命值、法力值、怒气值、能量值或快乐值。";
OPTION_TOOLTIP_TARGETOFTARGET3 = "当你独自冒险时显示目标的目标。";
OPTION_TOOLTIP_TARGETOFTARGET2 = "当你在一个小队中时显示目标的目标。";
OPTION_TOOLTIP_TARGETOFTARGET1 = "当你在一个团队中时显示目标的目标。";
OPTION_TOOLTIP_TARGETOFTARGET5 = "总是显示目标的目标。";
OPTION_TOOLTIP_TARGETOFTARGET4 = "当你在一个团队或小队中时显示目标的目标。";
OPTION_TOOLTIP_TARGETOFTARGET_ALWAYS = "总是显示目标的目标。";
OPTION_TOOLTIP_TARGETOFTARGET_PARTY = "当你在一个小队中时显示目标的目标。";
OPTION_TOOLTIP_TARGETOFTARGET_RAID = "当你在团队中时显示目标的目标。";
OPTION_TOOLTIP_TARGETOFTARGET_RAID_AND_PARTY = "当你在一个团队或小队中时显示目标的目标。";
OPTION_TOOLTIP_TARGETOFTARGET_SOLO = "当你独自冒险时显示目标的目标。";
OPTION_TOOLTIP_TERRAIN_HIGHLIGHTS = "开启地形和水中的镜像高光。关闭这个选项有时可以提高运行速度。";
OPTION_TOOLTIP_TERRAIN_TEXTURE = "设置地形材质细节。";
OPTION_TOOLTIP_TEXTURE_DETAIL = "控制所有材质细节的层次。降低这个数值可以提高运行速度。";
OPTION_TOOLTIP_TRILINEAR = "开启高质量材质过滤。关闭此选项可以提高运行速度。";
OPTION_TOOLTIP_UI_SCALE = "改变用户游戏界面尺寸";
OPTION_TOOLTIP_USE_DESKTOP_GAMMA = "使用桌面Gamma设定。";
OPTION_TOOLTIP_USE_REFRESH = "改变显示器刷新率。越高的刷新率可以让屏幕的闪动率看上去更低。";
OPTION_TOOLTIP_USE_RESOLUTION = "改变游戏分辨率。降低分辨率可以提高游戏运行速度。";
OPTION_TOOLTIP_USE_UBERTOOLTIPS = "开启屏幕右下角的具体信息提示。";
OPTION_TOOLTIP_USE_UISCALE = "选择则可以使用用户界面比例调节，不选择的话就使用系统默认比例。";
OPTION_TOOLTIP_USE_WEATHER_SHADER = "如果天气效果会导致你的计算机崩溃，就不要开启这一选项。";
OPTION_TOOLTIP_VERTEX_ANIMATION_SHADERS = "开启顶点着色器加速动画。开启这个选项将最大化画面表现。";
OPTION_TOOLTIP_VERTICAL_SYNC = "将你的游戏祯数与显示器刷新率同步。可以解决游戏中的图像无法正常显示的问题。";
OPTION_TOOLTIP_WATER_COLLISION = "设置视角，使其在你角色处于水面之上的时候镜头在水面之上，而当你在水下的时候，视角在水面之下。";
OPTION_TOOLTIP_WEATHER_DETAIL = "调节天气效果的表现强度。降低这个效果可以提高运行速度。";
OPTION_TOOLTIP_WINDOWED_MAXIMIZED = "钩选此项以最大化窗口并去除边框。";
OPTION_TOOLTIP_WINDOWED_MODE = "钩选此框，在非全屏幕的窗口模式下游戏。\n\n在窗口模式下，游戏将使用桌面Gamma值，并无法通过下面的滑杆调节。";
OPTION_TOOLTIP_WORLD_LOD = "开启地形多边型细节动态缩减，打开此选项可以提高运行速度。";
OTHER_MESSAGES = "其它信息";
OUTBID = "开价被压过";
OUTBID_BY = "更高的出价";
OUT_OF_ENERGY = "能量值不足";
OUT_OF_FOCUS = "集中值不足";
OUT_OF_HEALTH = "生命值不足";
OUT_OF_MANA = "法力值不足";
OUT_OF_RAGE = "怒气值不足";
PAGE_NUMBER = "第%d页"; -- Generic page number, as in "Page 3"
PALADIN_INTELLECT_TOOLTIP = "提高你的法力值上限和使用法术造成致命一击的几率。\n使你更快地提升武器技能熟练度。";
PALADIN_STRENGTH_TOOLTIP = "提高你的近战武器的攻击强度。\n使你用盾牌格挡攻击时减免更多的伤害。";
PARENS_TEMPLATE = "（%s）"; -- Generic template to add parentheses around a string
PARRIED = "招架"; -- Parried an attack
PARRY = "招架"; -- Parry an attack
PARTY = "队伍";
PARTYKILLOTHER = "%s被%s干掉了！";
PARTY_CHAT_BUBBLES_TEXT = "显示组队聊天泡泡";
PARTY_INVITE = "邀请"; -- Invite a player to join your party
PARTY_LEAVE = "离开队伍"; -- Leave your current party
PARTY_MESSAGE = "队伍聊天"; -- Message to your party
PARTY_OPTIONS_LABEL = "组队选项";
PARTY_PROMOTE = "提升为队长"; -- Promote a party member to be the new leader
PARTY_QUEST_STATUS_NONE = "你附近没有正在做该任务的队友。"; -- Indicates that a party member is out of range
PARTY_QUEST_STATUS_ON = "附近正在做该任务的队友："; -- Indicates if a party member is also on the quest
PARTY_UNINVITE = "取消邀请"; -- Kick a player out of the group
PASS = "放弃";
PASSIVE_PARENS = "（被动技能）"; -- Passive label in parentheses
PERCENT_SYMBOL = "%%";
PERIODICAURADAMAGEOTHEROTHER = "%4$s的%5$s使%1$s受到了%2$d点%3$s伤害。"; -- Bob suffers 5 frost damage from Jeff's Ice Nova.
PERIODICAURADAMAGEOTHERSELF = "你受到%d点%s伤害（%s的%s）。"; -- You suffer 3 frost damage from Rabbit's Ice Nova.
PERIODICAURADAMAGESELFOTHER = "你的%4$s使%1$s受到了%2$d点%3$s伤害。"; -- Rabbit suffers 3 frost damage from your Ice Nova.
PERIODICAURADAMAGESELFSELF = "你受到%d点%s伤害（你的%s）。";
PERIODICAURAHEALOTHEROTHER = "%s获得%d点生命值（%s的%s）。"; -- Bob gains 10 health from Jane's Rejuvenation.
PERIODICAURAHEALOTHERSELF = "你因%2$s的%3$s而获得了%1$d点生命值。"; -- You gain 12 health from John's Rejuvenation.
PERIODICAURAHEALSELFOTHER = "%s获得%d点生命值（你的%s）。"; -- Bob gains 10 health from your Rejuvenation.
PERIODICAURAHEALSELFSELF = "你因%2$s而获得了%1$d点生命值。"; -- You gain 10 health from Rejuvenation.
PERIODIC_MESSAGES = "周期效果";
PET = "宠物"; -- Generic pet string
PETITION_CREATOR = "由%s制造";
PETITION_GAME_MASTER = "请求和GM联系";
PETITION_NUM_SIGNATURES_P1 = "%d签名";
PETITION_NUM_SIGNATURES = "%d签名";
PETITION_TITLE = "请愿：%s";
PETTAME_ANOTHERSUMMONACTIVE = "你已经有一个召唤生物了";
PETTAME_CREATUREALREADYOWNED = "目标生物已经被控制";
PETTAME_DEAD = "你的宠物已经死亡";
PETTAME_INTERNALERROR = "内部宠物错误";
PETTAME_INVALIDCREATURE = "没有找到生物";
PETTAME_NOPETAVAILABLE = "你无法召唤宠物";
PETTAME_NOTDEAD = "你的宠物还未死亡";
PETTAME_NOTTAMEABLE = "目标生物无法被驯服";
PETTAME_TOOHIGHLEVEL = "生物等级太高，无法驯服";
PETTAME_TOOMANY = "你的宠物太多了";
PETTAME_UNITSCANTTAME = "你不能驯服生物";
PETTAME_UNKNOWNERROR = "未知的驯服错误";
PET_ABANDON = "放弃"; -- Dismiss the pet permanently
PET_ACTION_ATTACK = "攻击"; -- Tells pet to attack
PET_ACTION_DISMISS = "解散"; -- Dismisses your pet
PET_ACTION_FOLLOW = "跟随"; -- Tells pet to follow the player
PET_ACTION_WAIT = "停留"; -- Tells pet to wait
PET_AGGRESSIVE = "攻击型"; -- Set the pet to aggressive mode
PET_ATTACK = "攻击"; -- Tell the pet to attack your current target
PET_DAMAGE_PERCENTAGE = "造成%d%%的普通伤害";
PET_DEFENSIVE = "防御型"; -- Set the pet to defensive mode
PET_DIET_TEMPLATE = "食物：%s";
PET_DISMISS = "解散"; -- Release the pet
PET_FOLLOW = "跟随"; -- Tell the pet to follow you
PET_HAPPINESS1 = "不高兴状态";
PET_HAPPINESS2 = "满足";
PET_HAPPINESS3 = "快乐";
PET_LOYALTY_GAIN = "宠物的忠诚度提高了。";
PET_LOYALTY_LOSS = "宠物的忠诚度降低了。";
PET_MODE_AGGRESSIVE = "攻击型"; -- Pet is in "aggressive" mode
PET_MODE_DEFENSIVE = "防御型"; -- Pet is in "defensive" mode
PET_MODE_PASSIVE = "被动型"; -- Pet is in "passive" mode
PET_PAPERDOLL = "宠物具体信息";
PET_PASSIVE = "被动型"; -- Set the pet to passive mode
PET_RENAME = "重命名"; -- Rename the pet
PET_RENAME_CONFIRMATION = "你确定要将宠物命名为'%s'吗？";
PET_RENAME_LABEL = "输入你想要给宠物起的名字：";
PET_SPELLS_TEMPLATE = "驯兽能力：%s";
PET_SPELL_NOPATH = "无路径可供你的宠物使用";
PET_TIME_LEFT_MINUTES = "剩余%d分钟";
PET_TIME_LEFT_SECONDS = "剩余%d秒";
PET_TYPE_DEMON = "恶魔";
PET_TYPE_PET = "宠物";
PET_WAIT = "停留"; -- Tell the pet to wait here
PHONG_SHADING = "阴影平滑";
PHYSICAL_HARASSMENT = "行为骚扰";
PHYSICAL_HARASSMENT_DESCRIPTION = "任何个人采取行动不断使别的玩家沮丧，心情恶化或者别的招惹行为的话，此人将被认为在采取行为骚扰或者“消极游戏”。然而，请确定这种行为是在PvP准则范围之外的。";
PHYSICAL_HARASSMENT_TEXT1 = "以下行为不被认为是行为骚扰：";
PHYSICAL_HARASSMENT_TEXT6 = "“拉火车”（将敌人引到另一个玩家面前，强迫其作战）";
PHYSICAL_HARASSMENT_TEXT5 = "“守尸体”（在一名玩家的尸体旁等待，并在他/她复活后对其进行攻击）";
PHYSICAL_HARASSMENT_TEXT4 = "“守怪物”（为了重复杀死某一种怪物而占据一处怪物刷新的地点）";
PHYSICAL_HARASSMENT_TEXT3 = "反复攻击和/或杀死NPC";
PHYSICAL_HARASSMENT_TEXT2 = "攻击其他玩家正在攻击的怪物或NPC";
PIXEL_SHADERS = "光影效果";
PLAYER = "玩家";
PLAYER_COUNT_ALLIANCE = "%d联盟玩家";
PLAYER_COUNT_ALLIANCE_P1 = "%d联盟玩家";
PLAYER_COUNT_HORDE = "%d部落玩家";
PLAYER_COUNT_HORDE_P1 = "%d部落玩家";
PLAYER_LEVEL = "等级%d %s%s"; -- Character's level, race, and class
PLAYER_LEVEL_UP = "升级";
PLAYER_LOGOUT_FAILED = "登出失败";
PLAYER_LOGOUT_FAILED_ERROR = "由于你现在无法坐下，所以无法登出。"; -- logout failure. Right now being mounted is only way logging out can fail
PLAYER_NOT_FOUND = "无法找到指定玩家";
PLAYER_OFFLINE = "离线";
PLAYER_OPTIONS_LABEL = "玩家选项";
PLAYER_STATUS = "玩家状态";
PLAYTIME_TIRED = "您的在线时间已经超过3个小时。在接下来的游戏过程中，您将只能获得一半的游戏收益。如果您下线并休息%d小时，该状态就会解除。建议您下线休息一段时间。";
PLAYTIME_UNHEALTHY = "您的累计在线时间已经超过了5小时，将无法获得游戏收益，也无法提交已经完成的任务。您的休息状态会在下线时间累计达到%d小时之后恢复。请退出游戏并进行一些休息和锻炼。";
PLUS_AMMO_DAMAGE_TEMPLATE = "每秒伤害+%g";
PLUS_AMMO_SCHOOL_DAMAGE_TEMPLATE = "每秒+%g点%s伤害";
PLUS_DAMAGE_TEMPLATE = "+ %d - %d伤害";
PLUS_DAMAGE_TEMPLATE_WITH_SCHOOL = "+ %d - %d %s伤害";
PLUS_SINGLE_DAMAGE_TEMPLATE = "+ %d伤害";
POTION_TIMER = "你已经喝得太饱了。"; -- The player is trying to drink a potion before the cooldown timer expires
POWERGAINOTHEROTHER = "%1$s从%4$s的%5$s获得了%2$d点%3$s。"; -- Bob gains 5 mana from Fred's spell.
POWERGAINOTHERSELF = "你从%3$s的%4$s获得了%1$d点%2$s。"; -- You gain 5 mana from Bob's spell.
POWERGAINSELFOTHER = "%1$s从%4$s获得了%2$d点%3$s。"; -- Bob gains 5 mana from spell.
POWERGAINSELFSELF = "你从%3$s获得了%1$d点%2$s。"; -- You gain 5 mana from spell.
PREFERENCES = "偏好选择";
PREV = "上一页"; -- Abbreviation for "previous"
PRIEST_INTELLECT_TOOLTIP = "提高你的法力值上限和使用法术造成致命一击的几率。\n使你更快地提升武器技能熟练度。";
PRIMARY = "主要";
PRIMARY_SKILLS = "主要技能："; -- A character's "primary skills"
PROCRESISTOTHEROTHER = "%s抵抗了%s的%s。";
PROCRESISTOTHERSELF = "你抵抗了%s的%s。";
PROCRESISTSELFOTHER = "%s抵抗了你的%s。";
PROCRESISTSELFSELF = "你抵抗了你的%s。";
PROC_EVENT0_DESC = "No Proc Trigger Assigned. Tell Kevin.";
PROC_EVENT1024_DESC = "当被法术击中的时候";
PROC_EVENT128_DESC = "挥舞时";
PROC_EVENT16_DESC = "躲闪时";
PROC_EVENT1_DESC = "击中目标时";
PROC_EVENT2048_DESC = "当被%s法术击中的时候";
PROC_EVENT256_DESC = "施放时";
PROC_EVENT2_DESC = "被击中时";
PROC_EVENT32_DESC = "招架时";
PROC_EVENT3_DESC = "击中目标或被击中时";
PROC_EVENT4_DESC = "杀死目标时";
PROC_EVENT512_DESC = "施放%s时";
PROC_EVENT64_DESC = "格挡时";
PROC_EVENT8_DESC = "每个tick";
PROFANITY_FILTER = "语言过滤器";
PROFESSION_CONFIRMATION1 = "你只能学习两个专业。你要学习|cffffd200%s|r作为你的第一个专业吗？";
PROFESSION_CONFIRMATION2 = "你只能学习两个专业。你要学习|cffffd200%s|r作为你的第二个专业吗？";
PROFFESSION_CONFIRMATION2 = "你只能学习两个专业。你要把%s作为你的第二个专业吗？";
PROFICIENCIES = "精通：";
PROFICIENCIES_COLON = "精通："; -- Character's proficiencies, with colon
PROFICIENCY_NEEDED = "你不会使用这种物品。"; -- The player does not have a proficiency for that item type
PROMOTE = "提升";
PUBLICNOTE_BUTTON_TOOLTIP = "点击浏览你的公共信息";
PUBLIC_NOTE = "公共信息";
PURCHASE = "购买";
PURCHASED_BY_COLON = "购买者：";
PVPFFA = "自由PvP";
PVP_DISABLED = "PvP未开启";
PVP_ENABLED = "PvP";
PVP_MEDAL1 = "Protector of Stormwind";
PVP_MEDAL2 = "Overlord of Orgrimmar";
PVP_MEDAL3 = "Thane of Ironforge";
PVP_MEDAL4 = "High Sentinel of Darnassus";
PVP_MEDAL5 = "Deathlord of the Undercity";
PVP_MEDAL6 = "Chieftain of Thunderbluff";
PVP_MEDAL8 = "Voodoo Boss of Sen�jin";
PVP_MEDAL7 = "Avenger of Gnomeregan";
PVP_MEDAL9 = "Bruiser of Sparkwater";
PVP_MEDAL10 = "Avenger of Quel'thalas";
PVP_MEDAL11 = "Warchief";
PVP_MEDAL12 = "Developer";
PVP_MEDAL13 = "World Boss";
PVP_MEDAL14 = "Turtle God";
PVP_MEDAL15 = "Scarab Lord";
PVP_MEDAL16 = "Conqueror of Naxxramas";
PVP_MEDAL17 = "Expedition Leader";

PVP_MEDAL18 = "Footman";
PVP_MEDAL19 = "Grunt";
PVP_MEDAL20 = "Deathguard";
PVP_MEDAL21 = "Headhunter";
PVP_MEDAL22 = "Priestess of the Moon";
PVP_MEDAL23 = "Druid of the Wild";
PVP_MEDAL24 = "Druid of the Claw";
PVP_MEDAL25 = "Druid of the Talon";
PVP_MEDAL26 = "Mountaineer";
PVP_MEDAL27 = "Mekgineer";
PVP_MEDAL28 = "Bruiser";
PVP_MEDAL29 = "Merchant";
PVP_MEDAL30 = "Trader";
PVP_MEDAL31 = "Sunbringer";
PVP_MEDAL32 = "Cleric";
PVP_MEDAL33 = "Peasant";
PVP_MEDAL34 = "Peon";
PVP_MEDAL35 = "Magus";
PVP_MEDAL36 = "Bishop";
PVP_MEDAL37 = "Spiritwalker";
PVP_MEDAL38 = "Child of Cenarius";
PVP_MEDAL39 = "Watcher";
PVP_MEDAL40 = "Blood Mage";
PVP_MEDAL41 = "Farstrider";

PVP_MEDAL42 = "Seeker of Knowledge";
PVP_MEDAL43 = "Inquisitor";
PVP_MEDAL44 = "Unstoppable Force";
PVP_MEDAL45 = "Brewmaster";
PVP_MEDAL46 = "Merciless Gladiator";

PVP_MEDAL47 = "Blood Ring Champion";
PVP_MEDAL48 = "Stormwielder";
PVP_MEDAL49 = "Sulfuron Champion";
PVP_MEDAL50 = "Guardian of Tirisfal";
PVP_MEDAL51 = "Starkiller";
PVP_MEDAL52 = "Immortal";

PVP_MEDAL53 = "Admiral";
PVP_MEDAL54 = "Mayor";
PVP_MEDAL55 = "Captain";
PVP_MEDAL56 = "Sailor";
PVP_MEDAL57 = "Queen";
PVP_MEDAL58 = "King";
PVP_MEDAL59 = "Lord";
PVP_MEDAL60 = "Lady";
PVP_MEDAL61 = "Doctor";
PVP_MEDAL62 = "Medic";

-- RP ranks END

PVP_MEDAL63 = "Lorekeeper";
PVP_MEDAL64 = "Penus";

PVP_POLICY_URL = "|cffffd200http://www.wowchina.com|r";
PVP_RANK_10_0_FEMALE = "石头守卫";
PVP_RANK_10_0 = "石头守卫";
PVP_RANK_10_1 = "骑士";
PVP_RANK_10_1_FEMALE = "骑士";
PVP_RANK_11_0 = "血卫士";
PVP_RANK_11_0_FEMALE = "血卫士";
PVP_RANK_11_1 = "骑士中尉";
PVP_RANK_11_1_FEMALE = "骑士中尉";
PVP_RANK_12_0 = "军团士兵";
PVP_RANK_12_0_FEMALE = "军团士兵";
PVP_RANK_12_1 = "骑士队长";
PVP_RANK_12_1_FEMALE = "骑士队长";
PVP_RANK_13_0 = "百夫长";
PVP_RANK_13_0_FEMALE = "百夫长";
PVP_RANK_13_1 = "护卫骑士";
PVP_RANK_13_1_FEMALE = "护卫骑士";
PVP_RANK_14_0_FEMALE = "勇士";
PVP_RANK_14_0 = "勇士";
PVP_RANK_14_1_FEMALE = "少校";
PVP_RANK_14_1 = "少校";
PVP_RANK_15_0_FEMALE = "中将";
PVP_RANK_15_0 = "中将";
PVP_RANK_15_1 = "司令";
PVP_RANK_15_1_FEMALE = "司令";
PVP_RANK_16_0 = "将军";
PVP_RANK_16_0_FEMALE = "将军";
PVP_RANK_16_1 = "统帅";
PVP_RANK_16_1_FEMALE = "统帅";
PVP_RANK_17_0_FEMALE = "督军";
PVP_RANK_17_0 = "督军";
PVP_RANK_17_1 = "元帅";
PVP_RANK_17_1_FEMALE = "元帅";
PVP_RANK_18_0_FEMALE = "高阶督军";
PVP_RANK_18_0 = "高阶督军";
PVP_RANK_18_1 = "大元帅";
PVP_RANK_18_1_FEMALE = "大元帅";
PVP_RANK_19_0 = "领袖"; -- PvP Leader NPC
PVP_RANK_19_0_FEMALE = "领袖"; -- PvP Leader NPC
PVP_RANK_19_1_FEMALE = "领袖"; -- PvP Leader NPC
PVP_RANK_19_1 = "领袖"; -- PvP Leader NPC
PVP_RANK_1_0 = "无赖";
PVP_RANK_1_0_FEMALE = "无赖";
PVP_RANK_1_1 = "无赖";
PVP_RANK_1_1_FEMALE = "无赖";
PVP_RANK_2_0 = "逃犯";
PVP_RANK_2_0_FEMALE = "逃犯";
PVP_RANK_2_1 = "逃犯";
PVP_RANK_2_1_FEMALE = "逃犯";
PVP_RANK_3_0_FEMALE = "流亡者";
PVP_RANK_3_0 = "流亡者";
PVP_RANK_3_1 = "流亡者";
PVP_RANK_3_1_FEMALE = "流亡者";
PVP_RANK_4_0 = "耻辱者";
PVP_RANK_4_0_FEMALE = "耻辱者";
PVP_RANK_4_1 = "耻辱者";
PVP_RANK_4_1_FEMALE = "耻辱者";
PVP_RANK_5_0_FEMALE = "侦察兵";
PVP_RANK_5_0 = "侦察兵";
PVP_RANK_5_1 = "列兵";
PVP_RANK_5_1_FEMALE = "列兵";
PVP_RANK_6_0 = "步兵";
PVP_RANK_6_0_FEMALE = "步兵";
PVP_RANK_6_1 = "下士";
PVP_RANK_6_1_FEMALE = "下士";
PVP_RANK_7_0 = "中士";
PVP_RANK_7_0_FEMALE = "中士";
PVP_RANK_7_1 = "中士";
PVP_RANK_7_1_FEMALE = "中士";
PVP_RANK_8_0_FEMALE = "高阶军士";
PVP_RANK_8_0 = "高阶军士";
PVP_RANK_8_1 = "军士长";
PVP_RANK_8_1_FEMALE = "军士长";
PVP_RANK_9_0 = "一等军士长";
PVP_RANK_9_0_FEMALE = "一等军士长";
PVP_RANK_9_1_FEMALE = "士官长";
PVP_RANK_9_1 = "士官长";
PVP_RANK_CIVILIAN = "平民"; -- Non-combatant NPC
PVP_RANK_CIVILIAN_FEMALE = "平民"; -- Non-combatant NPC
PVP_RANK_LEADER = "阵营领袖";
PVP_TOGGLE_OFF_VERBOSE = "当你身处友方领地，并且在5分钟内不进行任何PvP动作之后，你将离开PvP状态。";
PVP_TOGGLE_ON_VERBOSE = "你现在被标记为可以进行PvP战斗，该状态将一直持续到你关闭PvP战斗标记。";
QUALITY = "质量";
QUESTLOG_BUTTON = "任务日志";
QUESTLOG_NO_QUESTS_TEXT = "没有正在进行的任务";
QUESTS_COLON = "任务："; -- Header for the available quests, with colon
QUEST_ACCEPT = "%s开始%s\n你也想这样吗？"; -- First %s is a player name, second %s is a quest title
QUEST_COMPLETE = "任务完成"; -- Tells the player that a quest was completed
QUEST_DESCRIPTION = "描述"; -- Header for the quest description
QUEST_FACTION_NEEDED = "%s：%s / %s"; -- faction name, current reaction, required reaction
QUEST_FAILED = "任务失败"; -- The player did something funny with quest items, causing the quest to fail
QUEST_FAILED_TAG = "- 失败"; -- Displayed next to a failed quest in the log
QUEST_HARD = "（困难）"; -- Indicates that a quest may be difficult for the player
QUEST_ITEMS_NEEDED = "%s：%d/%d"; -- Lists items obtained for the selected quest
QUEST_LOG = "任务日志"; -- Screen that shows info for a player's quests
QUEST_LOG_COUNT_TEMPLATE = "任务：|cffffffff%d/%d|r";
QUEST_MONSTERS_KILLED = "已杀死%s：%d/%d"; -- Lists the monsters killed for the selected quest
QUEST_OBJECTIVES = "任务目标"; -- Header for the quest objectives
QUEST_OBJECTS_FOUND = "%s：%d/%d"; -- Lists the game objects found for the selected quest
QUEST_REWARDS = "奖励"; -- Quest rewards title
QUEST_TIMERS = "任务计时"; -- Quest timers title
QUEST_WATCH_NO_OBJECTIVES = "这项任务没有可以追踪的任务目标";
QUEST_WATCH_TOOLTIP = "按住Shift点击一个任务可以将其加入你的任务追踪列表，或者从任务追踪列表中删除。";
QUEST_WATCH_TOO_MANY = "你最多同时追踪%d个任务";
QUEUE_TIME_UNAVAILABLE = "不可用";
QUIT = "退出";
QUIT_NOW = "立刻退出"; -- Gives the time in minutes/seconds until you logout
QUIT_TIMER = "%d%s后退出游戏";
RACE = "种族";
RACE_CLASS_ONLY = "%s专用。"; -- An item can only be used by a certain race/class
RACIAL_SKILLS = "%s技能："; -- %s is the race
RAGE = "怒气"; -- Generic "rage"
RAGE_COST = "%d怒气";
RAGE_COST_PER_TIME = "%d怒气，外加每秒%d";
RAGE_POINTS = "怒气";
RAID = "团队"; -- Generic "Raid" text
RAIDOPTIONS_MENU = "团队设置";
RAID_AND_PARTY = "团队&小队";
RAID_ASSISTANT_TOKEN = "A"; -- Token to denote a raid assistant
RAID_BOSS_MESSAGE = "首领怪物";
RAID_CONTROL = "团队管理";
RAID_DESCRIPTION = "团队是超过5个人的队伍，这是为了击败高等级的特定挑战而准备的大型队伍模式。\n\n|cffffffff- 团队成员无法获得非团队任务所需的物品或者杀死怪物的纪录。\n\n- 在团队中，你通过杀死怪物获得的经验值相对普通小队要少。\n\n- 团队让你可以赢得用其它方法根本无法通过的挑战。|r";
RAID_GROUPS = "团队";
RAID_INFO = "团队信息";
RAID_INFORMATION = "团队信息";
RAID_INFO_DESC = "你的已保存的团队副本信息。";
RAID_INSTANCE_INFO_FMT = "%s (ID=%lx)：%s";
RAID_INSTANCE_INFO_HDR = "团队副本倒计时：";
RAID_INSTANCE_WARNING_HOURS = "警告！%s将在%d小时后重置。";
RAID_INSTANCE_WARNING_HOURS_P1 = "警告！%s将在%d小时后重置。";
RAID_INSTANCE_WARNING_MIN = "警告！%s将在%d分钟后重置！";
RAID_INSTANCE_WARNING_MIN_P1 = "警告！%s将在%d分钟后重置！";
RAID_INSTANCE_WARNING_MIN_SOON_P1 = "警告！%s将在%d分钟后重置。请退出这个区域，否则你将会被传送到炉石绑定点！";
RAID_INSTANCE_WARNING_MIN_SOON = "警告！%s将在%d分钟后重置。请退出这个区域，否则你将会被传送到炉石绑定点！";
RAID_INSTANCE_WELCOME = "欢迎来到%s。这个团队副本预计在%d天%d小时%d分钟后重置。";
RAID_LEADER_TOKEN = "L"; -- Token used to denote a raid leader
RAID_MEMBERS_AFK = "下列玩家正处于暂离状态：%s";
RAID_MEMBER_NOT_READY = "%s 还未准备就绪";
RAID_MESSAGE = "团队";
RAID_TARGET_1 = "星形";
RAID_TARGET_2 = "圆形";
RAID_TARGET_3 = "菱形";
RAID_TARGET_4 = "三角";
RAID_TARGET_5 = "月亮";
RAID_TARGET_6 = "方块";
RAID_TARGET_7 = "十字";
RAID_TARGET_8 = "骷髅";
RAID_TARGET_ICON = "团队目标图标";
RAID_WARNING_MESSAGE = "团队通知";
RANDOM_ROLL_RESULT = "%s掷出%d（%d-%d）"; -- Example: Jeremy rolls 4 (1-10)
RANGEDSLOT = "远程"; -- Ranged slot
RANGED_ATTACK = "远程攻击";
RANGED_ATTACK_POWER = "远程攻击强度";
RANGED_ATTACK_POWER_TOOLTIP = "使远程武器的每秒伤害提高%.1f。";
RANGED_ATTACK_TOOLTIP = "远程攻击命中率";
RANGED_DAMAGE_TOOLTIP = "远程伤害";
RANK = "级别";
RANK_COLON = "级别：";
RANK_POSITION = "排名";
RARITY = "稀有程度";
READY_CHECK = "就位确认";
READY_CHECK_MESSAGE = "%s正在进行就位确认。\n|cffffffff你准备好了吗？|r";
READY_CHECK_NO_AFK = "无人处于暂离状态";
READY_CHECK_START = "开始就位确认…";
READY_CHECK_YOU_WERE_AFK = "你在就位确认时处于暂离状态";
RECOVER_CORPSE = "现在复活吗？";
RECOVER_CORPSE_INSTANCE = "你必须进入副本才能捡回你的尸体。";
RECOVER_CORPSE_TIMER = "%d%s后复活"; -- Gives the time in minutes/seconds until you can recover your corpse
REFLECT = "反射";
REFRESH = "刷新";
REFRESH_RATE = "刷新";
RELICSLOT = "圣物";
REMOVE = "移除";
REMOVE_CHAT_DELAY_TEXT = "移除聊天标签弹出延迟";
REMOVE_FRIEND = "删除好友"; -- Remove a friend from your friends list
REMOVE_GUILDMEMBER_LABEL = "你确定想要从公会中开除%s吗？";
RENAME_CHAT_WINDOW = "重命名窗口";
RENAME_GUILD = "公会重命名";
RENAME_GUILD_LABEL = "输入新的公会名：";
REPAIR_ALL_ITEMS = "修理所有物品";
REPAIR_AN_ITEM = "修理一件物品";
REPAIR_COST = "修理花费：";
REPAIR_ITEMS = "修理物品";
REPLACE_ENCHANT = "你要将\"%s\"替换为\"%s\"吗？";
REPLY_MESSAGE = "回复"; -- Reply to the last player to send you a tell
REPORT = "举报";
REPORT_PHYSICAL_HARASSMENT = "报告行为骚扰";
REPORT_VERBAL_HARASSMENT = "报告语言骚扰";
REPUTATION = "声望"; -- Character's reputation
REPUTATION_AT_WAR_DESCRIPTION = "决定该派别的成员对你的反应。如果你钩选了交战状态框，那么你就可以攻击他们。如果你没有钩选交战状态框，则不会对他们进行攻击。";
REPUTATION_FACTION_DESCRIPTION = "这里列出的是你与各个阵营派别的声望。";
REPUTATION_STANDING_DESCRIPTION = "声望条越偏右，你在该阵营派别中的声望就越高。声望从低到高分为多个级别：仇恨、敌对、冷淡、中立、友善、尊敬、崇敬、崇拜。";
REPUTATION_STATUS_AT_PEACE = "反选此框即进入和平状态。";
REPUTATION_STATUS_AT_WAR = "你正在与该派别处于交战状态。";
REPUTATION_STATUS_NOT_AT_PEACE = "钩选此框即进入交战状态。";
REPUTATION_STATUS_PERMANENT_AT_PEACE = "你不能与己方派别开战！";
REPUTATION_STATUS_PERMANENT_AT_WAR = "你无法与%s保持和平关系。";
REQUEST_SIGNATURE = "要求签名";
REQUIRED_MONEY = "需要金钱：";
REQUIRES_LABEL = "需要："; -- Requirements to train in a skill
RESET = "重置";
RESETS_IN = "重置时间";
RESET_INSTANCES = "重置所有副本";
RESET_TO_DEFAULT = "恢复默认设置";
RESET_TUTORIALS = "重置教程";
RESIST = "抵抗"; -- Resisted a spell
RESISTANCE0_NAME = "护甲"; -- Physical resistance
RESISTANCE1_NAME = "神圣抗性"; -- Resistance to Holy Magic
RESISTANCE2_NAME = "火焰抗性"; -- Resistance to Fire Magic
RESISTANCE3_NAME = "自然抗性"; -- Resistance to Nature Magic
RESISTANCE4_NAME = "冰霜抗性"; -- Resistance to Frost Magic
RESISTANCE5_NAME = "暗影抗性"; -- Resistance to Shadow Magic
RESISTANCE6_NAME = "奥术抗性"; -- Resistance to arcane magic
RESISTANCE_EXCELLENT = "极高";
RESISTANCE_FAIR = "一般";
RESISTANCE_GOOD = "较高";
RESISTANCE_LABEL = "抗性";
RESISTANCE_NONE = "无";
RESISTANCE_POOR = "较低";
RESISTANCE_TEMPLATE = "%d%s";
RESISTANCE_TOOLTIP_SUBTEXT = "提高你对%s系攻击、法术和技能的抵抗能力。\n对%d级敌人的抗性：|cffffffff%s|r";
RESISTANCE_TYPE0 = "护甲";
RESISTANCE_TYPE1 = "神圣";
RESISTANCE_TYPE2 = "火焰";
RESISTANCE_TYPE3 = "自然";
RESISTANCE_TYPE4 = "冰霜";
RESISTANCE_TYPE5 = "暗影";
RESISTANCE_TYPE6 = "奥术";
RESISTANCE_VERYGOOD = "很高";
RESIST_TRAILER = "(%d点被抵抗)"; -- Appended to attack strings in the combat log when damage is resisted. (NOTE SPACE AT BEGINNING)
RESOLUTION = "分辨率"; -- Set resolution tag for the options screen
RESURRECTABLE = "可复活";
RESURRECT_REQUEST = "%s想要复活你。一旦这样复活，你将会进入复活虚弱状态"; -- Another player is attempting to resurrect this player
RESURRECT_REQUEST_NO_SICKNESS = "%s想要复活你";
RESURRECT_REQUEST_NO_SICKNESS_TIMER = "%s要复活你，%d%s内生效";
RESURRECT_REQUEST_TIMER = "%s要复活你，%d%s内生效。一旦这样复活，你将会进入复活虚弱状态。";
RETURN_TO_GAME = "返回游戏";
RETURN_TO_WORLD = "离开战场";
REWARD_CHOICES = "你可以从这些奖励品中选择一件："; -- Gives the player a list of possible quest reward choices
REWARD_CHOOSE = "选择你的奖励品："; -- The player must choose one of these items
REWARD_ITEMS = "你还将得到："; -- These items will always be given as a reward
REWARD_ITEMS_ONLY = "你将得到："; -- Same as above, used when the player has no chosen items
REWARD_SPELL = "你将学会：";
REWARD_TRADESKILL_SPELL = "你将学会如何制造：";
RIGHT_CLICK_MESSAGE = "|cffffffff<右键点击> 设置PvP选项|r";
ROGUE_AGILITY_TOOLTIP = "提高你的近战武器和远程武器的攻击强度，并提高用所有武器造成致命一击的几率。\n提高你的护甲值和躲避攻击的几率。";
SALE_PRICE_COLON = "售价：";
SAVE = "保存";
SAY = "说";
SAY_MESSAGE = "说"; -- Normal chat message
SCORE_FLAGS_CAPTURED = "军旗\n夺取";
SCORE_FLAGS_RETURNED = "军旗\n归还";
SCORE_HONORABLE_KILLS = "荣誉\n击杀";
SCORE_HONOR_GAINED = "获得\n荣誉";
SCORE_KILLING_BLOWS = "杀死\n敌人";
SCREENSHOT_FAILURE = "截图失败";
SCREENSHOT_SUCCESS = "截图完成";
SEARCH = "搜索";
SEARCHING_FOR_ITEMS = "搜索物品";
SECONDARY = "次要";
SECONDARYHANDSLOT = "副手"; -- Off hand slot
SECONDARY_SKILLS = "次要技能："; -- A character's "secondary skills"
SECONDS = "秒"; -- Seconds of time
SECONDS_P1 = "秒";
SECONDS_ABBR_P1 = "秒";
SECONDS_ABBR = "秒";
SECOND_ONELETTER_ABBR = "%d s";
SELECT_CATEGORY = "——> 选择一个类别";
SELFKILLOTHER = "你杀死了%s！";
SENDMAIL = "发件箱";
SENDMAIL_TEXT = "在这里放置物品和/或钱币";
SEND_BUG = "提交Bug"; -- Submit a bug to Q/A
SEND_LABEL = "发送";
SEND_MAIL_COST = "邮资：";
SEND_MESSAGE = "发送信息"; -- Send a message to a friend
SEND_MONEY = "发送钱币";
SEND_MONEY_CONFIRMATION = "确定要将下列数量发送给%s吗？";
SEND_SUGGEST = "发送建议"; -- Send a suggestion to Q/A
SERVER_CHANNELS = "服务器频道";
SET_GUILDMOTD_LABEL = "设置今日公会通知：";
SET_GUILDOFFICERNOTE_LABEL = "设置公会官员信息";
SET_GUILDPLAYERNOTE_LABEL = "设置玩家信息";
SHAMAN_INTELLECT_TOOLTIP = "提高你的法力值上限和使用法术造成致命一击的几率。\n使你更快地提升武器技能熟练度。";
SHAMAN_STRENGTH_TOOLTIP = "提高你的近战武器的攻击强度。\n使你用盾牌格挡攻击时减免更多的伤害。";
SHARE_QUEST = "共享任务";
SHARE_QUEST_TEXT = "如果你队伍的成员符合条件的话\n你可以和他分享一个任务。\n有些任务，比如由物品激发的\n任务，无法进行分享。";
SHIELD_BLOCK_TEMPLATE = "%d格挡";
SHIRTSLOT = "衬衣"; -- Body slot
SHOULDERSLOT = "肩部"; -- Shoulder slot
SHOW_BATTLEFIELDMINIMAP_PLAYERS = "显示队友";
SHOW_BUFFS = "显示增益效果"; -- For the raid pullout dropdown menu
SHOW_BUFF_DURATION_TEXT = "增益持续时间";
SHOW_CASTABLE_BUFFS_TEXT = "显示可施放增益";
SHOW_CLOAK = "显示披风";
SHOW_COMBAT_TEXT_TEXT = "开启浮动战斗信息";
SHOW_DAMAGE_TEXT = "目标伤害显示";
SHOW_DEBUFFS = "显示负面效果"; -- For the raid pullout dropdown menu
SHOW_DISPELLABLE_DEBUFFS_TEXT = "显示可驱散效果";
SHOW_FACTION_ON_MAINSCREEN = "显示为经验条";
SHOW_FRIENDS_LIST = "显示好友列表";
SHOW_GUILD_NAMES = "玩家公会名";
SHOW_HELM = "显示头盔";
SHOW_IGNORE_LIST = "显示忽略列表";
SHOW_LOOT_SPAM = "详细拾取信息";
SHOW_MULTIBAR1_TEXT = "显示左下方动作条";
SHOW_MULTIBAR2_TEXT = "显示右下方动作条";
SHOW_MULTIBAR3_TEXT = "显示右边动作条";
SHOW_MULTIBAR4_TEXT = "显示右边动作条2";
SHOW_NEWBIE_TIPS_TEXT = "显示详细的工具提示信息";
SHOW_NPC_NAMES = "NPC姓名";
SHOW_OFFLINE_MEMBERS = "显示离线成员";
SHOW_OWN_NAME = "显示本人名字";
SHOW_PARTY_BACKGROUND_TEXT = "显示小队背景";
SHOW_PARTY_PETS_TEXT = "显示队友宠物";
SHOW_PET_MELEE_DAMAGE = "宠物伤害";
SHOW_PET_NAMEPLATES = "宠物姓名栏";
SHOW_PET_SPELL_DAMAGE = "显示宠物法术伤害";
SHOW_PLAYER_NAMES = "玩家名字";
SHOW_PLAYER_TITLES = "玩家头衔";
SHOW_QUEST_FADING_TEXT = "立即显示任务文本";
SHOW_TARGET_OF_TARGET_TEXT = "显示目标的目标";
SHOW_TIPOFTHEDAY_TEXT = "显示每日小窍门";
SHOW_TUTORIALS = "显示教程";
SHOW_UNIT_NAMES = "显示单位名称";
SIGN_CHARTER = "签名";
SILVER = "银币"; -- Mid value coin denomination
SIMPLECASTOTHEROTHER = "%1$s对%3$s施放了%2$s。";
SIMPLECASTOTHERSELF = "%s对你施放了%s。";
SIMPLECASTSELFOTHER = "你对%2$s施放了%1$s。";
SIMPLECASTSELFSELF = "你施放了%s。";
SIMPLEPERFORMOTHEROTHER = "%1$s对%3$s使用%2$s。";
SIMPLEPERFORMOTHERSELF = "%s对你使用%s。";
SIMPLEPERFORMSELFOTHER = "你对%2$s使用%1$s。";
SIMPLEPERFORMSELFSELF = "你使用%s。";
SIMPLE_CHAT_TEXT = "简易聊天模式";
SINGLE_DAMAGE_TEMPLATE = "%d点伤害";
SKILL = "技能";
SKILLS = "技能"; -- Generic "skills"
SKILL_DESCRIPTION = "|cffffffff%s|r %s";
SKILL_INCREMENT_COST = "消耗%s%d|r技能点来升级。";
SKILL_INCREMENT_COST_SINGULAR = "消耗%s%d|r技能点来升级。";
SKILL_LEARNING_COST = "学习该技能：%s%d|r 技能点。";
SKILL_LEARNING_COST_SINGULAR = "学习该技能：%s%d|r 技能点。";
SKILL_LEVEL = "技能等级"; -- Skill level in a particular skill line
SKILL_POINTS_TOOLTIP = "技能点数是用来在世界各地的训练师处\n学习技能的。"; -- Tooltip description for skill points
SKILL_RANK_UP = "你的%s技能提高到了%d。"; -- Tells player when a skill rank goes up
SLASH_ASSIST4 = "/assist"; -- Assist
SLASH_ASSIST1 = "/a"; -- Assist
SLASH_ASSIST2 = "/协助"; -- Assist
SLASH_ASSIST3 = "/a"; -- Assist
SLASH_BATTLEGROUND1 = "/bg"; -- Send to battleground party
SLASH_BATTLEGROUND3 = "/bg";
SLASH_BATTLEGROUND4 = "/battleground";
SLASH_BATTLEGROUND2 = "/battleground"; -- Send to battleground party
SLASH_CAST1 = "/施放";
SLASH_CAST2 = "/spell";
SLASH_CAST4 = "/法术";
SLASH_CAST3 = "/cast";
SLASH_CHANNEL4 = "/csay";
SLASH_CHANNEL2 = "/csay";
SLASH_CHANNEL3 = "/c"; -- Send to channel (/# works too)
SLASH_CHANNEL1 = "/c"; -- Send to channel (/# works too)
SLASH_CHATLOG2 = "/聊天记录"; -- enable/disable chat log file output
SLASH_CHATLOG1 = "/chatlog"; -- enable/disable chat log file output
SLASH_CHAT_AFK2 = "/暂离"; -- Set your AFK flag
SLASH_CHAT_AFK1 = "/afk"; -- Set your AFK flag
SLASH_CHAT_ANNOUNCE3 = "/宣布"; -- Turn join/leave announcments on/off
SLASH_CHAT_ANNOUNCE4 = "/ann";
SLASH_CHAT_ANNOUNCE2 = "/禁止公告";
SLASH_CHAT_ANNOUNCE1 = "/announce"; -- Turn join/leave announcments on/off
SLASH_CHAT_BAN2 = "/ban"; -- Ban from channel
SLASH_CHAT_BAN1 = "/ban"; -- Ban from channel
SLASH_CHAT_CINVITE4 = "/chatinvite";
SLASH_CHAT_CINVITE3 = "/cinvite"; -- Invite a player to a channel
SLASH_CHAT_CINVITE1 = "/cinvite"; -- Invite a player to a channel
SLASH_CHAT_CINVITE2 = "/聊天邀请";
SLASH_CHAT_DND2 = "/请勿打扰"; -- Set your DND flag
SLASH_CHAT_DND1 = "/dnd"; -- Set your DND flag
SLASH_CHAT_DND3 = "/请勿打扰"; -- Set your DND flag
SLASH_CHAT_HELP5 = "/chathelp";
SLASH_CHAT_HELP2 = "/频道帮助";
SLASH_CHAT_HELP3 = "/聊天帮助"; -- Chat help
SLASH_CHAT_HELP1 = "/chat"; -- Chat help
SLASH_CHAT_HELP4 = "/聊天帮助"; -- Chat help
SLASH_CHAT_KICK2 = "/频道剔除"; -- Kick off a channel
SLASH_CHAT_KICK1 = "/ckick"; -- Kick off a channel
SLASH_CHAT_MODERATE2 = "/修改"; -- Set a channel's moderated flag
SLASH_CHAT_MODERATE1 = "/moderate"; -- Set a channel's moderated flag
SLASH_CHAT_MODERATOR4 = "/moderator";
SLASH_CHAT_MODERATOR1 = "/mod"; -- Set a moderator
SLASH_CHAT_MODERATOR3 = "/mod"; -- Set a moderator
SLASH_CHAT_MODERATOR2 = "/缓和器";
SLASH_CHAT_MUTE5 = "/squelch";
SLASH_CHAT_MUTE6 = "/禁声";
SLASH_CHAT_MUTE3 = "/unvoice";
SLASH_CHAT_MUTE2 = "/压制";
SLASH_CHAT_MUTE1 = "/mute"; -- Mute a player on a channel
SLASH_CHAT_MUTE4 = "/静声"; -- Mute a player on a channel
SLASH_CHAT_OWNER1 = "/owner"; -- Display or change channel owner
SLASH_CHAT_OWNER2 = "/owner"; -- Display or change channel owner
SLASH_CHAT_PASSWORD5 = "/pass";
SLASH_CHAT_PASSWORD4 = "/密码"; -- Set channel password
SLASH_CHAT_PASSWORD2 = "/pass";
SLASH_CHAT_PASSWORD1 = "/password"; -- Set channel password
SLASH_CHAT_PASSWORD3 = "/密码"; -- Set channel password
SLASH_CHAT_UNBAN2 = "/unban"; -- Unban
SLASH_CHAT_UNBAN1 = "/unban"; -- Unban
SLASH_CHAT_UNMODERATOR4 = "/unmoderator";
SLASH_CHAT_UNMODERATOR3 = "/unmod"; -- Remove a mod
SLASH_CHAT_UNMODERATOR2 = "/解除缓和";
SLASH_CHAT_UNMODERATOR1 = "/unmod"; -- Remove a mod
SLASH_CHAT_UNMUTE6 = "/voice";
SLASH_CHAT_UNMUTE1 = "/unmute"; -- Give a player voice on a channel
SLASH_CHAT_UNMUTE3 = "/声音";
SLASH_CHAT_UNMUTE4 = "/开声"; -- Give a player voice on a channel
SLASH_CHAT_UNMUTE5 = "/unsquelch";
SLASH_CHAT_UNMUTE2 = "/unsquelch";
SLASH_CLEAR1 = "/clear"; -- Clears whatever chat window has focus
SLASH_CLEAR2 = "/clear";
SLASH_COMBATLOG2 = "/combatlog"; -- enable/disable combat log file output
SLASH_COMBATLOG1 = "/战斗日志"; -- enable/disable combat log file output
SLASH_CONSOLE2 = "/控制台";
SLASH_CONSOLE1 = "/console";
SLASH_DUEL1 = "/对决";
SLASH_DUEL2 = "/duel";
SLASH_DUEL_CANCEL2 = "/认输";
SLASH_DUEL_CANCEL6 = "/forfeit";
SLASH_DUEL_CANCEL5 = "/concede";
SLASH_DUEL_CANCEL4 = "/yield";
SLASH_DUEL_CANCEL3 = "/forfeit";
SLASH_DUEL_CANCEL1 = "/yield";
SLASH_EMOTE8 = "/me"; -- Emote
SLASH_EMOTE3 = "/emote"; -- Emote
SLASH_EMOTE5 = "/表情"; -- Emote
SLASH_EMOTE4 = "/emote"; -- Emote
SLASH_EMOTE2 = "/表情"; -- Emote
SLASH_EMOTE6 = "/em"; -- Emote
SLASH_EMOTE7 = "/emote"; -- Emote
SLASH_EMOTE1 = "/e"; -- Emote
SLASH_FOLLOW1 = "/f";
SLASH_FOLLOW2 = "/跟随";
SLASH_FOLLOW6 = "/fol";
SLASH_FOLLOW4 = "/f";
SLASH_FOLLOW3 = "/fol";
SLASH_FOLLOW5 = "/follow";
SLASH_FRIENDS3 = "/friends";
SLASH_FRIENDS4 = "/friend";
SLASH_FRIENDS1 = "/friends";
SLASH_FRIENDS2 = "/朋友";
SLASH_GUILD8 = "/guild"; -- Guild
SLASH_GUILD7 = "/gu"; -- Guild
SLASH_GUILD1 = "/g"; -- Guild
SLASH_HARDCORE1 = "/h"; -- Hardcore
SLASH_GUILD3 = "/gu"; -- Guild
SLASH_GUILD4 = "/公会"; -- Guild
SLASH_GUILD5 = "/公会"; -- Guild
SLASH_GUILD2 = "/gc"; -- Guild
SLASH_GUILD6 = "/gc"; -- Guild
SLASH_GUILD_DEMOTE3 = "/公会降级"; -- Guild demote
SLASH_GUILD_DEMOTE1 = "/gdemote"; -- Guild demote
SLASH_GUILD_DEMOTE2 = "/公会降职"; -- Guild demote
SLASH_GUILD_DEMOTE4 = "/guilddemote"; -- Guild demote
SLASH_GUILD_DISBAND4 = "/guilddisband"; -- Guild disband
SLASH_GUILD_DISBAND3 = "/公会解散"; -- Guild disband
SLASH_GUILD_DISBAND1 = "/gdisband"; -- Guild disband
SLASH_GUILD_DISBAND2 = "/guilddisband"; -- Guild disband
SLASH_GUILD_HELP3 = "/公会帮助"; -- Guild help
SLASH_GUILD_HELP4 = "/公会帮助"; -- Guild help
SLASH_GUILD_HELP2 = "/guildhelp"; -- Guild help
SLASH_GUILD_HELP1 = "/ghelp"; -- Guild help
SLASH_GUILD_HELP5 = "/ghelp"; -- Guild help
SLASH_GUILD_INFO2 = "/guildinfo"; -- Guild information
SLASH_GUILD_INFO3 = "/公会信息"; -- Guild information
SLASH_GUILD_INFO1 = "/ginfo"; -- Guild information
SLASH_GUILD_INFO4 = "/guildinfo"; -- Guild information
SLASH_GUILD_INVITE4 = "/guildinvite"; -- Guild invite
SLASH_GUILD_INVITE3 = "/公会邀请"; -- Guild invite
SLASH_GUILD_INVITE2 = "/guildinvite"; -- Guild invite
SLASH_GUILD_INVITE1 = "/ginvite"; -- Guild invite
SLASH_GUILD_LEADER4 = "/guildleader"; -- Guild leader
SLASH_GUILD_LEADER3 = "/公会领袖"; -- Guild leader
SLASH_GUILD_LEADER2 = "/公会领袖"; -- Guild leader
SLASH_GUILD_LEADER1 = "/gleader"; -- Guild leader
SLASH_GUILD_LEAVE4 = "/guildquit"; -- Guild leave
SLASH_GUILD_LEAVE1 = "/gquit"; -- Guild leave
SLASH_GUILD_LEAVE2 = "/退出公会"; -- Guild leave
SLASH_GUILD_LEAVE3 = "/gquit"; -- Guild leave
SLASH_GUILD_MOTD4 = "/guildmotd"; -- Guild MOTD
SLASH_GUILD_MOTD2 = "/guildmotd"; -- Guild MOTD
SLASH_GUILD_MOTD3 = "/公会信息"; -- Guild MOTD
SLASH_GUILD_MOTD1 = "/gmotd"; -- Guild MOTD
SLASH_GUILD_PROMOTE4 = "/guildpromote"; -- Guild promote
SLASH_GUILD_PROMOTE2 = "/guildpromote"; -- Guild promote
SLASH_GUILD_PROMOTE1 = "/gpromote"; -- Guild promote
SLASH_GUILD_PROMOTE3 = "/公会提升"; -- Guild promote
SLASH_GUILD_ROSTER4 = "/guildroster"; -- Guild roster
SLASH_GUILD_ROSTER1 = "/groster"; -- Guild roster
SLASH_GUILD_ROSTER3 = "/公会名册"; -- Guild roster
SLASH_GUILD_ROSTER2 = "/guildroster"; -- Guild roster
SLASH_GUILD_UNINVITE4 = "/guildremove"; -- Guild uninvite
SLASH_GUILD_UNINVITE3 = "/公会剔除"; -- Guild uninvite
SLASH_GUILD_UNINVITE2 = "/公会删除"; -- Guild uninvite
SLASH_GUILD_UNINVITE1 = "/gremove"; -- Guild uninvite
SLASH_GUILD_WHO6 = "/whoguild";
SLASH_GUILD_WHO5 = "/gwho";
SLASH_GUILD_WHO4 = "/glist";
SLASH_GUILD_WHO2 = "/gwho";
SLASH_GUILD_WHO3 = "/whoguild";
SLASH_GUILD_WHO1 = "/glist";
SLASH_HELP6 = "/help"; -- Help
SLASH_HELP2 = "/帮助"; -- Help
SLASH_HELP5 = "/h"; -- Help
SLASH_HELP4 = "/h"; -- Help
SLASH_HELP3 = "/?"; -- Help
SLASH_HELP1 = "/h"; -- Help
SLASH_IGNORE1 = "/屏蔽";
SLASH_IGNORE2 = "/ignore";
SLASH_INSPECT3 = "/ins"; -- Inspect
SLASH_INSPECT4 = "/inspect"; -- Inspect
SLASH_INSPECT2 = "/观察"; -- Inspect
SLASH_INSPECT1 = "/ins"; -- Inspect
SLASH_INVITE5 = "/inv"; -- Invite
SLASH_INVITE4 = "/i"; -- Invite
SLASH_INVITE6 = "/invite"; -- Invite
SLASH_INVITE1 = "/i"; -- Invite
SLASH_INVITE3 = "/邀请"; -- Invite
SLASH_INVITE2 = "/inv"; -- Invite
SLASH_JOIN2 = "/频道";
SLASH_JOIN6 = "/channel";
SLASH_JOIN3 = "/频道";
SLASH_JOIN1 = "/join"; -- Join (chat channel)
SLASH_JOIN5 = "/加入"; -- Join (chat channel)
SLASH_JOIN7 = "/chan";
SLASH_JOIN4 = "/加入"; -- Join (chat channel)
SLASH_LEAVE6 = "/chatleave";
SLASH_LEAVE7 = "/chatexit";
SLASH_LEAVE2 = "/离开频道";
SLASH_LEAVE4 = "/离开"; -- Leave (chat channel)
SLASH_LEAVE3 = "/退出频道";
SLASH_LEAVE5 = "/离开"; -- Leave (chat channel)
SLASH_LEAVE1 = "/leave"; -- Leave (chat channel)
SLASH_LIST_CHANNEL2 = "/频道查询";
SLASH_LIST_CHANNEL6 = "/chatwho";
SLASH_LIST_CHANNEL4 = "/聊天列表"; -- List channels or members
SLASH_LIST_CHANNEL5 = "/聊天列表"; -- List channels or members
SLASH_LIST_CHANNEL3 = "/频道信息";
SLASH_LIST_CHANNEL1 = "/chatlist"; -- List channels or members
SLASH_LIST_CHANNEL7 = "/chatinfo";
SLASH_LOGOUT4 = "/camp"; -- Logout
SLASH_LOGOUT2 = "/camp"; -- Logout
SLASH_LOGOUT1 = "/登出"; -- Logout
SLASH_LOGOUT3 = "/logout"; -- Logout
SLASH_LOOT_FFA1 = "/随意拾取";
SLASH_LOOT_FFA2 = "/ffa";
SLASH_LOOT_MASTER2 = "/master";
SLASH_LOOT_MASTER1 = "/队长分配";
SLASH_LOOT_ROUNDROBIN2 = "/roundrobin";
SLASH_LOOT_ROUNDROBIN1 = "/轮流拾取";
SLASH_MACRO1 = "/宏";
SLASH_MACRO3 = "/m";
SLASH_MACRO4 = "/macro";
SLASH_MACRO2 = "/m";
SLASH_MACROHELP2 = "/macrohelp";
SLASH_MACROHELP3 = "/宏帮助";
SLASH_MACROHELP1 = "/宏帮助";
SLASH_OFFICER4 = "/osay"; -- Officer
SLASH_OFFICER1 = "/o"; -- Officer
SLASH_OFFICER2 = "/官员发言"; -- Officer
SLASH_OFFICER3 = "/o"; -- Officer
SLASH_PARTY1 = "/p"; -- Party
SLASH_PARTY2 = "/party"; -- Party
SLASH_PARTY4 = "/party";
SLASH_PARTY3 = "/p"; -- Party
SLASH_PLAYED2 = "/游戏时间"; -- Time played
SLASH_PLAYED1 = "/played"; -- Time played
SLASH_PROMOTE3 = "/pr"; -- Promote
SLASH_PROMOTE2 = "/升职"; -- Promote
SLASH_PROMOTE1 = "/pr"; -- Promote
SLASH_PROMOTE4 = "/promote"; -- Promote
SLASH_PVP1 = "/pvp";
SLASH_PVP2 = "/pvp";
SLASH_QUIT1 = "/离开"; -- Quit
SLASH_QUIT3 = "/quit"; -- Quit
SLASH_QUIT2 = "/退出"; -- Quit
SLASH_QUIT4 = "/exit"; -- Quit
SLASH_RAID4 = "/rsay";
SLASH_RAID3 = "/ra";
SLASH_RAID6 = "/rsay";
SLASH_RAID1 = "/团队";
SLASH_RAID5 = "/ra";
SLASH_RAID2 = "/raid";
SLASH_RAID_INFO1 = "/raidinfo"; -- Returns information about your saved raid instances
SLASH_RAID_INFO2 = "/raidinfo";
SLASH_RAID_WARNING1 = "/rw";
SLASH_RAID_WARNING2 = "/rw";
SLASH_RANDOM1 = "/random";
SLASH_RANDOM5 = "/rand";
SLASH_RANDOM3 = "/随机";
SLASH_RANDOM4 = "/random";
SLASH_RANDOM6 = "/rnd";
SLASH_RANDOM2 = "/rand";
SLASH_RANDOM7 = "/roll";
SLASH_READYCHECK1 = "/readycheck";
SLASH_READYCHECK2 = "/readycheck";
SLASH_REMOVEFRIEND3 = "/removefriend";
SLASH_REMOVEFRIEND4 = "/remfriend";
SLASH_REMOVEFRIEND1 = "/删除好友";
SLASH_REMOVEFRIEND2 = "/remfriend";
SLASH_REPLY2 = "/回复"; -- Reply
SLASH_REPLY4 = "/reply"; -- Reply
SLASH_REPLY3 = "/回复"; -- Reply
SLASH_REPLY1 = "/r"; -- Reply
SLASH_SAVEGUILDROSTER1 = "/saveguildroster";
SLASH_SAVEGUILDROSTER2 = "/saveguildroster";
SLASH_SAY1 = "/s"; -- Say
SLASH_SAY3 = "/s"; -- Say
SLASH_SAY2 = "/说"; -- Say
SLASH_SAY4 = "/say"; -- Say
SLASH_SCRIPT1 = "/脚本";
SLASH_SCRIPT4 = "/run";
SLASH_SCRIPT3 = "/script";
SLASH_SCRIPT2 = "/run";
SLASH_TARGET3 = "/target";
SLASH_TARGET4 = "/tar";
SLASH_TARGET2 = "/tar";
SLASH_TARGET1 = "/目标";
SLASH_TIME2 = "/time"; -- Time
SLASH_TIME1 = "/时间"; -- Time
SLASH_TRADE3 = "/tr"; -- Trade
SLASH_TRADE2 = "/交易"; -- Trade
SLASH_TRADE1 = "/tr"; -- Trade
SLASH_TRADE4 = "/trade"; -- Trade
SLASH_UNIGNORE1 = "/取消屏蔽";
SLASH_UNIGNORE2 = "/unignore";
SLASH_UNINVITE6 = "/un"; -- Uninvite
SLASH_UNINVITE4 = "/移除"; -- Uninvite
SLASH_UNINVITE8 = "/kick";
SLASH_UNINVITE5 = "/u"; -- Uninvite
SLASH_UNINVITE2 = "/un"; -- Uninvite
SLASH_UNINVITE7 = "/uninvite"; -- Uninvite
SLASH_UNINVITE3 = "/踢除"; -- Uninvite
SLASH_UNINVITE1 = "/u"; -- Uninvite
SLASH_VOICEMACRO1 = "/v";
SLASH_VOICEMACRO2 = "/v";
SLASH_WHISPER8 = "/t"; -- Whisper
SLASH_WHISPER5 = "/发送"; -- Whisper
SLASH_WHISPER4 = "/告诉"; -- Whisper
SLASH_WHISPER3 = "/t"; -- Whisper
SLASH_WHISPER1 = "/w"; -- Whisper
SLASH_WHISPER6 = "/w"; -- Whisper
SLASH_WHISPER2 = "/密语"; -- Whisper
SLASH_WHISPER9 = "/tell"; -- Whisper
SLASH_WHISPER10 = "/send"; -- Whisper
SLASH_WHISPER7 = "/whisper"; -- Whisper
SLASH_WHO2 = "/who"; -- Get a list of players
SLASH_WHO1 = "/查询"; -- Get a list of players
SLASH_YELL6 = "/yell"; -- Yell
SLASH_YELL3 = "/sh"; -- Yell
SLASH_YELL5 = "/大喊"; -- Yell
SLASH_YELL4 = "/大叫"; -- Yell
SLASH_YELL2 = "/大喊"; -- Yell
SLASH_YELL7 = "/sh"; -- Yell
SLASH_YELL1 = "/y"; -- Yell
SLURRED_SPEECH = "%s ……嗝儿！"; -- Used for drunken mumbles
SMART_PIVOT = "智能调节";
SOCIALS = "社交"; -- Chat options, called "socials"...
SOCIAL_BUTTON = "社交";
SOLD_BY_COLON = "销售者：";
SOLO = "独身";
SORT_QUEST = "任务分类"; -- Text for the quest sorting tab
SOUNDOPTIONS_MENU = "声音设置"; -- Title for sound options menu
SOUND_VOLUME = "音效音量"; -- Sound options tag
SPEAKERMODE = "通告模式";
SPEAKERMODE_HEADPHONES = "头戴式耳机";
SPEAKERMODE_STEREO = "立体声";
SPEAKERMODE_SURROUND = "环绕音效";
SPECIAL_SKILLS = "%s技能："; -- %s is the class specialization
SPEED = "速度"; -- Generic "speed"
SPELLBLOCKEDOTHEROTHER = "%s的%s被%s格挡过去。";
SPELLBLOCKEDOTHERSELF = "%s的%s被格挡过去。";
SPELLBLOCKEDSELFOTHER = "你的%s被%s格挡了。";
SPELLBOOK = "法术书"; -- The player's spellbook
SPELLBOOK_ABILITIES_BUTTON = "法术书和技能";
SPELLBOOK_BUTTON = "法术书";
SPELLCASTGOOTHER = "%s施放了%s。";
SPELLCASTGOOTHERTARGETTED = "%1$s对%3$s施放了%2$s。";
SPELLCASTGOSELF = "你施放了%s。";
SPELLCASTGOSELFTARGETTED = "你对%2$s施放了%1$s。";
SPELLCASTOTHERSTART = "%s开始施放%s。";
SPELLCASTSELFSTART = "你开始施放%s。";
SPELLDEFLECTEDOTHEROTHER = "%s的%s被%s偏斜了。";
SPELLDEFLECTEDOTHERSELF = "%s的%s被偏斜了。";
SPELLDEFLECTEDSELFOTHER = "你的%s被%s偏斜了。";
SPELLDEFLECTEDSELFSELF = "你偏斜了你的%s。";
SPELLDISMISSPETOTHER = "%s的%s被解散了。";
SPELLDISMISSPETSELF = "你的%s被解散了。";
SPELLDODGEDOTHEROTHER = "%s的%s被%s闪躲过去。";
SPELLDODGEDOTHERSELF = "%s的%s被闪躲过去。";
SPELLDODGEDSELFOTHER = "你的%s被%s躲闪过去了。";
SPELLDODGEDSELFSELF = "你躲闪了你的%s。";
SPELLDURABILITYDAMAGEALLOTHEROTHER = "%s对%s施放%s：所有的物品损坏。";
SPELLDURABILITYDAMAGEALLOTHERSELF = "%s对你施放%s：所有的物品损坏。";
SPELLDURABILITYDAMAGEALLSELFOTHER = "你对%s施放了%s：所有物品损坏。";
SPELLDURABILITYDAMAGEOTHEROTHER = "%1$s对%3$s施放了%2$s，造成%4$s点伤害。";
SPELLDURABILITYDAMAGEOTHERSELF = "%s对你施放了%s，造成%s点伤害。";
SPELLDURABILITYDAMAGEOTHERSELF = "%s casts %s on you: %s damaged.";
SPELLDURABILITYDAMAGESELFOTHER = "你对%2$s施放了%1$s，造成%3$s点伤害。"; -- Example:  You cast Destruction on Fred: helmet damaged.
SPELLEVADEDOTHEROTHER = "%s的%s被%s闪避过去。";
SPELLEVADEDOTHERSELF = "%s的%s被闪避过去。";
SPELLEVADEDSELFOTHER = "你的%s被%s闪避过去了。";
SPELLEVADEDSELFSELF = "你闪避了你的%s。";
SPELLEXTRAATTACKSOTHER = "%1$s通过%3$s获得了%2$d次额外攻击。"; -- Victim gains 3 extra attacks through Thrash.
SPELLEXTRAATTACKSOTHER_SINGULAR = "%1$s通过%3$s获得了%2$d次额外攻击。";
SPELLEXTRAATTACKSSELF = "你通过%2$s获得了%1$d次额外攻击。"; -- You gain 3 extra attacks through Thrash.
SPELLEXTRAATTACKSSELF_SINGULAR = "你通过%2$s获得%1$d次额外攻击。";
SPELLFAILCASTOTHER = "%s施放%s失败：%s。";
SPELLFAILCASTSELF = "你施放%s失败：%s";
SPELLFAILPERFORMOTHER = "%s进行%s失败：%s。";
SPELLFAILPERFORMSELF = "你施展%s失败：%s";
SPELLHAPPINESSDRAINOTHER = "%s的%s失去了%d点快乐值。";
SPELLHAPPINESSDRAINSELF = "你的%s失去了%d点快乐值。";
SPELLIMMUNEOTHEROTHER = "%s的%s施放失败。%s对此免疫。";
SPELLIMMUNEOTHERSELF = "%s的%s施放失败。你对此免疫。";
SPELLIMMUNESELFOTHER = "你的%s施放失败。%s对此免疫。";
SPELLIMMUNESELFSELF = "你的%s失败了。你对此免疫。";
SPELLINTERRUPTOTHEROTHER = "%s打断了%s的%s。";
SPELLINTERRUPTOTHERSELF = "%s打断了你的%s。";
SPELLINTERRUPTSELFOTHER = "你打断了%s的%s。";
SPELLLOGABSORBOTHEROTHER = "%s的%s被%s吸收了。";
SPELLLOGABSORBOTHERSELF = "你吸收了%s的%s。";
SPELLLOGABSORBSELFOTHER = "你的%s被%s吸收了。";
SPELLLOGABSORBSELFSELF = "你吸收了你的%s。";
SPELLLOGCRITOTHEROTHER = "%s的%s对%s造成%d点致命一击伤害。";
SPELLLOGCRITOTHERSELF = "%s的%s对你造成%d点致命一击伤害。";
SPELLLOGCRITSCHOOLOTHEROTHER = "%s的%s致命一击对%s造成%d点%s伤害。";
SPELLLOGCRITSCHOOLOTHERSELF = "%s的%s致命一击对你造成%d点%s伤害。";
SPELLLOGCRITSCHOOLSELFOTHER = "你的%s致命一击对%s造成%d点%s伤害。";
SPELLLOGCRITSCHOOLSELFSELF = "你的%s致命一击对你造成%d点%s伤害。";
SPELLLOGCRITSELFOTHER = "你的%s对%s造成%d的致命一击伤害。";
SPELLLOGCRITSELFSELF = "你的%s致命一击对你造成%d点伤害。";
SPELLLOGOTHEROTHER = "%s的%s击中%s造成%d点伤害。";
SPELLLOGOTHERSELF = "%s的%s击中你造成%d点伤害。";
SPELLLOGSCHOOLOTHEROTHER = "%s的%s击中%s造成%d点%s伤害。";
SPELLLOGSCHOOLOTHERSELF = "%s的%s击中你造成%d点%s伤害。";
SPELLLOGSCHOOLSELFOTHER = "你的%s击中%s造成%d点%s伤害。";
SPELLLOGSCHOOLSELFSELF = "你的%s击中你造成%d点%s伤害。";
SPELLLOGSELFOTHER = "你的%s击中%s造成%d点伤害。";
SPELLLOGSELFSELF = "你的%s击中你造成了%d点伤害。";
SPELLMISSOTHEROTHER = "%s的%s没有击中%s。";
SPELLMISSOTHERSELF = "%s的%s没有击中你。";
SPELLMISSSELFOTHER = "你的%s没有击中%s。";
SPELLMISSSELFSELF = "你的%s未击中你。";
SPELLPARRIEDOTHEROTHER = "%s的%s被%s招架了。";
SPELLPARRIEDOTHERSELF = "%s的%s被招架了。";
SPELLPARRIEDSELFOTHER = "你的%s被%s招架了。";
SPELLPARRIEDSELFSELF = "你招架了你的%s。";
SPELLPERFORMGOOTHER = "%s使用%s。";
SPELLPERFORMGOOTHERTARGETTED = "%1$s对%3$s使用%2$s。";
SPELLPERFORMGOSELF = "你使用%s。";
SPELLPERFORMGOSELFTARGETTED = "你对%2$s使用%1$s。";
SPELLPERFORMOTHERSTART = "%s开始施展%s。";
SPELLPERFORMSELFSTART = "你开始进行%s。";
SPELLPOWERDRAINOTHEROTHER = "%1$s的%2$s从%5$s身上吸取了%3$d点%4$s。";
SPELLPOWERDRAINOTHERSELF = "%1$s的%2$s从你身上吸取了%3$d点%4$s。";
SPELLPOWERDRAINSELFOTHER = "你的%1$s从%4$s身上吸取了%2$d点%3$s。";
SPELLPOWERDRAINSELFSELF = "你的%s从你身上吸取了%d点%s。";
SPELLPOWERLEECHOTHEROTHER = "%1$s的%2$s从%5$s身上吸取了%3$d点%4$s，%6$s获得了%7$d点%8$s。";
SPELLPOWERLEECHOTHERSELF = "%1$s的%2$s从你身上吸取了%3$d点%4$s。%5$s获得了%6$d点%7$s。";
SPELLPOWERLEECHSELFOTHER = "你的%1$s从%4$s身上吸取了%2$d点%3$s，并获得了%5$d点%6$s。";
SPELLREFLECTOTHEROTHER = "%s的%s被%s反弹回来。";
SPELLREFLECTOTHERSELF = "你反弹了%s的%s。";
SPELLREFLECTSELFOTHER = "你的%s被%s反弹回来。";
SPELLREFLECTSELFSELF = "你反射了你的%s。";
SPELLRESISTOTHEROTHER = "%s的%s被%s抵抗了。";
SPELLRESISTOTHERSELF = "%s的%s被抵抗了。";
SPELLRESISTSELFOTHER = "你的%s被%s抵抗了。";
SPELLRESISTSELFSELF = "你抵抗了你的%s。";
SPELLS = "法术"; -- Generic "spells"
SPELLSPLITDAMAGEOTHEROTHER = "%s的%s对%s造成了%d点伤害。";
SPELLSPLITDAMAGEOTHERSELF = "%s的%s对你造成了%d点伤害。";
SPELLSPLITDAMAGESELFOTHER = "你的%s对%s造成了%d点伤害。";
SPELLTEACHOTHEROTHER = "%s教会%s使用%s。";
SPELLTEACHOTHERSELF = "%s教会你使用%s。";
SPELLTEACHSELFOTHER = "你教会%s使用%s。";
SPELLTEACHSELFSELF = "你教会自己使用%s。";
SPELLTERSEPERFORM_OTHER = "%s使用%s。";
SPELLTERSEPERFORM_SELF = "你使用%s。";
SPELLTERSE_OTHER = "%s施放了%s。";
SPELLTERSE_SELF = "你施放了%s。";
SPELL_CAST_CHANNELED = "需引导";
SPELL_CAST_TIME_INSTANT = "瞬发法术"; -- Zero cast time
SPELL_CAST_TIME_INSTANT_NO_MANA = "瞬发";
SPELL_CAST_TIME_MIN = "%.3g分钟施法时间"; -- How long it takes to cast a spell
SPELL_CAST_TIME_RANGED = "攻击速度+%.3g秒";
SPELL_CAST_TIME_SEC = "%.3g秒施法时间"; -- How long it takes to cast a spell
SPELL_DETAIL = "法术细节";
SPELL_DURATION = "持续%s";
SPELL_DURATION_DAYS = "%.2f天";
SPELL_DURATION_HOURS = "%.2f小时";
SPELL_DURATION_MIN = "%.2f分钟";
SPELL_DURATION_SEC = "%.2f秒";
SPELL_DURATION_UNTIL_CANCELLED = "直到主动取消";
SPELL_EQUIPPED_ITEM = "需要%s";
SPELL_EQUIPPED_ITEM_NOSPACE = "需要%s";
SPELL_FAILED_AFFECTING_COMBAT = "你正处于交战状态";
SPELL_FAILED_ALREADY_BEING_TAMED = "该生物已经被驯化。";
SPELL_FAILED_ALREADY_HAVE_CHARM = "你已经控制了一个被魅惑的生物";
SPELL_FAILED_ALREADY_HAVE_SUMMON = "你已经控制了一个召唤生物";
SPELL_FAILED_ALREADY_OPEN = "已经打开";
SPELL_FAILED_AURA_BOUNCED = "已经有一个更强大的法术在发挥作用";
SPELL_FAILED_BAD_IMPLICIT_TARGETS = "没有目标";
SPELL_FAILED_BAD_TARGETS = "无效的目标";
SPELL_FAILED_CANT_BE_CHARMED = "目标无法被魅惑";
SPELL_FAILED_CANT_BE_DISENCHANTED = "物品无法被分解";
SPELL_FAILED_CANT_BE_PROSPECTED = "里面没有宝石";
SPELL_FAILED_CANT_CAST_ON_TAPPED = "目标已被其他玩家接触";
SPELL_FAILED_CANT_DUEL_WHILE_INVISIBLE = "你无法在隐形状态下开始决斗";
SPELL_FAILED_CANT_DUEL_WHILE_STEALTHED = "你无法在潜行状态下开始对决。";
SPELL_FAILED_CANT_STEALTH = "你与敌人的距离太近";
SPELL_FAILED_CASTER_AURASTATE = "你还不能那么做";
SPELL_FAILED_CASTER_DEAD = "你已经死亡";
SPELL_FAILED_CHARMED = "在被魅惑状态下无法那么做";
SPELL_FAILED_CHEST_IN_USE = "目标正在被使用";
SPELL_FAILED_CONFUSED = "无法在混乱状态下那样做";
SPELL_FAILED_DAMAGE_IMMUNE = "无法在免疫状态下那样做"; -- See ERR_USE_CANT_IMMUNE
SPELL_FAILED_DISENCHANT_WHILE_LOOTING = "无法在拾取物品时分解";
SPELL_FAILED_EQUIPPED_ITEM = "必须装备正确的物品";
SPELL_FAILED_EQUIPPED_ITEM_CLASS = "必须装备一个%s";
SPELL_FAILED_EQUIPPED_ITEM_CLASS_MAINHAND = "主手上必须装备%s";
SPELL_FAILED_EQUIPPED_ITEM_CLASS_OFFHAND = "必须在副手上装备一件%s";
SPELL_FAILED_ERROR = "内部错误";
SPELL_FAILED_FIZZLE = "失败";
SPELL_FAILED_FLEEING = "无法在逃跑状态下那样做";
SPELL_FAILED_FOOD_LOWLEVEL = "食物等级不够，不足以满足你的宠物。";
SPELL_FAILED_HIGHLEVEL = "目标等级太高";
SPELL_FAILED_IMMUNE = "免疫";
SPELL_FAILED_INTERRUPTED = "被打断";
SPELL_FAILED_INTERRUPTED_COMBAT = "被打断";
SPELL_FAILED_ITEM_ALREADY_ENCHANTED = "物品已经被附魔";
SPELL_FAILED_ITEM_GONE = "物品不存在";
SPELL_FAILED_ITEM_NOT_FOUND = "试图给一个不存在的物品附魔";
SPELL_FAILED_ITEM_NOT_READY = "物品还没有准备好";
SPELL_FAILED_LEVEL_REQUIREMENT = "你的角色等级不够";
SPELL_FAILED_LEVEL_REQUIREMENT_PET = "你的宠物等级不够";
SPELL_FAILED_LINE_OF_SIGHT = "目标不在视野中";
SPELL_FAILED_LOWLEVEL = "目标等级太低";
SPELL_FAILED_LOW_CASTLEVEL = "技能等级不够";
SPELL_FAILED_MAINHAND_EMPTY = "你拿武器的手是空着的";
SPELL_FAILED_MIN_SKILL = "你的技能不够高。需要%s（%d）。"; -- %s is the skill name, %d is the skill amount.
SPELL_FAILED_MOVING = "不能在移动中实施该动作";
SPELL_FAILED_NEED_AMMO = "使用弹药之前必须将弹药放在角色状态界面下方的弹药栏中";
SPELL_FAILED_NEED_AMMO_POUCH = "需要：%s";
SPELL_FAILED_NEED_EXOTIC_AMMO = "需要异种弹药：%s";
SPELL_FAILED_NOPATH = "没有可以行进的路径";
SPELL_FAILED_NOTHING_TO_DISPEL = "无可供驱散的法术";
SPELL_FAILED_NOTHING_TO_STEAL = "没有可以偷取的物品";
SPELL_FAILED_NOT_BEHIND = "你必须位于目标背后";
SPELL_FAILED_NOT_FISHABLE = "你不能在非渔区钓鱼";
SPELL_FAILED_NOT_HERE = "不能在这里使用";
SPELL_FAILED_NOT_INFRONT = "你必须站在目标前方";
SPELL_FAILED_NOT_IN_CONTROL = "你无法控制你自己的动作";
SPELL_FAILED_NOT_KNOWN = "法术尚未学会";
SPELL_FAILED_NOT_MOUNTED = "你正在骑乘状态";
SPELL_FAILED_NOT_ON_TAXI = "你正在飞行";
SPELL_FAILED_NOT_ON_TRANSPORT = "你正在运输工具上";
SPELL_FAILED_NOT_READY = "尚未恢复";
SPELL_FAILED_NOT_SHAPESHIFT = "你正在变形状态下";
SPELL_FAILED_NOT_STANDING = "你必须处于站立状态下才能那么做";
SPELL_FAILED_NOT_TRADEABLE = "你只能对自己拥有的物品使用这个";
SPELL_FAILED_NOT_TRADING = "你已试图给一件交易物品附魔，但不会进行交易";
SPELL_FAILED_NOT_UNSHEATHED = "你必须拔出武器才能那么做";
SPELL_FAILED_NOT_WHILE_FATIGUED = "疲倦时不能施法";
SPELL_FAILED_NOT_WHILE_GHOST = "无法在鬼魂状态下施放";
SPELL_FAILED_NOT_WHILE_TRADING = "当交易的时候，无法施法";
SPELL_FAILED_NO_AMMO = "弹药不足";
SPELL_FAILED_NO_CHAMPION = "你还没有选择一位勇士";
SPELL_FAILED_NO_CHARGES_REMAIN = "无使用次数";
SPELL_FAILED_NO_COMBO_POINTS = "这个技能需要连击点数";
SPELL_FAILED_NO_DUELING = "此地不允许进行决斗";
SPELL_FAILED_NO_EDIBLE_CORPSES = "附近没有可用的尸体";
SPELL_FAILED_NO_ENDURANCE = "耐力不足";
SPELL_FAILED_NO_FISH = "这里没有鱼。";
SPELL_FAILED_NO_ITEMS_WHILE_SHAPESHIFTED = "不能在变形状态下使用物品";
SPELL_FAILED_NO_MOUNTS_ALLOWED = "你不能在这里召唤坐骑！";
SPELL_FAILED_NO_PET = "你没有宠物";
SPELL_FAILED_ONLY_ABOVEWATER = "不能在游泳的时候使用";
SPELL_FAILED_ONLY_BATTLEGROUNDS = "只能在战场中使用";
SPELL_FAILED_ONLY_DAYTIME = "只能在白天使用";
SPELL_FAILED_ONLY_INDOORS = "只能在室内使用";
SPELL_FAILED_ONLY_MOUNTED = "在坐骑上才能使用";
SPELL_FAILED_ONLY_NIGHTTIME = "只能在夜晚使用";
SPELL_FAILED_ONLY_OUTDOORS = "只能在室外使用";
SPELL_FAILED_ONLY_SHAPESHIFT = "必须在%s中"; -- %s is the required stance/form
SPELL_FAILED_ONLY_STEALTHED = "你必须在潜行状态下";
SPELL_FAILED_ONLY_UNDERWATER = "只有在游泳的时候才能够使用";
SPELL_FAILED_OUT_OF_RANGE = "超出范围";
SPELL_FAILED_PACIFIED = "不能在平静状态使用这个能力";
SPELL_FAILED_POSSESSED = "你被占据了";
SPELL_FAILED_PREVENTED_BY_MECHANIC = "无法在%s时那样做"; -- %s is the mechanic state that prevented the action (e.g. disoriented, frozen, etc.)
SPELL_FAILED_PROSPECT_NEED_MORE = "需要%d %s。"; -- %d is the number of %s's required to prospect this type of material.
SPELL_FAILED_PROSPECT_WHILE_LOOTING = "无法在拾取时观察物品";
SPELL_FAILED_REAGENTS = "缺少材料：%s";
SPELL_FAILED_REPUTATION = "你的声望不够高";
SPELL_FAILED_REQUIRES_AREA = "你必须处于%s状态下。"; -- %s is an area name
SPELL_FAILED_REQUIRES_SPELL_FOCUS = "需要%s";
SPELL_FAILED_ROOTED = "你无法移动";
SPELL_FAILED_SILENCED = "不能在沉默状态中实施该动作";
SPELL_FAILED_SPELL_IN_PROGRESS = "另外一个动作正在进行中";
SPELL_FAILED_SPELL_LEARNED = "你已经学会了这个法术";
SPELL_FAILED_SPELL_UNAVAILABLE = "你无法使用这个法术";
SPELL_FAILED_SPELL_UNAVAILABLE_PET = "你的宠物无法学习这个技能";
SPELL_FAILED_STUNNED = "不能在昏迷状态实施该动作";
SPELL_FAILED_TARGETS_DEAD = "目标已经死亡";
SPELL_FAILED_TARGET_AFFECTING_COMBAT = "目标正处于战斗状态中";
SPELL_FAILED_TARGET_AURASTATE = "你还不能那么做";
SPELL_FAILED_TARGET_DUELING = "目标正在决斗中";
SPELL_FAILED_TARGET_ENEMY = "目标处于敌对状态";
SPELL_FAILED_TARGET_ENRAGED = "目标过于愤怒而无法被魅惑";
SPELL_FAILED_TARGET_FREEFORALL = "目标正处于自由PvP状态";
SPELL_FAILED_TARGET_FRIENDLY = "目标处于友好状态";
SPELL_FAILED_TARGET_IN_COMBAT = "目标无法进入战斗状态"; -- Spell failure message when trying to use a spell on something that is in combat when it should not be.
SPELL_FAILED_TARGET_IS_PLAYER = "不能将玩家作为目标";
SPELL_FAILED_TARGET_NOT_DEAD = "目标是活着的";
SPELL_FAILED_TARGET_NOT_GHOST = "你的军阶还不足以装备该物品";
SPELL_FAILED_TARGET_NOT_IN_INSTANCE = "目标必须在这个副本中";
SPELL_FAILED_TARGET_NOT_IN_PARTY = "目标和你不在同一个队伍中";
SPELL_FAILED_TARGET_NOT_IN_RAID = "目标不在你的小队或团队中";
SPELL_FAILED_TARGET_NOT_LOOTED = "必须先拿走该生物身上的物品";
SPELL_FAILED_TARGET_NOT_PLAYER = "目标不是玩家角色";
SPELL_FAILED_TARGET_NO_POCKETS = "没有物品可以偷窃";
SPELL_FAILED_TARGET_NO_WEAPONS = "目标没有装备任何武器";
SPELL_FAILED_TARGET_UNSKINNABLE = "这个生物无法被剥皮";
SPELL_FAILED_TOO_CLOSE = "目标过于接近";
SPELL_FAILED_TOO_MANY_OF_ITEM = "你已经拥有过多的同类物品";
SPELL_FAILED_TOO_MANY_SKILLS = "你的宠物不能学习更多技能了";
SPELL_FAILED_TOTEMS = "需要%s";
SPELL_FAILED_TRAINING_POINTS = "没有足够的训练点数";
SPELL_FAILED_TRANSFORM_UNUSABLE = "你无法使用新物品";
SPELL_FAILED_TRY_AGAIN = "尝试失败";
SPELL_FAILED_UNIT_NOT_BEHIND = "目标必须在你身后";
SPELL_FAILED_UNIT_NOT_INFRONT = "你必须面对目标";
SPELL_FAILED_UNKNOWN = "未知原因";
SPELL_FAILED_WRONG_PET_FOOD = "你的宠物不喜欢这种食物";
SPELL_FAILED_WRONG_WEATHER = "目前的气候不适合那样做";
SPELL_INSTANT_EFFECT = "即时效果";
SPELL_LASTING_EFFECT = "%s效果";
SPELL_MESSAGES = "法术信息";
SPELL_NOT_SHAPESHIFTED = "不能在变形状态下实施该动作";
SPELL_NOT_SHAPESHIFTED_NOSPACE = "不能在变形状态下实施该动作";
SPELL_ON_NEXT_RANGED = "攻击速度";
SPELL_ON_NEXT_SWING = "下一次近战武器攻击";
SPELL_OTHER_MESSAGES = "法术信息（倒计数）";
SPELL_PASSIVE = "被动"; -- This is a passive ability
SPELL_PASSIVE_EFFECT = "被动";
SPELL_POINTS_SPREAD_TEMPLATE = "%.1f到%.1f"; -- Min to max points for a spell
SPELL_RANGE = "%s码距离";
SPELL_RANGE_AREA = "施法者周围区域";
SPELL_REAGENTS = "材料：";
SPELL_RECAST_TIME_INSTANT = "即时冷却"; -- Zero recast time
SPELL_RECAST_TIME_MIN = "%.3g分钟冷却时间"; -- How long it takes to recast a spell
SPELL_RECAST_TIME_SEC = "%.3g秒冷却时间"; -- How long it takes to recast a spell
SPELL_REQUIRED_FORM = "需要%s";
SPELL_REQUIRED_FORM_NOSPACE = "需要%s";
SPELL_SCHOOL0_CAP = "物理";
SPELL_SCHOOL0_NAME = "物理";
SPELL_SCHOOL1_CAP = "神圣";
SPELL_SCHOOL1_NAME = "神圣";
SPELL_SCHOOL2_CAP = "火焰";
SPELL_SCHOOL2_NAME = "火焰";
SPELL_SCHOOL3_CAP = "自然";
SPELL_SCHOOL3_NAME = "自然";
SPELL_SCHOOL4_CAP = "冰霜";
SPELL_SCHOOL4_NAME = "冰霜";
SPELL_SCHOOL5_CAP = "暗影";
SPELL_SCHOOL5_NAME = "暗影";
SPELL_SCHOOL6_CAP = "奥术";
SPELL_SCHOOL6_NAME = "奥术";
SPELL_SCHOOLALL = "全部";
SPELL_SCHOOLMAGICAL = "魔法的";
SPELL_SKILL_LINE = "%s";
SPELL_STAT0_NAME = "力量"; -- enable/disable combat log file output
SPELL_STAT1_NAME = "敏捷";
SPELL_STAT2_NAME = "耐力";
SPELL_STAT3_NAME = "智力";
SPELL_STAT4_NAME = "精神";
SPELL_STATALL = "所有属性";
SPELL_TARGET_CENTER_CASTER = "施法者";
SPELL_TARGET_CENTER_LOC = "目标位置";
SPELL_TARGET_CHAIN_TEMPLATE = "目标%s，对%d个目标产生连锁效果";
SPELL_TARGET_CONE_TEMPLATE = "目标%1$s，位于%3$s面前锥形范围%2$d码内";
SPELL_TARGET_CREATURE_TYPE12_DESC = "所有%s";
SPELL_TARGET_CREATURE_TYPE13_DESC = "敌方%s";
SPELL_TARGET_CREATURE_TYPE1_DESC = "%s";
SPELL_TARGET_CREATURE_TYPE2_DESC = "友方%s";
SPELL_TARGET_CREATURE_TYPE3_DESC = "敌方%s";
SPELL_TARGET_CREATURE_TYPE8_DESC = "%s宠物";
SPELL_TARGET_CREATURE_TYPE_DEAD12_DESC = "所有已死亡%s";
SPELL_TARGET_CREATURE_TYPE_DEAD13_DESC = "死亡的敌方%s";
SPELL_TARGET_CREATURE_TYPE_DEAD1_DESC = "死亡的%s";
SPELL_TARGET_CREATURE_TYPE_DEAD2_DESC = "死亡的友方%s";
SPELL_TARGET_CREATURE_TYPE_DEAD3_DESC = "死亡的敌方%s";
SPELL_TARGET_CREATURE_TYPE_DEAD8_DESC = "死亡的%s宠物";
SPELL_TARGET_MULTIPLE_TEMPLATE = "目标%1$s，位于%3$s周围%2$d码的范围内";
SPELL_TARGET_TEMPLATE = "目标%s";
SPELL_TARGET_TYPE0_DESC = "施法者";
SPELL_TARGET_TYPE10_DESC = "副手物品";
SPELL_TARGET_TYPE11_DESC = "队伍";
SPELL_TARGET_TYPE12_DESC = "全部";
SPELL_TARGET_TYPE13_DESC = "敌人";
SPELL_TARGET_TYPE14_DESC = "队伍成员";
SPELL_TARGET_TYPE15_DESC = "主人";
SPELL_TARGET_TYPE16_DESC = "团队成员";
SPELL_TARGET_TYPE17_DESC = "团队成员";
SPELL_TARGET_TYPE1_DESC = "任何";
SPELL_TARGET_TYPE2_DESC = "友方";
SPELL_TARGET_TYPE3_DESC = "敌人";
SPELL_TARGET_TYPE4_DESC = "队伍成员";
SPELL_TARGET_TYPE5_DESC = "物品";
SPELL_TARGET_TYPE6_DESC = "所在地区";
SPELL_TARGET_TYPE7_DESC = "物体";
SPELL_TARGET_TYPE8_DESC = "宠物";
SPELL_TARGET_TYPE9_DESC = "主手物品";
SPELL_TARGET_TYPE_DEAD11_DESC = "死亡的队伍成员";
SPELL_TARGET_TYPE_DEAD12_DESC = "所有已死亡人物";
SPELL_TARGET_TYPE_DEAD13_DESC = "死亡的敌人";
SPELL_TARGET_TYPE_DEAD14_DESC = "死亡的队伍成员";
SPELL_TARGET_TYPE_DEAD16_DESC = "死亡的团队成员";
SPELL_TARGET_TYPE_DEAD17_DESC = "死亡的团队成员";
SPELL_TARGET_TYPE_DEAD1_DESC = "死亡的";
SPELL_TARGET_TYPE_DEAD2_DESC = "死亡的友方";
SPELL_TARGET_TYPE_DEAD3_DESC = "死亡的敌人";
SPELL_TARGET_TYPE_DEAD4_DESC = "死亡的队伍成员";
SPELL_TARGET_TYPE_DEAD8_DESC = "死亡的宠物";
SPELL_TIMER = "%s失败：你还没有完全恢复。"; -- The player is trying to cast a spell before the cooldown timer expires
SPELL_TIME_REMAINING_DAYS = "剩余%d天";
SPELL_TIME_REMAINING_DAYS_P1 = "剩余%d天";
SPELL_TIME_REMAINING_HOURS = "剩余%d小时";
SPELL_TIME_REMAINING_HOURS_P1 = "剩余%d小时";
SPELL_TIME_REMAINING_MIN = "剩余%d分钟"; -- Time remaining on a buff
SPELL_TIME_REMAINING_MIN_P1 = "剩余%d分钟";
SPELL_TIME_REMAINING_SEC = "剩余%d秒"; -- Time remaining on a buff
SPELL_TIME_REMAINING_SEC_P1 = "剩余%d秒";
SPELL_TOTEMS = "工具：";
SPELL_USE_ALL_ENERGY = "使用全部能量值";
SPELL_USE_ALL_FOCUS = "使用全部集中值";
SPELL_USE_ALL_HEALTH = "使用全部生命值";
SPELL_USE_ALL_MANA = "使用全部法力值"; -- Uses up all your mana
SPELL_USE_ALL_RAGE = "使用全部怒气值";
SPIRIT_COLON = "精神："; -- Character's spirit, with colon
SPIRIT_TOOLTIP = "提高生命值和法力值的回复速度。精神\n影响所有人物在战斗状态下和非战斗状态下的\n法力值和生命值回复速度。"; -- Tooltip description for "Spirit"
STABLED_PETS = "已存放的宠物：";
STABLES = "宠物存放处";
STABLE_SLOT_TEXT = "你想要再购买一个宠物存放栏位吗？";
STAMINA_COLON = "耐力："; -- Character's stamina, with colon
STAMINA_TOOLTIP = "提高生命值"; -- Tooltip description for "Stamina"
STANDING = "关系"; -- Label for your standing with a faction i.e. "Revered", "Hated"
STARTING_PRICE = "起始价格";
STARTUP_TEXT_LINE2 = "";
STARTUP_TEXT_LINE3 = "";
STARTUP_TEXT_LINE1 = "输入 '/help 以获取简略的命令列表。";
STARTUP_TEXT_LINE4 = "";
STATS_LABEL = "状态：";
STATUS_BAR_TEXT = "状态栏数值";
STAT_TEMPLATE = "%s状态";
STOP_IGNORE = "删除玩家"; -- Remove from ignore list
STRENGTH_COLON = "力量："; -- Character's strength, with colon
STRENGTH_TOOLTIP = "增加你的攻击强度和每秒伤害。\n力量不会影响致命一击的几率。力量\n不会提高你的格挡几率，但是可以增加\n你每次格挡所减少的伤害。这一部分\n减少的伤害一部分由你的力量决定，\n另一部分由盾牌本身的属性决定。";
STUCK_BUTTON2_TEXT = "如果还不行的话，请联系GM";
STUCK_BUTTON_TEXT = "自动脱离卡死";
STUCK_OPTION = "卡死选项";
STUN = "昏迷"; -- Stun in combat
STUNNED = "昏迷"; -- Stunned in combat
STUN_CAPS = "昏迷";
SUBMIT = "提交";
SUGGESTFRAME_TITLE = "建议和Bug"; -- Suggest frame title
SUGGEST_SUBMITTED = "建议已经提交。";
SUGGEST_SUBMIT_FAILED = "建议提交失败。";
SUGGEST_TOOLTIP_TEXT = "请输入你想要提交的Bug或者建议\n你的名字、种族、职业、等级和位置\n将被自动提交"; -- Gives the time in minutes/seconds until you logout
SYSTEM_MESSAGES = "系统信息";
TABARDSLOT = "公会徽章"; -- Tabard slot
TABARDVENDORALREADYSETGREETING = "你已经拥有了一个徽章，但是仍可随意试穿";
TABARDVENDORCOST = "花费：";
TABARDVENDORGREETING = "欢迎！请选择你的公会标志和颜色。";
TABARDVENDORNOGUILDGREETING = "只有公会首领才能购买公会徽章，但是你可以随意预览。";
TAKE_ATTACHMENTS = "拿取附件：";
TAKE_GM_SURVEY = "你愿意花时间填写一份GM调查问卷吗？";
TALENTS = "天赋";
TALENTS_BUTTON = "天赋"; -- Talent microbutton text
TALENT_POINTS_TOOLTIP = "天赋点数通过天赋界面分配，其作用是\n提升你的作战能力。"; -- Talent point description
TALENT_TRAINER = "天赋训练师"; -- Talent Trainer title
TAMEABLE = "可驯服";
TARGET = "目标";
TARGET_TOKEN_NOT_FOUND = "<没有目标>";
TASKS_COLON = "任务："; -- Header for the available tasks, with colon
TAXINODEYOUAREHERE = "你在这里";
TAXISAMENODE = "你已经在那里了！";
TERRAIN_HIGHLIGHTS = "地形高亮";
TERRAIN_MIP = "地形阴影质量";
TEST_TAG_TEST = "do not translate";
TEXTURE_DETAIL = "材质细节";
TICKET_STATUS1 = "你发出了求助信息。";
TICKET_TYPE1 = "游戏";
TICKET_TYPE2 = "骚扰";
TICKET_TYPE4 = "Bug";
TICKET_TYPE3 = "卡死";
TIME_DAYHOURMINUTESECOND = "%d天，%s小时，%d分钟，%d秒";
TIME_ELAPSED = "已用时间：";
TIME_PLAYED_LEVEL = "你在这个等级的游戏时间：%s"; -- The amount of time played at this level
TIME_PLAYED_MSG = "已玩本游戏的时间";
TIME_PLAYED_TOTAL = "总游戏时间：%s"; -- Total amount of time this character has played
TIME_REMAINING = "剩余时间：";
TIME_TEMPLATE_LONG = "%d天，%d小时，%d分钟，%d秒"; -- Generic template for displaying time
TIME_TO_PORT = "即将传送：";
TIME_TWELVEHOURAM = "%d:%02d AM"; -- %d = hour, %d = minute
TIME_TWELVEHOURPM = "%d:%02d PM"; -- %d = hour, %d = minute
TIME_TWENTYFOURHOURS = "%d：%02d"; -- %d = hour, %d = minute
TIME_UNTIL_DELETED = "信息保留时间"; -- Tooltip for mail that is marked deletable
TIME_UNTIL_RETURNED = "信息退回时间"; -- Tooltip for items marked returnable
TITLE_TEMPLATE = "%2$s - %1$s"; -- Character's title and faction, as in "Bob of the Carpenters' Guild"
TOGGLESTICKYCAMERA = "切换视角";
TOOLTIP_TALENT_LEARN = "点击这里学习";
TOOLTIP_TALENT_NEXT_RANK = "下一级：";
TOOLTIP_TALENT_PREREQ = "需要%d点%s";
TOOLTIP_TALENT_PREREQ_P1 = "需要%d点%s";
TOOLTIP_TALENT_RANK = "等级 %d/%d";
TOOLTIP_TALENT_TIER_POINTS = "需要%d点%s系天赋";
TOOLTIP_UNIT_LEVEL = "等级 %s";
TOOLTIP_UNIT_LEVEL_CLASS = "等级 %s %s";
TOOLTIP_UNIT_LEVEL_CLASS_TYPE = "等级 %s %s（%s）";
TOOLTIP_UNIT_LEVEL_TYPE = "等级 %s（%s）";
TOO_FAR_TO_LOOT = "你离尸体太远，无法拾取！"; -- The player is too far away to loot a corpse
TRACK_QUEST = "追踪任务";
TRADE = "交易"; -- Trade with another player
TRADEFRAME_ENCHANT_SLOT_LABEL = "不会被交易";
TRADEFRAME_NOT_MODIFIED_TEXT = "物品未被改变";
TRADESKILL_LOG_FIRSTPERSON = "你制造了%s。";
TRADESKILL_LOG_THIRDPERSON = "%s制造了%s。";
TRADESKILL_SERVICE_LEARN = "配方"; -- Services that teach a recipe
TRADESKILL_SERVICE_STEP = "发展技能"; -- Services that raise potential in a trade skill
TRADE_SKILLS = "专业"; -- Trade skills, such as weaponcrafting, armorsmithing, etc.
TRADE_SKILL_TITLE = "%s"; -- Title for the trade skill frame
TRADE_WITH_QUESTION = "和%s交易吗？"; -- Asks the player if he would like to trade with another player
TRAIN = "训练"; -- Train in a skill
TRAINER_CAST_TIME_INSTANT = "施放时间：|cffffffff即时|r";
TRAINER_CAST_TIME_MIN = "施放时间：|cffffffff%d分|r";
TRAINER_CAST_TIME_SEC = "施放时间：|cffffffff%d秒|r";
TRAINER_COOLDOWN_TIME_INSTANT = "冷却时间：|cffffffff即时|r";
TRAINER_COOLDOWN_TIME_MIN = "冷却时间：|cffffffff%d分|r";
TRAINER_COOLDOWN_TIME_SEC = "冷却时间：|cffffffff%d秒|r";
TRAINER_COST_SP = "技能点数 |cffffffff%d|r";
TRAINER_COST_SP_RED = "技能点数 |cffff2020%d|r";
TRAINER_COST_TP = "天赋点数 |cffffffff%d|r";
TRAINER_COST_TP_RED = "天赋点数 |cffff2020%d|r";
TRAINER_LIST_SP = "%d SP"; -- Used to format the Skill Point cost   in the trainer listings
TRAINER_LIST_TP = "%d TP"; -- Used to format training points in the pet trainer listing
TRAINER_MANA_COST = "法力消耗：|cffffffff%d|r";
TRAINER_MANA_COST_PER_TIME = "法力消耗：|cffffffff%d，外加%d每秒|r";
TRAINER_PET_LEVEL = "宠物等级 |cffffffff%d|r";
TRAINER_PET_LEVEL_RED = "宠物等级 |cffff2020%d|r";
TRAINER_PET_SPELL_LABEL = "- 宠物法术";
TRAINER_RANGE = "射程：|cffffffff%s (%d)|r";
TRAINER_REQ_ABILITY = "|cffffffff%s|r";
TRAINER_REQ_ABILITY_RED = "|cffff2020%s|r";
TRAINER_REQ_LEVEL = "等级 |cffffffff%d|r";
TRAINER_REQ_LEVEL_RED = "等级 |cffff2020%d|r";
TRAINER_REQ_SKILL_RANK = "%s (|cffffffff%d|r)";
TRAINER_REQ_SKILL_RANK_RED = "%s (|cffff2020%d|r)";
TRAINING_POINTS = "训练点数：";
TRAINING_POINTS_LABEL = "训练点数";
TRANSFER_ABORT_ERROR = "传输失败：未知错误"; -- Not supposed to see this one
TRANSFER_ABORT_MAX_PLAYERS = "传输失败：副本已满"; -- Too many players in instance
TRANSFER_ABORT_NOT_FOUND = "传输失败：副本未找到"; -- Something's wrong with the server
TRANSFER_ABORT_TOO_MANY_INSTANCES = "你在短时间内进入副本的次数过多。";
TRANSFER_ABORT_ZONE_IN_COMBAT = "无法在战斗进行时切换所在地区。";
TRILINEAR_FILTERING = "三线过滤";
TRINKET0SLOT = "饰品"; -- Trinket slot
TRINKET1SLOT = "饰品"; -- Trinket slot
TRIPLE_BUFFER = "三倍缓冲";
TURN_IN_ITEMS = "需要物品："; -- Lists the items needed to turn in to complete a quest
TUTORIAL26 = "你现在处于精力充沛状态，将在杀死怪物后获得一定的额外经验值奖励。";
TUTORIAL31 = "你已经拥有了一块炉石，它可以将你从现在的位置传送回你最后登记的那个旅店里。你每小时只能使用一次炉石。";
TUTORIAL45 = "如果你和另外一个玩家相处得很愉快，就把这个玩家加入好友名单吧！点击社交按钮然后将其加入你的好友名单。";
TUTORIAL27 = "如果你游到远离陆地的深水区，就会看到一个疲倦槽。当你的疲倦槽完全耗尽时，你就会溺水死亡。";
TUTORIAL28 = "游泳与走路基本相同，但是你可以按住鼠标右键并移动鼠标，以此来改变游泳的方向。";
TUTORIAL16 = "你可以打开角色状态中的声望界面来查看你与世界上的各个组织的关系。";
TUTORIAL37 = "你可以通过吃食物来加快生命值的回复速度，点击屏幕底部动作条中的食物图标就可以吃食物。但是在战斗中是不能吃食物的。";
TUTORIAL47 = "你正在通过集合石等待加入一支小队。集合石指示器图标就在微缩地图的旁边。你可以点击该指示器将自己从集合石队列中移除。";
TUTORIAL41 = "你可以按Enter键然后输入一条信息。你旁边的玩家都能够收到这条信息。";
TUTORIAL40 = "你可以在人物出现地附近找到训练师来学习新的技能，当然他们不会全都站在你面前，所以你得要四处走动一下才能找到他们。";
TUTORIAL19 = "由其他玩家控制的人物头上的名字会以蓝色来表示。";
TUTORIAL38 = "你的人物可以学习最多两种专业，使你能够采集或制造珍贵的物品。你可以向各大主城中的守卫询问各种专业的训练师所在的位置。";
TUTORIAL11 = "你可以通过吃食物来加快生命值的回复速度，点击屏幕底部动作条中的食物图标就可以吃食物。但是在战斗中是不能吃食物的。";
TUTORIAL43 = "如果你和另外一个玩家相处得很愉快，就把这个玩家加入好友名单吧！点击社交按钮然后将其加入你的好友名单。";
TUTORIAL48 = "你正在等待进入战场的队列中。你可以将鼠标移至微缩地图旁的战场图标上以查看你的状态。";
TUTORIAL24 = "要想给你的角色穿戴各种装备的话，你可以打开角色面板，然后把想要装备的物品从背包中拖到面板中。";
TUTORIAL12 = "你可以通过喝饮料来加快法力值的回复速度。点击屏幕底部的动作条中的饮料图标来喝饮料。如果你在喝饮料的过程中做出其它动作或者进行战斗的话，法力值加速回复的效果就会消失。";
TUTORIAL9 = "右键点击物品就可以使用它。如果你想要不打开背包就使用物品的话，可以事先把要使用的物品拖入动作条中。";
TUTORIAL1 = "那些能交给你任务的人物头顶上会出现一个感叹号，你可以右键点击这个人物与其交谈。";
TUTORIAL6 = "你可以点击屏幕底部左边一系列动作条中的快捷键来使用法术和特殊技能。";
TUTORIAL2 = "你可以使用ASDW键或者方向键来控制角色的移动，同时按下鼠标左右键也可以控制移动。";
TUTORIAL34 = "你现在处于鬼魂状态，可以通过回去找到自己的尸体或者和旁边的灵魂医者对话来复活。你的尸体的位置会在地图上标示出来，右上角的小地图上也会有箭头指向你的尸体所在的方向。";
TUTORIAL5 = "右键点击目标移动到作战范围进入战斗模式";
TUTORIAL10 = "你可以把背包放入屏幕右下角的空余的背包空格处，然后就可以点击打开这个背包了。";
TUTORIAL21 = "你可以点击屏幕中央下部的金色杯子图标来打开任务日志，然后查看自己的任务。";
TUTORIAL25 = "你现在处于鬼魂状态，可以通过回去找到自己的尸体或者和旁边的灵魂医者对话来复活。你的尸体的位置会在地图上标示出来，右上角的小地图上也会有箭头指向你的尸体所在的方向。";
TUTORIAL18 = "你可以右键点击对方的头像然后选择邀请来邀请另外一个玩家加入你的队伍。";
TUTORIAL35 = "你刚刚点击了一个飞行管理员，他/她是专门训练飞行兽并提供飞行运输服务的人。只需一小笔花费，你就可以飞往任何你曾经接触过的飞行管理员那里。当你发现一座新的城镇之后，最好先去找到该城镇的飞行管理员，以便日后能够快速抵达这里。";
TUTORIAL36 = "你可以通过吃食物来加快生命值的回复速度，点击屏幕底部动作条中的食物图标就可以吃食物。但是在战斗中是不能吃食物的。";
TUTORIAL33 = "你现在处于鬼魂状态，可以通过回去找到自己的尸体或者和旁边的灵魂医者对话来复活。你的尸体的位置会在地图上标示出来，右上角的小地图上也会有箭头指向你的尸体所在的方向。";
TUTORIAL44 = "如果你和另外一个玩家相处得很愉快，就把这个玩家加入好友名单吧！点击社交按钮然后将其加入你的好友名单。";
TUTORIAL22 = "如果你和另外一个玩家相处得很愉快，就把这个玩家加入好友名单吧！点击社交按钮然后将其加入你的好友名单。";
TUTORIAL13 = "你可以在天赋界面学习一个新的天赋。天赋界面可以通过点击动作界面中的人物角色界面下的天赋图标进入。";
TUTORIAL15 = "你可以点击技能图标打开技能页面，然后将法术或者技能的图标拖到动作条中。你也可以在技能页面中点击图标来使用相应的法术或者技能。";
TUTORIAL17 = "在收到其他玩家发来的悄悄话之后，你可以按R键来进行回复，或者直接输入/tell <对方的名字>然后输入你想要发出的信息。";
TUTORIAL50 = "钥匙链可以帮助你存放地下城的入口钥匙。钥匙链显示在你的动作条上的背包左边，它只能用来存放永久性的钥匙。";
TUTORIAL23 = "你可以按回车键然后输入一条信息，你旁边的玩家都可以收到这条信息。";
TUTORIAL30 = "你现在在旅店中，可以向旅店主人要一块炉石。你的人物在和怪物长期作战之后会逐渐感到疲倦，这时你就最好回到旅店中休息。如果你在旅店中登出，那么8小时之后你就可恢复到精力充沛状态。";
TUTORIAL14 = "你可以在角色出现地附近找到训练师来学习新的技能，当然他们不会全都站在你面前，所以你得要四处走动一下才能找到他们。";
TUTORIAL7 = "右键点击生物的尸体来对它进行搜索，然后再用右键点击物品将其放入背包中。";
TUTORIAL8 = "物品都会被放入你的背包。你可以点击屏幕右下角的背包图标打开背包。将鼠标移动到物品上可以查看它的属性。";
TUTORIAL20 = "右键点击商人处的物品便可购买此物品，前提是你有足够的金钱。在打开商人货物界面的情况下点击自己背包中的某个物品可以将其出售给商人。";
TUTORIAL39 = "你可以邀请其他玩家与你组队，共同冒险和作战。组队使你可以轻松完成许多困难的任务，而且组队时还可以获得一定的经验值奖励。";
TUTORIAL32 = "你进入了PvP模式，在这种状态下你的头像旁会显示你所属阵营的标志，敌对阵营的玩家可以自由攻击你。";
TUTORIAL29 = "当你的人物潜入水中时，你会看到一个呼吸槽。呼吸槽一旦用光，你的人物就会开始溺水。";
TUTORIAL3 = "你可以按住左键来调整视角，而按住右键可以让你的人物与视野保持同时旋转。";
TUTORIAL4 = "左键选择目标，右键进行相应的互动。";
TUTORIAL46 = "你可以在人物出现地附近找到训练师来学习新的技能，当然他们不会全都站在你面前，所以你得要四处走动一下才能找到他们。";
TUTORIAL42 = "如果你碰到什么新的东西，在你屏幕底部将会出现一个帮助按钮。点击这个按钮来获取如何和这个世界相关内容进行交互的简单介绍。这些小提示从最基础的内容开始，然后慢慢会出现一些进阶内容。\n\n感谢你参加游戏，祝你在你的冒险旅程中好运！";
TUTORIAL49 = "你现在可以加入战场了。点击对话框中的“加入战斗”按钮或右键点击微缩地图上的战场图标以加入战场。";
TUTORIAL_TITLE43 = "视角按键";
TUTORIAL_TITLE48 = "回复对话";
TUTORIAL_TITLE49 = "锁定目标";
TUTORIAL_TITLE3 = "视角按键";
TUTORIAL_TITLE2 = "移动";
TUTORIAL_TITLE6 = "法术和技能";
TUTORIAL_TITLE34 = "玩家对玩家战斗";
TUTORIAL_TITLE33 = "玩家对玩家战斗";
TUTORIAL_TITLE46 = "团队";
TUTORIAL_TITLE32 = "玩家对玩家战斗";
TUTORIAL_TITLE38 = "专业";
TUTORIAL_TITLE44 = "视角按键";
TUTORIAL_TITLE42 = "欢迎来到魔兽世界！";
TUTORIAL_TITLE40 = "组队";
TUTORIAL_TITLE39 = "组队";
TUTORIAL_TITLE20 = "正在购买物品";
TUTORIAL_TITLE47 = "集合石";
TUTORIAL_TITLE18 = "组队";
TUTORIAL_TITLE11 = "进食";
TUTORIAL_TITLE10 = "背包";
TUTORIAL_TITLE9 = "使用物品";
TUTORIAL_TITLE8 = "行囊";
TUTORIAL_TITLE7 = "正在拾取";
TUTORIAL_TITLE31 = "炉石";
TUTORIAL_TITLE50 = "钥匙链";
TUTORIAL_TITLE23 = "聊天";
TUTORIAL_TITLE17 = "回复对话";
TUTORIAL_TITLE16 = "声望";
TUTORIAL_TITLE15 = "法术和技能书";
TUTORIAL_TITLE13 = "学习天赋中";
TUTORIAL_TITLE12 = "喝水";
TUTORIAL_TITLE22 = "好友";
TUTORIAL_TITLE21 = "任务日志";
TUTORIAL_TITLE37 = "破损的物品";
TUTORIAL_TITLE36 = "玩家对玩家战斗";
TUTORIAL_TITLE35 = "旅行";
TUTORIAL_TITLE30 = "旅店";
TUTORIAL_TITLE29 = "呼吸";
TUTORIAL_TITLE28 = "游泳";
TUTORIAL_TITLE27 = "疲倦";
TUTORIAL_TITLE26 = "精力充沛";
TUTORIAL_TITLE25 = "死亡";
TUTORIAL_TITLE24 = "可装备的物品";
TUTORIAL_TITLE14 = "训练师";
TUTORIAL_TITLE5 = "战斗模式";
TUTORIAL_TITLE19 = "玩家";
TUTORIAL_TITLE41 = "精英任务";
TUTORIAL_TITLE4 = "锁定目标";
TUTORIAL_TITLE1 = "提供任务者";
TUTORIAL_TITLE45 = "视角按键";
TWOHANDEDWEAPONBEINGWIELDED = "不能装备在副手上，因为你现在持有的是双手武器！";
TWO_HANDED = "双手武器"; -- Weapons that require two hands
UIOPTIONS_MENU = "界面设置";
UI_SCALE = "UI缩放";
UKNOWNBEING = "未知生物";
UNAVAILABLE = "不可用"; -- Generic Unavailable tag
UNBIND = "解除键位";
UNITDESTROYEDOTHER = "%s被摧毁了。";
UNITDIESOTHER = "%s死亡了。";
UNITDIESSELF = "你死了。";
UNITNAME_TITLE = "%s"; -- %s is the title of the unit
UNITNAME_TITLE_CHARM = "%s的仆从"; -- %s is the name of the unit's charmer
UNITNAME_TITLE_CREATION = "%s的创造物";
UNITNAME_TITLE_GUARDIAN = "%s的守护者";
UNITNAME_TITLE_MINION = "%s的仆从";
UNITNAME_TITLE_PET = "%s的宠物"; -- %s is the name of the unit's summoner
UNIT_LETHAL_LEVEL_DEAD_TEMPLATE = "等级未知 尸体"; -- Level unknown for higher level units
UNIT_LETHAL_LEVEL_TEMPLATE = "等级未知"; -- Level unknown for higher level units
UNIT_LEVEL_DEAD_TEMPLATE = "等级%d 尸体";
UNIT_LEVEL_TEMPLATE = "等级 %d"; -- Level of a unit
UNIT_PLUS_LEVEL_TEMPLATE = "等级%d 精英"; -- Level of a plus-mob unit
UNIT_PVP_NAME = "%s %s"; -- The first %s is the rank, and the second %s is the name
UNIT_SKINNABLE = "可剥皮"; -- The unit is a dead creature which can be skinned for its pelt
UNIT_TYPE_LETHAL_LEVEL_TEMPLATE = "等级未知 %s"; -- As in Level ?? Beast
UNIT_TYPE_LEVEL_TEMPLATE = "等级%d %s"; -- As in Level 10 Beast
UNIT_TYPE_PLUS_LEVEL_TEMPLATE = "等级%d 精英%s"; -- As in Level 10+ Beast
UNKNOWN = "未知"; -- Generic "unknown"
UNKNOWNOBJECT = "未知目标";
UNLEARN = "忘却这个技能";
UNLEARN_SKILL = "你要忘却%s？";
UNLEARN_SKILL_TOOLTIP = "忘却这个技能";
UNLOCK_WINDOW = "解锁窗口";
USABLE_ITEMS = "可用物品";
USE = "使用"; -- Generic "Use" tag
USED = "已经学会"; -- Spell already learned text
USE_ITEM = "使用物品"; -- Use an item
USE_NO_DROP = "使用该物品后会将它和你绑定"; -- Tells player he is about to use a no drop item
USE_SOULSTONE = "复活"; -- Use your soulstone if you have one
USE_UBERTOOLTIPS = "详细提示信息";
USE_UISCALE = "使用UI缩放";
USE_WEATHER_SHADER = "天气遮罩";
VERBAL_HARASSMENT = "语言骚扰";
VERBAL_HARASSMENT_DESCRIPTION = "一个玩家对于另外一个玩家的语言攻击。";
VERBAL_HARASSMENT_TEXT2 = "要求他/她停止骚扰，并将这个玩家加入你的屏蔽名单中。";
VERBAL_HARASSMENT_TEXT3 = "保证你不再遭受更多的骚扰。";
VERBAL_HARASSMENT_TEXT4 = "报告言论中包含种族歧视、性和/或暴力内容的行为。此类行为均被视为骚扰。";
VERBAL_HARASSMENT_TEXT1 = "以下是一些对付语言骚扰的方法：";
VERTEX_ANIMATION_SHADERS = "顶点动态遮罩";
VERTICAL_SYNC = "垂直同步";
VICTORY_TEXT0 = "部落获胜";
VICTORY_TEXT1 = "联盟获胜";
VIDEOOPTIONS_MENU = "视频设置";
VOICEMACRO_0_Dw_0 = "帮助我！";
VOICEMACRO_0_Dw_0_FEMALE = "帮助我！";
VOICEMACRO_0_Dw_1 = "我需要帮助！";
VOICEMACRO_0_Dw_1_FEMALE = "我需要帮助！";
VOICEMACRO_0_Gn_0 = "你能帮助我吗？";
VOICEMACRO_0_Gn_0_FEMALE = "你能帮助我吗？";
VOICEMACRO_0_Gn_1 = "请帮助我！";
VOICEMACRO_0_Gn_1_FEMALE = "请帮助我！";
VOICEMACRO_0_Gn_2_FEMALE = "我需要帮助！";
VOICEMACRO_0_Hu_0 = "我需要协助！";
VOICEMACRO_0_Hu_0_FEMALE = "我需要帮助！";
VOICEMACRO_0_Hu_1 = "帮帮我！";
VOICEMACRO_0_Hu_1_FEMALE = "帮助我！";
VOICEMACRO_0_Ni_0 = "帮帮我！";
VOICEMACRO_0_Ni_0_FEMALE = "帮帮我！";
VOICEMACRO_0_Ni_1 = "来我这边！";
VOICEMACRO_0_Ni_1_FEMALE = "协助我！";
VOICEMACRO_0_Ni_2 = "协助我！";
VOICEMACRO_0_Or_0 = "我需要帮助！";
VOICEMACRO_0_Or_0_FEMALE = "我需要帮助！";
VOICEMACRO_0_Or_1 = "帮帮我！";
VOICEMACRO_0_Or_1_FEMALE = "帮帮我！";
VOICEMACRO_0_Sc_0 = "我需要帮助！";
VOICEMACRO_0_Sc_0_FEMALE = "需要你的帮助！";
VOICEMACRO_0_Sc_1 = "帮助我！";
VOICEMACRO_0_Sc_1_FEMALE = "帮助我！";
VOICEMACRO_0_Ta_0 = "支援我！";
VOICEMACRO_0_Ta_0_FEMALE = "帮帮我！";
VOICEMACRO_0_Ta_1 = "帮助我！";
VOICEMACRO_0_Ta_1_FEMALE = "帮助我！";
VOICEMACRO_0_Ta_2 = "帮帮我！";
VOICEMACRO_0_Tr_0 = "帮助我！";
VOICEMACRO_0_Tr_0_FEMALE = "帮助我！";
VOICEMACRO_0_Tr_1 = "协助我！";
VOICEMACRO_0_Tr_1_FEMALE = "协助我！";
VOICEMACRO_10_Dw_0 = "开火！";
VOICEMACRO_10_Dw_0_FEMALE = "开枪！";
VOICEMACRO_10_Dw_1 = "开火！";
VOICEMACRO_10_Dw_1_FEMALE = "开火！";
VOICEMACRO_10_Gn_0 = "快，开火！";
VOICEMACRO_10_Gn_0_FEMALE = "射击！";
VOICEMACRO_10_Gn_1 = "射击！";
VOICEMACRO_10_Gn_1_FEMALE = "开火！";
VOICEMACRO_10_Hu_0 = "自由射击！";
VOICEMACRO_10_Hu_0_FEMALE = "攻击！";
VOICEMACRO_10_Hu_1 = "开火！";
VOICEMACRO_10_Hu_1_FEMALE = "开火！";
VOICEMACRO_10_Ni_0 = "自由射击！";
VOICEMACRO_10_Ni_0_FEMALE = "快点攻击！";
VOICEMACRO_10_Ni_1 = "开火！";
VOICEMACRO_10_Ni_1_FEMALE = "开火！";
VOICEMACRO_10_Or_0 = "射击！";
VOICEMACRO_10_Or_0_FEMALE = "开火！";
VOICEMACRO_10_Or_1 = "让我们解决它！";
VOICEMACRO_10_Or_1_FEMALE = "开火！";
VOICEMACRO_10_Or_2_FEMALE = "射击！";
VOICEMACRO_10_Sc_0 = "开枪！";
VOICEMACRO_10_Sc_0_FEMALE = "开火！";
VOICEMACRO_10_Sc_1 = "开火！";
VOICEMACRO_10_Sc_1_FEMALE = "开火！";
VOICEMACRO_10_Ta_0 = "释放你的怒气吧！";
VOICEMACRO_10_Ta_0_FEMALE = "射击！";
VOICEMACRO_10_Ta_1 = "现在攻击！";
VOICEMACRO_10_Ta_1_FEMALE = "现在攻击！";
VOICEMACRO_10_Tr_0 = "射击！";
VOICEMACRO_10_Tr_0_FEMALE = "向他们射击！";
VOICEMACRO_10_Tr_1 = "干掉他们!";
VOICEMACRO_10_Tr_1_FEMALE = "干掉他们!";
VOICEMACRO_12_Dw_0 = "发生了什么？";
VOICEMACRO_12_Dw_0_FEMALE = "你好。";
VOICEMACRO_12_Dw_1 = "你好。";
VOICEMACRO_12_Dw_1_FEMALE = "你好啊。";
VOICEMACRO_12_Dw_2 = "你好。";
VOICEMACRO_12_Dw_2_FEMALE = "你好啊。";
VOICEMACRO_12_Dw_3 = "你好啊。";
VOICEMACRO_12_Gn_0 = "你好。";
VOICEMACRO_12_Gn_0_FEMALE = "你好。";
VOICEMACRO_12_Gn_1 = "你好。";
VOICEMACRO_12_Gn_1_FEMALE = "你好。";
VOICEMACRO_12_Gn_2 = "打招呼.";
VOICEMACRO_12_Gn_2_FEMALE = "你好啊。";
VOICEMACRO_12_Gn_3 = "你好啊。";
VOICEMACRO_12_Hu_0 = "你好。";
VOICEMACRO_12_Hu_0_FEMALE = "你好。";
VOICEMACRO_12_Hu_1 = "好消息";
VOICEMACRO_12_Hu_1_FEMALE = "你好。";
VOICEMACRO_12_Hu_2 = "欢呼。";
VOICEMACRO_12_Hu_2_FEMALE = "你好。";
VOICEMACRO_12_Hu_3 = "幸会。";
VOICEMACRO_12_Ni_0 = "你好。";
VOICEMACRO_12_Ni_0_FEMALE = "你好。";
VOICEMACRO_12_Ni_1 = "幸会。";
VOICEMACRO_12_Ni_1_FEMALE = "你好。";
VOICEMACRO_12_Ni_2 = "你好。";
VOICEMACRO_12_Ni_2_FEMALE = "幸会。";
VOICEMACRO_12_Ni_3_FEMALE = "你好。";
VOICEMACRO_12_Or_0 = "你好。";
VOICEMACRO_12_Or_0_FEMALE = "你好。";
VOICEMACRO_12_Or_1 = "嘿。";
VOICEMACRO_12_Or_1_FEMALE = "嘿。";
VOICEMACRO_12_Or_2 = "你好。";
VOICEMACRO_12_Or_2_FEMALE = "你好。";
VOICEMACRO_12_Sc_0 = "你好。";
VOICEMACRO_12_Sc_0_FEMALE = "你好。";
VOICEMACRO_12_Sc_1 = "你好啊。";
VOICEMACRO_12_Sc_1_FEMALE = "你好啊。";
VOICEMACRO_12_Sc_2 = "你好。";
VOICEMACRO_12_Sc_2_FEMALE = "嘿。";
VOICEMACRO_12_Ta_0 = "你好。";
VOICEMACRO_12_Ta_0_FEMALE = "你好。";
VOICEMACRO_12_Ta_1 = "你好。";
VOICEMACRO_12_Ta_1_FEMALE = "你好。";
VOICEMACRO_12_Ta_2 = "你好。";
VOICEMACRO_12_Ta_2_FEMALE = "你好啊。";
VOICEMACRO_12_Tr_0 = "嘿，叫你呢。";
VOICEMACRO_12_Tr_0_FEMALE = "你好。";
VOICEMACRO_12_Tr_1 = "嘿，伙计。";
VOICEMACRO_12_Tr_1_FEMALE = "嘿，叫你呢。";
VOICEMACRO_12_Tr_2 = "发生了什么？";
VOICEMACRO_12_Tr_2_FEMALE = "你好。";
VOICEMACRO_13_Dw_0 = "保重。";
VOICEMACRO_13_Dw_0_FEMALE = "好长啊。";
VOICEMACRO_13_Dw_1 = "愿我们能再次相见。";
VOICEMACRO_13_Dw_1_FEMALE = "再见。";
VOICEMACRO_13_Dw_2 = "再见。";
VOICEMACRO_13_Dw_2_FEMALE = "再见。";
VOICEMACRO_13_Gn_0 = "保重。";
VOICEMACRO_13_Gn_0_FEMALE = "保重。";
VOICEMACRO_13_Gn_1 = "再见。";
VOICEMACRO_13_Gn_1_FEMALE = "再见。";
VOICEMACRO_13_Gn_2 = "那很有趣。";
VOICEMACRO_13_Gn_2_FEMALE = "这可是个教训。";
VOICEMACRO_13_Gn_3 = "我会想念你的。";
VOICEMACRO_13_Gn_3_FEMALE = "我会一直珍稀我们在一起的时光的。";
VOICEMACRO_13_Hu_0 = "保重。";
VOICEMACRO_13_Hu_0_FEMALE = "再见。";
VOICEMACRO_13_Hu_1 = "愿我们能再次相见。";
VOICEMACRO_13_Hu_1_FEMALE = "愿我们能再次相见。";
VOICEMACRO_13_Hu_2 = "那很有趣。";
VOICEMACRO_13_Hu_2_FEMALE = "那很有趣。";
VOICEMACRO_13_Ni_0 = "祝你一路顺风。";
VOICEMACRO_13_Ni_0_FEMALE = "再见。";
VOICEMACRO_13_Ni_1 = "再见。";
VOICEMACRO_13_Ni_1_FEMALE = "我祝你一切安好。";
VOICEMACRO_13_Ni_2 = "我祝你一切安好。";
VOICEMACRO_13_Ni_2_FEMALE = "愿我们能再次相见。";
VOICEMACRO_13_Or_0 = "愿我们能够再次见面。";
VOICEMACRO_13_Or_0_FEMALE = "保持强大。";
VOICEMACRO_13_Or_1 = "保持强大。";
VOICEMACRO_13_Or_1_FEMALE = "愿我们百战常胜。";
VOICEMACRO_13_Or_2 = "愿你的剑真的如此锋利。";
VOICEMACRO_13_Or_2_FEMALE = "NPC商人没有物品出售或者没有反映";
VOICEMACRO_13_Sc_0 = "再见。";
VOICEMACRO_13_Sc_0_FEMALE = "再见。";
VOICEMACRO_13_Sc_1 = "祝你一天过得不愉快。";
VOICEMACRO_13_Sc_1_FEMALE = "这是真的。";
VOICEMACRO_13_Sc_2 = "保持乐观。";
VOICEMACRO_13_Sc_2_FEMALE = "祝你好运。";
VOICEMACRO_13_Ta_0 = "祝你一路顺风。";
VOICEMACRO_13_Ta_0_FEMALE = "保重。";
VOICEMACRO_13_Ta_1 = "愿我们能再次相见。";
VOICEMACRO_13_Ta_1_FEMALE = "再见。";
VOICEMACRO_13_Ta_2 = "愿你一路顺风。";
VOICEMACRO_13_Ta_2_FEMALE = "愿我们能再次相见。";
VOICEMACRO_13_Tr_0 = "再见。";
VOICEMACRO_13_Tr_0_FEMALE = "再见。";
VOICEMACRO_13_Tr_1 = "这是真的。";
VOICEMACRO_13_Tr_1_FEMALE = "好长啊。";
VOICEMACRO_13_Tr_2 = "回头见。";
VOICEMACRO_13_Tr_2_FEMALE = "再见。";
VOICEMACRO_14_Dw_0 = "是的。";
VOICEMACRO_14_Dw_0_FEMALE = "当然。";
VOICEMACRO_14_Dw_1 = "当然。";
VOICEMACRO_14_Dw_1_FEMALE = "真直接。";
VOICEMACRO_14_Dw_2 = "是的。";
VOICEMACRO_14_Dw_2_FEMALE = "当然。";
VOICEMACRO_14_Dw_3 = "真直接。";
VOICEMACRO_14_Gn_0 = "当然。";
VOICEMACRO_14_Gn_0_FEMALE = "当然。";
VOICEMACRO_14_Gn_1 = "是的。";
VOICEMACRO_14_Gn_1_FEMALE = "当然。";
VOICEMACRO_14_Gn_2 = "确实如此。";
VOICEMACRO_14_Gn_2_FEMALE = "确实如此。";
VOICEMACRO_14_Hu_0 = "当然。";
VOICEMACRO_14_Hu_0_FEMALE = "是的。";
VOICEMACRO_14_Hu_1 = "听上去不错。";
VOICEMACRO_14_Hu_1_FEMALE = "当然。";
VOICEMACRO_14_Hu_2 = "噢噢。";
VOICEMACRO_14_Hu_2_FEMALE = "为什么不。";
VOICEMACRO_14_Ni_0 = "是的。";
VOICEMACRO_14_Ni_0_FEMALE = "是的。";
VOICEMACRO_14_Ni_1 = "当然。";
VOICEMACRO_14_Ni_1_FEMALE = "当然。";
VOICEMACRO_14_Ni_2 = "自然。";
VOICEMACRO_14_Ni_2_FEMALE = "自然。";
VOICEMACRO_14_Or_0 = "你好。";
VOICEMACRO_14_Or_0_FEMALE = "你好。";
VOICEMACRO_14_Or_1 = "Zug zug.";
VOICEMACRO_14_Or_1_FEMALE = "Zug zug.";
VOICEMACRO_14_Or_2 = "（兽人步兵）";
VOICEMACRO_14_Or_2_FEMALE = "（兽人步兵）";
VOICEMACRO_14_Or_3 = "是的。";
VOICEMACRO_14_Or_3_FEMALE = "噢噢。";
VOICEMACRO_14_Sc_0 = "是的。";
VOICEMACRO_14_Sc_0_FEMALE = "是的。";
VOICEMACRO_14_Sc_1 = "当然。";
VOICEMACRO_14_Sc_1_FEMALE = "为什么不。";
VOICEMACRO_14_Sc_2 = "我认为是这样的。";
VOICEMACRO_14_Sc_2_FEMALE = "我认为是这样的。";
VOICEMACRO_14_Ta_0 = "是的。";
VOICEMACRO_14_Ta_0_FEMALE = "是的。";
VOICEMACRO_14_Ta_1 = "我是那么认为的。";
VOICEMACRO_14_Ta_1_FEMALE = "自然。";
VOICEMACRO_14_Ta_2 = "事情应该就是这样。";
VOICEMACRO_14_Ta_2_FEMALE = "当然。";
VOICEMACRO_14_Tr_0 = "当然可以，伙计。";
VOICEMACRO_14_Tr_0_FEMALE = "当然。";
VOICEMACRO_14_Tr_1 = "当然。";
VOICEMACRO_14_Tr_1_FEMALE = "挖它。";
VOICEMACRO_14_Tr_2 = "我一直感觉得到你。";
VOICEMACRO_14_Tr_2_FEMALE = "是的，伙计。";
VOICEMACRO_14_Tr_3 = "搞笑语音";
VOICEMACRO_15_Dw_0 = "不。";
VOICEMACRO_15_Dw_0_FEMALE = "不。";
VOICEMACRO_15_Dw_1 = "噢噢。";
VOICEMACRO_15_Dw_1_FEMALE = "随你喜欢去拿吧。";
VOICEMACRO_15_Dw_2 = "不可能。";
VOICEMACRO_15_Dw_2_FEMALE = "门都没有";
VOICEMACRO_15_Dw_3 = "决不。";
VOICEMACRO_15_Gn_0 = "不。";
VOICEMACRO_15_Gn_0_FEMALE = "不可能。";
VOICEMACRO_15_Gn_1 = "不可能。";
VOICEMACRO_15_Gn_1_FEMALE = "决不。";
VOICEMACRO_15_Gn_2 = "我不这么认为。";
VOICEMACRO_15_Gn_2_FEMALE = "我不这么认为。";
VOICEMACRO_15_Hu_0 = "不。";
VOICEMACRO_15_Hu_0_FEMALE = "不。";
VOICEMACRO_15_Hu_1 = "不。";
VOICEMACRO_15_Hu_1_FEMALE = "不可能。";
VOICEMACRO_15_Hu_2 = "我不这么认为。";
VOICEMACRO_15_Hu_2_FEMALE = "我不这么认为。";
VOICEMACRO_15_Hu_3 = "不。";
VOICEMACRO_15_Ni_0 = "不。";
VOICEMACRO_15_Ni_0_FEMALE = "不。";
VOICEMACRO_15_Ni_1 = "我不这么认为。";
VOICEMACRO_15_Ni_1_FEMALE = "我不这么认为。";
VOICEMACRO_15_Ni_2 = "完全不是。";
VOICEMACRO_15_Ni_2_FEMALE = "完全不是。";
VOICEMACRO_15_Or_0 = "不。";
VOICEMACRO_15_Or_0_FEMALE = "不。";
VOICEMACRO_15_Or_1 = "你一定是在开玩笑。";
VOICEMACRO_15_Or_1_FEMALE = "决不。";
VOICEMACRO_15_Or_2 = "（兽人步兵）";
VOICEMACRO_15_Or_2_FEMALE = "（兽人步兵）";
VOICEMACRO_15_Sc_0 = "不。";
VOICEMACRO_15_Sc_0_FEMALE = "不。";
VOICEMACRO_15_Sc_1 = "决不。";
VOICEMACRO_15_Sc_1_FEMALE = "决不。";
VOICEMACRO_15_Sc_2 = "我不这么认为。";
VOICEMACRO_15_Sc_2_FEMALE = "我不这么认为。";
VOICEMACRO_15_Ta_0 = "不。";
VOICEMACRO_15_Ta_0_FEMALE = "不。";
VOICEMACRO_15_Ta_1 = "我不这么认为。";
VOICEMACRO_15_Ta_1_FEMALE = "我不这么认为。";
VOICEMACRO_15_Ta_2 = "决不。";
VOICEMACRO_15_Ta_2_FEMALE = "事情不应该是这样的。";
VOICEMACRO_15_Tr_0 = "不。";
VOICEMACRO_15_Tr_0_FEMALE = "不可能。";
VOICEMACRO_15_Tr_1 = "不可能。";
VOICEMACRO_15_Tr_1_FEMALE = "不可能。";
VOICEMACRO_15_Tr_2 = "决不。";
VOICEMACRO_15_Tr_2_FEMALE = "决不。";
VOICEMACRO_15_Tr_3 = "噢噢。";
VOICEMACRO_16_Dw_0 = "谢谢你。";
VOICEMACRO_16_Dw_0_FEMALE = "谢谢你。";
VOICEMACRO_16_Dw_1 = "非常感谢。";
VOICEMACRO_16_Dw_1_FEMALE = "啊，你太好了。";
VOICEMACRO_16_Dw_2 = "啊，你太好了。";
VOICEMACRO_16_Dw_2_FEMALE = "非常感谢。";
VOICEMACRO_16_Dw_3 = "愿你再次表现的如此慷慨而且其程度可以是现在的一百倍。";
VOICEMACRO_16_Dw_3_FEMALE = "愿你再次表现的如此慷慨而且其程度可以是现在的一百倍。";
VOICEMACRO_16_Gn_0 = "谢谢你。";
VOICEMACRO_16_Gn_0_FEMALE = "你真慷慨。";
VOICEMACRO_16_Gn_1 = "我欠你的人情。";
VOICEMACRO_16_Gn_1_FEMALE = "你真好。";
VOICEMACRO_16_Gn_2 = "请允许我表示我深深的感谢。";
VOICEMACRO_16_Gn_2_FEMALE = "非常感谢。";
VOICEMACRO_16_Hu_0 = "谢谢你。";
VOICEMACRO_16_Hu_0_FEMALE = "谢谢你。";
VOICEMACRO_16_Hu_1 = "你真慷慨。";
VOICEMACRO_16_Hu_1_FEMALE = "你真慷慨。";
VOICEMACRO_16_Hu_2 = "你真好。";
VOICEMACRO_16_Hu_2_FEMALE = "非常感谢。";
VOICEMACRO_16_Ni_0 = "谢谢你。";
VOICEMACRO_16_Ni_0_FEMALE = "谢谢你。";
VOICEMACRO_16_Ni_1 = "太棒了。";
VOICEMACRO_16_Ni_1_FEMALE = "你真慷慨。";
VOICEMACRO_16_Ni_2 = "非常感谢。";
VOICEMACRO_16_Ni_2_FEMALE = "你太好了。";
VOICEMACRO_16_Or_0 = "我会还你这个人情的。";
VOICEMACRO_16_Or_0_FEMALE = "我不会忘记这个的。";
VOICEMACRO_16_Or_1 = "我不会忘记这个的。";
VOICEMACRO_16_Or_1_FEMALE = "谢谢你。";
VOICEMACRO_16_Or_2 = "谢谢你。";
VOICEMACRO_16_Or_2_FEMALE = "谢谢。";
VOICEMACRO_16_Sc_0 = "我欠你的人情。";
VOICEMACRO_16_Sc_0_FEMALE = "谢谢你。";
VOICEMACRO_16_Sc_1 = "啊，谢谢你。";
VOICEMACRO_16_Sc_1_FEMALE = "啊，谢谢你。";
VOICEMACRO_16_Sc_2 = "感谢你。";
VOICEMACRO_16_Sc_2_FEMALE = "你真慷慨。";
VOICEMACRO_16_Ta_0 = "你真慷慨。";
VOICEMACRO_16_Ta_0_FEMALE = "谢谢你。";
VOICEMACRO_16_Ta_1 = "你真好。";
VOICEMACRO_16_Ta_1_FEMALE = "你真好。";
VOICEMACRO_16_Ta_2 = "我感谢你。";
VOICEMACRO_16_Ta_2_FEMALE = "你太好了。";
VOICEMACRO_16_Ta_3 = "愿你的祖先会永远指引你前进。";
VOICEMACRO_16_Tr_0 = "谢谢。";
VOICEMACRO_16_Tr_0_FEMALE = "谢谢。";
VOICEMACRO_16_Tr_1 = "非常感谢。";
VOICEMACRO_16_Tr_1_FEMALE = "非常感谢。";
VOICEMACRO_16_Tr_2 = "我欠你一个人情。";
VOICEMACRO_16_Tr_2_FEMALE = "我欠你一个人情。";
VOICEMACRO_17_Dw_0 = "没问题。";
VOICEMACRO_17_Dw_0_FEMALE = "没问题。";
VOICEMACRO_17_Dw_1 = "愿意为您效劳。";
VOICEMACRO_17_Dw_1_FEMALE = "愿意为您效劳。";
VOICEMACRO_17_Dw_2 = "别提了。";
VOICEMACRO_17_Dw_2_FEMALE = "不客气。";
VOICEMACRO_17_Gn_0 = "没问题。";
VOICEMACRO_17_Gn_0_FEMALE = "随时效劳。";
VOICEMACRO_17_Gn_1 = "随时效劳。";
VOICEMACRO_17_Gn_1_FEMALE = "这是我为朋友该做的。";
VOICEMACRO_17_Gn_2 = "不客气。";
VOICEMACRO_17_Gn_2_FEMALE = "不客气。";
VOICEMACRO_17_Hu_0 = "我一直都会乐意帮助你的。";
VOICEMACRO_17_Hu_0_FEMALE = "没问题。";
VOICEMACRO_17_Hu_1 = "没有你办不成的事情。";
VOICEMACRO_17_Hu_1_FEMALE = "随时效劳。";
VOICEMACRO_17_Hu_2 = "随时愿意为你服务。";
VOICEMACRO_17_Hu_2_FEMALE = "不客气。";
VOICEMACRO_17_Ni_0 = "不客气。";
VOICEMACRO_17_Ni_0_FEMALE = "愿意为您效劳。";
VOICEMACRO_17_Ni_1 = "愿意为您效劳。";
VOICEMACRO_17_Ni_1_FEMALE = "没什么。";
VOICEMACRO_17_Ni_2 = "我很荣幸。";
VOICEMACRO_17_Ni_2_FEMALE = "我很荣幸。";
VOICEMACRO_17_Or_0 = "我预期的一样。";
VOICEMACRO_17_Or_0_FEMALE = "随时效劳。";
VOICEMACRO_17_Or_1 = "为了荣耀。";
VOICEMACRO_17_Or_1_FEMALE = "这是我为朋友该做的。";
VOICEMACRO_17_Or_2 = "没什么。";
VOICEMACRO_17_Or_2_FEMALE = "不客气。";
VOICEMACRO_17_Sc_0 = "随时效劳。";
VOICEMACRO_17_Sc_0_FEMALE = "随时效劳。";
VOICEMACRO_17_Sc_1 = "呃。";
VOICEMACRO_17_Sc_1_FEMALE = "呃。";
VOICEMACRO_17_Sc_2_FEMALE = "这不意味着我不喜欢你。";
VOICEMACRO_17_Ta_0 = "没什么。";
VOICEMACRO_17_Ta_0_FEMALE = "愿意为您效劳。";
VOICEMACRO_17_Ta_1 = "我一直都会乐意帮助你的。";
VOICEMACRO_17_Ta_1_FEMALE = "我一直都会乐意帮助你的。";
VOICEMACRO_17_Ta_2 = "这是我为联盟该做的。";
VOICEMACRO_17_Ta_2_FEMALE = "不用谢。";
VOICEMACRO_17_Tr_0 = "没问题。";
VOICEMACRO_17_Tr_0_FEMALE = "没问题。";
VOICEMACRO_17_Tr_1 = "随时效劳，伙计。";
VOICEMACRO_17_Tr_1_FEMALE = "愿意为您效劳。";
VOICEMACRO_17_Tr_2 = "你帮我抓背，我帮你抓背。";
VOICEMACRO_17_Tr_2_FEMALE = "随时效劳，伙计。";
VOICEMACRO_18_Dw_0 = "祝贺你。";
VOICEMACRO_18_Dw_0_FEMALE = "祝贺你。";
VOICEMACRO_18_Dw_1 = "太好了。";
VOICEMACRO_18_Dw_1_FEMALE = "出发了。";
VOICEMACRO_18_Dw_2 = "太棒了。";
VOICEMACRO_18_Dw_2_FEMALE = "哦，那真不错。";
VOICEMACRO_18_Dw_3 = "干得好。";
VOICEMACRO_18_Dw_3_FEMALE = "太棒了。";
VOICEMACRO_18_Dw_4 = "出发了。";
VOICEMACRO_18_Gn_0 = "祝贺你。";
VOICEMACRO_18_Gn_0_FEMALE = "祝贺你。";
VOICEMACRO_18_Gn_1 = "干得好。";
VOICEMACRO_18_Gn_1_FEMALE = "出发了。";
VOICEMACRO_18_Gn_2 = "干得好。";
VOICEMACRO_18_Gn_2_FEMALE = "太棒了。";
VOICEMACRO_18_Gn_3_FEMALE = "太好了。";
VOICEMACRO_18_Hu_0 = "祝贺你。";
VOICEMACRO_18_Hu_0_FEMALE = "祝贺你。";
VOICEMACRO_18_Hu_1 = "太棒了。";
VOICEMACRO_18_Hu_1_FEMALE = "太棒了。";
VOICEMACRO_18_Hu_2 = "太好了。";
VOICEMACRO_18_Hu_2_FEMALE = "哈哈。";
VOICEMACRO_18_Ni_0 = "祝贺你。";
VOICEMACRO_18_Ni_0_FEMALE = "祝贺你。";
VOICEMACRO_18_Ni_1 = "你是有价值的人。";
VOICEMACRO_18_Ni_1_FEMALE = "干得好。";
VOICEMACRO_18_Ni_2 = "太棒了。";
VOICEMACRO_18_Ni_2_FEMALE = "太棒了。";
VOICEMACRO_18_Or_0 = "你应该感到光荣。";
VOICEMACRO_18_Or_0_FEMALE = "很有技巧性地完成了。";
VOICEMACRO_18_Or_1 = "干得好。";
VOICEMACRO_18_Or_1_FEMALE = "还不错。";
VOICEMACRO_18_Or_2 = "你是有价值的人。";
VOICEMACRO_18_Or_2_FEMALE = "祝贺你。";
VOICEMACRO_18_Sc_0 = "祝贺你。";
VOICEMACRO_18_Sc_0_FEMALE = "祝贺你。";
VOICEMACRO_18_Sc_1 = "干得好。";
VOICEMACRO_18_Sc_1_FEMALE = "总是别的什么人。";
VOICEMACRO_18_Sc_2 = "干得好。";
VOICEMACRO_18_Sc_2_FEMALE = "好吧，你难道不感到幸运吗。";
VOICEMACRO_18_Sc_3_FEMALE = "今天显然是你的日子。";
VOICEMACRO_18_Ta_0 = "干得好。";
VOICEMACRO_18_Ta_0_FEMALE = "干得好。";
VOICEMACRO_18_Ta_1 = "你是有价值的人。";
VOICEMACRO_18_Ta_1_FEMALE = "太棒了。";
VOICEMACRO_18_Ta_2 = "命运对着你微笑。";
VOICEMACRO_18_Ta_2_FEMALE = "命运对着你微笑。";
VOICEMACRO_18_Tr_0 = "干得好。";
VOICEMACRO_18_Tr_0_FEMALE = "干得好。";
VOICEMACRO_18_Tr_1 = "太好了。";
VOICEMACRO_18_Tr_1_FEMALE = "出发了。";
VOICEMACRO_18_Tr_2 = "他们会世代颂扬你的发现的。";
VOICEMACRO_18_Tr_2_FEMALE = "太好了。";
VOICEMACRO_19_Dw_0 = "废话够了。让我们开始吧。";
VOICEMACRO_19_Dw_0_FEMALE = "搞笑语音";
VOICEMACRO_19_Dw_1 = "你是不是想要摸摸我的胡子，是不是？";
VOICEMACRO_19_Dw_1_FEMALE = "我会让你知道我能用我的大腿打磨钢。";
VOICEMACRO_19_Dw_2 = "搞笑语音";
VOICEMACRO_19_Dw_2_FEMALE = "搞笑语音";
VOICEMACRO_19_Dw_3 = "你很漂亮。我喜欢你的头发。这是你的饮料。你现在准备好了没有？";
VOICEMACRO_19_Dw_3_FEMALE = "搞笑语音。";
VOICEMACRO_19_Dw_4 = "你从哪里来的？那并不重要。";
VOICEMACRO_19_Dw_4_FEMALE = "不要再罗嗦了，我知道你认为所有的矮人女性都一个样子。";
VOICEMACRO_19_Dw_5 = "搞笑语音";
VOICEMACRO_19_Gn_0 = "搞笑语音";
VOICEMACRO_19_Gn_0_FEMALE = "你很可爱。";
VOICEMACRO_19_Gn_1 = "搞笑语音";
VOICEMACRO_19_Gn_1_FEMALE = "搞笑语音";
VOICEMACRO_19_Gn_2 = "搞笑语音";
VOICEMACRO_19_Gn_2_FEMALE = "你能够完全组成一句句子是个附加功能。";
VOICEMACRO_19_Gn_3 = "嘿，样子不错。";
VOICEMACRO_19_Gn_3_FEMALE = "搞笑语音";
VOICEMACRO_19_Gn_4_FEMALE = "搞笑语音";
VOICEMACRO_19_Hu_0 = "How you doing?";
VOICEMACRO_19_Hu_0_FEMALE = "You've got me all a flutter.";
VOICEMACRO_19_Hu_1 = "If I said you had a good body, would you hold it against me?";
VOICEMACRO_19_Hu_1_FEMALE = "My turn offs are rude people, mean people, and people who aren't nice.";
VOICEMACRO_19_Hu_2 = "Hey, baby cakes.";
VOICEMACRO_19_Hu_2_FEMALE = "I need a hero.";
VOICEMACRO_19_Hu_3 = "What's your sign?";
VOICEMACRO_19_Hu_4 = "Are you tired, cause you've been running through my mind all day.";
VOICEMACRO_19_Hu_5 = "Your tag's showing.  It says, \"Made in heaven.\"";
VOICEMACRO_19_Ni_0 = "You're an Emerald Dream come true.";
VOICEMACRO_19_Ni_0_FEMALE = "If I wasn't purple, you'd see I was blushing.";
VOICEMACRO_19_Ni_1 = "I hope you're not afraid of snakes.";
VOICEMACRO_19_Ni_1_FEMALE = "Sure I've got exotic piercings.";
VOICEMACRO_19_Ni_2 = "Baby, I'm mortal now.  Time's a wasting.";
VOICEMACRO_19_Ni_2_FEMALE = "I'm the type of girl my mother warned me about.";
VOICEMACRO_19_Ni_3 = "I'm a force of nature.";
VOICEMACRO_19_Ni_3_FEMALE = "There's nothing like sleeping in the forest under the moonlight.";
VOICEMACRO_19_Ni_4 = "Wanna bring out the animal in me?";
VOICEMACRO_19_Or_0 = "That armor looks good on you.  It would also look good on my floor.";
VOICEMACRO_19_Or_0_FEMALE = "You had me at zug zug.";
VOICEMACRO_19_Or_1 = "This is true love.  Do you think this happens every day?";
VOICEMACRO_19_Or_1_FEMALE = "I'll give you crazy love.";
VOICEMACRO_19_Or_2 = "You have six different smiles; one for when you're angry, one for when you tear flesh, one for when you chew flesh, one for when you loot bodies, one for when you skin game, and one for when you want to kill something.";
VOICEMACRO_19_Or_2_FEMALE = "I like men who aren't afraid to cry; cry uncle.";
VOICEMACRO_19_Or_3 = "I love you like a fat kid loves cake.";
VOICEMACRO_19_Or_3_FEMALE = "You'll do.  Let's go.";
VOICEMACRO_19_Or_4 = "Lady, from the moment I see you I... I did not expect to get this far.";
VOICEMACRO_19_Or_4_FEMALE = "Don't talk, just follow me.";
VOICEMACRO_19_Or_5 = "Um, you look like a lady.";
VOICEMACRO_19_Or_5_FEMALE = "Let's not ruin this moment with chit chat.";
VOICEMACRO_19_Sc_0 = "I don't smell that bad for a dead dude, do I?";
VOICEMACRO_19_Sc_0_FEMALE = "Nice butt.";
VOICEMACRO_19_Sc_1 = "You have beautiful skin; no maggot holes at all.";
VOICEMACRO_19_Sc_1_FEMALE = "One good thing about being dead, biological clock seems to have stopped.";
VOICEMACRO_19_Sc_2 = "If rot was hot, I'd be a volcano.";
VOICEMACRO_19_Sc_2_FEMALE = "Us Undead girls really know how to have a good time because, after all, what's the worst thing that could happen?";
VOICEMACRO_19_Sc_3 = "Once you go dead, you never go back.";
VOICEMACRO_19_Sc_3_FEMALE = "I can't wait to suck the juice out of your eyeballs.";
VOICEMACRO_19_Sc_4 = "Check my breath.  Is it bad enough for you?";
VOICEMACRO_19_Sc_4_FEMALE = "I don't care that much about romance.  I fell in love before, and look what happened to me.";
VOICEMACRO_19_Sc_5 = "Don't mind the drool.  It's just embalming fluid.";
VOICEMACRO_19_Sc_5_FEMALE = "I don't need to get funky.  I'm already there.";
VOICEMACRO_19_Ta_0 = "You move me.";
VOICEMACRO_19_Ta_0_FEMALE = "I'm tired of the same old bull.";
VOICEMACRO_19_Ta_1 = "Hey, you into leather?";
VOICEMACRO_19_Ta_1_FEMALE = "I want a man with soft hands.  Preferably four of them.";
VOICEMACRO_19_Ta_2 = "Free rides for the ladies.";
VOICEMACRO_19_Ta_2_FEMALE = "I've got big soulful eyes, long eyelashes, and a wet tongue.  What more could a guy want?";
VOICEMACRO_19_Ta_3 = "Hey, you work out?";
VOICEMACRO_19_Ta_3_FEMALE = "Come over here, sailor.";
VOICEMACRO_19_Ta_4 = "You know, older bulls really only have one function.";
VOICEMACRO_19_Ta_4_FEMALE = "Wanna see some good clog dancing?";
VOICEMACRO_19_Ta_5 = "Are you comfortable with complicated machinery?";
VOICEMACRO_19_Tr_0 = "Want some of my jungle love?";
VOICEMACRO_19_Tr_0_FEMALE = "I know.  My natural beauty is intimidating.";
VOICEMACRO_19_Tr_1 = "We Trolls mate for life.  'Course, we believe in frequent reincarnation.";
VOICEMACRO_19_Tr_1_FEMALE = "When enraged, and in heat, a female Troll can mate over eighty times in one night.  Be you prepared?";
VOICEMACRO_19_Tr_2 = "You look pretty.  Pretty tasty.";
VOICEMACRO_19_Tr_2_FEMALE = "Aren't you going to axe me out?";
VOICEMACRO_19_Tr_3 = "I hope you're well rested.  You're going to need your strength.";
VOICEMACRO_19_Tr_3_FEMALE = "I won't bite you where it shows.";
VOICEMACRO_19_Tr_4_FEMALE = "You're the type I'd like to sink my teeth into.";
VOICEMACRO_1_Dw_0 = "Danger approaching!";
VOICEMACRO_1_Dw_0_FEMALE = "危险在靠近！";
VOICEMACRO_1_Dw_1 = "小心！";
VOICEMACRO_1_Dw_1_FEMALE = "小心！";
VOICEMACRO_1_Gn_0 = "危险在靠近！";
VOICEMACRO_1_Gn_0_FEMALE = "危险在靠近！";
VOICEMACRO_1_Hu_0 = "保护好你自己！";
VOICEMACRO_1_Hu_0_FEMALE = "保护好你自己！";
VOICEMACRO_1_Hu_1 = "保持警惕！";
VOICEMACRO_1_Hu_1_FEMALE = "保持警惕！";
VOICEMACRO_1_Ni_0 = "小心！";
VOICEMACRO_1_Ni_0_FEMALE = "危险！";
VOICEMACRO_1_Ni_1 = "保持警惕！";
VOICEMACRO_1_Ni_1_FEMALE = "保持警惕！";
VOICEMACRO_1_Or_0 = "看这个！";
VOICEMACRO_1_Or_0_FEMALE = "保持警惕！";
VOICEMACRO_1_Or_1 = "来了！";
VOICEMACRO_1_Or_1_FEMALE = "来了！";
VOICEMACRO_1_Or_2 = "保持警惕！";
VOICEMACRO_1_Sc_0 = "危险！";
VOICEMACRO_1_Sc_0_FEMALE = "危险！";
VOICEMACRO_1_Sc_1 = "来了！";
VOICEMACRO_1_Sc_1_FEMALE = "来了！";
VOICEMACRO_1_Ta_0 = "危险在靠近！";
VOICEMACRO_1_Ta_0_FEMALE = "危险在靠近！";
VOICEMACRO_1_Ta_1 = "保持警惕！";
VOICEMACRO_1_Ta_1_FEMALE = "保持警惕！";
VOICEMACRO_1_Ta_2_FEMALE = "小心！";
VOICEMACRO_1_Tr_0 = "危险！";
VOICEMACRO_1_Tr_0_FEMALE = "危险！";
VOICEMACRO_1_Tr_1 = "有危险的东西在靠近！";
VOICEMACRO_1_Tr_1_FEMALE = "有危险的东西在靠近！";
VOICEMACRO_20_Dw_0 = "Hi ho, hi ho... uh... second verse, same as the first.";
VOICEMACRO_20_Dw_0_FEMALE = "No, they're not real, but thanks for noticing.";
VOICEMACRO_20_Dw_1 = "Ah winter.  Yes, winter.";
VOICEMACRO_20_Dw_1_FEMALE = "I don't like to be underground.  It reminds me of death.";
VOICEMACRO_20_Dw_2 = "Oh, I'm having a wardrobe malfunction.  Oh, there's my hammer.";
VOICEMACRO_20_Dw_2_FEMALE = "I like my ale like I like my men; dark and rich.";
VOICEMACRO_20_Dw_3 = "I don't have a drinking problem.  I drink, I get drunk, I fall down, no problem.";
VOICEMACRO_20_Dw_3_FEMALE = "It's like my father always used to say, \"Shut up, and get out.\"";
VOICEMACRO_20_Dw_4 = "I don't drink anymore.  'Course, I don't drink any less, either.";
VOICEMACRO_20_Dw_4_FEMALE = "My uncle has brass balls.  No, really!";
VOICEMACRO_20_Dw_5 = "I like my beer like I like my women; stout and bitter.";
VOICEMACRO_20_Dw_5_FEMALE = "I give myself a Dutch oven pedicure every night.  I've got no foot fungus at all.  My toes are pristine.";
VOICEMACRO_20_Dw_6 = "Oh, I'm just a social drinker.  Every time someone says, \"I'll have a drink\", I say, \"So shall I.\"";
VOICEMACRO_20_Gn_0 = "You know, I really wish I had a garden where I could put a couple of human statues.";
VOICEMACRO_20_Gn_0_FEMALE = "I apologize profusely for any inconvenience my murderous rampage may have caused.";
VOICEMACRO_20_Gn_1 = "I think that last vendor short changed me.  Oh, that was a bad one.";
VOICEMACRO_20_Gn_1_FEMALE = "I've discovered that getting pummeled by a blunt weapon can be quite painful.";
VOICEMACRO_20_Gn_2 = "I do hope to find some interesting gadgets around here.  I do love tinkering with things.";
VOICEMACRO_20_Gn_2_FEMALE = "You know, squirrels can be deadly when cornered.";
VOICEMACRO_20_Gn_3 = "I had an idea for a device that you could put small pieces of bread in to cook, but in the end I really didn't think there would be much of a market for it.";
VOICEMACRO_20_Gn_3_FEMALE = "Some day, I hope to find the nuggets on a chicken.";
VOICEMACRO_20_Gn_4 = "I'd like to give a shout out to my boys in Gnomeregan.  Keeping it real Big-T, Snoop-Pup, and Little Dees.  Y'all are short, but you're real, baby.";
VOICEMACRO_20_Gn_5 = "I look bigger in those mirrors where things look bigger.";
VOICEMACRO_20_Hu_0 = "Cover for me.  I gotta whiz behind a tree.";
VOICEMACRO_20_Hu_0_FEMALE = "Why does everyone automatically assume I know tailoring and cooking?";
VOICEMACRO_20_Hu_1 = "So, an Orc walks into a bar with a parrot on his shoulder.  The bartender says, \"Hey, where'd you get that?\"  The parrot says, \"Durotar.  They got 'em all over the place.\"";
VOICEMACRO_20_Hu_1_FEMALE = "Do you ever feel like you're not in charge of your own destiny, like you're being controlled by an invisible hand?";
VOICEMACRO_20_Hu_2 = "A duck walked into an apothecary and he said, \"Give me some chapstick, and put it on my bill.\"";
VOICEMACRO_20_Hu_2_FEMALE = "Sometimes, I have trouble CONTROLLING THE VOLUME OF MY VOICE!";
VOICEMACRO_20_Hu_3 = "How does a Tauren hide in a cherry tree?  He paints his hooves red.";
VOICEMACRO_20_Hu_3_FEMALE = "I like to fart in the tub.";
VOICEMACRO_20_Hu_4 = "A guy walked up to me and said, \"I'm a teepee, I'm a wigwam, I'm a teepee, I'm a wigwam.\"  I said, \"Relax, man, you're too tense.\"";
VOICEMACRO_20_Hu_4_FEMALE = "Me and my girlfriends exchange clothes all the time.  We're all the same size.";
VOICEMACRO_20_Hu_5 = "So, I have this idea for a great movie.  It's about two Gnomes who find a bracelet of power, and they have to take it to the Burning Steppes and cast it into The Cauldron.  They form the Brotherhood of the Bracelet.  Along the way, they're trailed by a murloc named Gottom who's obsessed with the bracelet, and nine bracelet bogeymen.  It could be a three parter called \"Ruler of the Bracelet\".  The first part would be called \"The Brotherhood of the Bracelet\", followed by \"A Couple of Towers\", with the climactic ending called \"Hey, the King's Back\".";
VOICEMACRO_20_Hu_5_FEMALE = "I can't find anywhere to get my nails done.";
VOICEMACRO_20_Hu_6_FEMALE = "I can't wait till this quest is done, and I can look for another Garibaldi artifact.";
VOICEMACRO_20_Ni_0 = "Last night I went to an awesome stag party.";
VOICEMACRO_20_Ni_0_FEMALE = "You know I have to keep moving at night or I'll disappear.";
VOICEMACRO_20_Ni_1 = "You know those Ancient Protectors in Darnassas?  They're not that old.";
VOICEMACRO_20_Ni_1_FEMALE = "Actually, I'm more of a morning elf.";
VOICEMACRO_20_Ni_2 = "Man, I was halfway through the Emerald Dream when I had to pee.";
VOICEMACRO_20_Ni_2_FEMALE = "You know, Wisps are actually pretty useful for personal hygiene.";
VOICEMACRO_20_Ni_3 = "Is that thing sharp?  Could that thing cut me?  I'm not immortal now, you know.";
VOICEMACRO_20_Ni_3_FEMALE = "I think that guys just use the Emerald Dream as an excuse to avoid calling me back.";
VOICEMACRO_20_Ni_4 = "I don't know about you, but I can't understand a thing those Wisps say.  I usually just nod.";
VOICEMACRO_20_Ni_4_FEMALE = "Oh, I'm dancing again.  I hope all your friends are enjoying the show.";
VOICEMACRO_20_Ni_5 = "Who wants to live forever?";
VOICEMACRO_20_Ni_6 = "What?  I didn't hear that.";
VOICEMACRO_20_Ni_7 = "I don't mind the Gnomes, but I'm always worried about tripping over one.";
VOICEMACRO_20_Or_0 = "I will crush and destroy and... ooo... shiny...";
VOICEMACRO_20_Or_0_FEMALE = "Get between me and my food, and you'll lose a hand.";
VOICEMACRO_20_Or_1 = "It's not easy being green.";
VOICEMACRO_20_Or_1_FEMALE = "I have no respect for people with small piercings.  I say go full hog.  Put a spear through your head.";
VOICEMACRO_20_Or_2 = "Man, dawg, you know, it's like I'm feeling you, but I'm not feeling you, you know?";
VOICEMACRO_20_Or_2_FEMALE = "I feel very feminine, and I'll beat the crap out of anyone who disagrees.";
VOICEMACRO_20_Or_3 = "I come from the Orcs.  We eat with spoons and forks.  We love to eat our pork.";
VOICEMACRO_20_Or_3_FEMALE = "What's estrogen?  Can you eat it?";
VOICEMACRO_20_Or_4 = "Orc smash!";
VOICEMACRO_20_Or_4_FEMALE = "I need to get my chest waxed again.";
VOICEMACRO_20_Or_5 = "Stop poking me!  Well, that was okay.";
VOICEMACRO_20_Or_5_FEMALE = "Man, I think that boar meat's coming back on me.  I gotta hit the can.  Anyone have a hearthstone?";
VOICEMACRO_20_Sc_0 = "I can't stand the smell of Orcs.";
VOICEMACRO_20_Sc_0_FEMALE = "I'd paint my toenails, but I'm not sure where they fell off.";
VOICEMACRO_20_Sc_1 = "I'm dead, and I'm pissed.";
VOICEMACRO_20_Sc_1_FEMALE = "Yes, they're real.  They're not mine, but they're real.";
VOICEMACRO_20_Sc_2 = "Roses are gray, violets are gray.  I'm dead and color blind.";
VOICEMACRO_20_Sc_2_FEMALE = "I'm in a rotten mood.";
VOICEMACRO_20_Sc_3 = "Hey diddle diddle, the mucus and the spittle, the corpse sank in the lagoon.  The murloc said, \"Ahhh...\" to see such a sight, and the Dwarf spanked the baboon.";
VOICEMACRO_20_Sc_3_FEMALE = "This stinks.";
VOICEMACRO_20_Sc_4 = "Anyone have any odorant?  Either \"wet dog\", \"fresh garbage\", or \"low tide\" would do.";
VOICEMACRO_20_Sc_4_FEMALE = "You know, once you're dead, nothing smells bad anymore.  Rotten eggs?  No problem.  Dead fish?  Like a spring breeze.";
VOICEMACRO_20_Sc_5_FEMALE = "You don't need deodorant when you don't have any armpits.";
VOICEMACRO_20_Sc_6_FEMALE = "Ah, doornails.";
VOICEMACRO_20_Sc_7_FEMALE = "I heard a knee slapper once, and skipped my kneecap right across a lake.";
VOICEMACRO_20_Ta_0 = "Mess with the bull, you get the horns.";
VOICEMACRO_20_Ta_0_FEMALE = "In my native tongue, my name means Dances with Tassels.";
VOICEMACRO_20_Ta_1 = "Here's the beef.";
VOICEMACRO_20_Ta_1_FEMALE = "I once laughed so hard, I milked all over the floor.";
VOICEMACRO_20_Ta_2 = "Homogenized?  No way.  I like the ladies.";
VOICEMACRO_20_Ta_2_FEMALE = "Happy Taurens come from Mulgore.";
VOICEMACRO_20_Ta_3 = "You know, Taurens are born hunters.  You ever see a Tauren catch a salmon out of a stream?  It really is quite exciting.  And, have you ever seen a Tauren stalk a python?  'Course you haven't.  That's because Taurens are so adept at blending in with their surroundings.";
VOICEMACRO_20_Ta_3_FEMALE = "You know how hard it is to get your groove on with the spirit of your great grandmother looking over you?";
VOICEMACRO_20_Ta_4 = "Moo.  Are you happy now?";
VOICEMACRO_20_Tr_0 = "Cooking's done.  Stew here.";
VOICEMACRO_20_Tr_0_FEMALE = "Strong halitosis be but one of my feminine traits.";
VOICEMACRO_20_Tr_1 = "I like my women dumpy and droopy with halitosis.";
VOICEMACRO_20_Tr_1_FEMALE = "I feel pretty, oh so pretty (spit).";
VOICEMACRO_20_Tr_2 = "I got a shrunken head.  I just came out of the pool.";
VOICEMACRO_20_Tr_2_FEMALE = "I got all this and personality, too.";
VOICEMACRO_20_Tr_3 = "New Troll here.";
VOICEMACRO_20_Tr_3_FEMALE = "The way to a man's heart be through his stomach, but I go through the rib cage.";
VOICEMACRO_20_Tr_4 = "I kill two dwarves in the morning, I kill two dwarves at night.  I kill two dwarves in the afternoon, and then I feel alright. I kill two dwarves in time of peace and two in time of war. I kill two dwarves before I kill two dwarves, and then I kill two more.";
VOICEMACRO_20_Tr_4_FEMALE = "If cannibalism be wrong, I don't want to be right.";
VOICEMACRO_20_Tr_5 = "I heard if you cut off an extremity, it'll regenerate a little bigger.  Don't believe it.";
VOICEMACRO_2_Dw_0 = "Charge!";
VOICEMACRO_2_Dw_0_FEMALE = "Charge!";
VOICEMACRO_2_Dw_1 = "For Khaz Modan!";
VOICEMACRO_2_Dw_1_FEMALE = "By Muridan's beard!";
VOICEMACRO_2_Gn_0 = "Get 'em!";
VOICEMACRO_2_Gn_0_FEMALE = "For Gnomeregan!";
VOICEMACRO_2_Gn_1 = "Attack!";
VOICEMACRO_2_Gn_1_FEMALE = "Charge forth!";
VOICEMACRO_2_Gn_2 = "For Gnomeregan!";
VOICEMACRO_2_Hu_0 = "Charge!";
VOICEMACRO_2_Hu_0_FEMALE = "Attack!";
VOICEMACRO_2_Hu_1 = "To battle!";
VOICEMACRO_2_Hu_1_FEMALE = "To battle!";
VOICEMACRO_2_Ni_0 = "For Cenarius!";
VOICEMACRO_2_Ni_0_FEMALE = "By Elune!";
VOICEMACRO_2_Ni_1 = "Attack!";
VOICEMACRO_2_Ni_1_FEMALE = "Attack for the goddess!";
VOICEMACRO_2_Ni_2_FEMALE = "Charge forth!";
VOICEMACRO_2_Or_0 = "Destroy them!";
VOICEMACRO_2_Or_0_FEMALE = "Attack!";
VOICEMACRO_2_Or_1 = "Slay them all!";
VOICEMACRO_2_Or_1_FEMALE = "Slay them all!";
VOICEMACRO_2_Or_2 = "Break their bones!";
VOICEMACRO_2_Or_2_FEMALE = "Leave none alive!";
VOICEMACRO_2_Sc_0 = "For the forsaken!";
VOICEMACRO_2_Sc_0_FEMALE = "Rend flesh with me!";
VOICEMACRO_2_Sc_1 = "Rend flesh with me!";
VOICEMACRO_2_Sc_1_FEMALE = "For the forsaken!";
VOICEMACRO_2_Ta_0 = "For Kalimdor!";
VOICEMACRO_2_Ta_0_FEMALE = "For Kalimdor!";
VOICEMACRO_2_Ta_1 = "Unleash your fury!";
VOICEMACRO_2_Ta_1_FEMALE = "Attack!";
VOICEMACRO_2_Ta_2 = "Charge!";
VOICEMACRO_2_Tr_0 = "For Zul'jin!";
VOICEMACRO_2_Tr_0_FEMALE = "For Zul'jin!";
VOICEMACRO_2_Tr_1 = "Now, we kill!";
VOICEMACRO_2_Tr_1_FEMALE = "We bring the pain to them!";
VOICEMACRO_3_Dw_0 = "快跑！";
VOICEMACRO_3_Dw_0_FEMALE = "快跑！";
VOICEMACRO_3_Dw_1 = "快跑！";
VOICEMACRO_3_Dw_1_FEMALE = "快跑！";
VOICEMACRO_3_Dw_2 = "快跑！";
VOICEMACRO_3_Gn_0 = "快跑！";
VOICEMACRO_3_Gn_0_FEMALE = "快跑！";
VOICEMACRO_3_Gn_1 = "让我们离开这里！";
VOICEMACRO_3_Gn_1_FEMALE = "让我们离开这里！";
VOICEMACRO_3_Gn_2 = "撤退！";
VOICEMACRO_3_Gn_2_FEMALE = "撤退！";
VOICEMACRO_3_Hu_0 = "快跑！";
VOICEMACRO_3_Hu_0_FEMALE = "快跑！";
VOICEMACRO_3_Hu_1 = "撤退！";
VOICEMACRO_3_Hu_1_FEMALE = "撤退！";
VOICEMACRO_3_Ni_0 = "撤退！";
VOICEMACRO_3_Ni_0_FEMALE = "快跑！";
VOICEMACRO_3_Ni_1 = "散开！";
VOICEMACRO_3_Ni_1_FEMALE = "我们的敌人太强了！";
VOICEMACRO_3_Or_0 = "撤退！";
VOICEMACRO_3_Or_0_FEMALE = "撤退！";
VOICEMACRO_3_Or_1 = "快跑！";
VOICEMACRO_3_Or_1_FEMALE = "快跑！";
VOICEMACRO_3_Sc_0 = "靠边站！";
VOICEMACRO_3_Sc_0_FEMALE = "快跑！"; -- Undead Female voicemacro 'Run' var0
VOICEMACRO_3_Sc_1 = "赶快回去！";
VOICEMACRO_3_Sc_1_FEMALE = "靠边站！"; -- Undead Female voicemacro 'Run' var1
VOICEMACRO_3_Ta_0 = "快跑！";
VOICEMACRO_3_Ta_0_FEMALE = "撤退！";
VOICEMACRO_3_Ta_1 = "撤退！";
VOICEMACRO_3_Ta_1_FEMALE = "躲起来！";
VOICEMACRO_3_Tr_0 = "快跑！";
VOICEMACRO_3_Tr_0_FEMALE = "快跑啊！";
VOICEMACRO_3_Tr_1 = "离开这里！";
VOICEMACRO_3_Tr_1_FEMALE = "离开这里！";
VOICEMACRO_4_Dw_0 = "一起上！";
VOICEMACRO_4_Dw_0_FEMALE = "一起上！";
VOICEMACRO_4_Dw_1 = "攻击这个！";
VOICEMACRO_4_Dw_1_FEMALE = "攻击这个！";
VOICEMACRO_4_Dw_2 = "帮我干掉这个家伙！";
VOICEMACRO_4_Gn_0 = "请和我一起做战！";
VOICEMACRO_4_Gn_0_FEMALE = "你能帮助我吗？";
VOICEMACRO_4_Gn_1 = "请帮我在这里做战！";
VOICEMACRO_4_Gn_1_FEMALE = "嗨，帮帮我！";
VOICEMACRO_4_Hu_0 = "协助我的攻击！";
VOICEMACRO_4_Hu_0_FEMALE = "协助我的攻击！";
VOICEMACRO_4_Hu_1 = "从这里发起攻击！";
VOICEMACRO_4_Hu_1_FEMALE = "从这里发起攻击！";
VOICEMACRO_4_Ni_0 = "攻击我的敌人！";
VOICEMACRO_4_Ni_0_FEMALE = "攻击这个敌人！";
VOICEMACRO_4_Ni_1 = "干掉我的敌人！";
VOICEMACRO_4_Ni_1_FEMALE = "在这里！";
VOICEMACRO_4_Ni_2_FEMALE = "攻击我的敌人！";
VOICEMACRO_4_Or_0 = "从这里发起攻击！";
VOICEMACRO_4_Or_0_FEMALE = "从这里发起攻击！";
VOICEMACRO_4_Or_1 = "和我一起攻上去！";
VOICEMACRO_4_Or_1_FEMALE = "让我们一起奋战！";
VOICEMACRO_4_Or_2 = "让我们一起奋战！";
VOICEMACRO_4_Or_2_FEMALE = "一起上！";
VOICEMACRO_4_Sc_0 = "和我一起作战！";
VOICEMACRO_4_Sc_0_FEMALE = "屠杀吧!";
VOICEMACRO_4_Sc_1 = "帮助我一起战斗！";
VOICEMACRO_4_Sc_1_FEMALE = "帮助我一起战斗！";
VOICEMACRO_4_Ta_0 = "和我并肩作战！";
VOICEMACRO_4_Ta_0_FEMALE = "和我并肩作战！";
VOICEMACRO_4_Ta_1 = "和我一起作战！";
VOICEMACRO_4_Ta_1_FEMALE = "和我一起作战！";
VOICEMACRO_4_Tr_0 = "攻击这个敌人！";
VOICEMACRO_4_Tr_0_FEMALE = "攻击这个敌人！";
VOICEMACRO_4_Tr_1 = "帮助我！";
VOICEMACRO_4_Tr_1_FEMALE = "这是我们攻击的目标！";
VOICEMACRO_5_Dw_0 = "我没有法力值了！";
VOICEMACRO_5_Dw_0_FEMALE = "我没有法力值了！";
VOICEMACRO_5_Dw_1 = "我需要更多的法力值！";
VOICEMACRO_5_Dw_1_FEMALE = "我需要更多的法力值！";
VOICEMACRO_5_Gn_0 = "我需要法力值！";
VOICEMACRO_5_Gn_0_FEMALE = "我的法力值快用完了！";
VOICEMACRO_5_Gn_1 = "我的法力值不足！";
VOICEMACRO_5_Gn_1_FEMALE = "我的法力值不足！";
VOICEMACRO_5_Hu_0 = "我没有法力值了！";
VOICEMACRO_5_Hu_0_FEMALE = "我没有法力值了！";
VOICEMACRO_5_Hu_1 = "我的法力值很少！";
VOICEMACRO_5_Hu_1_FEMALE = "我需要更多的法力值！";
VOICEMACRO_5_Ni_0 = "我的法力值很少！";
VOICEMACRO_5_Ni_0_FEMALE = "我的法力值很少！";
VOICEMACRO_5_Ni_1 = "我的法力值要用光了！";
VOICEMACRO_5_Ni_1_FEMALE = "我的法力值要用光了！";
VOICEMACRO_5_Or_0 = "我需要法力值！";
VOICEMACRO_5_Or_0_FEMALE = "我需要法力值！";
VOICEMACRO_5_Or_1 = "我的法力值很少！";
VOICEMACRO_5_Or_1_FEMALE = "我的法力值很少！";
VOICEMACRO_5_Sc_0 = "我的法力值要用光了！";
VOICEMACRO_5_Sc_0_FEMALE = "我需要法力值！";
VOICEMACRO_5_Sc_1 = "我需要法力值！";
VOICEMACRO_5_Sc_1_FEMALE = "我的法力值用完了！";
VOICEMACRO_5_Ta_0 = "我需要更多的法力值！";
VOICEMACRO_5_Ta_0_FEMALE = "我必须补充我的法力值！";
VOICEMACRO_5_Ta_1 = "我的法力值用完了！";
VOICEMACRO_5_Ta_1_FEMALE = "我需要更多的法力值！";
VOICEMACRO_5_Tr_0 = "我的法力值很少！";
VOICEMACRO_5_Tr_0_FEMALE = "我的法力值不足！";
VOICEMACRO_5_Tr_1 = "我需要更多的法力值！";
VOICEMACRO_5_Tr_1_FEMALE = "我需要更多的法力值！";
VOICEMACRO_6_Dw_0 = "我来带路。";
VOICEMACRO_6_Dw_0_FEMALE = "我来带路。";
VOICEMACRO_6_Dw_1 = "跟着我。";
VOICEMACRO_6_Dw_1_FEMALE = "跟着我！";
VOICEMACRO_6_Dw_2 = "跟着我，快。";
VOICEMACRO_6_Gn_0 = "我来带路。";
VOICEMACRO_6_Gn_0_FEMALE = "跟着我。";
VOICEMACRO_6_Gn_1 = "跟着我。";
VOICEMACRO_6_Hu_0 = "跟着我。";
VOICEMACRO_6_Hu_0_FEMALE = "跟着我。";
VOICEMACRO_6_Hu_1 = "我来带路。";
VOICEMACRO_6_Hu_1_FEMALE = "我来带路。";
VOICEMACRO_6_Ni_0 = "我来带路。";
VOICEMACRO_6_Ni_0_FEMALE = "我来带路。";
VOICEMACRO_6_Ni_1 = "跟着我。";
VOICEMACRO_6_Ni_1_FEMALE = "跟着我。";
VOICEMACRO_6_Or_0 = "跟着我。";
VOICEMACRO_6_Or_0_FEMALE = "跟着我。";
VOICEMACRO_6_Or_1 = "来。";
VOICEMACRO_6_Or_1_FEMALE = "来。";
VOICEMACRO_6_Sc_0 = "这条路。";
VOICEMACRO_6_Sc_0_FEMALE = "走这边。";
VOICEMACRO_6_Sc_1 = "我来带路。";
VOICEMACRO_6_Sc_1_FEMALE = "跟着我。";
VOICEMACRO_6_Ta_0 = "跟我来。";
VOICEMACRO_6_Ta_0_FEMALE = "跟我来。";
VOICEMACRO_6_Ta_1 = "我来带路。";
VOICEMACRO_6_Ta_1_FEMALE = "跟着我。";
VOICEMACRO_6_Tr_0 = "跟着我。";
VOICEMACRO_6_Tr_0_FEMALE = "我来带路。";
VOICEMACRO_6_Tr_1 = "你跟着我。";
VOICEMACRO_6_Tr_1_FEMALE = "你跟着我。";
VOICEMACRO_7_Dw_0 = "在这里等着。";
VOICEMACRO_7_Dw_0_FEMALE = "待在这里。";
VOICEMACRO_7_Dw_1 = "待着不要动。";
VOICEMACRO_7_Dw_1_FEMALE = "在这里等着。";
VOICEMACRO_7_Dw_2 = "待在这里。";
VOICEMACRO_7_Gn_0 = "在这里等一会。";
VOICEMACRO_7_Gn_0_FEMALE = "在这里等一会。";
VOICEMACRO_7_Gn_1 = "请等在这里。";
VOICEMACRO_7_Gn_1_FEMALE = "请等在这里。";
VOICEMACRO_7_Hu_0 = "待着不要动。";
VOICEMACRO_7_Hu_0_FEMALE = "待着不要动。";
VOICEMACRO_7_Hu_1 = "待在这里。";
VOICEMACRO_7_Hu_1_FEMALE = "待在这里。";
VOICEMACRO_7_Hu_2 = "在这里等着。";
VOICEMACRO_7_Hu_2_FEMALE = "在这里等着。";
VOICEMACRO_7_Ni_0 = "待在这里。";
VOICEMACRO_7_Ni_0_FEMALE = "待在这里。";
VOICEMACRO_7_Ni_1 = "在这里等着。";
VOICEMACRO_7_Ni_1_FEMALE = "在这里等着。";
VOICEMACRO_7_Or_0 = "待在这里。";
VOICEMACRO_7_Or_0_FEMALE = "待着。";
VOICEMACRO_7_Or_1 = "待在这里。";
VOICEMACRO_7_Or_1_FEMALE = "在这里等着。";
VOICEMACRO_7_Sc_0 = "等等。";
VOICEMACRO_7_Sc_0_FEMALE = "待在这里。";
VOICEMACRO_7_Sc_1 = "待在这里。";
VOICEMACRO_7_Sc_1_FEMALE = "不要移动。";
VOICEMACRO_7_Ta_0 = "休息一会。";
VOICEMACRO_7_Ta_0_FEMALE = "休息一下。";
VOICEMACRO_7_Ta_1 = "待在这里。";
VOICEMACRO_7_Ta_1_FEMALE = "待在这里。";
VOICEMACRO_7_Ta_2 = "等在这里。";
VOICEMACRO_7_Tr_0 = "你待在这里。";
VOICEMACRO_7_Tr_0_FEMALE = "你待在这里。";
VOICEMACRO_7_Tr_1 = "待着不要动。";
VOICEMACRO_7_Tr_1_FEMALE = "待着不要动。";
VOICEMACRO_7_Tr_2 = "不要四处乱走。";
VOICEMACRO_7_Tr_2_FEMALE = "不要四处乱走。";
VOICEMACRO_8_Dw_0 = "我需要治疗！";
VOICEMACRO_8_Dw_0_FEMALE = "我需要治疗！";
VOICEMACRO_8_Dw_1 = "治疗我的伤口！";
VOICEMACRO_8_Dw_1_FEMALE = "治疗我的伤口！";
VOICEMACRO_8_Dw_2 = "治疗我！";
VOICEMACRO_8_Dw_2_FEMALE = "治疗我！";
VOICEMACRO_8_Gn_0 = "请治疗我！";
VOICEMACRO_8_Gn_0_FEMALE = "请治疗我！";
VOICEMACRO_8_Gn_1 = "你能帮忙治疗我一下吗？";
VOICEMACRO_8_Gn_1_FEMALE = "你能帮忙治疗我一下吗？";
VOICEMACRO_8_Hu_0 = "我需要治疗！";
VOICEMACRO_8_Hu_0_FEMALE = "我需要治疗。";
VOICEMACRO_8_Hu_1 = "治疗我！";
VOICEMACRO_8_Hu_1_FEMALE = "治疗我！";
VOICEMACRO_8_Ni_0 = "治疗我！";
VOICEMACRO_8_Ni_0_FEMALE = "治疗我。";
VOICEMACRO_8_Ni_1 = "我需要治疗！";
VOICEMACRO_8_Ni_1_FEMALE = "我需要治疗！";
VOICEMACRO_8_Or_0 = "治疗我！";
VOICEMACRO_8_Or_0_FEMALE = "治疗我！";
VOICEMACRO_8_Or_1 = "我需要治疗！";
VOICEMACRO_8_Or_1_FEMALE = "我需要治疗！";
VOICEMACRO_8_Sc_0 = "治疗我！";
VOICEMACRO_8_Sc_0_FEMALE = "治疗我！";
VOICEMACRO_8_Sc_1 = "治疗我！";
VOICEMACRO_8_Sc_1_FEMALE = "治疗我！";
VOICEMACRO_8_Ta_0 = "治疗我！";
VOICEMACRO_8_Ta_0_FEMALE = "治疗我！";
VOICEMACRO_8_Ta_1 = "我需要治疗！";
VOICEMACRO_8_Ta_1_FEMALE = "我需要治疗！";
VOICEMACRO_8_Tr_0 = "治疗我！";
VOICEMACRO_8_Tr_0_FEMALE = "我情况不妙!";
VOICEMACRO_8_Tr_1 = "治疗我！";
VOICEMACRO_8_Tr_1_FEMALE = "治疗我！";
VOICEMACRO_8_Tr_2 = "我情况不太妙啊!";
VOICEMACRO_8_Tr_2_FEMALE = "治疗我！";
VOICEMACRO_LABEL = "谈话";
VOICEMACRO_LABEL_AID1 = "协助";
VOICEMACRO_LABEL_ATTACKMYTARGET1 = "assist";
VOICEMACRO_LABEL_ATTACKMYTARGET2 = "as";
VOICEMACRO_LABEL_CHARGE1 = "次";
VOICEMACRO_LABEL_CHEER1 = "cheer";
VOICEMACRO_LABEL_CONGRATULATIONS1 = "grats";
VOICEMACRO_LABEL_CONGRATULATIONS3 = "congratulations";
VOICEMACRO_LABEL_CONGRATULATIONS2 = "congrats";
VOICEMACRO_LABEL_FLEE1 = "flee";
VOICEMACRO_LABEL_FLEE2 = "run";
VOICEMACRO_LABEL_FLIRT1 = "flirt";
VOICEMACRO_LABEL_FOLLOW1 = "followme";
VOICEMACRO_LABEL_FOLLOWME1 = "followme";
VOICEMACRO_LABEL_FOLLOWME3 = "跟着我";
VOICEMACRO_LABEL_FOLLOWME2 = "跟随";
VOICEMACRO_LABEL_GOODBYE1 = "goodbye";
VOICEMACRO_LABEL_GOODBYE2 = "bye";
VOICEMACRO_LABEL_HEALME1 = "heal";
VOICEMACRO_LABEL_HEALME2 = "治疗我";
VOICEMACRO_LABEL_HELLO1 = "hello";
VOICEMACRO_LABEL_HELP1 = "helpme";
VOICEMACRO_LABEL_HELPME2 = "帮我";
VOICEMACRO_LABEL_HELPME1 = "help";
VOICEMACRO_LABEL_INCOMING1 = "incoming";
VOICEMACRO_LABEL_INCOMING2 = "inc";
VOICEMACRO_LABEL_JOKE1 = "silly";
VOICEMACRO_LABEL_NO1 = "no";
VOICEMACRO_LABEL_OPENFIRE1 = "火焰";
VOICEMACRO_LABEL_OPENFIRE2 = "开火";
VOICEMACRO_LABEL_OUTOFMANA1 = "oom";
VOICEMACRO_LABEL_OUTOFMANA2 = "法力值不足";
VOICEMACRO_LABEL_RASPBERRY1 = "rasp";
VOICEMACRO_LABEL_RASPBERRY2 = "raspberry";
VOICEMACRO_LABEL_SILLY1 = "silly";
VOICEMACRO_LABEL_THANKYOU3 = "thank";
VOICEMACRO_LABEL_THANKYOU1 = "thankyou";
VOICEMACRO_LABEL_THANKYOU2 = "thanks";
VOICEMACRO_LABEL_TRAIN1 = "train";
VOICEMACRO_LABEL_WAITHERE1 = "等待";
VOICEMACRO_LABEL_WAITHERE2 = "在这里等待";
VOICEMACRO_LABEL_YES1 = "yes";
VOICEMACRO_LABEL_YOUREWELCOME1 = "welcome";
VOICEMACRO_LABEL_YOUREWELCOME2 = "welc";
VSABSORBOTHEROTHER = "%s发起攻击，%s吸收了所有伤害。";
VSABSORBOTHERSELF = "%s发起攻击，你吸收了所有伤害。";
VSABSORBSELFOTHER = "你发动攻击，%s吸收了所有伤害。";
VSBLOCKOTHEROTHER = "%s发起了攻击。%s格挡住了。";
VSBLOCKOTHERSELF = "%s发起了攻击。你格挡开了。";
VSBLOCKSELFOTHER = "你发起了攻击。%s格挡住了。";
VSDEFLECTOTHEROTHER = "%s发起了攻击。%s闪开了。";
VSDEFLECTOTHERSELF = "%s发起了攻击。你闪开了。";
VSDEFLECTSELFOTHER = "你发起了攻击。%s闪开了。";
VSDODGEOTHEROTHER = "%s发起了攻击。%s闪躲开了。";
VSDODGEOTHERSELF = "%s发起了攻击。你闪躲开了。";
VSDODGESELFOTHER = "你发起了攻击。%s闪开了。";
VSENVIRONMENTALDAMAGE_DROWNING_OTHER = "%s现在处于溺水状态并损失%d生命值。";
VSENVIRONMENTALDAMAGE_DROWNING_SELF = "你现在处于溺水状态并损失%d生命值。";
VSENVIRONMENTALDAMAGE_FALLING_OTHER = "%s从高处掉落损失%d生命值。";
VSENVIRONMENTALDAMAGE_FALLING_SELF = "你从高处掉落损失%d生命值。";
VSENVIRONMENTALDAMAGE_FATIGUE_OTHER = "%s精疲力竭，损失了%d点生命值。";
VSENVIRONMENTALDAMAGE_FATIGUE_SELF = "你精疲力竭，损失了%d点生命值。";
VSENVIRONMENTALDAMAGE_FIRE_OTHER = "%s受到%d点火焰伤害。";
VSENVIRONMENTALDAMAGE_FIRE_SELF = "你受到%d点火焰伤害。";
VSENVIRONMENTALDAMAGE_LAVA_OTHER = "在岩浆中，%s损失了%d生命值。";
VSENVIRONMENTALDAMAGE_LAVA_SELF = "你泡在岩浆中，损失了%d点生命值。";
VSENVIRONMENTALDAMAGE_SLIME_OTHER = "在泥浆中，%s损失了%d生命值。";
VSENVIRONMENTALDAMAGE_SLIME_SELF = "你泡在泥浆中，损失了%d点生命值。";
VSEVADEOTHEROTHER = "%s发起了攻击。%s闪避开了。";
VSEVADEOTHERSELF = "%s发起了攻击。你闪避开了。";
VSEVADESELFOTHER = "你发起攻击。%s闪避开了。";
VSIMMUNEOTHEROTHER = "%s发起了攻击，但是%s对此免疫。";
VSIMMUNEOTHERSELF = "%s发起了攻击，但是你对此免疫。";
VSIMMUNESELFOTHER = "你发起了攻击，但是%s对此免疫。";
VSPARRYOTHEROTHER = "%s发起了攻击。%s招架住了。";
VSPARRYOTHERSELF = "%s发起了攻击。你招架住了。";
VSPARRYSELFOTHER = "你发起了攻击。%s招架住了。";
VSRESISTOTHEROTHER = "%s发起攻击。%s抵抗了所有伤害。";
VSRESISTOTHERSELF = "%s发起攻击。你抵抗了所有伤害。";
VSRESISTSELFOTHER = "你发起攻击。%s抵抗了所有伤害。";
VULNERABLE_TRAILER = "(+%d 易伤加成)"; -- Appended to attack strings in the combat log when damage is amplified by vulnerability. (NOTE SPACE AT BEGINNING)
WAISTSLOT = "腰部"; -- Waist slot
WARLOCK_INTELLECT_TOOLTIP = "提高你的法力值上限和使用法术造成致命一击的几率。\n使你更快地提升武器技能熟练度。";
WARRIOR_STRENGTH_TOOLTIP = "提高你的近战武器的攻击强度。\n使你用盾牌格挡攻击时减免更多的伤害。";
WATER_COLLISION = "水体碰撞";
WATER_DETAIL = "水面细节";
WEATHER_DETAIL = "天气强度";
WHISPER = "悄悄话";
WHISPER_MESSAGE = "悄悄话"; -- Whisper message to a particular player
WHO = "查询";
WHO_FRAME_SHOWN_TEMPLATE = "（显示%d）";
WHO_FRAME_TOTAL_TEMPLATE = "找到%d个人";
WHO_FRAME_TOTAL_TEMPLATE_P1 = "找到%d个人";
WHO_LIST = "名单列表";
WHO_LIST_FORMAT = "|Hplayer:%s|h[%s]|h: 等级 %d %s %s - %s"; -- Example: Jeremy: Level 10 Human Warrior - Westfall
WHO_LIST_GUILD_FORMAT = "|Hplayer:%s|h[%s]|h: 等级 %d %s %s <%s> - %s"; -- Example: Tigole: Level 40 Human Rogue <Legacy of Steel> - Westfall 
WHO_NUM_RESULTS = "共计%d个玩家";
WHO_NUM_RESULTS_P1 = "共计%d个玩家";
WHO_TAG_CLASS = "c-"; -- "class" tag for /who
WHO_TAG_GUILD = "g-";
WHO_TAG_NAME = "n-"; -- "name" tag for /who
WHO_TAG_RACE = "r-"; -- "race" tag for /who
WHO_TAG_ZONE = "z-"; -- "zone" tag for /who
WIDESCREEN_TAG = "（宽屏）";
WINDOWED_MAXIMIZED = "最大化";
WINDOWED_MODE = "窗口模式";
WORLDMAP_BUTTON = "世界地图"; -- World map microbutton text
WORLD_APPEARANCE = "世界外观";
WORLD_LOD = "世界细节层次";
WORLD_MAP = "世界地图"; -- Title for world map frame
WORLD_PORT_ROOT_TIMER = "你掉落穿过了世界。你将在这里逗留%d%s。";
WRISTSLOT = "手腕"; -- Wrist slot
WRONG_SLOT_FOR_ITEM = "物品无法放入该空位。"; -- The item can't go in that slot.
XP = "经验值"; -- Abbreviation for "experience"
XPBAR_LABEL = "经验槽";
YELL_MESSAGE = "呼喊"; -- Yell to a wide area
YES = "是"; -- Generic "yes"
YOU = "你"; -- "You", the player
YOUR_BID = "你的出价：";
YOU_LOOT_MONEY = "你拾取了%s";
ZONE = "地区"; -- Label for "Zone" field in the world map
ZONE_COLON = "地区：";
ZONE_UNDER_ATTACK = "|cffffff00%s遭到攻击！|r"; -- %s is the zone which is under attack by enemy players
ZOOM_IN = "放大";
ZOOM_OUT = "缩小"; -- Button text to zoom out your view in the world map
ZOOM_OUT_BUTTON_TEXT = "右键点击可以缩小地图";
